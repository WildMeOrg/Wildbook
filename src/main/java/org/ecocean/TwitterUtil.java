...
public class TwitterUtil {
    ...
    public static Twitter init(String context) {
        ...
        try {
            ...
            System.out.println("INFO: TwitterUtil.init(" + context +
                ") failed [usually no twitter.properties] -- " + ex.toString());
            tfactory = null;
            return null;
        }
        System.out.println("INFO: initialized TwitterUtil.tfactory on " + context +
            " for account " + me.getScreenName() + " [" + me.getId() + "]");
    ...
    public static QueryResult findTweets(String search)
    throws TwitterException {
        return findTweets(search, -1l);
    }

    public static QueryResult findTweets(String search, long sinceId)
    throws TwitterException {
    ...
    public static Status getTweet(long tweetId) {
        ...
        try {
            ...
            System.out.println("ERROR: TwitterUtil.getTweet(" + tweetId + ") threw " +
                tex.toString());
            return null;
        } catch (Exception ex) {
            System.out.println("ERROR: TwitterUtil.getTweet(" + tweetId + ") threw " +
    ...
    public static Status toStatus(MediaAsset ma) {
        if ((ma == null) || !(ma.getStore() instanceof TwitterAssetStore) ||
            !ma.hasLabel("_original") || (ma.getMetadata() == null) ||
 ...
 */
    ...
    public static Status sendTweet(String tweetText)
    throws TwitterException {
        if (tfactory == null) throw new TwitterException("TwitterUtil has not been initialized");
        if (tweetText == null)
            throw new TwitterException("TwitterUtil.sendTweet() got null tweetText");
    ...
    public static Status sendTweet(String tweetText, Long replyToId)
    throws TwitterException {
        if (tfactory == null) throw new TwitterException("TwitterUtil has not been initialized");
        if (tweetText == null)
            throw new TwitterException("TwitterUtil.sendTweet() got null tweetText");
