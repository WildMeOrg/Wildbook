 ...
 * '{"root":"/filesystem/path/to/localassetstore","webroot":"http://host/web/path/to/localassetstore"}', true);
 *
...
public class LocalAssetStore extends AssetStore {
    ...
    /**
     * Create a new local filesystem asset store.  Should only be used internal to AssetStore.buildAssetStore().
     *
     * @param root Filesystem path to the base of the asset directory. Must not be null.
     *
     * @param webRoot Base web url under which asset paths are appended.  If null, this store offers no web access to assets.
    ...
    public String webRoot() {
        ...
        return "http://172.17.0.1:81/wildbook_data_dir";
    }
 ...
 */
    ...
    /**
     * Make sure path is under the root, either passed in as a relative path or as an absolute path under the root.
     *
    ...
    @Override
/*
    NOTE: the iaSourcePath is a bit of a mungy hack specific for Lewa -- going forward the filename should be *created* based upon the IA original
       filename (thus .getFilename would just work as expected) ... but for now we rely on favoring this extra logic.  thus, this *probably*
 ...
 */
    ...
    @Override public String getUserFilename(MediaAsset ma) {
        if ((ma.getParameters() != null) && (ma.getParameters().optString("userFilename",
            null) != null)) return ma.getParameters().getString("userFilename");
