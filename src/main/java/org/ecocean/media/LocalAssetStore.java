 ...
 *
 * To create a new store outside of Java, you can directly insert a row into the assetstore table
 * like so (adjusting paths as needed):
 *
 * insert into assetstore (name,type,config,writable) values ('local store', 'LOCAL',
 * '{"root":"/filesystem/path/to/localassetstore","webroot":"http://host/web/path/to/localassetstore"}',
 * true);
 *
 * Then ensure your webserver is configured to serve the filesystem path at the webroot.
 *
 * If you have only one asset store defined, it will be considered the default (see
 * AssetStore.loadDefault()).
 */
public class LocalAssetStore extends AssetStore {
     ...
     *
     * @param root Filesystem path to the base of the asset directory. Must not be null.
     *
     * @param webRoot Base web url under which asset paths are appended.  If null, this store offers
     * no web access to assets.
     *
    ...
    /**
     * Create a new local filesystem asset store.  Should only be used internal to
     ...
     * no web access to assets.
     */
    LocalAssetStore(final Integer id, final String name, final AssetStoreConfig config,
        final boolean writable) {
     ...
     *
     * @return The MediaAsset, or null if the path is invalid (not under the asset root or
     * nonexistent).
     */
    @Override public MediaAsset create(final JSONObject params)
    throws IllegalArgumentException {
        Path subpath = pathFromParameters(params, false); // file can not exist (for sake of
                                                          // subsequent copyIn() being called) ...
                                                          // sorry?
 ...
 */
    ...
    public MediaAsset create(File file)
    throws IllegalArgumentException {
 ...
 */
    ...
    throws IOException {
        ...
        params.put("path", subpath.toString()); // always store it relative, not absolute (in case
                                                // it was passed in as such)
    ...
    throws IOException {
        ...
        if (!(fromMA.getStore() instanceof LocalAssetStore) ||
 ...
 */
    ...
    /**
     * Make sure path is under the root, either passed in as a relative path or as an absolute path
     * under the root.
     *
    ...
    public static Path checkPath(final Path root, final Path path) {
        ...
        if (result.startsWith("..")) {
            throw new IllegalArgumentException("Path not under given root (root=" +
                    root.toString() + "; path=" + path.toString() + ")");
        }
        return result;
    }
    ...
    /**
     * Like checkPath(), but throws an IllegalArgumentException if the resulting file doesn't exist.
     *
     * @return Subpath to the file relative to the root.
     */
    ...
    @Override
/*
    NOTE: the iaSourcePath is a bit of a mungy hack specific for Lewa -- going forward the filename
       should be *created* based upon the IA original filename (thus .getFilename would just work as
       expected) ... but for now we rely on favoring this extra logic.  thus, this *probably*
    should not go outside of the lewa branch.  :(
 */
    public String getFilename(MediaAsset ma) {
        if ((ma.getParameters() != null) && (ma.getParameters().optString("iaSourcePath",
            null) != null)) {
