...
/**
 * MediaAssetMetadata is *machine-read* (i.e. not human input or confirmed) data *derived from* the
 * raw file itself. Most likely it is Exif(-ish) data, but may be some other analytical stuff.
 *  There should be a one-to-one mapping from MediaAsset to MediaAssetMetadata
 */
public class MediaAssetMetadata implements java.io.Serializable {
    ...
    public void setDataAsString(String d) {
        ...
        try {
            ...
            System.out.println(this + " -- error parsing data json string (" + d + "): " +
                je.toString());
            data = null;
        }
    }
    ...
    // even if it doesnt exist, this will return an empty one for convenience of calling
    // .optFOO(key) easily on it
    public JSONObject getAttributes() {
        if ((getData() == null) || (getData().optJSONObject("attributes") == null))
    ...
    // for now(?) this just searches down into exif structure, returns HashMap of key:values whose
    // keys match regex
...
/* sigh. ok, date/time created. sounds easy?  nope. here is an example of exif data *from a single
   image* ....

   Iptc: {    /// thanks for not following "yyyy:MM:dd HH:mm:ss" iptc.  :(
        Digital Date Created: "20140811", Time Created: "134915", Date Created: "Mon Aug 11 00:00:00
           PDT 2014",   ///who needs time, right?  and wtf is "134915" ... swatch time???
        Digital Time Created: "134915"
   }
   Exif SubIFD: {
        Sub-Sec Time Original: "70", Sub-Sec Time Digitized: "70", Date/Time Original: "2014:08:11
           13:49:15", Date/Time Digitized: "2014:08:11 13:49:15",
   }
   Exif IFD0: {
        Date/Time: "2014:10:20 16:06:40",   ///  WTF!?   ... and just our luck the recursing down
           found this first.  SIGH!!!!!!
   }
...
// System.out.println("MediaAssetMetadata.getDateTime() ----> " + matches);
        ...
        SimpleDateFormat dateParser = new SimpleDateFormat("yyyy:MM:dd HH:mm:ss"); // note: exif
                                                                                   // doesnt carry
                                                                                   // tz :(
        // TODO maybe?  we could also try just yyyy:MM:dd which sometimes exists.  sigh.  (etc x
        // infinity)
...
// System.out.println("* MediaAssetMetadata.getDateTime(): " + key + "[" + val + "] -> " + dt);
            ...
            // now we do a little quasi-weighting based on key (e.g. "original" is best, "digitized"
            // is kinda good; otherwise sketchy
...
/*
   GPS: {
   GPS Satellites: "10", GPS Latitude: "41.0° 31.0' 35.735999999990895"", GPS Map Datum: "WGS 84",
      GPS Date Stamp: "2014:08:11", GPS Version ID: "2.300", GPS Latitude Ref: "N", GPS Altitude
      Ref: "Sea level", GPS Longitude: "-69.0° 22.0' 45.62999999998169"", GPS Time-Stamp: "17:49:26
      UTC", GPS Longitude Ref: "W", GPS Altitude: "10 metres"
 */
...
// System.out.println("alt -> " + alt);
        ...
        if ((parts.length == 1) || "".equals(parts[1]) ||
            "m".equals(parts[1].toLowerCase().substring(0, 1))) return dbl;                                               //
                                                                                                                          // already
                                                                                                                          // in
                                                                                                                          // metres
                                                                                                                          // (we
                                                                                                                          ...
                                                                                                                          // not
                                                                                                                          // miles?)
        if ("f".equals(parts[1].toLowerCase().substring(0, 1))) return (dbl / 0.3048); // goodbye,
                                                                                       // feet
        System.out.println(
            "WARNING: MediaAssetMetadata._parseAltitude() unable to make sense of units, i think: "
            + alt);
...
/*
   ...
   - https://www.impulseadventure.com/photo/exif-orientation.html
    ...
    e.g.  https://iot.wildbook.org/obrowse.jsp?type=MediaAssetMetadata&id=253361 gives (only?)
