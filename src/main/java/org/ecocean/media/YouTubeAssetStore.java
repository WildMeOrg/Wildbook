...
public class YouTubeAssetStore extends AssetStore {
    ...
    @Override public JSONObject createParameters(File file, String grouping) {
        ...
        // will we even ever need this for a read-only system???  TODO so returning empty [or should
        // be null?]
        return p;
    }
    ...
    // most likely you want grabAndParse() really.  this is just a wrapper to YouTube.grab()
    // basically
    // how should we thread in bkgd??? ... probably this should by synchrous, but stuff can bg when
    // needed (e.g. extractMetadata)
    public static List<File> grab(MediaAsset ma)
    throws IOException {
        if (ma == null) return null;
        String id = idFromParameters(ma.getParameters());
        if (id == null) return null;
        // when fetched: (1) parse .json and set metadata.detailed; (2) create child assets (using
        // default store!) for video & thumb
    ...
    // 'wait' refers only for other process, if we need to do the grabbing, right now it will always
    // wait for that
...
// System.out.println("detailed -> " + detailed);
                if (detailed != null) {
                    ...
                    if (ma.getMetadata() != null) {
                        ma.getMetadata().getDataAsString(); // HACK gets around weird dn caching
                                                            // issue. :(
    ...
    // most likely you want extractFramesAndParse() really.  this is just a wrapper to
    // YouTube.extractFrames() basically
    ...
    throws IOException {
        ...
        // we attempt to create the final resting place for these files (which only works for
        // LocalAssetStore)
        // but this will work for any (writable, duh) AssetStore (since this will get us a temp dir
        // basically)
        ...
        if (dirP == null) dirP = Files.createTempDirectory("youtube_extract_" + ma.getId()); // fallback
                                                                                             // to
                                                                                             // tmp
    ...
    // returns success (frames extracted) -- passed in parent/_original MA ... maybe later we would
    // want to also allow _video child option?  TODO
    // 'wait' refers only for other process, if we need to do the grabbing, right now it will always
    // wait for that
    // NOTE: wait is untested!  TODO
    // note that uses same _processing derivation method as .grabAndParse() above.  this is cuz
    // these should not be run simultaneous anyway
...
// System.out.println("video child created: " + kid);
    ...
    throws java.io.IOException {
        ...
        // AssetStore astore = AssetStore.getDefault(myShepherd); //err, um think we should use
        // parent's (as it is a video stored somewhere, not a YouTubeAssetStore
        ...
        int offset = -1;
        // this assumes filename is like "frame00001.jpg" so needs to be altered if
        // config/youtube_extract.sh ever changes that
        try {
            offset = Integer.parseInt(f.getName().substring(5, 10)) - 1; // ffmpeg starts counting
                                                                         // at 1, so we adjust to be
                                                                         // 0-based
...
// System.out.println("frame child created: " + kid);
    ...
    throws IOException {
        ...
        // we (attempt to) let the basic stuff finish synchronously, so we have a populated data
        // chunk to save (hopefully)before the detailed one does
        try {
    ...
    /*
        note: this assumes the MediaAsset has been persisted, so we can create our own Shepherd
           object and affect it accordingly.... there are potentials for race conditions here for
           sure, especially if the caller has gone on to (or caused something to go on to) further
           alter the MediaAsset in memory etc.   use caution!
