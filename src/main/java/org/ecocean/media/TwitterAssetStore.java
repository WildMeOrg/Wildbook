...
/**
 * TwitterAssetStore references MediaAssets that reside on Twitter; tweets and their media currently
 * this is read-only but later could be writable with an API key if needed?
 *
 */
public class TwitterAssetStore extends AssetStore {
    ...
    // note: convention is a param.type of null (not set) means type is a tweet... otherwise must
    // specify (e.g. "media")
...
/*   TODO not really sure what to do with updateChild() and friends here..... hmmmm...
     presently opting to create these manually via entitiesAsMediaAssets() (which will utilize
        .parent property)
    ...
    //awkwardly named subset of the above which will be used to determine which should be derived
       with updateStandardChildren() public List<String> standardChildTypes() {
        return Arrays.asList(new String[]{"entity"});
    }
...
/*  regarding media (etc) in tweets:  seems .extended_entities.media is the same as .entities.media
   ???  but extended (duh?) might have more details?
        https://dev.twitter.com/overview/api/entities-in-twitter-objects
           https://dev.twitter.com/overview/api/entities
 */

///// note: might also want to walk .entities.urls --
...
/*   remove?  TODO public static List<MediaAsset> entitiesAsMediaAssetsGsonObj(MediaAsset ma, Long
   parentTweetId) {
        JSONObject raw = getRawJSONObject(ma);
        // System.out.println(raw.toString());
        AssetStore store = ma.getStore();
...
/*
    ...
    public Long parseParentTweetId(String label) throws Exception{
      Long returnVal = null;
      String PATTERN = "_parentTweet:(\\d+)"; //doesn't seem as robust as Pattern pattern =
