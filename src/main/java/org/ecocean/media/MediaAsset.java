 ...
 */
    ...
    public String getUUID() {
        ...
        // UUID v3 seems to take an arbitrary bytearray in, so we construct one that is basically
        // "Ma____" where "____" is the int id
    ...
    // note this function will not allow "invalid" (< 0) ids... so see above for hack for new
    // MediaAssets
    ...
    public JSONObject getParameters() {
        ...
        // System.out.println("NOTE: getParameters() on " + this + " was null, so trying to get from
        // parametersAsString()");
    ...
    ///note: really the only place that should call getParametersAsString or setParametersAsString
    // is datanucleus...
...
// System.out.println("hashCode on " + this + " = " + this.hashCode);
    ...
    // this is store-specific, and null should be interpreted to mean "i guess i dont really have
    // one"
    ...
    public String getRotationInfo() {
        ...
        for (String k : orient.keySet()) {
            if (k.toLowerCase().contains("thumb")) continue; // we skip exif sections with "thumb"
                                                             // in them
    ...
    // this is (seemingly?) always derived from MediaAssetMetadata, so .. yeah. make sure that is
    // set (see note by getMetadata() )
    ...
    public DateTime getDateTime() {
        ...
        if (adt != null) return DateTime.parse(adt); // lets hope it is in iso8601 format like it
                                                     // should be!
    ...
    // note: default behavior will add this to the features on this MediaAsset -- can pass false to
    // disable
...
/*    has been deprecated, cuz you should make a better choice about what you want the url of. see:
 ...
 */
    ...
    // right now "master" labelled image is used, if available, otherwise children are chosen by
    // allChildTypes() order....
    ...
    public URL safeURL(HttpServletRequest request) {
        ...
        // the throw-away Shepherd object is [mostly!] ok here since we arent returning the
        // MediaAsset it is used to find
    ...
    public MediaAsset bestSafeAsset(Shepherd myShepherd, HttpServletRequest request, String bestType
        ...
        // note, this next line means bestType may get bumped *up* for anon user.... so we should
        // TODO some logic in there if ever needed
        if (AccessControl.isAnonymous(request)) bestType = "mid";
        if (store instanceof URLAssetStore) bestType = "original"; // this is cuz it is assumed to
                                                                   // be a "public" url
        ...
        if (parentId != null) {
            ...
            if (!top.hasLabel("_original")) {
                // Commented out below because it prints 5k lines at a time when loading an
                // occurrence (!?!?)
                // System.out.println("INFO: " + this + " had a non-_original parent of " + top + ";
                // so using this");
                return this; // we stick with this cuz we are kinda at a dead end
            }
        }
        boolean gotBest = false;
        List<String> types = store.allChildTypes(); // note: do we need to care that top may have
                                                    // changed stores????
        for (String t : types) {
 ...
 */
    ...
    // this takes contents of this MediaAsset and copies it to the target (note MediaAssets must
    // exist with sufficient params already)
    // please note this uses *source* AssetStore for copying, which can/will affect how, for
    // example, credentials in aws s3 are chosen.
    // for tighter control of this, you can call copyAsset() (or copyAssetAny()?) directly on
    // desired store
    ...
    throws org.datanucleus.api.rest.orgjson.JSONException {
        ...
        // jobj.put("guid", "http://" + CommonConfiguration.getURLLocation(request) +
        // "/api/org.ecocean.media.MediaAsset/" + id);
        ...
        if ((getMetadata() != null) && (getMetadata().getData() != null) && (getMetadata().getData()
                .opt("attributes") != null)) {
 ...
 */
    ...
    public boolean hasKeyword(String keywordName) {
        if (keywords != null) {
            ...
            for (int i = 0; i < numKeywords; i++) {
                ...
                if ((keywordName.equals(kw.getIndexname()) || keywordName.equals(kw.getDisplayName()
                    ))) return true;
            }
        }
        return false;
    ...
    // piggybacks off metadata, so that must be set first, otherwise it matches corresponding mime
    // type (major/minor), case-insensitive
    ...
    public String[] getMimeType() {
        ...
        String mt = this.metadata.getAttributes().optString("contentType", null); // note:
                                                                                  // getAttributes
                                                                                  // always  returns
                                                                                  // a JSONObject
                                                                                  // (even if empty)
    ...
    // note: we are going to assume Metadata "will just be there" so no magical updates. if it is
    // null, it is null.
    // this implies basically that it is set once when the MediaAsset is created, so make sure that
    // happens, *cough*
    ...
    // handy cuz we dont need the actual file (if we have these values from elsewhere) and usually
    // the only stuff we "need"
 ...
 */
    public List<Encounter> assignEncounters(Shepherd myShepherd) {
        ...
        Map<String, Integer> partCt = new HashMap<String, Integer>(); // holds count for each type
                                                                      // of part
        int nonPartCt = 0;
        for (Annotation ann : annots) {
            ...
            if (ann.isTrivial() && (enc != null)) { // i guess it would be weird to have trivial
                                                    // with no enc, but we dont want it!
        ...
        if (myEncs.isEmpty()) {
            // this assumes we are dealing with all the same species (so makes one enc to clone),
            // based on first annotation
        ...
        if ((nonPartCt > 1) || hasDuplicateParts || !contiguousAnnots || (myEncs.size() > 1)) {
            ...
            for (Annotation ann : needsEncounter) {
                if (trivialAnnots.size() > 0) {
                    ...
                    Encounter tenc = tann.findEncounter(myShepherd); // should never be null, so
                                                                     // hope you arent here cuz of a
                                                                     // npe
                    ...
                    try {
                        Encounter newEnc = whichever.cloneWithoutAnnotations(myShepherd); // ok, not
                                                                                          // really
                                                                                          // random
    ...
    public int hashCode() {
        if (uuid == null) return Util.generateUUID().hashCode(); // random(ish) so we dont get two
                                                                 // users with no uuid equals! :/
