...
public class S3AssetStore extends AssetStore {
    ...
    /**
       ...
       https://docs.aws.amazon.com/AWSSdkDocsJava/latest/DeveloperGuide/credentials.html TODO
          possibly allow per-AssetStore or even per-MediaAsset credentials. these should be passed
          by reference (e.g. dont store them in, for example, MediaAsset parameters) perhaps to
          Profile or some properties etc?
     */
    ...
    throws IOException {
        ...
        if (!force && Files.exists(lpath)) return true; // we assume if we have it, then we should
                                                        // be cool
    ...
    @Override
    // NOTE the aws credentials will be pulled from the current instance ("this") S3AssetStore, so
    // must have access to both buckets
    // NOTE: *** s3 might give an "invalid key" if you try to copyObject a file immediately after it
    // was created.  ymmv.
 ...
 */
    ...
    @Override public URL webURL(final MediaAsset ma) {
        ...
        JSONObject params = ma.getParameters();
/*  meh, fooey on this per-asset setting; lets only use per-store setting Object up =
   getParameter(params, "urlAccessible");
 ...
 */
    ...
    @Override public JSONObject createParameters(File file, String grouping) {
        ...
        // note: this key is simply to try to encourage uniqueness, but can be later re-set with
        // something better if desired
    ...
    throws IOException {
        ...
        // ma.localPath() would have failed if we didnt have "bucket" and "key", so we can assume
        // (gulp) they exist
...
// System.out.println("S3AssetStore.convertToLocal() " + ma + " localizing " + lpath + " to " +
// targetPath);
        targetPath.getParent().toFile().mkdirs();
        Files.copy(lpath, targetPath, REPLACE_EXISTING); // replace, since bucket/key "should be"
                                                         // unique on S3!
