...
/*
    ...
    Basic usage:
    1. Instantiate a new FStatistics object for each allele 2. Use the loadIndividual(...) method to load the individuals and alleles from each of the
       populations being compared 3. Call your getter to get the FStatistic of interest: getF, getf, getTheta, etc.
            ...
            for (int offset = 0; offset < 2; offset++) {
                ...
                if (alleleIndex == s_numberOfAlleles) {
                    ...
                    for (int i = 0; i < s_numberOfPopulations; i++) {
                        // adds the new row
                        /*
                           s_genotypeFrequenciesPerPopulation[i] = (int*) realloc(s_genotypeFrequenciesPerPopulation[i],
                        ...
                        for (int j = 0; j < s_numberOfAlleles; j++) {
                            /*
                               s_genotypeFrequenciesPerPopulation[i][j] = (int) realloc(s_genotypeFrequenciesPerPopulation[i][j],
                                  s_numberOfAlleles*sizeof(int));
                               if (!s_genotypeFrequenciesPerPopulation[i][j]) {
                                s_clear();
                                throw EggMemoryError();
