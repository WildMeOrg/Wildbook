...
/*
    ...
    You should have received a copy of the GNU General Public License along with EggLib.  If not,
       see <http://www.gnu.org/licenses/>.

    Ported from the EggLib C++ library to Java by Jason Holmberg.

    Basic usage:
    1. Instantiate a new FStatistics object for each allele 2. Use the loadIndividual(...) method to
       load the individuals and alleles from each of the populations being compared 3. Call your
       getter to get the FStatistic of interest: getF, getf, getTheta, etc.
    ...
    /*
       void reserve(int numberOfIndividuals) {
        if (numberOfIndividuals==0) {
            throw new Exception("FStatistics: cannot reserve memory for zero sites for F-statistics
               analysis");
        }

        if (numberOfIndividuals<d_numberOfGenotypes) {
            throw new Exception("FStatistics: cannot reserve less memory than what is currently
               used");
        }
...
/* ****************************************************************** */
    ...
    private void s_compute() {
        if (!d_flag)
            System.out.println("FStatistics: cannot compute frequency statistics: data not loaded");
        if (s_flag)
            System.out.println("FStatistics: inconsistency: frequency statistics already computed");
        ...
        // s_genotypeFrequenciesPerPopulation[i] = null;
        // }
        if (s_numberOfPopulations < 2)
            System.out.println(
                "FStatistics: cannot compute frequency statistics: not enough different populations");
    ...
    void processPopulations() {
        for (int genotype = 0; genotype < d_numberOfGenotypes; genotype++) {
            ...
            if (populationIndex == s_numberOfPopulations) {
                ...
                // memory
                /*
                   s_populationLabelMapping = (int) realloc(s_populationLabelMapping,
                      s_numberOfPopulations*sizeof(int));
                   if (!s_populationLabelMapping) {
                    s_clear();
                    throw EggMemoryError();
                   }
                   s_populationFrequencies = (int) realloc(s_populationFrequencies,
                      s_numberOfPopulations*sizeof(int));
            ...
            // one of previous populations
                ...
                s_populationFrequencies.set(populationIndex,
                    s_populationFrequencies.get(populationIndex) + 1);
            }
        }
    }
        ...
        for (int genotype = 0; genotype < d_numberOfGenotypes; genotype++) {
            ...
            for (int offset = 0; offset < 2; offset++) {
                ...
                if (alleleIndex == s_numberOfAlleles) {
                    ...
                    // adds the allele
                    // s_alleleValueMapping = (int) realloc(s_alleleValueMapping,
                    // s_numberOfAlleles*sizeof(int));
                    ...
                    /*
                       s_alleleFrequenciesTotal = (int) realloc(s_alleleFrequenciesTotal,
                          s_numberOfAlleles*sizeof(int));
                    ...
                    for (int i = 0; i < s_numberOfPopulations; i++) {
                        /*
                           s_alleleFrequenciesPerPopulation[i] = (int)
                              realloc(s_alleleFrequenciesPerPopulation[i],
                              s_numberOfAlleles*sizeof(int));
                    ...
                    // new row
                    /*
                       s_genotypeFrequenciesTotal = (int*) realloc(s_genotypeFrequenciesTotal,
                          s_numberOfAlleles*sizeof(int));
                    ...
                    for (int i = 0; i < s_numberOfAlleles; i++) {
                        /*
                           s_genotypeFrequenciesTotal[i] = (int)
                              realloc(s_genotypeFrequenciesTotal[i], s_numberOfAlleles*sizeof(int));
                    ...
                    for (int i = 0; i < s_numberOfPopulations; i++) {
                        // adds the new row
                        /*
                           s_genotypeFrequenciesPerPopulation[i] = (int*)
                              realloc(s_genotypeFrequenciesPerPopulation[i],
                              s_numberOfAlleles*sizeof(int));
                        ...
                        for (int j = 0; j < s_numberOfAlleles; j++) {
                            /*
                               s_genotypeFrequenciesPerPopulation[i][j] = (int)
                                  realloc(s_genotypeFrequenciesPerPopulation[i][j],
                                  s_numberOfAlleles*sizeof(int));
                               if (!s_genotypeFrequenciesPerPopulation[i][j]) {
                                s_clear();
                                throw EggMemoryError();
                ...
                s_alleleFrequenciesTotal.set(alleleIndex,
                    s_alleleFrequenciesTotal.get(alleleIndex) + 1);
...
/* ****************************************************************** */
    ...
    private void w_compute() {
        ...
        for (int allele = 0; allele < s_numberOfAlleles; allele++) {
            w_pbar[allele] = 0.;
            for (int population = 0; population < s_numberOfPopulations; population++) {
                w_pbar[allele] += 0.5 * s_alleleFrequenciesPerPopulation[population][allele]; // absolule
                                                                                              // frquency
            }
        ...
        for (int allele = 0; allele < s_numberOfAlleles; allele++) {
            ...
            for (int population = 0; population < s_numberOfPopulations; population++) {
                w_ssquare[allele] += (s_populationFrequencies.get(population) *
                    (0.5 * s_alleleFrequenciesPerPopulation[population][allele] /
                    s_populationFrequencies.get(population) - w_pbar[allele]) *
                    (0.5 * s_alleleFrequenciesPerPopulation[population][allele] /
                    s_populationFrequencies.get(population) - w_pbar[allele]));
            }
            ...
            for (int population = 0; population < s_numberOfPopulations; population++) {
                ...
                for (int otherAllele = 0; otherAllele < s_numberOfAlleles; otherAllele++) {
                    if (allele != otherAllele) {
                        H += ((s_genotypeFrequenciesPerPopulation[population][allele][otherAllele] +
        ...
        for (int allele = 0; allele < s_numberOfAlleles; allele++) {
            w_a[allele] = (w_nbar / w_nc) *
                ...
                (w_pbar[allele] * (1 - w_pbar[allele]) - w_ssquare[allele] *
                (s_numberOfPopulations - 1.) / s_numberOfPopulations - w_hbar[allele] / 4.));
        }
        ...
        /* equation (3) of WC84) */
        for (int allele = 0; allele < s_numberOfAlleles; allele++) {
            w_b[allele] = (w_nbar / (w_nbar - 1)) *
                (w_pbar[allele] * (1 - w_pbar[allele]) - w_ssquare[allele] *
                ((s_numberOfPopulations - 1.) / s_numberOfPopulations) - w_hbar[allele] *
                (2 * w_nbar - 1) / (4 * w_nbar));
        }
...
/* ****************************************************************** */
    ...
    public int genotypeFrequencyPerPopulation(int populationIndex, int alleleIndex1,
        int alleleIndex2) {
        if (!s_flag) s_compute();
