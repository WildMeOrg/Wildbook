...
/*
 ...
 ++shared;
                ...
                case 2:
                        ...
                        {
                                if  (genes1[i] == genes2[i] || genes1[i+1] == genes2[i+1] ||
                                   genes1[i] == genes2[i+1] || genes1[i+1] == genes2[i])
                                {
 ++shared;	// at least one allele in common at this locus if ((genes1[i] == genes2[i] &&
   genes1[i+1] == genes2[i+1]) || (genes1[i] == genes2[i+1] && genes1[i+1] == genes2[i]))
 ...
 */
        ...
        default:
            ...
            for (int l = 0; l < na; l += ploidy) {              // l loops over loci
                for (int j = l; j < l + ploidy; ++j) {                  // loops over alleles at one
                                                                        // locus
                    for (int k = l; k < l + ploidy; ++k) {                      // loops over
                                                                                // alleles at same
                                                                                // locus in other
                                                                                // individual
       ...
       public float wtCountSharedAlleles (int genes1[], int genes2[])
       {
            ...
            default:
                    ...
                    for (int l=0; l < numLoci; ++l)	// l loops over loci
                    {
                            ...
                            {
                                    for (int k=i; k < i + ploidy; ++k)	// loops over alleles at
                                       same locus in other individual
                                    {
                                            if ((genes1[j] == genes2[k]) && !used[k])
                                            {
                                                    used[k] = true;
                                                    shared += numAlleleCopies -
                                                       alleleCounts[l][genes1[j]];
                                                    break;		// found a match for this
                                                       allele, now quit
                                            }
                                    }
