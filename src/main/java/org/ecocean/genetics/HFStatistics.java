/*
    ...
    EggLib is free software: you can redistribute it and/or modify it under the terms of the GNU
       General Public License as published by the Free Software Foundation, either version 3 of the
       License, or (at your option) any later version.

    EggLib is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even
    ...
    /*
       private void reserve(int numberOfIndividuals) {
        ...
        if (numberOfIndividuals<d_numberOfGenotypes) {
            throw EggArgumentValueError("HFStatistics: cannot reserve less memory than what is
               currently used");
        }
    ...
    public void loadIndividual(int genotype, int populationLabel) {
        ...
        /*
           if (d_numberOfGenotypes>d_reserved) {
            ...
            d_populationLabels = (int*) realloc(d_populationLabels,
               d_numberOfGenotypes*sizeof(int));
            if (!d_populationLabels) throw EggMemoryError();
           }
         */
...
/* ****************************************************************** */
    ...
    private void s_compute() {
        // if (!d_flag) throw new Exception("HFStatistics: cannot compute frequency statistics: data
        // not loaded");
        // if (s_flag) throw new Exception("HFStatistics: inconsistency: frequency statistics
        // already computed");
        ...
        /*
           // makes memory allocation accordingly s_alleleFrequenciesPerPopulation = (int**)
              malloc(s_numberOfPopulations*sizeof(int*));
           ...
           if (s_numberOfPopulations<2) throw EggRuntimeError("HFStatistics: cannot compute
              frequency statistics: not enough different populations");
         */
...
/* ****************************************************************** */
    ...
    private void w_compute() {
        ...
        if (!s_flag) s_compute();
        // if (w_flag) throw new Exception("HFStatistics: inconsistency: F-statistics already
        // computed");
        ...
        int sum_nisquare = 0;
        for (int i = 0; i < s_numberOfPopulations; i++) {
            sum_nisquare += s_populationFrequencies.get(i).intValue() *
                s_populationFrequencies.get(i).intValue();
        ...
        for (int allele = 0; allele < s_numberOfAlleles; allele++) {
            w_pbar[allele] = 0.;
            for (int population = 0; population < s_numberOfPopulations; population++) {
                w_pbar[allele] += s_alleleFrequenciesPerPopulation[population][allele]; // absolule
                                                                                        // frquency
            }
        ...
        for (int allele = 0; allele < s_numberOfAlleles; allele++) {
            ...
            for (int population = 0; population < s_numberOfPopulations; population++) {
                w_ssquare[allele] += (s_populationFrequencies.get(population).intValue() *
                    (1. * s_alleleFrequenciesPerPopulation[population][allele] /
                    s_populationFrequencies.get(population).intValue() - w_pbar[allele]) *
                    (1. * s_alleleFrequenciesPerPopulation[population][allele] /
                    s_populationFrequencies.get(population).intValue() - w_pbar[allele]));
            }
        ...
        for (int allele = 0; allele < s_numberOfAlleles; allele++) {
            w_T1[allele] = // (w_nbar/w_nc)* () ???
                ...
                (s_numberOfPopulations - 1.) / s_numberOfPopulations);
        }
