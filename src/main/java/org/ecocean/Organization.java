...
public class Organization implements java.io.Serializable {
            ...
            for (Organization org : kids) {
                if (this.isRelatedTo(org)) {
                    System.out.println("WARNING: " + this +
                        " .setChildren() failed due to related to " + org);
                    return false;
                }
            }
        }
    ...
    public List<Organization> addChild(Organization kid) {
        if (this.isRelatedTo(kid)) {
            System.out.println("WARNING: " + this + " .addChild(" + kid +
                ") failed due to relatedness");
            return null;
        }
    ...
    private void membersReciprocate(List<User> mems) {
        if (mems == null) return;
        for (User mem : mems) {
            if ((mem.getOrganizations() != null) && !mem.getOrganizations().contains(this))
                mem.getOrganizations().add(this);
        }
    }
    ...
    public static Organization load(String id, Shepherd myShepherd) {
        ...
        try {
            o = ((Organization)(myShepherd.getPM().getObjectById(
                myShepherd.getPM().newObjectIdInstance(Organization.class, id), true)));
