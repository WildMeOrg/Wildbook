/*
    idea here is "simple": a gateway to all IA calls, mostly (now) cleaned up from
       identity/IBEISIA.java (guessing)
 ...
 * probably should "leverage" Queue stuff where applicable?
      - possibly there is a NEED for both variations (as suggested by drew): an asynchronous
       * (queued) and synchronous (not)
...
public class IA {
    ...
    /*  NOTE: methods for both intaking a single element or a list.  thoughts:
        - these should be treated as different in that an IA framework might batch together the list
           in some way (i.e. difference between sending as list vs iterating over list with
           intake(each element)
 ...
 */
    ...
    // i think objects ingested here must(?) be persisted (and committed), as we have to assume (or
    // we know)
    // that these processes will use queues which operate in different (Shepherd) threads and will
    // thus try
    // to find the objects via the db.  :/
    // parentTask is optional, but *will NOT* set task as child automatically. is used only for
    // inheriting params
    ...
    public static Task intakeMediaAssets(Shepherd myShepherd, List<MediaAsset> mas,
        ...
        // in multi-species case we make sure we have a parent task and add each species task as a
        // child
    ...
    public static Task intakeMediaAssetsOneSpecies(Shepherd myShepherd, List<MediaAsset> mas,
        ...
        // what we do *for now* is punt to "legacy" IBEISIA queue stuff... but obviously this should
        ...
        // mimicking intakeAnnotations, we assume the first mediaAsset is representative of all of
        // them wrt Taxonomies, configs etc.
        ...
        try {
            for (MediaAsset ma : mediaAssets) {
                ...
                if ((assetsWithMissingAcmids.size() >= batchThreshold) ||
                    ...
                    if (assetsWithMissingAcmids.size() > 0) { // if count gets to the end and
                                                              // assetsWithMissingAcmids is still
                                                              // empty, no need to do any of this
                        try {
    ...
    public static Task intakeAnnotations(Shepherd myShepherd, List<Annotation> anns,
        ...
        //// slightly complicated bc we need to create child tasks only if there are multiple
        // iaClasses
        // if (annses.size() == 1) return intakeAnnotationsOneIAClass(myShepherd, annses.get(0),
        // parentTask);
        ...
        // return topTask;
        // }
        // public static Task intakeAnnotationsOneIAClass(Shepherd myShepherd, List<Annotation>
        // anns, final Task parentTask) {
        ...
        /*
            what we do *for now* is punt to "legacy" IBEISIA queue stuff... but obviously this
               should be expanded as needed for this we use IBEISIA.identOpts to decide how many
               flavors of identification we need to do!   if have more than one we need to make a
               set of subtasks
         */

/*
        String iaClass = anns.get(0).getIAClass(); //IAClass is a standard with image analysis that
           identifies the featuretype used for identification List<JSONObject> opts = null;
 ...
 */
        ...
        for (List<Annotation> annsOneIAClass : annotsByIaClass) {
            ...
            // now we remove ones with default=false (they may get added in below via
            // matchingAlgorithms param (via newOpts)
            if (opts != null) {
            ...
            JSONObject newTaskParams = new JSONObject(); // we merge parentTask.parameters in with
                                                         // opts from above
            ...
            if (opts.size() == 1 && annotsByIaClass.size() == 1) {
                ...
                for (int i = 0; i < opts.size(); i++) {
                    ...
                    newTaskParams.put("ibeis.identification",
                        ((opts.get(i) == null) ? "DEFAULT" : opts.get(i)));                                        //
                                                                                                                   // overwrites
                                                                                                                   // each
                                                                                                                   // time
            ...
            for (int i = 0; i < opts.size(); i++) {
                ...
                try {
                    if (fastlane) {
                        // if fastlane and a smaller, bespoke request, get this into the faster
                        // queue
    ...
    public static void handleRest(JSONObject jin) {
        ...
        try {
            ...
            myShepherd.storeNewTask(topTask);
            JSONObject opt = jin.optJSONObject("opt"); // should use this to decide how to branch
            ...
            if ((alist != null) && (alist.length() > 0)) {
                ...
                // okay, if we are sending another ID job from the hburger menu, the media asset
                // needs to be added to your top level 'root' task,
                ...
                // i cant think of a scenario where we would get here and accidently double-add
                // mas... but jic
    ...
    // (optional!) Taxonomy will append "_Scientific_name" to label and try that.  if not available,
    // then try just label.
    ...
    public static String getProperty(String context, String label, Taxonomy tax) { // no-default
                                                                                   // version
