...
/*
    ...
    Note: this is using a Queue as a more complicated example, thus:
        ...
        -- intakeMediaAssets() and intakeAnnotations() both use addToQueue() to publish a
           (meaningless) job to the Queue

    A Queue is not strictly necessary.  The intake methods could have simply done whatever
       processing necessary (e.g. send the objects to a blackbox IA service) in real-time.  The
       Queue just demostrates how to allow intake methods to asynchronously stack up jobs in the
       background.

    TODO -- mockup a TestPlugin callback, once we figure out how that will get routed thru
       IAGateway!!
