...
// NOTE!  this steals **a lot** from IBEISIA right now. eventually lets move it all here and kill
// that off!
    ...
    public void _OLD_prime() {
        ...
        Runnable r = new Runnable() {
            public void run() {
                ...
                try {
                    // think we can checkFirst on both of these -- no need to re-send anything
                    // during priming
...
/*
    note: sendMediaAssets() and sendAnnotations() need to be *batched* now in small chunks,
       particularly sendMediaAssets().
    this is because we **must** get the return value from the POST, in order that we can map the
       corresponding (returned) acmId values.  if we *timeout* in the POST, this *will not happen*.
        and it is a lengthy process on the IA side: as IA must grab the image over the network and
       generate the acmId from it!  hence, batchSize... which we kind of guestimate and cross our
       fingers.
 */
    ...
    throws RuntimeException, MalformedURLException, IOException, NoSuchAlgorithmException,
        ...
        // sometimes (i.e. when we already did the work, like priming) we dont want to check IA
        // first
        ...
        for (int i = 0; i < mas.size(); i++) {
            ...
            if (t == null) {
                ...
                map.get("image_unixtime_list").add((int)Math.floor(t.getMillis() / 1000)); // IA
                                                                                           // wants
                                                                                           // seconds
                                                                                           // since
                                                                                           // epoch
            }
            if ((i == (mas.size() - 1)) || ((i > 0) && (i % batchSize == 0))) { // end of all; or
                                                                                // end of a batch
                if (acmList.size() > 0) {
    ...
    throws RuntimeException, MalformedURLException, IOException, NoSuchAlgorithmException,
        ...
        URL url = new URL(u);
        int ct = 0;
        // may be different shepherd, but findIndividualId() below will only work if its all
        // persisted anyway. :/
        /*
    ...
    public static List<String> acmIdsFromResponse(JSONObject rtn) {
        ...
        for (int i = 0; i < rtn.getJSONArray("response").length(); i++) {
            if (rtn.getJSONArray("response").optJSONObject(i) == null) {
                // IA returns null when it cant localize/etc, so we need to add this to keep array
                // length the same
                ids.add(null);
            } else {
