...
public class Task implements java.io.Serializable {
    ...
    // private List<Object> objects = null;  //in some perfect world i could figure out how to
    // persist this.  :/  oh, for a wb base class.
...
/*
    ...
    public boolean isTypeIdentification() {
        if (this.isTypeDetection()) return false;  // we trust this a little more if
           (this.hasObjectAnnotations()) return true;
        if (this.parameters == null) return false;
    ...
    public boolean initiatedWithIdentification() {
        if (this.parameters == null) return false;  // not sure how i feel about this return
           !this.getParameters().optBoolean("skipIdent", false);
    }
 */
    ...
    // not sure if these two are mutually exclusive by definition, but lets assume not (wtf would
    // that even mean? i dunno)
    ...
    public JSONObject getParameters() { // only return as JSONObject!  TODO probably validate
                                        // content below?
    ...
    public int hashCode() {
        if (id == null) return Util.generateUUID().hashCode(); // random(ish) so we dont get two
    ...
    public String getStatus(Shepherd myShepherd) {
        ...
        if (logs != null && logs.size() > 0) {
            ...
            // if(status.equals("waiting to queue"))System.out.println("islObj:
            // "+islObj.toString());
        }
        return status;
    }
