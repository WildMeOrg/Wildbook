...
public class StoredQuery {
    ...
    // FUTURE: invalidateAfter - number of milliseconds after the lastExecuted time after which the stored results are invalidated and the query must
    // be re-run, non-persistent, Long
    ...
    public void setName(String newName) {
        if (newName == null) { this.name = null; } else { this.name = newName; }
        this.updateModified();
    }

    public String getUUID() { return uuid; }
    public void setUUID(String newUUID) {
        if (newUUID == null) { this.uuid = null; } else { this.uuid = newUUID; }
        this.updateModified();
    }

    public String getQueryString() { return queryString; }
    public void setQueryString(String newQS) {
        if (newQS == null) { this.queryString = null; } else { this.queryString = newQS; }
        this.updateModified();
    }
    ...
    public void setCorrespondingIACacheName(String cacheName) {
        if (cacheName == null) { this.correspondingIACacheName = null; } else {
            this.correspondingIACacheName = cacheName;
        }
        this.updateModified();
    }
    ...
    /*
       FUTURE: setCachedResult FUTURE: getCachedResult FUTURE: invalidateCachedResult - immediately invalidates cachedResult FUTURE:
          setInvalidateAfter Value of -1 never invalidates cachedResult if not null FUTURE: executeQuery
     */
