...
public class Annotation implements java.io.Serializable {
    ...
    private String iaClass; // This is just how it gonna be for now. Swap the methods to draw from Taxonomy
                            // later if ya like?
    ...
    // this is used to decide "should we match against this" problem is: that is not
    // very (IA-)algorithm agnostic
    ...
    // quality indicates the fidelity of the annotation, e.g. the overall image
    // quality of a picture.
    // This is useful e.g. for researchers who want to account for a bias where
    // "better" images are
    // more likely to produce matches.
    private Double quality;
    // distinctiveness indicates the real-wold distinctiveness of the feature *being
    // recorded*, independent
    // of the recording medium. Useful e.g. for researchers who want to account for
    // a bias where more distinct
    ...
    // For setting the iaClass returned from detection... No more mangled species
    // names sent to identification
    ...
    /*
     * public Annotation(MediaAsset ma, String species, int x, int y, int w, int h,
     * float[] tm) {
    ...
    // but migrating to features needs a legacy-compatible version!
    // ouch
    ...
    public boolean isTrivial() {
        ...
        for (Feature ft : getFeatures()) {
            if (ft.isUnity())
                return true; // TODO what *really* of multiple features?? does "just one unity" make sense?
        }
    ...
    public boolean isIAReady() {
        ...
        if (ma == null)
            return false;
        if (ma.getHeight() == 0 && ma.getWidth() == 0)
            return false;
        return true;
    }
    ...
    // (viewpoint == null || viewpoint == 'up' || viewpoint == 'upfront' ||
    // viewpoint == 'upfrontright'
    // || viewpoint == 'upright' || viewpoint == 'upbackright' || viewpoint ==
    // 'upback'
    // || viewpoint == 'upbackleft' || viewpoint == 'upfront' || viewpoint ==
    // 'upfrontleft')
    ...
    public static String[] getViewpointAndNeighbors(String vp) {
        ...
        try {
            ...
            if ((vp == null) || !isValidViewpoint(vp))
                return null;
    ...
    /*
     * // response comes from ia thus: "response": [{"score": 0.9783339699109396,
     * "species": "giraffe_reticulated", "viewpoint": "right"}] public static
     * JSONObject iaViewpointFromAnnotUUID(String uuid, String context) throws
     * RuntimeException, MalformedURLException, IOException,
    ...
    // TODO what should we do for multiple features that point to more than one
    // MediaAsset ?
    ...
    // detaches this Annotation from MediaAsset by removing the corresponding
    // feature *from the MediaAsset*
    ...
    // returns null if not MediaAsset (whaaa??), otherwise a list (possibly empty)
    // of siblings on the MediaAsset
    public List<Annotation> getSiblings() {
        ...
        for (Annotation ann : this.getMediaAsset().getAnnotations()) { // fyi .getAnnotations() doesnt return null
            if (!ann.getId().equals(this.getId()))
                sibs.add(ann);
        }
        return sibs;
    }
    ...
    /*
     * this is an ordering value which equates to
     * "where in the MediaAsset this Annotation lies compared to siblings".
     * what this means is greatly open to interpretation! "english reading order"
     * (top-down/left-right) might seem a decent choice, but after some
     * discussion, for boundingBox Annotations, i am going to go with left-most
     * x-value of the bounding box. NOTE: this is dependent on FeatureType,
    ...
    public int comparePositional(Annotation other) {
        if (other == null)
            return 0;
        if ((Util.collectionSize(this.getFeatures()) * Util.collectionSize(other.getFeatures())) == 0)
            return 0; // no features, oops
        // i have *no idea* how we should handle *multiple Features* here... so i am
        // just going to look for any non-zero response as "useful" :(
        // thus we do NxM comparing all Feature combinations; which seems like it might
        // suck but i think we "almost always" only have one!
    ...
    public void setIAExtractedKeywords(Shepherd myShepherd, Taxonomy taxy) {
        if (taxy == null)
            return;
        if (!this.hasMediaAsset() || !this.hasIAClass())
            return;
        IAJsonProperties iaConf = IAJsonProperties.iaConfig();
        String keywordString = iaConf.getKeywordString(taxy, this.getIAClass());
        if (!Util.stringExists(keywordString))
            return; // no keyword to save (according to config) scenario
    ...
    public int[] getBbox() {
        ...
        if ((bbox[2] < 1) || (bbox[3] < 1)) {
            // note: do NOT use toString() in here! it references .getBbox() !! see:
            // recursion
    ...
    /*
     * we no longer make a MediaAsset "from an Annotation", but rather from its
     * associated Feature(s) public MediaAsset createMediaAsset() throws
     * IOException {
     * if (mediaAsset == null) return null;
     * if (this.isTrivial()) return null; //we shouldnt make a new MA that is
     * identical, right?
    ...
    public String toString() {
        return new ToStringBuilder(this)
                ...
                .append("bbox", getBbox())
                /*
                 * //.append("transform", ((getTransformMatrix == null) ? null :
                 * Arrays.toString(getTransformMatrix())))
    ...
    // *for now* this will only(?) be called from an Encounter, which means that
    // Encounter must be sanitized
    ...
    /**
     * returns only the MediaAsset sanitized JSON, because whenever UI queries our
     * DB (regardless of class query), all they want in return are
    ...
    public ArrayList<Annotation> getMatchingSetForTaxonomy(Shepherd myShepherd, String genus,
        ...
        if (!Util.stringExists(genus) || !Util.stringExists(specificEpithet))
            ...
            filter = "SELECT FROM org.ecocean.Annotation WHERE matchAgainst && acmId != null && enc.annotations.contains(this) && enc.genus == '"
    ...
    // currently not taking params, but we can add later if we find useful (for now
    // just trusting filter!)
    static public ArrayList<Annotation> getMatchingSetForFilter(Shepherd myShepherd,
        ...
        while (it.hasNext()) {
            ...
            if (!IBEISIA.validForIdentification(ann))
                continue;
            if (acmidList.contains(ann.getAcmId()))
                continue;
            anns.add(ann);
    ...
    // will construnct "&& (viewpoint == null || viewpoint == 'x' || viewpoint ==
    // 'y')" for use above
    ...
    private String getMatchingSetFilterViewpointClause(Shepherd myShepherd) {
        ...
        if (viewpoints == null)
            return "";
        // else if(getSpecies(myShepherd)!=null &&
        // getSpecies(myShepherd).equals("Tursiops truncatus")) ||
        // (getSpecies(myShepherd)!=null &&
        // getSpecies(myShepherd).equals("Orcinus orca"))||
        // (getSpecies(myShepherd)!=null && getSpecies(myShepherd).equals("Tursiops
        // aduncus")) ||
        // (getSpecies(myShepherd)!=null && getSpecies(myShepherd).equals("Tursiops
        // sp.")) || (getSpecies(myShepherd)!=null &&
        ...
        // if explicitly told to ignore viewpoint matching, skip this step
                ...
                getTaxonomy(myShepherd)) != null && IA.getProperty(myShepherd.getContext(),
                        "ignoreViewpointMatching", getTaxonomy(myShepherd)).equals("true"))
            return "";
    ...
    // note, we are give *full* task.parameters; by convention, we only act on
    ...
    private String getMatchingSetFilterFromParameters(JSONObject taskParams) {
        ...
        if (expandedLocationIds.size() > 0) {
            ...
            // loc ID's were breaking for Hawaiian names with apostrophe(s) and stuff, so
            // overkill now
        ...
        if ((owner != null) && (userId != null)) {
            for (int i = 0; i < owner.length(); i++) {
                ...
                // if (opt.equals("org")) {
                // f += " && enc.submitterOrganization == user.organization[0]";
                // }
     ...
     * getMatchingSetXXX() above ...
     * this is a string that uniquely identifies the matchingSet, dependent of
     * content (e.g. cant be based on content uuids)
     */
    public String getCurvrankDailyTag(JSONObject taskParams) {
        ...
        if ((owner != null) && (userId != null)) {
            for (int i = 0; i < owner.length(); i++) {
                if ("me".equals(owner.optString(i, null)))
                    mineOnly = true;
            }
        }
        if (mineOnly) {
        ...
        // now locations, which we want sorted and lowercase, to ensure consistency in
        // multi-value names
        List<String> locs = new ArrayList<String>();
        if (j.optString("locationId", null) != null)
            locs.add(j.getString("locationId"));
        JSONArray larr = j.optJSONArray("locationIds");
        if (larr != null) {
            for (int i = 0; i < larr.length(); i++) {
                String val = larr.optString(i, null);
                if ((val != null) && !locs.contains(val))
                    locs.add(val);
            }
        }
        if (locs.size() > 0) {
     ...
     * MediaAsset ma = this.getMediaAsset();
     * System.out.println("  >> findEncounterDeep() -> ma = " + ma +
     * " .... getting Annotations:");
     * if (ma == null) return null;
     * ArrayList<Annotation> anns = ma.getAnnotations();
     * for (Annotation ann : anns) {
     * System.out.println("  >> findEncounterDeep() -> ann = " + ann);
     * //question: do we *only* look for trivial here? seems like we would want
     * that... cuz we crawl hierarchy only in weird video cases etc if
     * (ann.isTrivial()) return ann.findEncounterDeep(myShepherd); //recurse! (and
     * effectively bottom-out here... do or die
     * }
     * return null; //fall thru, no luck!
     * }
     */

    // this is a little tricky. the idea is the end result will get us an Encounter,
    // which *may* be new
    // if it is new, its pretty straight forward (uses findEncounter) .. if not,
    // creation is as follows:
    // look for "sibling" Annotations on same MediaAsset. if one of them has an
    // Encounter, we clone that.
    // additionally, if one is a trivial annotation, we drop it after. if no
    // siblings are found, we create
    // an Encounter based on this Annotation (which may get us something, e.g.
    // species, date, loc)
    // ###### NOTE: this is going away due to WB-945. (see
    // MediaAsset.assignEncounters() instead) ######
    public Encounter toEncounterDEPRECATED(Shepherd myShepherd) {
        // fairly certain this will *never* happen as code currently stands. this
        // (Annotation) is always new, and
        ...
        if ((sibs == null) || (sibs.size() < 1)) { // no sibs, we make a new Encounter!
            ...
            // this taxonomy only works when its twitter-sourced data cuz otherwise this is
            // just null
        ...
        /*
         * ok, we have sibling Annotations. if one is trivial, we just go for it and
         * replace that one.
         * is this wise? well, if it is the *only* sibling then probably the MediaAsset
         * was attached to the Annotation using legacy (non-IA)
         * methods, and we are "zooming in" on the actual animal. or *one of* the actual
         * animals -- if there are others, they should get added in
         * subsequent iterations of toEncounter().
         * in theory.
         * 
         * the one case that is still to be considered ( TODO ) is when (theoretically)
         * detection *improves* and we will want a new detection to
         * replace a *non-trivial* Annotation. but we arent considering that just now!
         */

        // so now we look for a trivial annot to replace.
        // we currently now allow (via import!) there to be *more than one trivial
        // annotation* on a media asset!
        // as such, this will find the first trivial available and use that. since we
        // have no way to know which annot
        // from detection lines up with which encounter (via the trivial annot), we just
        // randomly replace basically. alas!
        Encounter someEnc = null; // this is in case we fall thru (no trivial annot), we can clone some of this
                                  // for new one
        for (Annotation ann : sibs) {
            ...
            if (someEnc == null)
                someEnc = enc; // use the first one we find to base new one (below) off of, if necessary
        }
        ...
        if (someEnc != null) {
            for (Annotation ann : sibs) {
                ...
                if ((ann.getIAClass() == null || this.getIAClass() == null) ||
                        ann.getIAClass().equals(this.getIAClass())) {
                    break;
                }
                // if these two intersect and have a different detected class they are allowed
                // to reside on the same encounter
        ...
        // if we fall thru, we have no trivial annot, so just get a new Encounter for
        // this Annotation
        ...
        /*
         * NOTE: for now i am snipping out this sibling stuff! youtube-sourced frames
         * used this but now doesnt... here for prosperity...
         * System.out.println(".toEncounter() on " + this +
         * " found no Encounter.... trying to find one on siblings or make one....");
         * List<Annotation> sibs = this.getSiblings();
         * Annotation sourceSib = null;
         * Encounter sourceEnc = null;
         * if (sibs != null) {
         * //we look for one that has an Encounter, favoring the trivial one (so we can
         * replace it) otherwise any will do for (Annotation sib : sibs)
         * {
         ...
         * }
         * 
         * System.out.println(" * sourceSib = " + sourceSib + "; sourceEnc = " +
         * sourceEnc);
         * if (sourceSib == null) return new Encounter(this); //from scratch it is then!
         * 
         * if (sourceSib.isTrivial()) {
         * System.out.println("INFO: annot.toEncounter() replaced trivial " + sourceSib
         * + " with " + this + " on " + sourceEnc);
     ...
     * {
     * if (mediaAsset == null) return "<!-- Annotation.toHtmlElement(): " + this +
     * " has no MediaAsset -->";
    ...
    public Annotation revertToTrivial(Shepherd myShepherd, boolean force)
            throws IOException {
        if (this.isTrivial())
            throw new IOException("Already a trivial Annotation: " + this);
        Encounter enc = this.findEncounter(myShepherd);
        if (enc == null)
    ...
    // creates a new Annotation with the basic properties duplicated (but no
    // "linked" objects, like Features etc)
    ...
    public static boolean isValidViewpoint(String vp) {
        if (vp == null)
            return true; // ?? is this desired behavior?
    ...
    public static ArrayList<Encounter> checkForConflictingIDsforAnnotation(Annotation annot,
        ...
        String filter = "SELECT FROM org.ecocean.Encounter WHERE individual!=null && individual.individualID != \""
    ...
    /*
     * these will update(/create) AnnotationLite.cache for this Annotation note: use
     * sparingly? i.e. should only happen when (related) taxonomy or
    ...
    // they all are chained together; basically no gap between any single *or
    // cluster* of these annots
    // note: this skips all trivial annots, cuz those are always going to intersect
    // everything
    ...
    // note: this basically is "id-equivalence" rather than *content* equivalence,
    // so will not compare semantic similarity of 2 annots
    public boolean equals(final Object o2) {
        ...
        if ((this.id == null) || (two == null) || (two.getId() == null))
            return false;
    ...
    public int hashCode() {
        if (id == null)
            return Util.generateUUID().hashCode(); // random(ish) so we dont get two users with no uuid equals! :/
        return id.hashCode();
    }
}
