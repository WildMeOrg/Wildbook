...
package org.ecocean.timeseries.spatialindex.spatialindex;

public class Region implements IShape, Cloneable
{
  ...
  /*
   * Decide whether Region b is temporally closeto this Region.
   */
  public boolean withinTime(final Region b, double time)
  {
    if ( (this.getLow(org.ecocean.timeseries.core.Point.TIME_DIM) - b.getHigh(org.ecocean.timeseries.core.Point.TIME_DIM)) > time )
      return false;
    if ( (b.getLow(org.ecocean.timeseries.core.Point.TIME_DIM) - this.getHigh(org.ecocean.timeseries.core.Point.TIME_DIM)) > time )
      return false;
    
    return true;
  }
  
  public double getMinimumSpatialDistance(final Region b)
  {
    if (m_pLow.length != b.m_pLow.length) 
    ...
    {
      double x = 0.0;
      if ( cIndex != org.ecocean.timeseries.core.Point.TIME_DIM )
      {
        if (b.m_pHigh[cIndex] < m_pLow[cIndex])
          x = Math.abs(b.m_pHigh[cIndex] - m_pLow[cIndex]);
        else if (m_pHigh[cIndex] < b.m_pLow[cIndex])
          x = Math.abs(b.m_pLow[cIndex] - m_pHigh[cIndex]);
        ret += x * x;
      }
    }
  ...
  public double getMaximumSpatialDistance(final Region b)
  {
    if (m_pLow.length != b.m_pLow.length) 
      throw new IllegalArgumentException("getMinimumDistance: Shape has the wrong number of dimensions.");
    ...
    for (int cIndex = 0; cIndex < m_pLow.length; cIndex++)
    {
      ...
      if ( cIndex != org.ecocean.timeseries.core.Point.TIME_DIM )
      {
        if (b.m_pHigh[cIndex] > m_pHigh[cIndex])
          x = b.m_pHigh[cIndex] - m_pLow[cIndex];
        else 
          x = m_pHigh[cIndex] - b.m_pLow[cIndex];
        ret += x * x;
      }
    }
	...
	public boolean contains(final Point p)
	{
		if (m_pLow.length != p.m_pCoords.length) {
			System.err.println("region dim:" + m_pLow.length + "point dim:" + p.m_pCoords.length);
