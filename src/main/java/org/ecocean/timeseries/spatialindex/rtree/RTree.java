...
package org.ecocean.timeseries.spatialindex.rtree;

import java.util.*;

import java.io.*;

import org.ecocean.timeseries.spatialindex.spatialindex.*;

import org.ecocean.timeseries.spatialindex.storagemanager.*;
  ...
  implements ISpatialIndex
{
  ...
  int m_rootID;
  int m_headerID; // a unique index identifier that identifies this RTree index
  ...
  public RTree(PropertySet ps, IStorageManager sm)
  {
    ...
    if (var != null)
    {
      if (!(var instanceof Integer))
        throw new IllegalArgumentException("Property IndexIdentifier must be an Integer");
      ...
      while (queue.size() != 0)
      {
        ...
        if (first.m_pEntry instanceof Node)
        {
          ...
          for (int cChild = 0; cChild < n.m_children; cChild++)
          {
            ...
            {
              e = 
                  new Data(n.m_pData[cChild], n.m_pMBR[cChild], n.m_pIdentifier[cChild]);
            }
            else
            {
            ...
            int loc = 
              Collections.binarySearch(queue, e2, new NNEntryComparator());
            if (loc >= 0)
              queue.add(loc, e2);
            else
              queue.add((-loc - 1), e2);
  ...
  public PropertySet getIndexProperties()
  {
    ...
    pRet.setProperty("NearMinimumOverlapFactor", 
                     new Integer(m_nearMinimumOverlapFactor));

    // split distribution factor
    pRet.setProperty("SplitDistributionFactor", 
                     new Double(m_splitDistributionFactor));
  ...
  public boolean isIndexValid()
  {
    ...
    while (!st.empty())
    {
      ...
      for (int cDim = 0; cDim < m_dimension; cDim++)
      {
        ...
        {
          tmpRegion.m_pLow[cDim] = 
              Math.min(tmpRegion.m_pLow[cDim], e.m_pNode.m_pMBR[cChild].m_pLow[cDim]);
          tmpRegion.m_pHigh[cDim] = 
              Math.max(tmpRegion.m_pHigh[cDim], e.m_pNode.m_pMBR[cChild].m_pHigh[cDim]);
        }
      }
      ...
      if (e.m_pNode.m_level != 0)
      {
        for (int cChild = 0; cChild < e.m_pNode.m_children; cChild++)
        {
          ValidateEntry tmpEntry = 
            new ValidateEntry(e.m_pNode.m_pMBR[cChild], 
          ...
          {
            int i = 
              ((Integer) nodesInLevel.get(new Integer(tmpEntry.m_pNode.m_level))).intValue();
            nodesInLevel.put(new Integer(tmpEntry.m_pNode.m_level), 
                             new Integer(i + 1));
          }
    ...
    {
      int i1 = 
        ((Integer) nodesInLevel.get(new Integer(cLevel))).intValue();
    ...
    throws IOException
  {
    ...
    ds.flush();
    m_headerID = 
        m_pStorageManager.storeByteArray(m_headerID, bs.toByteArray());
  }
  ...
  protected void insertData_impl(byte[] pData, Region mbr, int id, 
                                 int level, boolean[] overflowTable)
  {
    //assert mbr.getDimension() == m_dimension;
  ...
  protected int writeNode(Node n)
    throws IllegalStateException
  {
    byte[] buffer = null;
  ...
  public String toString()
  {
    ...
    {
      s += 
          "Near minimum overlap factor: " + m_nearMinimumOverlapFactor + "\n" + 
          "Reinsert factor: " + m_reinsertFactor + "\n" + 
          "Split distribution factor: " + m_splitDistributionFactor + "\n";
    }

    s += 
        "Utilization: " + 100 * m_stats.getNumberOfData() / (m_stats.getNumberOfNodesInLevel(0) * 
                                                             m_leafCapacity) + 
        "%" + "\n" + m_stats;
  ...
  class NNComparator
    implements INearestNeighborComparator
  {
