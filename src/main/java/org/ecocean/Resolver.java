 ...
 */
    ...
    public static boolean validType(String t) {
        ...
        return (t.equals(TYPE_RETIRE) || t.equals(TYPE_SPLIT) || t.equals(TYPE_MERGE) ||
                   t.equals(TYPE_NEW));
    }

    public static Resolver retireMarkedIndividual(MarkedIndividual indiv,
        HttpServletRequest request, Shepherd myShepherd) {
        if (indiv == null)
            throw new RuntimeException(
                      "null MarkedIndividual passed to Resolver.retireMarkedIndividual()");
    ...
    public static Resolver mergeMarkedIndividuals(List<MarkedIndividual> indivs,
        HttpServletRequest request, Shepherd myShepherd) {
    ...
    public static Resolver splitMarkedIndividuals(List<MarkedIndividual> indivs,
        HttpServletRequest request, Shepherd myShepherd) {
    ...
    public static Resolver addNewMarkedIndividuals(List<Annotation> anns,
        HttpServletRequest request, Shepherd myShepherd) {
        ...
        for (Annotation ann : anns) {
            ...
            if (enc == null) { // we need an encounter to hold this annotation
                ...
                iss.put("message",
                    "Encounter " + enc.getCatalogNumber() + " is already assigned " +
                    enc.getIndividualID());
        ...
        JSONObject jr = new JSONObject();
/*  this should never happen now, since we create encounters if (encs.size() < 1) {
