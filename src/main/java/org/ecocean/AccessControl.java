/*
   note: in currenct incarnation, "anonymous" has username == null this should be assigned for "anonymous-owned" objects (as opposed to a null
      AccessControl value
 */
package org.ecocean;
...
public class AccessControl implements java.io.Serializable {
    ...
    public static User getUser(final HttpServletRequest request, Shepherd myShepherd) {
        if ((request == null) || (request.getUserPrincipal() == null) ||
            (request.getUserPrincipal().getName() == null)) return null;
    ...
    public static boolean canExport(String context, String exportType, Encounter enc) {
        ...
        if (!isExportEnabled(context, exportType)) {
            System.out.println("INFO: exporter '" + exportType +
                "' not enabled; cannot export enc id=" + enc.getCatalogNumber());
            return false;
        }
        ...
        if (!Util.booleanNotFalse(getExportProperty(context, exportType,
            "collaborationSecurityEnabled"))) return true;
        // we must have pairwise collab enabled, thus:
        if (!Collaboration.securityEnabled(context)) {
            System.out.println(
                "WARNING: export collaborationSecurityEnabled is set true, but system collaboration security is NOT enabled; failing to allow export for type "
                + exportType);
        ...
        if (exportUsername == null) {
            System.out.println("WARNING: export collaborationSecurityEnabled is set true, but no " +
                exportType + ".collaborationUsername was set; failing");
            return false;
        }
    ...
    public static String getExportProperty(String context, String exportType, String label,
        ...
        if (p == null) {
            System.out.println("getExportProperty(" + label +
                ") has no properties; export.properties unavailable?");
            return null;
        }
