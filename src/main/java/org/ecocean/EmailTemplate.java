 ...
 * must only reference remotely hosted images, not use embedded ones.</p>
 *
 * <p>A plain text file is mandatory, but the HTML version is optional. If both are specified, the plain text should be specified first. For HTML
 * emails a MIME-multipart message will be sent using the content of both files. If just the text version is present then a simple plain text email
 * will be sent.</p>
 *
...
public final class EmailTemplate {
    ...
    /**
     * Creates a new email template, using the subject and files provided The files are used to create body text representations for both the plain
     * text and HTML style emails.
     *
     ...
     */
    public EmailTemplate(String subj, File plain, File html, Charset csP, Charset csH, String host,
     ...
     * @throws IOException if any of the input files cannot be loaded
     */
    public EmailTemplate(String subj, File plain, File html)
    throws IOException {
        this.subj = new TemplateFiller(subj);
        this.plainBody = new TemplateFiller(plain);
        this.plainBody.setText(StringEscapeUtils.unescapeHtml4(this.plainBody.getText()));
    ...
    /**
     * Creates a new email template, using the subject and body provided, which will send a plain text email.
     *
    ...
    /**
     * Creates a new email template, using the subj and body provided, which will send a plain text email using the mail host specified.
     *
     ...
     */
    public void replace(String search, File replace)
    throws IOException {
    ...
    /**
     * Searches and replaces one or all occurrences of the specified regular expression search term with the specified replacement.
     *
     * @param search regex search term
     * @param replace regex replacement term
    ...
    /**
     * Resets the template content to the base-state. This allows the template to be re-used multiple times.
     */
    public void reset() {
        subj.reset();
        plainBody.reset();
    ...
    /**
     * Sets the base-state of the email template to the current content. The base state is the state to which the template reverts when the
    ...
    /**
     * Opens the mail session ready to send multiple emails using the same connection, before calling {@code closeSession()} at the end.
     *
    ...
    public boolean openSession(String from) {
        try {
            ...
            if (useStartTLS)
                props.setProperty("mail." + protocol + ".starttls.enable",
                    Boolean.toString(useStartTLS));
     ...
     */
    public int send(String from, Collection<String> to, Collection<String> cc,
        Collection<String> bcc, boolean html) {
    ...
    /**
     * Sends this email to the recipients specified. If an HTML format has been included in the template, then a call with the {@code html} parameter
     * as {@code true} will attempt to send the email as a multipart/alternative MIME type.
     *
     * @param to email recipients
     * @param cc email recipients (CC)
     * @param bcc email recipients (BCC)
     ...
     */
    private int send(Collection<String> to, Collection<String> cc, Collection<String> bcc,
        boolean html) {
     ...
     */
    private Address[] convertAddresses(Collection<String> x)
     ...
     * @return number of successful messages sent
     */
    public boolean sendSingle(String from, Collection<String> to, Collection<String> cc,
        Collection<String> bcc, boolean html) {
    ...
    /**
     * Creates an email in multipart/alternative format to cater for both HTML and text-only email clients.
     *
     * @return Multipart instance
     * @throws MessagingException if a problem occurs while creating the message
     */
    private Multipart createMultipartEmail()
    throws MessagingException {
        ...
        part.setContent(plainBody.getText(),
            String.format("text/plain; charset=\"%s\"", getPlainTextCharset().name()));
        mp.addBodyPart(part);

        // Add HTML content.
        part = new MimeBodyPart();
        part.setContent(htmlBody.getText(),
            String.format("text/html; charset=\"%s\"", getHtmlTextCharset().name()));
        mp.addBodyPart(part);
     ...
     */
    public static File[] resolveTemplatesFromRoot(File path, String baseName)
    throws IOException {
        Objects.requireNonNull(path);
        Objects.requireNonNull(baseName);
        File fP = getTemplateFile(path, baseName, new String[] { ".txt", ".TXT", "" });
        File fH = getTemplateFile(path, baseName,
            new String[] { ".html", ".HTML", ".htm", ".HTM" });
     ...
     * @throws IOException if a problem occurs in locating the template files
     */
    public static File[] resolveTemplatesFromRoot(String path, String baseName)
    throws IOException {
        return resolveTemplatesFromRoot(new File(path), baseName);
    }

    /**
     * Loads a mail template using the specified pair of files, specified as plain text and HTML text in order.
     *
     ...
     */
    public static EmailTemplate load(File fP, File fH, Charset csP, Charset csH)
    throws IOException {
        ...
        if (fH != null) {
            ...
            Pattern pat = Pattern.compile("[<\\s]meta\\s.*\\scharset\\s*=\\s*\"?([^\"]+)\"?",
    ...
    /**
     * Attempts to extract an email subject line from the specified text, which should be prefixed with &quot;SUBJECT:&quot;. If found, the subject
     * line of the email is assigned, and the text returned has the subject line removed.
    ...
    /**
     * Loads a mail template using the specified name as a base for the files. Template files are assumed to have the extensions &quot;.txt&quot;
     * and &quot;.html&quot; (or &quot;.htm&quot;) for plain text and HTML messages respectively. A plain text message must be present to successfully
     * create an {@code EmailTemplate} instance, and an HTML message file can be optionally present.
     *
     * @param fP plain text email template file
     * @param fH HTML text email template file
     ...
     * @throws IOException if a problem occurs in loading the template
     */
    public static EmailTemplate load(File fP, File fH, Charset cs)
    throws IOException {
     ...
     * @throws IOException if a problem occurs in loading the template
     */
    public static EmailTemplate load(File path, String baseName, Charset csP, Charset csH)
    throws IOException {
     ...
     * @throws IOException if a problem occurs in loading the template
     */
    public static EmailTemplate load(File path, String baseName, Charset cs)
    throws IOException {
        File[] f = resolveTemplatesFromRoot(path, baseName);
     ...
     * @throws IOException if a problem occurs in loading the template
     */
    public static EmailTemplate load(String path, String baseName, Charset cs)
    throws IOException {
