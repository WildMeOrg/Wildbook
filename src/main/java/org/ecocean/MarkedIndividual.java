 ...
 * <code>Encounters</code> are added to MarkedIndividual objects as multiple encounters are associated with known marked individuals.
 * <p/>
 *
...
public class MarkedIndividual extends Base implements java.io.Serializable {
    ...
    public void addNameAndComment(Object keyHint, String name, User user, String message) {
        ...
        String commentPrefix = "On " + timeStamp + " " + user.getDisplayName() + " added name " +
            nameStr;
...
///////////////// TODO other setters!!!!  e.g. addNameByKey(s)
    ...
    public void addIncrementalProjectId(Project project) {
        if (project != null) {
            if (!hasNameKey(project.getProjectIdPrefix())) {
                ...
                try {
                    addNameByKey(project.getProjectIdPrefix(),
                        project.getNextIncrementalIndividualId());
                ...
                System.out.println("[ERROR]: Project Id not added to Individual " + getId() +
    ...
    public String refreshThumbnailUrl(Shepherd myShepherd, HttpServletRequest req)
    throws org.datanucleus.api.rest.orgjson.JSONException {
        org.datanucleus.api.rest.orgjson.JSONObject thumbJson = getExemplarThumbnail(myShepherd,
            req);
 ...
 */
    ...
    public Vector returnEncountersWithGPSData(boolean useLocales, boolean reverseOrder,
        ...
        for (int c = 0; c < myEncs.length; c++) {
            ...
            try {
                ...
                if ((temp.getDWCDecimalLatitude() != null) &&
                    (temp.getDWCDecimalLongitude() != null)) {
                ...
                System.out.println(
                    "Hit exception in MarkedIndividual.returnEncountersWithGPSData for Encounter: "
                    + catalogNumber);
                e.printStackTrace();
            }
        }
    ...
    public double averageLengthInYear(int year) {
        ...
        for (int c = 0; c < encounters.size(); c++) {
            Encounter temp = (Encounter)encounters.get(c);
            if ((temp.getYear() == year) &&
                ((temp.getSizeAsDouble() != null) && (temp.getSize() > 0))) {
                total += temp.getSize();
                numLengths++;
            }
        }
        ...
        for (int c = 0; c < encounters.size(); c++) {
            ...
            if (temp.getYear() == year) {
                if ((temp.getSizeAsDouble() != null) && (temp.getSize() > 0)) {
                    if ((temp.getSizeGuess().equals("directly measured")) ||
                        ((allowGuideGuess) &&
                        (temp.getSizeGuess().equals("guide/researcher's guess")))) {
                        total += temp.getSize();
                        numLengths++;
                    }
                }
    ...
    public boolean wasSightedInPeriod(int m_startYear, int m_startMonth, int m_endYear,
        int m_endMonth) {
        ...
        for (int c = 0; c < encounters.size(); c++) {
            ...
            if ((temp.getDateInMilliseconds() != null) &&
                (temp.getDateInMilliseconds() >= gcMin.getTimeInMillis()) &&
                (temp.getDateInMilliseconds() <= gcMax.getTimeInMillis())) {
                return true;
            }
        }
        return false;
    ...
    public boolean wasSightedInPeriod(int m_startYear, int m_startMonth, int m_startDay,
        int m_endYear, int m_endMonth, int m_endDay, String locCode) {
        int endYear = m_endYear;
        int endMonth = m_endMonth;
        int endDay = m_endDay;
        int startYear = m_startYear;
        ...
        for (int c = 0; c < encounters.size(); c++) {
            Encounter temp = (Encounter)encounters.get(c);
            if ((temp.getLocationID() != null) && (!temp.getLocationID().trim().equals("")) &&
                (temp.getLocationID().trim().equals(locCode))) {
                if ((temp.getDateInMilliseconds() != null) &&
                    (temp.getDateInMilliseconds() >= gcMin.getTimeInMillis()) &&
                    (temp.getDateInMilliseconds() <= gcMax.getTimeInMillis())) {
                    return true;
    ...
    public boolean wasSightedInPeriod(int m_startYear, int m_startMonth, int m_startDay,
        int m_endYear, int m_endMonth, int m_endDay) {
        int endYear = m_endYear;
        int endMonth = m_endMonth;
        int endDay = m_endDay;
        int startYear = m_startYear;
    ...
    public boolean wasSightedInPeriodLeftOnly(int m_startYear, int m_startMonth, int m_endYear,
        int m_endMonth) {
        int endYear = m_endYear;
        int endMonth = m_endMonth;
        int startYear = m_startYear;
 ...
 */
    ...
    public boolean wasSightedInLocationCode(String locationCode) {
        for (int c = 0; c < encounters.size(); c++) {
            try {
                Encounter temp = (Encounter)encounters.get(c);
                if ((temp.getLocationID() != null) && (!temp.getLocationID().trim().equals("")) &&
                    (temp.getLocationID().trim().equals(locationCode))) {
                    return true;
                }
    ...
    public ArrayList<String> participatesInTheseVerbatimEventDates() {
        ...
        for (int c = 0; c < encounters.size(); c++) {
            Encounter temp = (Encounter)encounters.get(c);
            if ((temp.getVerbatimEventDate() != null) &&
                (!vbed.contains(temp.getVerbatimEventDate()))) {
    ...
    public boolean wasSightedInVerbatimEventDate(String ved) {
        for (int c = 0; c < encounters.size(); c++) {
            Encounter temp = (Encounter)encounters.get(c);
            if ((temp.getVerbatimEventDate() != null) &&
                (temp.getVerbatimEventDate().equals(ved))) {
                return true;
            }
        }
        return false;
    ...
    public long getEarliestSightingTime() {
        ...
        for (int c = 0; c < encounters.size(); c++) {
            Encounter temp = (Encounter)encounters.get(c);
            if ((temp.getDateInMilliseconds() != null) &&
                (temp.getDateInMilliseconds() < lowestTime) && (temp.getYear() > 0))
    ...
    public double avgLengthInPeriod(int m_startYear, int m_startMonth, int m_endYear,
        ...
        for (int c = 0; c < encounters.size(); c++) {
            ...
            } else if ((temp.getYear() == startYear) && (temp.getYear() < endYear) &&
                (temp.getMonth() >= startMonth)) {
                if ((temp.getSizeAsDouble() != null) && (temp.getSize() > 0)) {
                    avgLength += temp.getSize();
                    numMeasurements++;
                }
            } else if ((temp.getYear() > startYear) && (temp.getYear() == endYear) &&
                (temp.getMonth() <= endMonth)) {
                if ((temp.getSizeAsDouble() != null) && (temp.getSize() > 0)) {
                    avgLength += temp.getSize();
                    numMeasurements++;
                }
            } else if ((temp.getYear() >= startYear) && (temp.getYear() <= endYear) &&
                (temp.getMonth() >= startMonth) && (temp.getMonth() <= endMonth)) {
                if ((temp.getSizeAsDouble() != null) && (temp.getSize() > 0)) {
                    avgLength += temp.getSize();
                    numMeasurements++;
                }
    ...
    public Double getAverageMeasurementInPeriod(int m_startYear, int m_startMonth, int m_endYear,
        int m_endMonth, String measurementType) {
    ...
    public Double getAverageBiologicalMeasurementInPeriod(int m_startYear, int m_startMonth,
        int m_endYear, int m_endMonth, String measurementType) {
        double avgMeasurement = 0;
        int numMeasurements = 0;
        int endYear = m_endYear;
        int endMonth = m_endMonth;
                    ...
                    if (temp.hasMeasurement(measurementType)) {
                        ...
                        if ((enc.getYear() > startYear) && (enc.getYear() < endYear)) {
                            if (temp.getBiologicalMeasurement(measurementType) != null) {
                                avgMeasurement += temp.getBiologicalMeasurement(
                                    measurementType).getValue();
                                numMeasurements++;
                            }
                        } else if ((enc.getYear() == startYear) && (enc.getYear() < endYear) &&
                            (enc.getMonth() >= startMonth)) {
                            if (temp.getBiologicalMeasurement(measurementType) != null) {
                                avgMeasurement += temp.getBiologicalMeasurement(
                                    measurementType).getValue();
                                numMeasurements++;
                            ...
                            (enc.getMonth() >= startMonth) && (enc.getMonth() <= endMonth)) {
                            if (temp.getBiologicalMeasurement(measurementType) != null) {
                                avgMeasurement += temp.getBiologicalMeasurement(
                                    measurementType).getValue();
                                numMeasurements++;
        ...
        for (int c = 0; c < encounters.size(); c++) {
            ...
            if (temp.getLocationCode().startsWith(locCode)) {
                if ((temp.getDateInMilliseconds() != null) &&
                    ...
                    if ((temp.getNumRightSpots() > 0) && (temp.getNumSpots() > 0)) {
                        leftRightTogether = true;
                    }
                }
            }
        }
    ...
    public Float getMaxDistanceBetweenTwoSightings() {
        ...
        if (numEncs > 1) {
            for (int y = 0; y < numEncs; y++) {
                ...
                if ((thisEnc.getLatitudeAsDouble() != null) &&
                    (thisEnc.getLongitudeAsDouble() != null)) {
                    for (int z = (y + 1); z < numEncs; z++) {
                        Encounter nextEnc = (Encounter)encounters.get(z);
                        if ((nextEnc.getLatitudeAsDouble() != null) &&
                            (nextEnc.getLongitudeAsDouble() != null)) {
                            try {
                                Float tempMaxDistance = distFrom(new Float(
                                    thisEnc.getLatitudeAsDouble()),
                                    new Float(thisEnc.getLongitudeAsDouble()),
                                    new Float(nextEnc.getLatitudeAsDouble()),
                                    new Float(nextEnc.getLongitudeAsDouble()));
                                ...
                                e.printStackTrace();
                                System.out.println(
                                    "Hit an NPE when calculating distance between: " +
                                    thisEnc.getCatalogNumber() + " and " +
                                    nextEnc.getCatalogNumber());
                            }
                        }
                    }
                }
            ...
            for (int y = 0; y < numEncs; y++) {
                ...
                for (int z = (y + 1); z < numEncs; z++) {
                    ...
                    if ((thisEnc.getDateInMilliseconds() != null) &&
                        (nextEnc.getDateInMilliseconds() != null)) {
                        long tempMaxTime = Math.abs(thisEnc.getDateInMilliseconds().longValue() -
                            nextEnc.getDateInMilliseconds().longValue());
 ...
 */
    ...
    public String getFomattedMSMarkersString(String[] loci) {
        ...
        for (int i = 0; i < numLoci; i++) {
            ...
            if ((alleles.size() > 0) && (alleles.get(0) != null)) {
                sb.append(alleles.get(0) + " ");
            } else { sb.append("--- "); }
            if ((alleles.size() >= 2) && (alleles.get(1) != null)) {
                sb.append(alleles.get(1) + " ");
            } else { sb.append("--- "); }
        }
        return sb.toString();
    }
    ...
    public Float getMinDistanceBetweenTwoMarkedIndividuals(MarkedIndividual otherIndy) {
        ...
        if ((encounters != null) && (encounters.size() > 0) &&
            (otherIndy.getEncounters() != null) && (otherIndy.getEncounters().size() > 0)) {
            int numEncs = encounters.size();
                    ...
                    if ((thisEnc.getLatitudeAsDouble() != null) &&
                        ...
                        for (int z = 0; z < numOtherEncs; z++) {
                            ...
                            if ((nextEnc.getLatitudeAsDouble() != null) &&
                                (nextEnc.getLongitudeAsDouble() != null)) {
                                try {
                                    Float tempMinDistance = distFrom(new Float(
                                        thisEnc.getLatitudeAsDouble()),
                                        new Float(thisEnc.getLongitudeAsDouble()),
                                        new Float(nextEnc.getLatitudeAsDouble()),
                                        new Float(nextEnc.getLongitudeAsDouble()));
...
// Returns a somewhat rest-like JSON object containing the metadata
    ...
    // Returns a somewhat rest-like JSON object containing the metadata
    public JSONObject uiJson(HttpServletRequest request, boolean includeEncounters)
    throws JSONException {
    ...
    public String getUrl(HttpServletRequest request) {
        return request.getScheme() + "://" + CommonConfiguration.getURLLocation(request) +
                   "/individuals.jsp?number=" + this.getIndividualID();
    }
    ...
    public ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> getExemplarImages(
        Shepherd myShepherd, HttpServletRequest req)
    throws JSONException {
        return getExemplarImages(myShepherd, req, 5);
    }

    public ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> getExemplarImages(
        Shepherd myShepherd, HttpServletRequest req, int numResults)
    throws JSONException {
        return getExemplarImages(myShepherd, req, numResults, "_mid");
    }

    public ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> getExemplarImages(
        Shepherd myShepherd, HttpServletRequest req, int numResults, String imageSize)
    throws JSONException {
        ...
        List<MediaAsset> profilePhotos = myShepherd.getKeywordPhotosForIndividual(this,
            kwReadableNames, 5);
        ...
        if (profilePhotos == null || profilePhotos.size() < numResults) {
            ...
            List<MediaAsset> otherPhotos = myShepherd.getKeywordPhotosForIndividual(this,
                noKeywordNames, (numResults - assets.size()));
        ...
        for (MediaAsset ma : assets) {
            if (ma != null) {
                ...
                if ((j != null) && (ma.getMimeTypeMajor() != null) &&
                    (ma.getMimeTypeMajor().equals("image"))) {
    ...
    public ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> getExemplarImagesWithKeywords(
        HttpServletRequest req, List<String> kwNames)
    throws JSONException {
        ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> al = new ArrayList<org.datanucleus.api.rest.orgjson.JSONObject>();
    ...
    public ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> getBestKeywordPhotos(
        HttpServletRequest req, List<String> kwNames, Shepherd myShepherd)
    throws JSONException {
        return getBestKeywordPhotos(req, kwNames, false, myShepherd);
    }

    public ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> getBestKeywordPhotos(
        HttpServletRequest req, List<String> kwNames, boolean tryNoKeywords, Shepherd myShepherd)
    throws JSONException {
        ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> al = new ArrayList<org.datanucleus.api.rest.orgjson.JSONObject>();
        List<MediaAsset> assets = new ArrayList<MediaAsset>();
    ...
    public org.datanucleus.api.rest.orgjson.JSONObject getExemplarImage(Shepherd myShepherd,
        HttpServletRequest req)
    throws JSONException {
        ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> al = getExemplarImages(myShepherd,
            req, 0);
    ...
    public org.datanucleus.api.rest.orgjson.JSONObject getExemplarThumbnail(Shepherd myShepherd,
        HttpServletRequest req)
    throws org.datanucleus.api.rest.orgjson.JSONException {
        ArrayList<org.datanucleus.api.rest.orgjson.JSONObject> al = getExemplarImages(myShepherd,
            req, 0, "_thumb");
    ...
    public String getExemplarPhotographer() {
        for (Encounter enc : this.getDateSortedEncounters()) {
            if ((enc.getDynamicPropertyValue("PublicView") == null) ||
                (enc.getDynamicPropertyValue("PublicView").equals("Yes"))) {
                if ((enc.getDynamicPropertyValue("ShowPhotographer") == null) ||
                    (enc.getDynamicPropertyValue("ShowPhotographer").equals("Yes"))) {
    ...
    public String collaborationLockHtml(HttpServletRequest request) {
        ...
        return "<div class=\"row-lock " + collabClass +
                   " collaboration-button\" data-multiuser=\"" + data + "\">&nbsp;</div>";
    }
    ...
    public static List<MarkedIndividual> findByNames(Shepherd myShepherd, String regex,
        ...
        // Simplified query since taxonomy filtering is now handled in findNameIds
        String jdoql =
            "SELECT FROM org.ecocean.MarkedIndividual WHERE names.id == "
            + String.join(" || names.id == ", nameIds);
    ...
    public static MarkedIndividual withName(Shepherd myShepherd, String name, String genus,
        ...
        if (inds.size() > 1)
            System.out.println("WARNING! MarkedIndividual.withName called for name " + name +
                ". THERE ARE " + inds.size() +
                " INDIVIDUALS WITH THIS NAME AND WE'RE RETURNING ONLY ONE.");
        return inds.get(0);
    }
    ...
    public static List<String> findNameIds(Shepherd myShepherd, String regex) {
        return findNameIds(myShepherd, regex, null, null);
    }
    ...
    public static boolean initNamesCache(final Shepherd myShepherd) {
        if ((NAMES_CACHE != null) && (NAMES_CACHE.size() > 0) && (NAMES_KEY_CACHE != null) &&
            (NAMES_KEY_CACHE.size() > 0)) return false;
    ...
    public static Map<Integer, String> updateNamesCache(final Shepherd myShepherd) {
        ...
        for (Object m : c) {
            currentIndy++;
            myShepherd.setAction("MarkedIndividual.initNamesCache_" + currentIndy + "_" +
                numIndividuals);
            MarkedIndividual ind = (MarkedIndividual)m;
            if (ind.names == null) continue;
            NAMES_CACHE.put(ind.names.getId(),
                ind.getId() + ";" + String.join(";", ind.names.getAllValues()).toLowerCase());
            NAMES_KEY_CACHE.put(ind.names.getId(),
                ind.getId() + ";" + String.join(";", ind.getNameKeys()).toLowerCase());
        }
        query.closeAll();
        return NAMES_CACHE;
    }
    ...
    public void mergeIndividual(MarkedIndividual other, String username, Shepherd myShepherd) {
        ...
        String filter =
            "select from org.ecocean.scheduled.ScheduledIndividualMerge where primaryIndividual.individualID =='"
    ...
    public String getMergedComments(MarkedIndividual other, String username) {
        ...
        mergedComments += "<p>This individual merged with individual " + other.getIndividualID() +
    ...
    public void mergeAndThrowawayIndividual(MarkedIndividual other, String username,
        Shepherd myShepherd) {
