    ...
    public String getDisplayName(Object keyHint, HttpServletRequest request, Shepherd myShepherd) {
        ...
        if (request!=null&&request.getUserPrincipal()!=null) {
          ...
          try {
            ...
            if (user!=null) {
              String projectContextId = null; // short circuit this because kitizen doesn't use project, and User.java doesn't yet need to be modified to include all of this: user.getProjectIdForPreferredContext();
              if (Util.stringExists(projectContextId)) {
                Project project = null; // short circuit this because kitizen doesn't use project, and Shepherad.java doesn't yet need to be modified to include all of this: nameShepherd.getProject(projectContextId);
                if (project!=null) {
  ...
  **/
  ...
  public String getExemplarPhotographer() {
    ...
    public void mergeIndividual(MarkedIndividual other, String username, Shepherd myShepherd) {
      ...
      List<SocialUnit> units= null; // short circuit this because kitizen doesn't use social units, and Shepherd.java doesn't yet need to be modified to include all of this:myShepherd.getAllSocialUnitsForMarkedIndividual(other);
      if(units!=null && units.size()>0) {
        for(SocialUnit unit:units) {
          Membership member= null; // short circuit this because kitizen doesn't use social units, and user.java doesn't yet need to be modified to include all of this:unit.getMembershipForMarkedIndividual(other);
          if(member!=null){
