...
public class YouTube {
    ...
    public static void init(String context) {
        ...
        Properties googleProps = org.ecocean.ShepherdProperties.getProperties(
            "googleKeys.properties", "");
        ...
        youtubeOauthCreds = new com.google.api.services.youtube.YouTube.Builder(HTTP_TRANSPORT,
    ...
    public static JSONObject simpleInfo(String id)
    throws RuntimeException, MalformedURLException, IOException, NoSuchAlgorithmException,
        InvalidKeyException {
        if (!validId(id)) return null;
        URL url = new URL("https://www.youtube.com/oembed?url=http://www.youtube.com/watch?v=" +
            id + "&format=json");
        return RestClient.get(url);
        // TODO handle 404 for bad ids, etc....
    }

    /* /usr/local/bin/youtube_get.sh ID tempdir will generate:
       -rw-rw-r-- 1 jon jon   253156 Sep  8 00:10 V_ozofiGPJo.jpg
       -rw------- 1 jon jon    30282 Sep  8 00:10 V_ozofiGPJo.info.json
       -rw-rw-r-- 1 jon jon 36398939 Aug 23 08:13 V_ozofiGPJo.mp4 use .json to populate ma.metadata.detailed  -- note: set .detailed = { processing:
          true } before background grab
     */
    ...
    public static List<File> grab(String id, File targetDir) {
        ...
        try {
            ...
            throw new RuntimeException("YouTube.grab(" + id + ", " + targetDir + ") failed: " +
                    ex.toString());
        }
        File[] flist = targetDir.listFiles();
        if ((flist == null) || (flist.length < 1))
            throw new RuntimeException(targetDir + " is empty; grab failed");
    ...
    public static List<File> extractFrames(File videoFile, File targetDir) {
        ...
        String[] cmd = new String[] {
            "/usr/local/bin/youtube_extract.sh", videoFile.toString(), targetDir.toString()
        };
        ProcessBuilder pb = new ProcessBuilder();
        pb.command(cmd);
        ...
        try {
            ...
            throw new RuntimeException("YouTube.extractFrames(" + videoFile + ", " + targetDir +
                    ") failed: " + ex.toString());
        }
        File[] flist = targetDir.listFiles();
        if ((flist == null) || (flist.length < 1))
            throw new RuntimeException(targetDir + " is empty; extractFrames failed");
        return Arrays.asList(flist);
    }
 ...
 */
    ...
    public static List<SearchResult> searchByKeyword(String keyword, long pubAfter,
        String context) {                                                                              // pubAfter is ms since epoch
        ...
        try {
            ...
            com.google.api.services.youtube.YouTube.Search.List search =
                youtubeAPIKey.search().list("id,snippet");
            search.setKey(apiKey);
            search.setQ(keyword);
            if (pubAfter > 0)
                search.setPublishedAfter(new com.google.api.client.util.DateTime(pubAfter));
            ...
            search.setFields(
                "items(id/kind,id/videoId,snippet/title,snippet/thumbnails/default/url,snippet/description,snippet/publishedAt)");
            ...
            System.err.println("ERROR: YouTube.searchByKeyword() had a service error: " +
                e.getDetails().getCode() + " : " + e.getDetails().getMessage());
        } catch (IOException e) {
            System.err.println("ERROR: YouTube.searchByKeyword() had an IOException: " +
...
// commentSnippet.setVideoId(videoId);
            ...
            com.google.api.services.youtube.YouTube.CommentThreads.Insert
                commentThreadsInsertRequest = youtubeOauthCreds.commentThreads().insert(
                parameters.get("part").toString(), commentThread);
...
// System.out.println(response);
    ...
    /* public static String sendReply(String commentId, String commentToPost) {  //pubAfter is ms since epoch if (!isActive2()) throw new
       RuntimeException("YouTube API refresh token not active (invalid token?)");
       ...
       //          String parentId = firstComment.getId();
         ...
         com.google.api.services.youtube.YouTube.Comments.Insert commentsInsertRequest = youtube.comments().insert(parameters.get("part").toString(),
            comment);
    ...
    public static void postOccurrenceMessageToYouTubeIfAppropriate(String message, Occurrence occur,
        Shepherd myShepherd, String context) {
        ...
        if ((occur.getSocialMediaSourceID() != null) &&
            (occur.hasMediaAssetFromRootStoreType(myShepherd, AssetStoreType.YouTube))) {
            if (!isActive()) init(context);
            System.out.println(
                "This occurrence has a YouTube Media Asset, so let's try to post to the OP the message I was given: "
                + message);
    ...
    /**
     * Returns a long String of comments and replies for an Occurrence that was derived from a YouTube video. Use this String for generic NLP
     * processing
     *
    ...
    /**
     * Returns an array of comments and replies for an Occurrence that was derived from a YouTube video. Use this array for fine-grained NLP
     * processing.
     *
     * @param occur The Occurrence that derived from a YouTube video.
     * @param context Standard context used in Wildbook.
    ...
    public static List<CommentThread> getVideoCommentsList(Occurrence occur, String context) { // pubAfter is ms since epoch
        ...
        try {
            ...
            CommentThreads.List commentThreadsListByVideoIdRequest =
                youtubeOauthCreds.commentThreads().list(parameters.get("part").toString());
            ...
            CommentThreadListResponse commentsResponse =
                commentThreadsListByVideoIdRequest.execute();
    ...
    public static String getVideoDescription(Occurrence occur, Shepherd myShepherd) {
        ...
        if ((occur != null) && (occur.hasMediaAssetFromRootStoreType(myShepherd,
            AssetStoreType.YouTube))) {
                ...
                if (assets != null) {
                    ...
                    if ((parentRoot.getMetadata() != null) &&
                        ...
                        if (parentRoot.getMetadata().getData().optJSONObject("detailed") != null) {
                            desc = parentRoot.getMetadata().getData().getJSONObject(
                                "detailed").optString("description", "[no description]");
                        }
                    }
                }
            }
        ...
        if ((occur != null) && (occur.hasMediaAssetFromRootStoreType(myShepherd,
            ...
            if ((occur.getEncounters() != null) && (occur.getEncounters().size() > 0)) {
                ...
                if (assets != null) {
                    ...
                    if ((parentRoot.getMetadata() != null) &&
                        (parentRoot.getMetadata().getData() != null)) {
                        if (parentRoot.getMetadata().getData().getJSONObject(
                            "detailed").optJSONArray("tags") != null) {
                            tags = parentRoot.getMetadata().getData().getJSONObject(
                                "detailed").getJSONArray("tags").toString();
                        }
                    }
                }
            }
                ...
                if (assets != null) {
                    ...
                    if ((parentRoot.getMetadata() != null) &&
                        ...
                        if (parentRoot.getMetadata().getData().optJSONObject("basic") != null) {
                            title = parentRoot.getMetadata().getData().getJSONObject(
                                "basic").optString("title", "[unknown]");
                        }
                    }
                }
            }
    ...
    public static String getVideoPublishedAt(Occurrence occur, String context) { // pubAfter is ms since epoch
        ...
        try {
            ...
            com.google.api.services.youtube.YouTube.Videos.List listVideosRequest =
                youtubeAPIKey.videos().list(parameters.get("part").toString());
            ...
            e.printStackTrace();
            System.err.println("There was a service error in publishedAt: " +
                e.getDetails().getCode() + " : " + e.getDetails().getMessage());
        } catch (Throwable t) {
            t.printStackTrace();
        }
    ...
    /*
     * Given a MediaAsset from a YouTubeAssetStore, annotate the Encounters of its derived Occurrence with any Encounter.locationID and Encounter date
     * information that can be extracted from the title, comments, tags, and descriptions, including replies to our own automated comments.
     *
     * This is the simple form of the method for calling without megtrics assessment.
     *
     *
     */
    public static void annotateChildrenOfYouTubeMediaAssetWithDateLocation(MediaAsset ma,
        String rootDir, Shepherd myShepherd, String context) {
        annotateChildrenOfYouTubeMediaAssetWithDateLocation(ma, rootDir, myShepherd, context,
            new AtomicInteger(0), new AtomicInteger(0), new AtomicInteger(0), new AtomicInteger(0),
            new AtomicInteger(0), new ArrayList<MediaAsset>(), new ArrayList<MediaAsset>(), true,
     ...
     * information that can be extracted from the title, comments, tags, and descriptions, including replies to our own automated comments.
     *
     * This method includes the ability to pass in variables for counting/statistics purposes and to test the method with the ability to not persist
     * changes.
     *
    ...
    public static String annotateChildrenOfYouTubeMediaAssetWithDateLocation(MediaAsset ma,
        String rootDir, Shepherd myShepherd, String context, AtomicInteger numVideosWithID,
        AtomicInteger numVideos, AtomicInteger numUncuratedVideos, AtomicInteger numCommentedVideos,
        AtomicInteger numCommentedVideosReplies, ArrayList<MediaAsset> goodDataVideos,
        ArrayList<MediaAsset> poorDataVideos, boolean persistDifferences,
        AtomicInteger numDatesFound, AtomicInteger numLocationIDsFound) {
        ...
        if ((ma.getMetadata() != null)) {
            ...
            if (md.getData() != null) {
                ...
                String videoID = ma.getMetadata().getData().getJSONObject("detailed").optString(
                    "id");
                ...
                if (md.getData().optJSONObject("basic") != null) {
                    videoTitle = md.getData().getJSONObject("basic").optString("title").replaceAll(
                ...
                if (videoDescription.length() > 1000) {
                    videoDescriptionShort = videoDescription.substring(0, 1000);
                }
                // video tags
                ...
                if (md.getData().getJSONObject("detailed") != null) {
                    videoDescription = md.getData().getJSONObject("detailed").optString(
                        "description").replaceAll(",", " ").replaceAll("\n", " ").replaceAll("'",
                        "").replaceAll("\"", "").replaceAll("’", "").replaceAll("′",
                        "").toLowerCase();
                    videoTags = md.getData().getJSONObject("detailed").getJSONArray(
                ...
                // JDOQL query
                String qFilter =
                    "SELECT FROM org.ecocean.Encounter WHERE (occurrenceRemarks.indexOf('" +
                    videoID + "') != -1)";
                ...
                for (int y = 0; y < numEncs; y++) {
                    ...
                    if ((enc.getState() != null) &&
                        ((enc.getState().equals("approved")) ||
                        (enc.getState().equals("unidentifiable")))) {
                ...
                if (numEncs > 0) {
                    ...
                    if (enc.getDate() != null)
                        currentDate = enc.getDate().replaceAll("Unknown", "");
                    if (enc.getLocationID() != null)
                        currentLocationID = enc.getLocationID().replaceAll("None", "");
                        ...
                        if ((comments == null) || (comments.size() == 0)) {
                            ...
                            for (int f = 0; f < numComments; f++) {
                                ...
                                if ((commentSnippet != null) &&
                                    (commentSnippet.getAuthorDisplayName() != null)) {
                                ...
                                videoComments += "<li style=\"" + style + "\">" + authorName +
                                    ": " +
                                    DetectTranslate.translateIfNotEnglish(
                                    topLevelComment.getSnippet().getTextDisplay());

                                videoCommentsClean += DetectTranslate.translateIfNotEnglish(
                                    topLevelComment.getSnippet().getTextDisplay()).toLowerCase() +
                                    " ";
                                if (ct.getReplies() != null) {
                                    ...
                                    if (numReplies > 0) {
                                        ...
                                        for (int g = 0; g < numReplies; g++) {
                                            ...
                                            videoComments += "<li>" +
                                                DetectTranslate.translateIfNotEnglish(
                                                reply.getSnippet().getTextDisplay()) + "</li>";
                                            videoCommentsClean +=
                                                DetectTranslate.translateIfNotEnglish(
                                                reply.getSnippet().getTextDisplay()).toLowerCase() +
                                                " ";
                                        }
                                        videoComments += "</ul>\n";
                                    }
                    ...
                    sbOriginalText.append(videoTitle + " " + videoDescription + " " + videoTags +
                        " " + videoCommentsClean);
                    ...
                    sb.append(videoTitle + " " + videoDescription + " " + videoTags + " " +
                        videoCommentsClean);

                    // get video date with SUTime
                    String newDetectedDate = "";
                    try {
                        newDetectedDate = SUTime.parseDateStringForBestDate(rootDir, sb.toString(),
                    ...
                    if (persistDifferences) {
                        ...
                        for (int y = 0; y < numEncs; y++) {
                            ...
                            // chosenStyleDate+="year: "+thisEnc.getYear()+";millis:"+thisEnc.getDateInMilliseconds()+";locationID:
                            ...
                            if ((newLocationID != null) && (!newLocationID.trim().equals(""))) {
                                ...
                                if ((thisEnc.getLocationID() == null) ||
                                    (thisEnc.getLocationID().trim().equals("")) ||
                                    (thisEnc.getLocationID().trim().equals("None"))) {
                                    thisEnc.setLocationID(newLocationID);
                                    madeAChange = true;
                                } else if (!thisEnc.getLocationID().trim().equals(
                                    newLocationID.trim())) {
                                    ...
                                    if (newLocationID.trim().startsWith(
                                        thisEnc.getLocationID().trim())) {
                                        thisEnc.setLocationID(newLocationID.trim());
                                        madeAChange = true;
                                    }
                                    // if the Encounter is not yet approved, then we can reset it as well since it's uncurated and may have been
                                    // incorrectly detected with older values
                                    else if ((thisEnc.getState() != null) &&
                                        (thisEnc.getState().equals("auto_sourced"))) {
                                        thisEnc.setLocationID(newLocationID.trim());
                                        madeAChange = true;
                                    }
                                }
                            ...
                            if ((newDetectedDate != null) && (!newDetectedDate.trim().equals(""))) {
                                ...
                                if ((thisEnc.getDateInMilliseconds() == null) ||
                                    (thisEnc.getYear() <= 0)) {
                    ...
                    resultsHTML =
                        "<tr><td><a target=\"_blank\" href=\"https://www.whaleshark.org/occurrence.jsp?number="
                        + occurID + "\">" + occurID +
                        "</a></td><td><a target=\"_blank\" href=\"https://www.youtube.com/watch?v="
                        + videoID + "\">" + videoID + "</a></td><td>" + currentDate +
                        "</td><td><p style=\"" + chosenStyleDate + "\">" + newDetectedDate +
                        "</p></td><td>" + currentLocationID + "</td><td><p style=\"" +
                        chosenStyleLocation + "\">" + newLocationID + "</p></td><td>" + videoTitle +
                        "</td><td>" + videoDescription + "</td><td>" + videoComments + "</td><td>" +
                        videoCommentsClean + "<br><br>LocID Words: " + locIDWords +
                        "</br></br></td><td>" + relativeDate + "</td></tr>";
                }
