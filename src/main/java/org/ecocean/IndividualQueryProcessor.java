...
public class IndividualQueryProcessor {
  ...
  //filter for location------------------------------------------
    ...
    if(request.getParameter("unidentifiable")==null) {
      if(filter.equals(SELECT_FROM_ORG_ECOCEAN_INDIVIDUAL_WHERE)){filter+="!enc.unidentifiable";}
      else{filter+=" && !enc.unidentifiable";}
    ...
    if(request.getParameter("approved")==null) {
      if(filter.equals(SELECT_FROM_ORG_ECOCEAN_INDIVIDUAL_WHERE)){filter+="!enc.approved";}
      else{filter+=" && !enc.approved";}
    ...
    if(request.getParameter("unapproved")==null) {
      if(filter.equals(SELECT_FROM_ORG_ECOCEAN_INDIVIDUAL_WHERE)){filter+="(!enc.approved && !enc.unidentifiable)";}
      else{filter+=" && (!enc.approved && !enc.unidentifiable)";}
  ...
  //------------------------------------------------------------------
    ...
    if((keywords!=null)&&(!keywords[0].equals("None"))){
          ...
          int kwLength=keywords.length;
            ...
            locIDFilter+=" )";
            if(filter.equals(SELECT_FROM_ORG_ECOCEAN_INDIVIDUAL_WHERE)){filter+="images.contains(photo) && photo.keywords.contains(word) && "+locIDFilter;}
            else{
              if(filter.indexOf("enc.images.contains(photo)")==-1){filter+=" && images.contains(photo)";}
      ...
      ArrayList<String> markers=myShepherd.getAllLoci();
        ...
        for(int h=0;h<numMarkers;h++){
            ...
            if(request.getParameter(marker)!=null){
              ...
              if(filter.equals(SELECT_FROM_ORG_ECOCEAN_INDIVIDUAL_WHERE)){filter+="enc.tissueSamples.contains(dce) && dce.analyses.contains(msanalysis) && msanalysis.loci.contains("+marker+") && "+locIDFilter;}
              else{
                if(filter.indexOf("enc.tissueSamples.contains(dce)")==-1){filter+=" && enc.tissueSamples.contains(dce)";}
    ...
    if((haplotypes!=null)&&(!haplotypes[0].equals("None"))){
          ...
          int kwLength=haplotypes.length;
            ...
            locIDFilter+=" )";
            if(filter.equals(SELECT_FROM_ORG_ECOCEAN_INDIVIDUAL_WHERE)){filter+="enc.tissueSamples.contains(dce) && dce.analyses.contains(analysis) && "+locIDFilter;}
            else{
              if(filter.indexOf("enc.tissueSamples.contains(dce)")==-1){filter+=" && enc.tissueSamples.contains(dce)";}
