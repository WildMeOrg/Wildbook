...
public class IdentityServiceLog implements java.io.Serializable {
    ...
    public IdentityServiceLog(String taskID, String[] objectIDs, String serviceName,
        String serviceJobID, JSONObject jstatus) {
        this(taskID, objectIDs, serviceName, serviceJobID, jstatus.toString());
    }

    public IdentityServiceLog(String taskID, String serviceName, String serviceJobID,
        JSONObject jstatus) {
        this(taskID, null, serviceName, serviceJobID, jstatus.toString());
    }

    public IdentityServiceLog(String taskID, String[] objectIDs, String serviceName,
        String serviceJobID, String status) {
        this.taskID = taskID;
    ...
    public IdentityServiceLog(String taskID, String objectID, String serviceName,
        String serviceJobID, JSONObject jstatus) {
    ...
    public JSONObject getStatusJson() {
        try {
            ...
            System.out.println(this + " -- error parsing status json string (" + status + "): " +
                je.toString());
            return null;
        }
    }

    public static ArrayList<IdentityServiceLog> load(String taskID, String serviceName,
        String serviceJobID, Shepherd myShepherd) {
        Extent cls = myShepherd.getPM().getExtent(IdentityServiceLog.class, true);
        Query qry = myShepherd.getPM().newQuery(cls,
            "this.taskID == \"" + taskID + "\" && this.serviceName == \"" + serviceName +
            "\" && this.serviceJobID == \"" + serviceJobID + "\"");
    ...
    public static ArrayList<IdentityServiceLog> loadByTaskID(String taskID, String serviceName,
        Shepherd myShepherd) {
    ...
    public static ArrayList<IdentityServiceLog> loadByServiceJobID(String serviceName,
        String serviceJobID, Shepherd myShepherd) {
// System.out.println("serviceName=(" + serviceName + ") serviceJobID=(" + serviceJobID + ")");
        Extent cls = myShepherd.getPM().getExtent(IdentityServiceLog.class, true);
        Query qry = myShepherd.getPM().newQuery(cls,
            "this.serviceName == \"" + serviceName + "\" && this.serviceJobID == \"" +
            serviceJobID + "\"");
    ...
    public static ArrayList<IdentityServiceLog> loadByObjectID(String serviceName, String objectID,
        Shepherd myShepherd) {
// System.out.println("serviceName=(" + serviceName + ") serviceJobID=(" + serviceJobID + ")");
        Extent cls = myShepherd.getPM().getExtent(IdentityServiceLog.class, true);
        ...
        // "\")");
        Query qry = myShepherd.getPM().newQuery(cls,
            "this.objectIDs != null && this.serviceName == \"" + serviceName + "\"");
    ...
    public static ArrayList<IdentityServiceLog> loadMostRecentByObjectID(String serviceName,
        String objectID, Shepherd myShepherd) {
// System.out.println("serviceName=(" + serviceName + ") serviceJobID=(" + serviceJobID + ")");
        Extent cls = myShepherd.getPM().getExtent(IdentityServiceLog.class, true);
...
/*
   IDENTITYSERVICELOG_ID | SERVICEJOBID | SERVICENAME |       STATUS       |                TASKID                |   TIMESTAMP   |
                    OBJECTID
   -----------------------+--------------+-------------+--------------------+--------------------------------------+---------------+--------------------------------------
                   737 | -1           | IBEISIA     | {"_action":"init"} | 6bc34656-0847-4a80-b093-2a52481d8b22 | 1459307111383 |
                      fd5b557e-337c-4239-9625-dfacc8822550 739 | -1           | IBEISIA     | {"_action":"init"} |
                      8fce30ef-a8e2-4a4f-8830-5a3273df9baf | 1459307241643 | fd5b557e-337c-4239-9625-dfacc8822550 745 | -1           | IBEISIA     |
                      {"_action":"init"} | 32bb426a-a03d-4386-bbbf-15d6259ef732 | 1459308840326 | fd5b557e-337c-4239-9625-dfacc8822550 (3 rows)

   (END)*/
    public static ArrayList<IdentityServiceLog> summaryForAnnotationId(String annId,
        Shepherd myShepherd) {
        Extent cls = myShepherd.getPM().getExtent(IdentityServiceLog.class, true);
    ...
    public static String[] findObjectIDs(ArrayList<IdentityServiceLog> logs) {
        for (IdentityServiceLog l : logs) {
            if ((l.getObjectIDs() != null) && (l.getObjectIDs().length > 0))
                return l.getObjectIDs();
        }
        return null;
    }
