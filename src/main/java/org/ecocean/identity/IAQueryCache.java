...
public class IAQueryCache {
    ...
    public static CachedQuery setTargetAnnotationsCache(String context, Annotation ann,
        JSONObject jobj, Shepherd myShepherd) {
        ...
        if (q == null) {
            ...
            log("INFO: setTargetAnnotationsCache(" + qname + ") created new CachedQuery id=" +
                q.getUUID());
        } else {
            q.setJSONSerializedQueryResult(jobj, true);
            log("INFO: setTargetAnnotationsCache(" + qname + ") updated CachedQuery id=" +
                q.getUUID());
        }
        return q;
    }
    ...
    public static JSONObject tryTargetAnnotationsCache(String context, Annotation ann,
        final JSONObject baseRtn, Shepherd myShepherd) {
        ...
        if ((idArr == null) || (nameArr == null) || (idArr.length() != nameArr.length())) {
            log(
                "WARNING: tryTargetAnnotationsCache() could not unpack idArr or nameArr from cache; WEIRD!");
            return null;
        }
