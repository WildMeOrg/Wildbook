 ...
 * <p>When an instance is created, a delegate {@code EmailTemplate} instance is created which handles loading of relevant text/HTML content templates.
 ...
 * <p>If the <em>type</em> parameter is null, then no type-template is loaded, and the base tag (<em>&#64;EMAIL_CONTENT&#64;</em>) is simply replaced
 * by the standard content tag (<em>&#64;TEXT_CONTENT&#64;</em>) before proceeding as usual.</p>
 *
 ...
 * </pre>
 * <p>then the standard content tag (<em>&#64;TEXT_CONTENT&#64;</em>) will be replaced with the specified text, and no other tag replacements will
 * occur during initialization.</p>
 * <p>If instead you want different text inserted for each of the plain/HTML templates, you must do the replacements after initialization:</p>
 * <pre>
 * String context = "context0";
 * String langCode = "en";
 * String from = "from&#64;bar.org";
 ...
 * </pre>
 *
 * <p>An optional REMOVEME section is also supported, which is designed to support inclusion of a section for removing a user from an email
 * subscription. This optional section can be delimited by
 * <em>&#64;REMOVEME_START&#64;</em> &amp; <em>&#64;REMOVEME_END&#64;</em>
 * tags in the plain text template, or by commented versions (<em>&lt;!--&#64;REMOVEME_START&#64;--&gt;</em> &amp;
 * <em>&lt;!--&#64;REMOVEME_END&#64;--&gt;</em>) in the HTML template. If a user-specific email-hash tag is specified in the tag map, these delimiters
 * are removed, and the section remains in place, otherwise the entire section is removed during initialization.</p>
 ...
 * </pre>
 *
 * <p>After initialization, and before invoking {@link #run()}, the template can still be used to make arbitrary tag replacements using the
...
public final class NotificationMailer implements Runnable {
     ...
     */
    public NotificationMailer(String context, String langCode, Collection<String> to,
        List<String> types, Map<String, String> map) {
        this(context, langCode, to, types, map, false);
    }
    public NotificationMailer(String context, String langCode, Collection<String> to,
        List<String> types, Map<String, String> map, boolean overrideReceiveEmails) {
        ...
        for (int i = 0; i < recips.size(); i++) {
            ...
            if (s == null || "".equals(s.trim())) { recips.remove(i); i--; } else {
                ...
                try {
                    ...
                    if ((myShepherd.getUserByEmailAddress(s) == null) ||
                        (!overrideReceiveEmails &&
                        !myShepherd.getUserByEmailAddress(s).getReceiveEmails())) {
                        recips.remove(s);
                        i--;
                    }
        ...
        try {
            ...
            if (map != null) {
                ...
                if (map.containsKey("@URL_LOCATION@") && map.containsKey(EMAIL_HASH_TAG) &&
                    map.containsKey(EMAIL_NOTRACK)) {
...
/*
    re: discussion with jh 2020-03-18, this is deprecated usage so commenting out -jv
          // Extra layer to help prevent chance of URL spoof attacks. String noTrack = map.get(EMAIL_NOTRACK);
 ...
 */
                    ...
                    mailer.replaceRegexInPlainText("(?s)@REMOVEME_START@.*@REMOVEME_END@", null,
                        false);
                    if (mailer.hasHtmlText())
                        mailer.replaceRegexInHtmlText("(?s)<!--@REMOVEME_START@.*@REMOVEME_END@-->",
                            null, false);
                }
            }
     ...
     * @param map map of search/replace strings for email template (if order is important, supply {@code LinkedHashMap}
     */
    public NotificationMailer(String context, String langCode, Collection<String> to, String type,
     ...
     * @param map map of search/replace strings for email template (if order is important, supply {@code LinkedHashMap}
     */
    public NotificationMailer(String context, String langCode, String to, List<String> types,
        Map<String, String> map) {
     ...
     * @param map map of search/replace strings for email template (if order is important, supply {@code LinkedHashMap}
     */
    public NotificationMailer(String context, String langCode, String to, String type,
        Map<String, String> map) {
        this(context, langCode, Arrays.asList(to), type, map);
    }
    public NotificationMailer(String context, String langCode, String to, String type,
        Map<String, String> map, boolean overrideReceiveEmails) {
        this(context, langCode, Arrays.asList(to), Arrays.asList(type), map, overrideReceiveEmails);
    }

    /**
     * Creates a new NotificationMailer instance. If the <em>type</em> parameter is null, the specified <em>text</em> is placed directly into the
     * standard email template, instead of also loading an type-template.
     * @param context webapp context
     * @param langCode language code for template loading
     * @param to email recipients
     * @param type email type ((e.g. <em>individualAddEncounter</em>)
    ...
    public NotificationMailer(String context, String langCode, Collection<String> to, String type,
        final String text) {
        this(context, langCode, to, type,
            new HashMap<String, String>() {{ put(STANDARD_CONTENT_TAG, text);
                                             System.out.println(
                                                 "NoteMailerHere1");
        }});
    }
    ...
    /**
     * Removes any initialized HTML text assigned to this mailer. This can be called to ensure a plain text only email is sent.
     */
    ...
    /**
     * Loads an email template from the specified email types. This method traverses the list of types until one is found that provides a valid email
     * template.
     *
     * @param langCode language code for template loading
     ...
     */
    private EmailTemplate loadEmailTemplate(String langCode, List<String> types)
    throws IOException {
    ...
    /**
     * Loads an email template for the specified email type. An email template references two files, one for each of plain/HTML text.
     *
     * @param langCode language code for template loading
     * @param type string specifying type of email (e.g. <em>individualAddEncounter</em>)
     * @return {@code EmailTemplate} instance
     */
    private EmailTemplate loadEmailTemplate(String langCode, String type)
    throws IOException {
    ...
    /**
     * Resolves mail template files using the specified filename base. Template files are assumed to have the extensions &quot;.txt&quot;
     * and &quot;.html&quot; (or &quot;.htm&quot;) for plain text and HTML messages respectively. This method doesn't attempt to load the files, just
     * resolves them to
     ...
     */
    private File[] resolveTemplatesFromRoot(String langCode, String baseName)
    throws IOException {
        Objects.requireNonNull(langCode);
        Objects.requireNonNull(baseName);
        String s = baseName + ".txt";
        File f = ServletUtilities.findResourceOnFileSystem(String.format("%s/%s/%s", SEARCH_PATH,
            langCode, s));
        if (f == null) {
            s = baseName + ".TXT";
            f = ServletUtilities.findResourceOnFileSystem(String.format("%s/%s/%s", SEARCH_PATH,
                langCode, s));
        }
        if (f == null)
            throw new FileNotFoundException(String.format(
                    "Failed to find plain text email template: %s.txt", baseName));
        return EmailTemplate.resolveTemplatesFromRoot(f.getParentFile(), baseName);
    }

    /**
     * Performs a string search/replace on the subject and body of the template. This method is a convenience to perform all replacements throughout.
     *
    ...
    /**
     * Searches and replaces one or all occurrences of the specified regular expression search term with the specified replacement. This method is a
     * convenience to perform all replacements throughout.
    ...
    /**
     * Creates a basic tag map for the specified encounter. This map can subsequently be enhanced with extra tags. Encounter tags included:
     * <ul>
     ...
     */
    public static Map<String, String> createBasicTagMap(HttpServletRequest req,
        MarkedIndividual ind, Encounter enc, String scheme) {
        Map<String, String> map = new HashMap<>();
    ...
    public static Map<String, String> createBasicTagMap(HttpServletRequest req,
        MarkedIndividual ind, Encounter enc) {
     ...
     */
    private static void addTags(Map<String, String> map, HttpServletRequest req,
        MarkedIndividual ind, String scheme) {
        Objects.requireNonNull(map);
        if (!map.containsKey("@URL_LOCATION@"))
            map.put("@URL_LOCATION@",
                String.format(scheme + "://%s", CommonConfiguration.getURLLocation(req)));
        if (ind != null) {
            map.put("@INDIVIDUAL_LINK@",
                String.format("%s/individuals.jsp?id=%s", map.get("@URL_LOCATION@"),
                ind.getIndividualID()));
