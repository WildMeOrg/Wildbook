 ...
 * contained in a workspace selected by name
 **/
...
public class WorkspaceServer extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String owner = request.getUserPrincipal() !=
            null ? request.getUserPrincipal().getName() : "testUser";
        if (request.getParameter("id") == null)
            throw new IOException("WorkspaceServer requires an \"id\" argument");
        ...
        try {
            ...
            if (wSpace == null) {
                ...
                System.out.println("doGet successfully grabbed workspace with id=" +
                    wSpace.getID() + " and queryArg=" + wSpace.getArgs());
            }
     ...
     * @requestParameter overwrite (optional) if "true", will overwrite an existing workspace e.g.
     * http://52.36.191.214/WorkspaceServer?id=testServer&args={class:org.ecocean.MarkedIndividual, range:3}&overwrite=true
     **/
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            JSONObject args = ((request.getParameter("args")) !=
                null) ? new JSONObject(request.getParameter("args")) : Util.requestParamsToJSON(
                request);
            String id = request.getParameter("id");
            ...
            if (inDB && !overwrite)
                throw new IOException("Workspace with id=" + id + " and owner=" + owner +
                        " already in database. Include request parameter overwrite=true if you wish to overwrite.");
            if (inDB) {
                wSpace.setArg(args);
                wSpace.setModified(new Date());
                out.println("modifying existing workspace with id=" + wSpace.getID() + ", name=" +
                    wSpace.getName() + ", owner=" + wSpace.getOwner() + ", args=" +
                    wSpace.getArgs() + ", created=" + wSpace.getCreated() + ", and modified =" +
                    wSpace.getModified());
                System.out.println("modifying existing workspace with id=" + wSpace.getID() +
                    ", name=" + wSpace.getName() + ", owner=" + wSpace.getOwner() + ", args=" +
                    wSpace.getArgs() + ", created=" + wSpace.getCreated() + ", and modified =" +
                    wSpace.getModified());
            } else {
                wSpace = new Workspace(id, owner, args);
                out.println("initializing new workspace with id=" + wSpace.id + ", name=" + id +
                    ", owner=" + wSpace.owner + ", args=" + args.toString() + ", and created=" +
                    wSpace.created);
                System.out.println("initializing new workspace with id=" + wSpace.id + ", name=" +
                    id + ", owner=" + wSpace.owner + ", args=" + args.toString() +
                    ", and created=" + wSpace.created);
            }
