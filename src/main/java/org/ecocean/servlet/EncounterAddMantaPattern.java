...
public class EncounterAddMantaPattern extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (request.getParameter("action") != null &&
            request.getParameter("action").equals("imageremove")) {
            action = "imageremove";
        } else if (request.getParameter("action") != null &&
            request.getParameter("action").equals("rescan")) {
            action = "rescan";
        } else if (request.getParameter("action") != null &&
            request.getParameter("action").equals("rescanRegional")) {
        ...
        // imageadd2 is added for the new candidate region selection tool and should eventually replace the original imageadd action
            ...
            request.getParameter("action").equals("imageadd2")) {
            action = "imageadd2";
        }
        try {
            if (action.equals("imageremove")) {
                ...
                try {
                    ...
                    boolean hasCR = MantaMatcherUtilities.checkEncounterHasMatcherFiles(enc,
                        shepherdDataDir);
                    if (!hasCR) {
                    ...
                    System.out.println(
                        "Error attempting to delete the old version of a submitted manta data image!!!!");
                ...
                try {
                    ...
                    if (mmaInputFile.exists())
                        mmaInputFile.delete();
                    String inputText = MantaMatcherUtilities.collateAlgorithmInput(myShepherd,
                        encountersDir, enc, spv);
                    PrintWriter pw = null;
                    try {
                    ...
                    resultComment.append(
                        "mmatch reported the following when trying to match image files:<br />");
                    BufferedReader brProc = new BufferedReader(new InputStreamReader(
                        proc.getInputStream()));
                    try {
                        String temp = null;
                        ...
                        locked = true;
                        resultComment.append(
                            "I hit an IOException while trying to execute mmprocess from the command line.");
                ...
                try {
                    ...
                    if (mmaInputFile.exists())
                        mmaInputFile.delete();
                    String inputText = MantaMatcherUtilities.collateAlgorithmInputRegional(
                        myShepherd, encountersDir, enc, spv);
                    PrintWriter pw = null;
                    try {
                        pw = new PrintWriter(mmaInputFile);
                        pw.print(inputText);
                ...
                while ((part = mp.readNextPart()) != null) {
                    ...
                    if (part.isFile()) {
                        ...
                        resultComment.append("I am trying to execute the command:<br/>").append(
                            procArgStr).append("<br />");

                        Process process = pb.start();
                        // Read ouput from process.
                        resultComment.append(
                            "mmprocess reported the following when trying to create the enhanced image file:<br />");
                        BufferedReader brProc = new BufferedReader(new InputStreamReader(
                        ...
                        if (enc != null && MantaMatcherUtilities.checkEncounterHasMatcherFiles(enc,
                            shepherdDataDir)) {
                ...
                if (rawPng != null) {
                    ...
                    if (enc == null) {
                        ...
                        if (!crImageFormat.equalsIgnoreCase(targetFormat)) {
                            System.out.println("cr format (" + crImageFormat +
                                ") differs from target format (" + targetFormat +
            ...
            if ((myShepherd.isEncounter(encounterNumber)) && !locked) {
                ...
                try {
                    ...
                    if (action.equals("imageadd") || action.equals("imageadd2")) {
                        add2shark.addComments("<p><em>" + user + " on " +
                            (new java.util.Date()).toString() + "</em><br>" +
                            "Submitted new mantamatcher data image.</p>");
                    } else if (action.equals("rescan")) {
                        add2shark.addComments("<p><em>" + user + " on " +
                            (new java.util.Date()).toString() + "</em><br>" +
                            "Performed global matching scan of mantamatcher feature data.</p>");
                    } else if (action.equals("rescanRegional")) {
                        add2shark.addComments("<p><em>" + user + " on " +
                            (new java.util.Date()).toString() + "</em><br>" +
                            "Performed regional matching scan of mantamatcher feature data.</p>");
                    } else {
                        add2shark.addComments("<p><em>" + user + " on " +
                            (new java.util.Date()).toString() + "</em><br>" +
                            "Removed mantamatcher data image.</p>");
                    }
                } catch (Exception le) {
                    locked = true;
                ...
                if (!locked) {
                    ...
                    if ((action.equals("imageadd")) || (action.equals("imageadd2"))) {
                        ...
                        out.println(
                            "<strong>Confirmed:</strong> I have successfully added your mantamatcher data image file.");
                        out.println(
                            "<p><strong>Additional comments from the operation</strong><br />" +
                            resultComment.toString() + "</p>");
                        ...
                        String resultsURL = request.getContextPath() +
                            "/MantaMatcher/displayResults?spv=" + spv.getDataCollectionEventID();
                        response.sendRedirect(resultsURL);
                    } else if (action.equals("rescanRegional")) {
                        String resultsURL = request.getContextPath() +
                            "/MantaMatcher/displayResultsRegional?spv=" +
                            spv.getDataCollectionEventID();
                        response.sendRedirect(resultsURL);
                    } else {
                        out.println(ServletUtilities.getHeader(request));
                        out.println(
                            "<strong>Confirmed:</strong> I have successfully removed your mantamatcher data image file.");
                        out.println("<p><a href=\"" + request.getScheme() + "://" +
                            CommonConfiguration.getURLLocation(request) +
                            "/encounters/encounter.jsp?number=" + encounterNumber +
                            "\">Return to encounter " + encounterNumber + "</a></p>\n");
                    ...
                    if (action.equals("imageadd")) {
                        ...
                        out.println(
                            "<strong>Step 2 Failed:</strong> I could not upload this patterning file. There may be a database error, or a incompatible image file format may have been uploaded.");
                    } else {
                        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                        out.println(
                            "<strong>Step 2 Failed:</strong> I could not remove this patterning file. There may be a database error.");
                    }
                    out.println("<p><a href=\"" + request.getScheme() + "://" +
                        CommonConfiguration.getURLLocation(request) +
                        "/encounters/encounter.jsp?number=" + encounterNumber +
                        "\">Return to encounter " + encounterNumber + "</a></p>\n");
                    out.println("<p><strong>Additional comments from the operation</strong><br />" +
                        resultComment.toString() + "</p>");
                    out.println(ServletUtilities.getFooter(context));
                }
            } else {
                out.println(ServletUtilities.getHeader(request));
                out.println(
                    "<strong>Error:</strong> I was unable to execute this action. I cannot find the encounter that you intended it for in the database.");
                out.println("<p><strong>Additional comments from the operation</strong><br />" +
                    resultComment.toString() + "</p>");
                out.println(ServletUtilities.getFooter(context));
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            ...
            out.println(ServletUtilities.getHeader(request));
            out.println(
                "<strong>Error:</strong> Algorithm scanning process was unexpectedly interrupted.");
            out.println("<p><strong>Additional comments from the operation</strong><br />" +
                resultComment.toString() + "</p>");
            out.println(ServletUtilities.getFooter(context));
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
