    ...
    throws ServletException, IOException {
        ...
        if (imageContents != null) {
            ...
            if ((request.getParameter("rightSide") != null) &&
                request.getParameter("rightSide").equals("true")) side = "right";
                ...
                try {
                    ...
                    try {
                        ...
                        add2shark.addComments("<p><em>" + user + " on " +
                            (new java.util.Date()).toString() + "</em><br>" + "Submitted new " +
                            side + "-side spot data graphic.</p>");
                    } catch (Exception le) {
                        locked = true;
            ...
            try {
                    ...
                    if (part.isFile()) {
                        ...
                        if ((fileName != null) && (myShepherd.isAcceptableImageFile(fileName))) {
                            ...
                            try {
                                ...
                                File jpegVersion = new File(thisEncounterDir,
                                    ("extract" + sideAddition + encounterNumber + ".jpg"));
                                ...
                                System.out.println(
                                    "Error attempting to delete the old JPEG version of a submitted spot data image!!!!");
                                removedJPEG = false;
                            }
                            long file_size = filePart.writeTo(new File(thisEncounterDir, fileName)
                                );
                        } else {
                            locked = true;
                            myShepherd.rollbackDBTransaction();
                ...
                if ((myShepherd.isEncounter(encounterNumber)) && !locked) {
                    ...
                    if (!locked) {
                        ...
                        out.println(
                            "<strong>Step 2 Confirmed:</strong> I have successfully uploaded your "
                            + side + "-side spot data image file.");
                        out.println("<p><a href=\"http://" +
                            CommonConfiguration.getURLLocation(request) +
                            "/encounters/encounter.jsp?number=" + encounterNumber +
                            "#spotpatternmatching\">Return to encounter " + encounterNumber +
                            "</a></p>\n");
                        ...
                        out.println(ServletUtilities.getHeader(request));
                        out.println(
                            "<strong>Step 2 Failed:</strong> This encounter is currently locked and modified by another user. Please try to resubmit your spot data and add this image again in a few seconds.");
                    ...
                    out.println(ServletUtilities.getHeader(request));
                    out.println(
                        "<strong>Error:</strong> I was unable to upload your file. I cannot find the encounter that you intended it for in the database, or the file type uploaded is not supported.");
                    out.println(ServletUtilities.getFooter(context));
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                }
