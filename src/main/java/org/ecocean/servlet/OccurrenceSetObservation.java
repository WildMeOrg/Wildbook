        ...
        if ((request.getParameter("number") != null) && (request.getParameter("name") != null)) {
            ...
            try {
                if (newValue.equals("null") || newValue.equals("") || newValue == null) {
                    ...
                    if (occ.getObservationByName(name) != null && newValue != null) {
                        ...
                        System.out.println("Resetting value for Observation named " + name +
                            " with new value " + newValue);
                        System.out.println("Did the value stick??? Name:" + existing.getName() +
                            " Value: " + existing.getValue());
                    } else {
                        obs = new Observation(name, newValue, occ.getClass().getSimpleName(),
                            occ.getID());
            ...
            if (!locked) {
                ...
                if (!newValue.equals("") && !oldValue.equals("null")) {
                    out.println("<strong>Success:</strong> Occurrence Observation " + name +
                        " has been updated to <i>" + newValue + "</i>.");
                } else if (oldValue.equals("null")) {
                    out.println("<strong>Success:</strong> Occurrence Observation " + name +
                        " has been created with a value of <i>" + newValue + "</i>.");
                } else {
                    out.println("<strong>Success:</strong> Occurrence Observation " + name +
                        " was removed. The old value was <i>" + oldValue + "</i>.");
                }
                out.println("<p><a href=\"" + request.getScheme() + "://" +
                    CommonConfiguration.getURLLocation(request) + redirectURL + "?number=" +
                ...
                out.println(
                    "<p><a href=\"individualSearchResults.jsp\">View all marked individuals</a></font></p>");
                out.println(ServletUtilities.getFooter(context));
                String message = "Occurrence " + request.getParameter("number") + " Observation " +
                ...
                out.println(ServletUtilities.getHeader(request));
                out.println("<strong>Failure:</strong> Occurrence Observation " + name +
                    " was NOT updated because another user is currently modifying this record. Please try to reset the value again in a few seconds.");
                out.println("<p><a href=\"" + request.getScheme() + "://" +
                    CommonConfiguration.getURLLocation(request) + redirectURL + "?number=" +
            ...
            out.println("<p><a href=\"" + request.getScheme() + "://" +
                ...
                request.getParameter("number") + "\">Return to survey #" +
                request.getParameter("number") + "</a></p>\n");
            List<String> allStates = CommonConfiguration.getIndexedPropertyValues("encounterState",
                context);
            int allStatesSize = allStates.size();
            if (allStatesSize > 0) {
                for (int i = 0; i < allStatesSize; i++) {
                    String stateName = allStates.get(i);
                    out.println("<p><a href=\"" + redirectURL + "?state=" + stateName +
                        "\">View all " + stateName + " surveys.</a></font></p>");
                }
            }
            out.println(
