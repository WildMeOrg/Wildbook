            ...
            if (removeFromMe != null) {
                ...
                try {
                    ...
                    enc2remove.addComments("<p><em>" + request.getRemoteUser() + " on " +
                        (new java.util.Date()).toString() + "</em><br>" + "Removed from " +
                        old_name + ".</p>");
                    setDateLastModified(enc2remove);
                    if (removeFromMe != null) {
                        name_s = removeFromMe.getName();
                        while (removeFromMe.getEncounters().contains(enc2remove)) {
                            System.out.println("IndividualRemoveEncounter checkpoint B.1 has enc=" +
                                enc2remove + " and ind=" + removeFromMe);
                            removeFromMe.removeEncounter(enc2remove);
                            System.out.println("IndividualRemoveEncounter checkpoint B.2 has enc=" +
                                enc2remove + " and ind=" + removeFromMe);
                        }
                        ...
                        if (removeFromMe.totalEncounters() == 0) {
                            ...
                            List<SocialUnit> units =
                                myShepherd.getAllSocialUnitsForMarkedIndividual(removeFromMe);
                ...
                if (!locked) {
                    ...
                    out.println("<strong>Success:</strong> Encounter #" +
                        request.getParameter("number") + " was successfully removed from " +
                        old_name + ".");
                    ...
                    out.println("<strong>Failure:</strong> Encounter #" +
                        request.getParameter("number") + " was NOT removed from " + old_name +
                ...
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                out.println(
                    "<strong>Error:</strong> You can't remove this encounter from a marked individual because that individual does not exist.");
                out.println(ServletUtilities.getFooter(context));
            }
        } else {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            out.println(
                "I did not receive enough data to remove this encounter from a marked individual.");
        }
        out.close();
