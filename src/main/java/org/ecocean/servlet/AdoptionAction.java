...
public class AdoptionAction extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        id = "adpt" + (new Integer(date.get(Calendar.DAY_OF_MONTH))) +
...
/*
      //return a forward to display.jsp System.out.println("Ending adoption data submission.");
      ...
      if (adoptionSuccess) {
        ...
        response.sendRedirect(request.getScheme()+"://" + CommonConfiguration.getURLLocation(request) + "/adoptions/adoptionFailure.jsp?message=" +
           failureMessage);
        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
      }
 */
        if ((fv.get("g-recaptcha-response") != null) &&
            !fv.get("g-recaptcha-response").equals("")) {
        ...
        for (FileItem item : formFiles) {
            /* this will actually write file to filesystem (or [FUTURE] wherever) TODO: either (a) undo this if any failure of writing encounter; or
               (b) dont write til success of enc. */
            // try {
        ...
        if ((validCaptcha == true) || (loggedIn == true)) {
            ...
            try {
                Adoption ad = new Adoption(id, adopterName, adopterEmail, adoptionStartDate,
                    adoptionEndDate);
                if (isEdit || emailEdit) {
                ...
                if (emailEdit == false) {
                    try {
                        ...
                        Map<String, String> tagMap = NotificationMailer.createBasicTagMap(request,
                            mi, ad);
                        NotificationMailer mailer = new NotificationMailer(emailContext, langCode,
                            to, type, tagMap);
                        NotificationMailer adminMailer = new NotificationMailer(emailContext,
                            langCode, CommonConfiguration.getNewSubmissionEmail(emailContext), type,
                            tagMap);
                        es.execute(mailer);
                ...
                System.out.println(
                    "The recaptcha passed but something went wrong saving the adoption.");
                e.printStackTrace();
            }
        }
        // }
        ...
        if ((adoptionSuccess) && (emailEdit == false)) {
            response.sendRedirect(request.getScheme() + "://" +
                CommonConfiguration.getURLLocation(request) + "/adoptions/adoptionSuccess.jsp?id=" +
                id);
        } else if ((adoptionSuccess) && (emailEdit == true)) {
            response.sendRedirect(request.getScheme() + "://" +
                CommonConfiguration.getURLLocation(request) + "/adoptions/editSuccess.jsp");
        } else {
            response.sendRedirect(request.getScheme() + "://" +
