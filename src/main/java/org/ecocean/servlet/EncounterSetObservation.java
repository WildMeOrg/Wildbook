    ...
    throws ServletException, IOException {
        ...
        if ((request.getParameter("number") != null) && (request.getParameter("name") != null)) {
            ...
            try {
                if (newValue.equals("null")) {
                    ...
                    changeMe.addComments("<p><em>" + request.getRemoteUser() + " on " +
                        (new java.util.Date()).toString() + "</em><br>Removed observation <em>" +
                        name + "</em>. The Value was <em>" + oldValue + ".</em></p>");
                } else {
                    if (changeMe.getObservationByName(name) != null && value != null) {
                        Observation existing = changeMe.getObservationByName(name);
                        existing.setValue(value);
                    } else {
                        obs = new Observation(name, newValue, changeMe.getClass().getSimpleName(),
                            changeMe.getCatalogNumber());
                        changeMe.addObservation(obs);
                        System.out.println("Success setting Observation!");
                        // changeMe.addComments("<p><em>" + request.getRemoteUser() + " on " + (new java.util.Date()).toString() + "</em><br>Set
                        // dynamic property <em>" + name + "</em> from <em>" + oldValue + "</em> to <em>" + newValue + "</em>.</p>");
                    }
                }
            } catch (Exception le) {
            ...
            if (!locked) {
                ...
                if (!newValue.equals("")) {
                    out.println("<strong>Success:</strong> Encounter Observation " + name +
                ...
                out.println("<p><a href=\"" + request.getScheme() + "://" +
                    ...
                    request.getParameter("number") + "\">Return to Encounter " +
                ...
                String message = "Encounter " + request.getParameter("number") + " Observation " +
                ...
                out.println(ServletUtilities.getHeader(request));
                out.println("<strong>Failure:</strong> Encounter Observation " + name +
            ...
            if (allStatesSize > 0) {
                for (int i = 0; i < allStatesSize; i++) {
                    ...
                    out.println("<p><a href=\"" + redirectURL + "?state=" + stateName +
                        "\">View all " + stateName + " encounter</a></font></p>");
                }
            }
            out.println(ServletUtilities.getFooter(context));
        }
