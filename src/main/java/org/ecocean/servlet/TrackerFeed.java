        ...
        try {
            ...
            if (matches[0].matchValue > 0) {
                ...
                for (int i = 0; i < matches.length; i++) {
                    ...
                    String finalscore = (new Double(mo.matchValue *
                        mo.adjustedMatchValue)).toString();
                    ...
                    moDate = enc.getDate();
                    if (enc.getSex() != null) { moSex = enc.getSex(); } else { moSex = "unknown"; }
                    ...
                    out.println("<encounter number=\"" + mo.encounterNumber + "\" date=\"" +
                        moDate + "\" sex=\"" + moSex + "\" assignedToShark=\"" + moIndividualName +
                        "\" size=\"" + moSize + " meters\" location=\"" + moLocation + "\">");
                    for (int k = 0; k < mo.scores.size(); k++) {
                        // map the spots
                        out.println("     <spot x=\"" + ((VertexPointMatch)mo.scores.get(k)).oldX +
                            "\" y=\"" + ((VertexPointMatch)mo.scores.get(k)).oldY + "\" />");
                    }
                    out.println("</encounter>");
                    out.println("<encounter number=\"" + num + "\" date=\"" + newEncDate +
                        "\" sex=\"" + mo.newSex + "\" assignedToShark=\"" + newEncShark +
                        "\" size=\"" + newEncSize + "\" location=\"" + newEncLocation + "\">");
                    for (int j = 0; j < mo.scores.size(); j++) {
                        // map the spots
                        out.println("     <spot x=\"" + ((VertexPointMatch)mo.scores.get(j)).newX +
                            "\" y=\"" + ((VertexPointMatch)mo.scores.get(j)).newY + "\" />");
                    }
                    out.println("</encounter>");
