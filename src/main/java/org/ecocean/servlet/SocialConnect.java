    ...
    throws ServletException, IOException {
        ...
        if ("facebook".equals(socialType)) {
            ...
            String callbackUrl = request.getScheme() + "://" +
                CommonConfiguration.getURLLocation(request) + "/SocialConnect?type=facebook";
            ...
            if (credentials != null) {
                ...
                if (fbuser != null)
                    System.out.println("user = " + user.getUsername() + "; fbuser = " +
                        fbuser.getUsername());
                if ((fbuser != null) && (fbuser.getUsername().equals(user.getUsername())) &&
                    (request.getParameter("disconnect") != null)) {
                    ...
                    session.setAttribute("error",
                        "looks like this account is already connected to an account");
                    response.sendRedirect("myAccount.jsp");
                    myShepherd.rollbackDBTransaction();
                    myShepherd.closeDBTransaction();
                    return;
            ...
            String callbackUrl = request.getScheme() + "://" +
                CommonConfiguration.getURLLocation(request) + "/SocialConnect?type=flickr";
            if (request.getParameter("disconnect") != null) callbackUrl += "&disconnect=1";
            try {
...
// http://localhost.wildme.org/a/SocialConnect?type=xxflickr&oauth_token=72157652805581432-a5b8f3598c13e2a6&oauth_verifier=d3325223f923442e
                ...
                if (fuser != null)
                    System.out.println("user = " + user.getUsername() + "; fuser = " +
                        fuser.getUsername());
                if ((fuser != null) && (fuser.getUsername().equals(user.getUsername())) &&
                    (request.getParameter("disconnect") != null)) {
...
/*
    ...
    // Trade the Request Token and Verfier for the Access Token System.out.println("Trading the Request Token for an Access Token...");
    Token accessToken = service.getAccessToken(requestToken, verifier);
...
/*
   ...
   if (yuser != null) System.out.println("user = " + user.getUsername() + "; yuser = " + yuser.getUsername());
                ...
                } else {  //lets do this user.setSocial("flickr", yahooProfile.getId());
                    //myShepherd.getPM().makePersistent(user);
                    session.setAttribute("message", "connected to flickr");
                    response.sendRedirect("myAccount.jsp");
                    return;
 ...
 */
    ...
    public HttpSession renewSession(final J2EContext context) {
        ...
        while (atts.hasMoreElements()) {
            attributes.put(atts.nextElement().toString(),
                session.getAttribute(atts.nextElement().toString()));
        }
        session.invalidate();
