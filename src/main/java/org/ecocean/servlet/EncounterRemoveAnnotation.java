    ...
    throws ServletException, IOException {
        ...
        try {
            if ((encID != null) && (myShepherd.isEncounter(encID))) {
                ...
                if (ServletUtilities.isUserAuthorizedForEncounter(enc,
                    request) && annotID != null && myShepherd.getAnnotation(annotID) != null &&
                    ...
                    if (!ann.isTrivial() &&
                        (ann.getSiblings() == null || ann.getSiblings().size() == 0)) {
                ...
                addErrorMessage(res,
                    "I don't know which Encounter you are refering to, or you don't have permission to modify this Encounter.");
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            }
        } catch (Exception e) {
            e.printStackTrace();
