...
public class EncounterRemoveImage extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            System.out.println("encounterRemoveImage: " + request.getParameter("number") + " " +
        ...
        if ((myShepherd.isEncounter(encounterNumber)) && (myShepherd.isSinglePhotoVideo(dcID))) {
            ...
            if (enc.getIndividualID() == null) {
                ...
                try {
                    ...
                    /*
                       ...
                       //remove copyrighted images try {

                        //File cpyrght=new File(((new
                           File(".")).getCanonicalPath()).replace('\\','/')+"/webapps/ROOT/encounters/"+encounterNumber+"/"+j+".jpg");
                    ...
                    enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                        ...
                        "Removed encounter image graphic: " + fileName + ".</p>");
                ...
                if (!locked) {
                    ...
                    out.println(
                        "<strong>Success!</strong> I have successfully removed the encounter image file. When returning to the encounter page, please make sure to refresh your browser to see the changes. Image changes will not be visible until you have done so.");
                    ...
                    String message = "An image file named " + fileName +
                        " has been removed from encounter#" + encounterNumber + ".";
                    ServletUtilities.informInterestedParties(request, encounterNumber, message,
                        context);
                } else {
                    out.println(ServletUtilities.getHeader(request));
                    out.println(
                        "<strong>Failure!</strong> This encounter is currently being modified by another user. Please wait a few seconds before trying to remove this image again.");
                ...
                out.println(ServletUtilities.getHeader(request));
                out.println(
                    "<strong>Error:</strong> I was unable to remove your image file. For data protection, you must first remove the encounter from the marked individual it is assigned to.");
                out.println(ServletUtilities.getFooter(context));
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            }
        } else {
            ...
            out.println(ServletUtilities.getHeader(request));
            out.println(
                "<strong>Error:</strong> I was unable to remove your image file. I cannot find the encounter that you intended it for in the database. +++");
            out.println(ServletUtilities.getFooter(context));
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
        }
        out.close();
