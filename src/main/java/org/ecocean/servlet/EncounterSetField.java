    ...
    throws ServletException, IOException {
        ...
        if (fieldTypeName == null) fieldType = DEFAULT_FIELD_TYPE;
            ...
            throw new IOException(
                      "EncounterSetField: incompatible with fieldType received from request: " +
                      fieldTypeName);
        }
        ...
        try {
            ...
            throw new IOException("EncounterSetField: there is no Encounter method called " +
                    fieldSetterName + " with a single " + fieldType.getName() + " argument.");
        }
        String value = request.getParameter("value");
        if (value == null) {
            throw new IOException(
                      "EncounterSetField: Could not get value from request. Recieved value=" +
                      value + " with field setter " + fieldSetterName);
        }
        System.out.println("EncounterSetField: proceeding towards calling Encounter." +
            fieldSetterName + "(" + value + ") for encounter " + encNum);
        ...
        if (myShepherd.isEncounter(encNum)) {
            ...
            try {
                ...
                enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                    (new java.util.Date()).toString() + "</em><br>Changed " + fieldName + " to " +
                    value + " using EncounterSetField servlet.</p>");
            } catch (Exception le) {
                locked = true;
                myShepherd.rollbackDBTransaction();
                myShepherd.closeDBTransaction();
                System.out.println("Exception submitting EncounterSetField(" + fieldName + ", " +
                    value + "):");
                le.printStackTrace();
            }
            if (!locked) {
                ...
                out.println("<strong>Success!</strong> I have successfully changed the " +
                    fieldName + " for encounter " + encNum + " to " + value + ".</p>");
                ...
                String message = "The " + fieldName + " for encounter " + encNum + " was set to " +
                    value + ".";
            } else {
                out.println(ServletUtilities.getHeader(request));
                out.println(
            ...
            out.println(ServletUtilities.getHeader(request));
            out.println(
                "<strong>Error:</strong> I was unable to set the encounter field. I cannot find the encounter that you intended in the database.");
            out.println(ServletUtilities.getFooter(context));
        }
        out.close();
        myShepherd.closeDBTransaction();
