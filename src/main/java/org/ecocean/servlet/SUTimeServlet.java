...
public class SUTimeServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        this.getServletContext().getRequestDispatcher("/sutime/su-header.jsp").include(request,
            response);
        addResults(request, response);
        this.getServletContext().getRequestDispatcher("/sutime/su-footer.jsp").include(request,
            response);
    }
    ...
    private String getRuleFilepaths(String... files) {
        String rulesDir = getServletContext().getRealPath("/WEB-INF/data/sutime/rules");
    ...
    private Properties getTimeAnnotatorProperties(HttpServletRequest request) {
        ...
        if (readRules) {
            ...
            if ("English".equalsIgnoreCase(rules)) {
                ruleFile = getRuleFilepaths("defs.sutime.txt", "english.sutime.txt",
        ...
        if (ruleFile != null) {
            ...
            props.setProperty("sutime.binder.1.xml",
                getServletContext().getRealPath("/WEB-INF/data/holidays/Holidays_sutime.xml"));
...
// props.setProperty("gutime.path", getServletContext().getRealPath("/packages/gutime"));
    ...
    private static void displayAnnotation(PrintWriter out, String query, Annotation anno,
        ...
        if (timexAnns.size() > 0) {
            ...
            for (CoreMap timexAnn : timexAnns) {
                ...
                int begin = timexAnn.get(CoreAnnotations.CharacterOffsetBeginAnnotation.class);
                int end = timexAnn.get(CoreAnnotations.CharacterOffsetEndAnnotation.class);
                out.print("<td>" + StringEscapeUtils.escapeHtml4(query.substring(begin,
                    end)) + "</td>");
                out.print("<td>" + ((timex.value() !=
                    null) ? StringEscapeUtils.escapeHtml4(timex.value()) : "") + "</td>");
                if (includeOffsets) {
                    out.print("<td>" + begin + "</td>");
                    out.print("<td>" + end + "</td>");
                    out.print("<td>" + timexAnn.get(CoreAnnotations.TokenBeginAnnotation.class) +
                        "</td>");
                    out.print("<td>" + timexAnn.get(CoreAnnotations.TokenEndAnnotation.class) +
                        "</td>");
                }
        ...
        for (CoreMap sentence : anno.get(CoreAnnotations.SentencesAnnotation.class)) {
            ...
            for (CoreLabel token : tokens) {
                String tokenOutput = StringEscapeUtils.escapeHtml4(token.word() + "/" +
                    token.tag());
    ...
    throws IOException {
        ...
        if (!StringUtils.isNullOrEmpty(query)) {
            ...
            if (timeAnnotator != null) {
                ...
                out.println("<h3>Annotated Text</h3> <em>(tagged using " + annotatorType +
                    "</em>)");
                displayAnnotation(out, query, anno, includeOffsets);
            } else {
                out.println("<br><br>Error creating annotator for " +
