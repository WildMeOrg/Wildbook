...
public class EncounterSetMeasurements extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (myShepherd.isEncounter(encNum)) {
                ...
                while (requestEventValues != null) {
                    ...
                    if (requestEventValues.id == null ||
                        ...
                        enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                            ...
                            "</em><br>Added measurement:<br><i>" + requestEventValues.type + " " +
                            requestEventValues.value + " " + requestEventValues.units + " (" +
                        ...
                        enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                            (new java.util.Date()).toString() + "</em><br>Changed measurement " +
                            requestEventValues.type + " from " + oldValue + " (" +
                            oldSamplingProtocol + ") to " + requestEventValues.value + " " +
                            requestEventValues.units + " (" + requestEventValues.samplingProtocol +
                            ")</i></p>");
                    }
            ...
            if (!locked) {
                ...
                out.println(
                    "<p><strong>Success!</strong> I have successfully set the following measurement values:");
                for (RequestEventValues requestEventValue : list) {
                    out.println(MessageFormat.format("<br/>{0} set to {1}", requestEventValue.type,
                        requestEventValue.value));
                }
            ...
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            out.println(
                "<strong>Error:</strong> I was unable to set the measurement(s). I cannot find the encounter that you intended in the database.");
            out.println(ServletUtilities.getFooter(context));
        }
        out.close();
        myShepherd.closeDBTransaction();
    ...
    // The parameter names are of the form "measurement0(id)", "measurement0(type)", "measurement0(value"), "measurement1(id)", "measurement1(type)",
    // "measurement1(value)" etc.
