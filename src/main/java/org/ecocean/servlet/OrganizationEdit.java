...
public class OrganizationEdit extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (searchUser != null) {
            ...
            String jdo =
                "SELECT FROM org.ecocean.Organization WHERE name.toLowerCase().matches('.*" +
                clean + ".*') || description.toLowerCase().matches('.*" + clean + ".*')";
            Query query = myShepherd.getPM().newQuery(jdo);
            Collection c = (Collection)query.execute();
            Iterator it = c.iterator();
            while (it.hasNext()) {
        ...
        if (user == null) {
            ...
            if (jsonIn.optString("mergeFrom", null) != null) {
                ...
                if (other == null) {
                    ...
                    System.out.println("INFO: OrganizationEdit mergeFrom is deleting " +
                        other.toJSONObject());
                    int added = org.mergeFrom(other);
                    rtn.put("message",
                        "merged " + added + " member(s) from [deleted] id=" + mergeFromId);
                ...
                rtn.put("deep", deep);
                rtn.put("message",
                    "removed " + rmCt + " member(s) from " + orgCt + " org(s) via " +
                    org.toString());
                ...
                if (kid == null) {
                    ...
                    if (kids == null) {
                        System.out.println("WARNING: failed OrganizationEdit addChild " + kid +
                            " onto " + org);
                        rtn.put("error", "failed to add child -- probably related already");
                    } else {
                        System.out.println("INFO: success OrganizationEdit addChild " + kid +
                            " onto " + org);
                ...
                if (kid == null) {
                    ...
                    if (ok) {
                        System.out.println("INFO: success OrganizationEdit removeChild " + kid +
                            " from " + org);
                        rtn.put("message", "removed " + kid + " from " + org);
                        rtn.put("success", true);
                    } else {
                        System.out.println("WARNING: failed OrganizationEdit removeChild " + kid +
                            " from " + org);
