 ...
 */
        ...
        if ("facebook".equals(socialType)) {
            ...
            String callbackUrl = request.getScheme() + "://" +
                CommonConfiguration.getURLLocation(request) + "/UserCreateSocial?type=facebook";
...
// TODO other fields?  --> https://pac4j.github.io/pac4j/apidocs/pac4j/org/pac4j/oauth/profile/facebook/FacebookProfile.html
                    fbuser = createUser(username, facebookProfile.getEmail(),
                        facebookProfile.getFirstName() + " " + facebookProfile.getFamilyName(),
                        context);
                    fbuser.setSocial("facebook", facebookProfile.getId());
                    // myShepherd.getPM().makePersistent(fbuser);
                    System.out.println("account " + fbuser.getUsername() +
                        " created via facebook!");
                    session.setAttribute("message", "new account created");

                    // account created, now lets try to log them in
                    UsernamePasswordToken token = new UsernamePasswordToken(fbuser.getUsername(),
                        fbuser.getPassword());
                    try {
...
// System.out.println("*** trying redirect?");
            ...
            String callbackUrl = request.getScheme() + "://" +
                CommonConfiguration.getURLLocation(request) + "/UserCreateSocial?type=flickr";
            try {
...
// System.out.println(oResponse.getBody());
                ...
                if (fuser != null) {
                    ...
                    // account created, now lets try to log them in
                    UsernamePasswordToken token = new UsernamePasswordToken(fuser.getUsername(),
                        fuser.getPassword());
                    try {
                        Subject subject = SecurityUtils.getSubject();
                        subject.login(token);
    ...
    private User createUser(String username, String emailAddress, String fullName, String context) {
        ...
        while (already != null) {
            count++;
            System.out.println("UserCreateSocial.createUser: username " + username +
                " already exists, appending " + count);
