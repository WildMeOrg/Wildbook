        ...
        try {
            ...
            if (annotInProject != null && "true".equals(annotInProject)) {
                ...
                if (Util.stringExists(acmId) && Util.stringExists(projectIdPrefix)) {
                    ...
                    Project project = myShepherd.getProjectByProjectIdPrefix(
                ...
                for (int i = 0; i < individualIdJSONArr.length(); i++) {
                    ...
                    if (Util.isUUID(individualId) && Util.stringExists(projectIdPrefix)) {
                        ...
                        if (projOwner != null) {
                            ...
                            if (Util.stringExists(projOwnerName)) {
                                individualData.put("projectOwner", projOwnerName);
                            }
                        }
                        if (Util.stringExists(projName)) {
                            individualData.put("projectName", projName);
                        }
                        if (Util.stringExists(projUuid)) {
                            individualData.put("projectUuid", projUuid);
                        }
                        if (project != null) {
                            String researchProjId = project.getProjectIdPrefix();
                            if (Util.stringExists(researchProjId)) {
                                MarkedIndividual individual = myShepherd.getMarkedIndividual(
                                    individualId);
                                if (individual != null) {
                                    List<String> namesList = individual.getNamesList(
                                        researchProjId);
                                    String projectIncrementalId = individual.getName(
                                        researchProjId);
                ...
                if (user != null) {
                    ...
                    for (Project project : userProjects) {
                        ...
                        if (Util.stringExists(currentInput)) {
                            ...
                            if (!projectIdPrefix.equals(currentInput) &&
                                !projectIdPrefix.contains(currentInput)) {
                                useProject = false;
                            }
                        }
                        if (useProject) {
                            List<MarkedIndividual> individuals =
                                myShepherd.getMarkedIndividualsFromProject(project);
                            for (MarkedIndividual individual : individuals) {
                                String projectIncrementalId = individual.getName(projectIdPrefix);
                                if (projectIncrementalId != null &&
                                    projectIncrementalId.contains(currentInput)) {
                                    JSONObject individualData = new JSONObject();
                                    individualData.put("projectIncrementalId",
                                        projectIncrementalId);
                                    individualData.put("individualId", individual.getId());
                                    individualData.put("individualDisplayName",
                                        individual.getDisplayName(request, myShepherd));
            ...
            if (individualIdsArr != null && individualIdsArr.length() > 0) {
                for (int i = 0; i < individualIdsArr.length(); i++) {
                    ...
                    if (Util.stringExists(individualId)) {
                        MarkedIndividual ind = myShepherd.getMarkedIndividual(individualId);
                        if (ind != null) {
                            List<Project> projects = myShepherd.getAllProjectsForMarkedIndividual(
                                ind);
                            JSONArray projectArr = new JSONArray();
                            if (projects != null) {
                                for (Project project : projects) {
                                    projectArr.put(project.asJSONObjectWithEncounterMetadata(
                                        myShepherd, request));
                                }
                            }
            ...
            if ((Util.stringExists(projectIdPrefix) || Util.stringExists(projectUUID)) &&
                !complete) {
                ...
                if (getEditPermission != null && "true".equals(getEditPermission)) {
                    ...
                    if (user != null && (user.hasRoleByName("admin",
                        myShepherd) || user.getId().equals(project.getOwnerId()))) {
