		...
		if(action!=null){
			...
			if ((action.equals("removeTask"))&&(request.getParameter("taskID")!=null)) {
				...
				if((myShepherd.isScanTask(request.getParameter("taskID")))&&(deletePermission)){
					...
					if(!locked) {
						...
						out.println("<p><a href=\""+request.getScheme()+"://"+"spotashark.com"+"/appadmin/scanTaskAdmin.jsp\">Return to scanTask administration page.</a></p>\n");
						...
						if(hasNeededSpots) {
							...
							locked=true;
					          ...
					          System.out.println("scanTaskHandler addTask: This is an existing scanTask...");
					                ...
					                if(request.getParameter("restart")==null){
					                  response.sendRedirect(request.getScheme()+"://" + "spotashark.com" + "/appadmin/scanTaskAdmin.jsp?task="+taskIdentifier);
                }
					}
					else{
						myShepherd.rollbackDBTransaction();
            ...
            //now build our jobs for the task
					...
					if(!locked){
						...
						out.println("<p><a href=\""+request.getScheme()+"://"+"spotashark.com"+"/appadmin/scanTaskAdmin.jsp?task="+taskIdentifier+ "#"+taskIdentifier+"\">Go to sharkGrid administration to monitor for completion.</a></p>\n");
						out.println(ServletUtilities.getFooter(context));
					}
					else{
						out.println(ServletUtilities.getHeader(request));
						out.println("<strong>Failure:</strong> The scan could not be created or was not fully created!");
						out.println("<p><a href=\""+request.getScheme()+"://"+"spotashark.com"+"/appadmin/scanTaskAdmin.jsp\">Go to sharkGrid administration.</a></p>\n");
						out.println(ServletUtilities.getFooter(context));
