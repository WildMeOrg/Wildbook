    ...
    throws ServletException, IOException {
        ...
        if (action != null) {
            if ((action.equals("removeTask")) && (request.getParameter("taskID") != null)) {
                ...
                if ((myShepherd.isScanTask(request.getParameter("taskID"))) && (deletePermission)) {
                    try {
                        ...
                        locked = true;
                        System.out.println(
                            "I encounter the following error while deleting a scanTask:");
                        e.printStackTrace();
                    ...
                    if (!locked) {
                        ...
                        out.println("<strong>Success:</strong> The scanTask <i>" +
                            request.getParameter("taskID") + "</i> has been removed.");
                        ...
                        out.println(ServletUtilities.getHeader(request));
                        out.println("<strong>Error:</strong> The scanTask <i>" +
                ...
                (request.getParameter("encounterNumber") != null)) {
                ...
                String taskIdentifier = "scan" + sideIdentifier +
                ...
                if (currentNumScanTasks < taskLimit) {
                    ...
                    if ((!myShepherd.isScanTask(taskIdentifier))) {
                        System.out.println(
                            "scanTaskHandler: This scanTask does not exist, so go create it...");

                        // check if this encounter has the needed spots to create the task
                        boolean hasNeededSpots = false;
                        Encounter enc = myShepherd.getEncounter(request.getParameter(
                            "encounterNumber"));
                        ...
                        if (hasNeededSpots) {
                            System.out.println(
                                "scanTaskHandler: I have needed spots...proceeding...");

                            st = new ScanTask(myShepherd, taskIdentifier, props2,
                                request.getParameter("encounterNumber"), writeThis);
                            ...
                            if (request.getParameterValues("locationID") != null) {
                                ...
                                for (int d = 0; d < locIDs.length; d++) {
                                    if ((locIDs[d] != null) && (!locIDs[d].trim().equals("")))
                                        st.addLocationIDToFilter(locIDs[d].trim());
                                }
                            } else if (enc.getLocationID() != null) {
                                st.addLocationIDToFilter(enc.getLocationID());
                                ArrayList<String> linkedLocs = myShepherd.getLinkedLocationIDs(
                                    enc.getLocationID());
                                if (linkedLocs != null) {
                            ...
                            if (!successfulStore) {
                                ...
                                System.out.println("Setting GM scanTaskSize: " + taskIdentifier +
                                    ": " + numComparisons);
                        ...
                        System.out.println(
                            "scanTaskHandler addTask: This is an existing scanTask...");
                        ...
                        if (request.getParameter("restart") == null) {
                            response.sendRedirect(request.getScheme() + "://" +
                                CommonConfiguration.getURLLocation(request) +
                                "/appadmin/scanTaskAdmin.jsp?task=" + taskIdentifier);
                        }
                    } else {
                        myShepherd.rollbackDBTransaction();
                        myShepherd.closeDBTransaction();
                ...
                if (!locked && successfulStore) {
                    try {
                        String jdoql = "";
                        if (request.getParameter("jdoql") != null) {
                            jdoql = request.getParameter("jdoql");
                        }
                        ...
                        es.execute(new ScanWorkItemCreationThread(taskIdentifier, isRightScan,
                            request.getParameter("encounterNumber"), writeThis, context, jdoql));
                    } catch (Exception e) {
                        System.out.println(
                            "I failed while constructing the workItems for a new scanTask.");
                        e.printStackTrace();
                        myShepherd.rollbackDBTransaction();
                        myShepherd.closeDBTransaction();
                        locked = true;
                    ...
                    if (!locked) {
                        ...
                        out.println("<p><a href=\"" + request.getScheme() + "://" +
                            ...
                            request.getParameter("encounterNumber") + "\">Return to encounter " +
                            request.getParameter("encounterNumber") + ".</a></p>\n");
                        out.println("<p><a href=\"" + request.getScheme() + "://" +
                            CommonConfiguration.getURLLocation(request) +
                            "/appadmin/scanTaskAdmin.jsp?task=" + taskIdentifier + "#" +
                            taskIdentifier +
                    ...
                    out.println(ServletUtilities.getHeader(request));
                    out.println(
                        "<strong>Failure:</strong> I have NOT added this scanTask to the queue.");
                    if (currentNumScanTasks < taskLimit) {
                        out.println("The unfinished task limit of " + taskLimit +
            ...
            /*
                   ...
                   //set up our properties java.util.Properties props2=new java.util.Properties();
                   String secondRun="true";
                   String rightScan="false";
                   boolean isRightScan=false;
                   boolean writeThis=true;
                   ...
                   //let's check if a scanTask for this exists System.out.println("scanTaskHandler: Checking whether this is a new scanTask...");
                   ...
                   //let's do a check to see if too many scanTasks are in the queue int taskLimit=gm.getScanTaskLimit();
                   int currentNumScanTasks=myShepherd.getNumUnfinishedScanTasks();
                   myShepherd.getPM().getFetchPlan().setGroup(FetchPlan.DEFAULT);
                   System.out.println("currentNumScanTasks is: "+currentNumScanTasks);
                   //int currentNumScanTasks=0;
                   if(currentNumScanTasks<taskLimit){
                           ...
                           //calculate the number of comparisons that can be made int numLeftSharks=leftSharks.size();
                           ...
                           System.out.println("scanTaskHandler: Under the limit, so proceeding to check for condiions for creating a new
                              scanTask...");
                           if((!myShepherd.isScanTask(taskIdentifier))){
                   ...
                   if(!locked&&successfulStore) {
                           ...
                           if(!locked){
                                   ...
                                   //confirm success out.println(ServletUtilities.getHeader(request));
                                   out.println("<strong>Success:</strong> Your scan was successfully added to the sharkGrid!");
                                   out.println("<p><a
                                      href=\"http://"+CommonConfiguration.getURLLocation(request)+"/appadmin/scanTaskAdmin.jsp?task="+taskIdentifier+"#"+taskIdentifier+"\">Return
                                      to sharkGrid administration.</a></p>\n");
                                   out.println(ServletUtilities.getFooter(context));
                           }
                           else{
                                   out.println(ServletUtilities.getHeader(request));
                                   out.println("<strong>Failure:</strong> The scan could not be created or was not fully created!");
                                   out.println("<p><a href=\"http://"+CommonConfiguration.getURLLocation(request)+"/appadmin/scanTaskAdmin.jsp\">Go to
                                      sharkGrid administration.</a></p>\n");
                                   out.println(ServletUtilities.getFooter(context));
                                           ...
                                           if(currentNumScanTasks<taskLimit){
                                                   out.println("The unfinished task limit of "+taskLimit+" has been filled. Please try adding the task
                                                      to the queue again after existing tasks have finished.");
                                           }
                                           out.println("<p><a
                                              href=\"http://"+CommonConfiguration.getURLLocation(request)+"/appadmin/scanTaskAdmin.jsp\">Go to
                                              sharkGrid administration.</a></p>\n");
                                           out.println(ServletUtilities.getFooter(context));
                   }
               }*/
            /*
                    ...
                    //let's check if a scanTask for this exists System.out.println("scanTaskHandler: Checking whether this is a new False Match
                       scanTask...");
                    ...
                    if(!locked&&successfulStore) {
                            ...
                            if(!locked){
                                    ...
                                    out.println("<strong>Success:</strong> Your scan was successfully added to the sharkGrid!");
                                    out.println("<p><a
                                       href=\"http://"+CommonConfiguration.getURLLocation(request)+"/appadmin/scanTaskAdmin.jsp"+"\">Return to
                                       sharkGrid administration.</a></p>\n");
                                    out.println(ServletUtilities.getFooter(context));
                            }
                            else{
                                    out.println(ServletUtilities.getHeader(request));
                                    out.println("<strong>Failure:</strong> The scan could not be created or was not fully created!");
                                    out.println("<p><a href=\"http://"+CommonConfiguration.getURLLocation(request)+"/appadmin/scanTaskAdmin.jsp\">Go
                                       to sharkGrid administration.</a></p>\n");
                                    out.println(ServletUtilities.getFooter(context));
                                            ...
                                            out.println("<strong>Failure:</strong> I have NOT added this scanTask to the queue.");
                                            if(currentNumScanTasks<taskLimit){
                                                    out.println("The unfinished task limit of "+taskLimit+" has been filled. Please try adding the
                                                       task to the queue again after existing tasks have finished.");
                                            }
                                            out.println("<p><a
                                               href=\"http://"+CommonConfiguration.getURLLocation(request)+"/appadmin/scanTaskAdmin.jsp\">Go to
                                               sharkGrid administration.</a></p>\n");
