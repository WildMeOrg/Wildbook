...
public class TissueSampleSetMicrosatelliteMarkers extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if ((request.getParameter("analysisID") != null) &&
            ...
            MicrosatelliteMarkersAnalysis msMarkers = new MicrosatelliteMarkersAnalysis(analysisID,
                sampleID, encounterNumber);
            ...
            try {
                numPloids = (new Integer(CommonConfiguration.getProperty("numPloids",
                    context))).intValue();
            } catch (Exception e) {
            ...
            while (hasMoreLoci) {
                System.out.println(request.toString());
                if ((request.getParameter("locusName" + numLoci) != null) &&
                    (!request.getParameter("locusName" + numLoci).trim().equals("")) &&
                    (request.getParameter(("allele" + numLoci + "0")) != null) &&
                    (!request.getParameter(("allele" + numLoci + "0")).trim().equals(""))) {
                    ...
                    try {
                        for (int q = 0; q < numPloids; q++) {
                            if ((request.getParameter("locusName" + numLoci) != null) &&
                                (!request.getParameter("locusName" + numLoci).trim().equals("")) &&
                                (request.getParameter(("allele" + numLoci + q)) != null) &&
                                (!request.getParameter(("allele" + numLoci + q)).equals(""))) {
                                alleles.add(new Integer(request.getParameter(("allele" + numLoci +
                                    q))));
            ...
            try {
                ...
                if (myShepherd.isGeneticAnalysis(sampleID, encounterNumber, analysisID,
                    ...
                    msMarkers = myShepherd.getMicrosatelliteMarkersAnalysis(sampleID,
                        encounterNumber, analysisID);

                    // now set the new values
                    msMarkers.setLoci(loci);
                } else {
                    msMarkers = new MicrosatelliteMarkersAnalysis(analysisID, sampleID,
                        encounterNumber, loci);
                }
                if (request.getParameter("processingLabTaskID") != null) {
                    msMarkers.setProcessingLabTaskID(request.getParameter("processingLabTaskID"));
                }
                if (request.getParameter("processingLabName") != null) {
                    msMarkers.setProcessingLabName(request.getParameter("processingLabName"));
                }
                if (request.getParameter("processingLabContactName") != null) {
                    msMarkers.setProcessingLabContactName(request.getParameter(
                        "processingLabContactName"));
                }
                if (request.getParameter("processingLabContactDetails") != null) {
                    msMarkers.setProcessingLabContactDetails(request.getParameter(
                ...
                enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                    ...
                    "Added or updated microsatellite markers of analysis " +
                    request.getParameter("analysisID") + " for tissue sample " +
                    request.getParameter("sampleID") + ".<br />" + msMarkers.getHTMLString());
            } catch (Exception le) {
                locked = true;
            ...
            if (!locked) {
                ...
                out.println(
                    "<strong>Success!</strong> I have successfully set the microsatellite markers for tissue sample "
            ...
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            out.println(
                "<strong>Error:</strong> I was unable to set the microsatellite markers. I cannot find the encounter or tissue sample that you intended it for in the database.");
            out.println(ServletUtilities.getFooter(context));
        }
        out.close();
        myShepherd.closeDBTransaction();
