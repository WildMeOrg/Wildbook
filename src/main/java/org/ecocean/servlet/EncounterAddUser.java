...
public class EncounterAddUser extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if ((request.getParameter("encounter") != null) && (request.getParameter("type") != null) &&
            ...
            (request.getParameter("email").indexOf("@") != -1) &&
            ...
            try {
                ...
                if (type.equals("submitter")) {
                    ...
                    changeMe.addComments("<p><em>" + request.getRemoteUser() + " on " +
                        (new java.util.Date()).toString() + "</em><br>Added user " +
            ...
            if (locked) {
                out.println(
                    "<strong>Failure:</strong> Encounter was NOT updated because another user is currently modifying this reconrd. Please try to reset the scarring again in a few seconds.");
            }
        } else {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            myShepherd.rollbackDBTransaction();
