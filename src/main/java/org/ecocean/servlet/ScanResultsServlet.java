...
public class ScanResultsServlet extends HttpServlet {
    ...
    public boolean writeXML(HttpServletRequest request, Vector results, String num,
        String newEncDate, String newEncShark, String newEncSize) {
        ...
        try {
            ...
            for (int i = 0; i < matches.length; i++) {
                ...
                match.addAttribute("adjustedpoints",
                    (new Double(mo.adjustedMatchValue)).toString());
                ...
                for (int k = 0; k < mo.scores.size(); k++) {
                    ...
                    spot.addAttribute("x",
                        (new Double(((VertexPointMatch)mo.scores.get(k)).oldX)).toString());
                    spot.addAttribute("y",
                        (new Double(((VertexPointMatch)mo.scores.get(k)).oldY)).toString());
                }
                ...
                for (int j = 0; j < mo.scores.size(); j++) {
                    ...
                    spot.addAttribute("x",
                        (new Double(((VertexPointMatch)mo.scores.get(j)).newX)).toString());
                    spot.addAttribute("y",
                        (new Double(((VertexPointMatch)mo.scores.get(j)).newY)).toString());
                }
            }
            ...
            File file = new File(Encounter.dir(shepherdDataDir,
                num) + "/lastFull" + fileAddition + "Scan.xml");
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            System.out.println(
                "scanResultsServlet: I successfully opened a stream from the applet.");

            // read the serialized results data from applet
            Vector results = (Vector)receiveObject(inputFromApplet);
            System.out.println(
                "scanResultsServlet: I successfully received the Vector, which had " +
                results.size() + " matchObjects in it.");
            ...
            if ((request.getParameter("writeThis") != null) &&
                (request.getParameter("writeThis").equals("true"))) {
