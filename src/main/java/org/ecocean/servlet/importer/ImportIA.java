...
public class ImportIA extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        boolean createOccurrences = Util.requestParameterSet(request.getParameter(
            "createOccurrences"));

        // whether to cluster all annots from an indiv into single encounters; usually related to above (default false)
        boolean clusterEncounters = Util.requestParameterSet(request.getParameter(
            "clusterEncounters"));

        out.println(
            "<h1>Starting ImportIA servlet | import task=<a href=\"obrowse.jsp?type=ImportTask&id="
            + itask.getId() + "\">" + itask.getId() + "</a></h1>");
        if (uid != null) {
            out.println("<p>submitter uid = <b>" + uid + "</b> (user => " + ((submitter ==
                null) ? "<i>invalid ID</i>" : submitter.getDisplayName()) + ")</p>");
        } else {
            out.println("<p><i>no submitter uid= passed</i></p>");
        }
        String urlSuffix = "/api/imageset/json/";
        if (!Util.requestParameterSet(request.getParameter("includeSpecial")))
            urlSuffix += "?is_special=False";
        ...
        if (imageSetRes == null && request.getParameter("doOnly") == null) {
            ...
            log(itask,
                "Got a result from IA but failed to parse fancyImageSetUUIDS. imageSetRes = " +
                imageSetRes);
            return;
        }
        int testingLimit = -1;
        ...
        for (int i = 0; i < fancyImageSetUUIDS.length(); i++) {
            ...
            JSONObject annotRes = getFromIA("/api/imageset/annot/uuid/json/?imageset_uuid_list=[" +
                fancyID + "]", context, out);
            ...
            if ((testingLimit > 0) && (annotUUIDs.size() >= testingLimit))
                annotUUIDs = annotUUIDs.subList(0, testingLimit);
            ...
            HashMap<String, ArrayList<Annotation> > annotGroups = new HashMap<String,
 ...
 */
                ...
                if (IBEISIA.unknownName(name) || !clusterEncounters) { // we need one encounter per annot for unknown!
                    for (Annotation ann : annotGroups.get(name)) {
                        if (hasEncounter.get(ann.getAcmId()) != null) {
                            log(itask,
                                "!! ann.acmId=" + ann.getAcmId() + " already has enc.id=" +
                                hasEncounter.get(ann.getAcmId()) + "; skipping");
                            continue;
                        }
                        ...
                        out.println("<p>Enc " + enc.getCatalogNumber() +
                            " from <a href=\"obrowse.jsp?type=Annotation&id=" + ann.getId() +
                            "\">Annot " + ann.getId() + "</a>");
                    ...
                    enc.setTaxonomy(IBEISIA.iaClassToTaxonomy(usable.get(0).getIAClass(),
                        myShepherd));
                    enc.setMatchedBy("IBEIS IA");
                    enc.setState("approved");
                    enc.addSubmitter(submitter);
                    ...
                    /*
                        ...
                        (see IBEISIA.iaDateTimeFromAnnotUUID() )  -- we might want to do that or fall back to that if the constructor fails to set
                           something.  tho... i think jasonp said annot times just come from images, so.

                        Similarly, it should also get lat/lon and species based upon IA values (if they have some).  note that IA also has sex,
                           however, it is stored on the "names" (i.e. individuals) so is not sighting-based ... so only useful to us if it is a new
                           individual.

                        Age, however, we dont store anywhere "lower", so we need to get that explicitly ... however IA has that on each Annotation, so
                           we just ... um.. i guess take the first one we can find?

                        note also that adding encounters to individuals should(!) gracefully adjust the individual accordingly (set sex/taxonomy *if
                           unset*)
                     */
                    String sex = null;
                    try {
                        sex = IBEISIA.iaSexFromAnnotUUID(annotGroups.get(name).get(0).getAcmId(),
                            context);
                    } catch (Exception ex) {}
                    Double age = null;
                    try {
                        // guess this assumes we have at least one annot and it has age; could walk thru if not?
                        age = IBEISIA.iaAgeFromAnnotUUID(annotGroups.get(name).get(0).getAcmId(),
                            context);
                    } catch (Exception ex) {}
                    if (age != null) enc.setAge(age);
                    if (sex != null) enc.setSex(sex);
                    ...
                    for (Annotation a : annotGroups.get(name)) {
                        annLog += " " + a;
                        annWeb += " <a href=\"obrowse.jsp?type=Annotation&id=" + a.getId() +
                            "\">Annot " + a.getId() + "</a> ";
                    }
                    log(itask, "name " + name + " created " + enc + " from " + annLog);
                    out.println("<p><b>Name " + name + "</b> Enc " + enc.getCatalogNumber() +
                        " from " + annWeb);
                    if (createOccurrences) {
                        myShepherd.beginDBTransaction();
                        if (occ == null) {
    ...
    private JSONObject getFromIA(String urlSuffix, String context, PrintWriter outForErrors)
    throws IOException {
        ...
        try {
            ...
            outForErrors.println("MalformedURLException on getFromIA()" + urlSuffix +
                "), which tried to GET " + restGetString);
    ...
    private static void log(ImportTask itask, String message) {
        ...
        System.out.println("ImportIA [" + itask.getId() + "] " +
            Util.prettyPrintDateTime(new DateTime()) + " " + message);
    }
}
