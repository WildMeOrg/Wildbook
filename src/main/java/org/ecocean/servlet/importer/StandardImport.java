...
public class StandardImport extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String individualScope = "user"; // accepts 'user', 'organization' and 'global'- default to
                                         // only matching in user catalog
    ...
    public void doImport(String filename, File dataFile, HttpServletRequest request,
        ...
        // initColIndexVariables(firstRow, colIndexMap, unusedColumns, skipCols, allColsMap,
        // feedback, sheet, committing, out); // IMPORTANT: this initializes the TabularFeedback

        // colIndexMap = makeColIndexMap(firstRow, skipCols, allColsMap, feedback, sheet,
        // committing, out);
        ...
        if (committing) {
            ...
            try {
                ...
                // CommonConfiguration.getURLLocation(request) + "/imports.jsp?taskId=" + taskID);
        ...
        out.println("<div class=\"col-sm-12 col-md-6 col-lg-6 col-xl-6\">"); // half page bootstrap
                                                                             // column
        ...
        if (!committing) {
            out.println("<div class=\"col-sm-12 col-md-6 col-lg-6 col-xl-6\">"); // half page
                                                                                 // bootstrap column
    ...
    public Occurrence loadOccurrence(Row row, Occurrence oldOcc, Encounter enc, Shepherd myShepherd,
        ...
        String groupBehavior = getString(row, "Occurrence.groupBehavior", colIndexMap, verbose,
            missingColumns, unusedColumns, feedback);
        // if no groupBehavior we want the behavior from an Encounter to copy over for occurrence
        // searches
    ...
    public Encounter loadEncounter(Row row, ArrayList<Annotation> annotations, String context,
        Shepherd myShepherd, Map<String, Integer> colIndexMap, boolean verbose, Set<String>
        missingColumns, Set<String> unusedColumns, String defaultSubmitterID, boolean committing,
        ...
        boolean hasTimeCategories = (year != null || month != null || day != null || hour != null ||
            minutes != null);
        // added sanity check for millis between 1900 and 2100.. some excel was giving 0 for millis
        // and making date stuff wierd
        if (millis != null && millis > -2208988800000L && millis < 4102444800000L) {
            if (hasTimeCategories) enc.setDateInMillisOnly(millis); // does not overwrite
                                                                    // day/month/etc
        ...
        // genotype
        /*
         * String markerAnalysisID = getStringOrInt(row,
         ...
         * if (markerAnalysisID!=null) markerAnalysisID =
         * markerAnalysisID+"-enc-"+encID.substring(0,Math.min(8,encID.length()));
         * if (markerAnalysisID!=null && !myShepherd.isGeneticAnalysis(markerAnalysisID)) {
         * markerAnalysisID = markerAnalysisID.replaceAll("_","-");
         * MicrosatelliteMarkersAnalysis microMark =
         * myShepherd.getMicrosatelliteMarkersAnalysis(markerAnalysisID);
         * if (microMark==null) {
        ...
        if (satelliteTag != null) {
            SatelliteTag tag = new SatelliteTag("", satelliteTag, ""); // note the empty fields. sat
                                                                       // tags are weird.
        ...
        try {
            for (String columnHeader : colIndexMap.keySet()) {
                if (columnHeader.contains(className + ".")) {
                    fieldNames.add(columnHeader.split(className + ".")[1]); // for Encounter.date
                                                                            // returns date
                }
            }
        } catch (Exception e) {
...
// private ArrayList<Annotation> loadAnnotationsFolderRow(Row row, AssetStore astore, Shepherd
// myShepherd) {
// ArrayList<Annotation> annots = new ArrayList<Annotation>();
...
// System.out.println("StandardImport ERROR: loadAnnotationsFolderRow called on non-directory (or
// empty?) path "+fullPath);
...
// }
    ...
    public MediaAsset getMediaAsset(Row row, int i, AssetStore astore, Shepherd myShepherd, Map<
        ...
        if (isUserUpload) {
            // user uploads currently flatten all images into a folder (TODO fix that!) so we trim
            // extensions
            try {
    ...
    // TODO in a perfect world, we would also check db for assets with same hash!!  but then we need
    // a shepherd.  SIGH
    ...
    public MarkedIndividual loadIndividual(Row row, Encounter enc, Shepherd myShepherd, boolean
        ...
        // add the entered name, make sure it's attached to either the labelled organization, or
        ...
        try {
            ...
            if (!newIndividual) {
                ...
                // System.out.println("loadIndividual notnew individual:
                // "+mark.getDisplayName(request, myShepherd));
            } else {
                enc.setIndividual(mark);
            }
            if (committing) {
 ...
 */
    ...
    public Integer getInteger(Row row, int i, TabularFeedback feedback) {
        ...
        try {
            ...
            // case for when we have a weird String-Double, which looks like a double in the excel
            // sheet, yet is cast as a String, AND has a leading apostrophe in its stored value that
            // prevents us from parsing it as a number.
            // e.printStackTrace();
            try {
    ...
    private User getUserForRowOrCurrent(Row row, Shepherd myShepherd, Map<String, Integer>
    ...
    private AssetStore getAssetStore(Shepherd myShepherd) {
        ...
        // AssetStore as = new LocalAssetStore("Oman Import", new File(assetStorePath).toPath(),
        // assetStoreURL, true);
    ...
    // cannot put this inside CellFeedback bc java inner classes are not allowed static methods or
    // vars (this is stupid).
    ...
    private boolean emptyAssetColumn(int i, HashMap<String, Integer> allColsMap, List<Integer>
        skipCols) {
        // FIX THIS: necessary because skipCols doesn't well handle open ended col names, like
        // mediaAsset
