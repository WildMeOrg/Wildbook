...
public class UpdateStandard extends HttpServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException,  IOException {
    ...
    for (String heading: Util.asSortedList(colIndexMap.keySet())) out.println("<li>"+colIndexMap.get(heading)+": "+heading+"</li>");
    out.println("</ul>");
    ...
    for (rownum=1+offset; rownum<rows&&rownum<(numRows+offset); rownum++) {
      ...
      try {
        ...
        if (verbose) {
          out.println("<li>Parsed row ("+rownum+")<ul>"
          +"<li> Enc "+getEncounterDisplayString(enc)+" <ul>");
          for (MediaAsset ma: enc.getMedia()) {
            out.println("<li>"+ma.toString()+", filename "+ma.getFilename()+"<ul>");
...
//  	localPath = localPath.replace(" ","\\ ");
  ...
  private Keyword getLabeledKeyword(Row row, int n, String label) {
    ...
    System.out.println("getLabeledKeyword success! row "+rownum+", n="+n+", label "+label);
  ...
  private ArrayList<Keyword> getLabeledKeywords(Row row, int n) {
    ...
    if (verbose) System.out.println("getLabeledKeywords returning "+keys.size()+" keywords on row "+rownum+", n="+n);
    return keys;
  }
