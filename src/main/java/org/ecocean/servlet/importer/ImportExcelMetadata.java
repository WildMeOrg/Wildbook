...
public class ImportExcelMetadata extends HttpServlet {
        ...
        try {
            ...
            while ((part = mp.readNextPart()) != null) {
                ...
                if (part.isFile()) {
                    ...
                    if (fileName != null) {
                        System.out.println("ImportExcelMetadata is trying to upload file " +
                            fileName);
            ...
            try {
                if (successfullyWroteFile) {
                    ...
                    for (int i = 1; i < numRows; i++) {
                        ...
                        if ((encNumber != null) && (!encNumber.equals(""))) {
                            ...
                            messages.append("<li>Row " + i +
                                ": could not find sample/encounter ID in the first column of row " +
                                i + ".</li>");
                            System.out.println(
                                "          Could not find sample/encounter ID in the first column of row "
                                + i + ".");
                        ...
                        if (individualID != null) {
                            enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                                ...
                                "Import SRGD process set marked individual to " + individualID +
                                ".</p>");

                            // enc.setIndividualID(individualID);
                            System.out.println("          Setting Individual ID for row " + i +
                                ". Value: " + individualID);
                        }
                        ...
                        if ((latitude != null) && (!latitude.equals(""))) {
                            try {
                                ...
                                System.out.println("          Setting latitude for row " + i +
                                    ". Value: " + latitude);
                            } catch (NumberFormatException nfe) {
                                messages.append("<li>Row " + i + " for sample ID " +
                                    enc.getCatalogNumber() +
                                    ": Latitude hit a NumberFormatException in row " + i +
                                    " and could not be imported. The listed value was: " +
                                    latitude + "</li>");
                            }
                        }
                        ...
                        if ((longitude != null) && (!longitude.equals(""))) {
                            try {
                                ...
                                System.out.println("          Setting longitude for row " + i +
                                    ". Value: " + longitude);
                            } catch (NumberFormatException nfe) {
                                nfe.printStackTrace();
                                messages.append("<li>Row " + i + " for sample ID " +
                        ...
                        if ((isoDate != null) && (!isoDate.equals(""))) {
                            ...
                            try {
                                ...
                                System.out.println("          Set date for encounter: " +
                                    enc.getDate());
                            } catch (IllegalArgumentException iae) {
                                iae.printStackTrace();
                                messages.append("<li>Row " + i +
                                    ": could not import the date and time for row: " + i +
                                    ". Cancelling the import for this row.</li>");
                                ok2import = false;
                            }
                        }
                        ...
                        if (line.length >= 6) {
                            if ((locationID != null) && (!locationID.equals(""))) {
                                ...
                                enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                                    (new java.util.Date()).toString() + "</em><br>" +
                                    "Import SRGD process set location ID to " + locationID +
                                    ".</p>");

                                System.out.println("          Setting location ID for row " + i +
                                    ". Value: " + locationID);
                            }
                        }
                        // line[6] get sex
                        ...
                        if (line.length >= 7) {
                            ...
                            if (occurrenceIDColumnNumber != -1) {
                                ...
                                if (myShepherd.isOccurrence(occurID)) {
                                    ...
                                    if (isNew) {
                                        occur.addComments("<p><em>" + request.getRemoteUser() +
                                            " on " + (new java.util.Date()).toString() +
                                            "</em><br>" + "Import SRGD process added encounter " +
                                            enc.getCatalogNumber() + ".</p>");
                                    }
                                } else {
                                    occur = new Occurrence(occurID, enc);
                                    occur.addComments("<p><em>" + request.getRemoteUser() + " on " +
                                        (new java.util.Date()).toString() + "</em><br>" +
                                        "Import SRGD process added encounter " +
                                        enc.getCatalogNumber() + ".</p>");
                        ...
                        if (ok2import) {
                            ...
                            if ((sex != null) && (!sex.equals(""))) {
                                SexAnalysis sexDNA = new SexAnalysis(("analysis_" +
                                    enc3.getCatalogNumber() + "_sex"), sex, enc3.getCatalogNumber(),
                                ...
                                enc3.addComments("<p><em>" + request.getRemoteUser() + " on " +
                                    ...
                                    sexDNA.getAnalysisID() + " for tissue sample " +
                                    ts.getSampleID() + ".<br />" + sexDNA.getHTMLString());
                            }
                            ...
                            if (line.length >= 8) {
                                ...
                                if ((haplo != null) && (!haplo.equals(""))) {
                                    ...
                                    MitochondrialDNAAnalysis mtDNA = new MitochondrialDNAAnalysis(
                                        ("analysis_" + enc3.getCatalogNumber()), haplo,
                                        enc3.getCatalogNumber(),
                                        ("sample_" + enc3.getCatalogNumber()));
                                    if (myShepherd.isGeneticAnalysis(ts.getSampleID(), encNumber,
                                        ("analysis_" + enc3.getCatalogNumber()),
                                        "MitochondrialDNA")) {
                                        mtDNA = myShepherd.getMitochondrialDNAAnalysis(
                                            ts.getSampleID(), encNumber,
                                            ("analysis_" + enc3.getCatalogNumber()));
                                    ...
                                    enc3.addComments("<p><em>" + request.getRemoteUser() + " on " +
                                        ...
                                        + mtDNA.getAnalysisID() + " for tissue sample " +
                                        ts.getSampleID() + ".<br />" + mtDNA.getHTMLString());
                            ...
                            if (line.length >= 9) {
                                for (int f = 8; f < numColumns; f++) {
                                    if (line.length > (f + 2)) {
                                        ...
                                        if ((l1 != null) && (l2 != null) && (!l1.equals("")) &&
                                            (!l2.equals("")) && (!locusName.equals("")) &&
                                            (headerNames[f].trim().toLowerCase().startsWith(
                                            "l_")) &&
                                            (headerNames[f +
                                            1].trim().toLowerCase().startsWith("l_")) &&
                                            (headerNames[f].trim().toLowerCase().equals(
                                            headerNames[f + 1].trim().toLowerCase()))) {
                                            // get allele values
                            ...
                            if (loci.size() > 0) {
                                ...
                                MicrosatelliteMarkersAnalysis microAnalysis =
                                    new MicrosatelliteMarkersAnalysis((ts.getSampleID() +
                                    "_msMarkerAnalysis"), ts.getSampleID(), enc.getCatalogNumber(),
                                    loci);
                                if (myShepherd.isGeneticAnalysis(ts.getSampleID(), encNumber,
                                    (ts.getSampleID() + "_msMarkerAnalysis"),
                                    "MicrosatelliteMarkers")) {
                                    microAnalysis = myShepherd.getMicrosatelliteMarkersAnalysis(
                                        ts.getSampleID(), encNumber,
                                        (ts.getSampleID() + "_msMarkerAnalysis"));
                                ...
                                enc3.addComments("<p><em>" + request.getRemoteUser() + " on " +
                                    ...
                                    + microAnalysis.getAnalysisID() + " for tissue sample " +
                                    ts.getSampleID() + ".<br />" + microAnalysis.getHTMLString());
                            }
                            myShepherd.commitDBTransaction();
                    ...
                    locked = true;
                    System.out.println(
                        "ImportSRGD: For some reason the import failed without exception.");
                }
            } catch (Exception le) {
                locked = true;
            ...
            if (!locked) {
                ...
                out.println(
                    "<p>The following error messages were reported during the import process:<br /><ul>"
                    + messages + "</ul></p>");
            ...
            out.println(ServletUtilities.getHeader(request));
            out.println(
                "<strong>Error:</strong> I was unable to import SRGD data as no file was specified.");
            out.println(ServletUtilities.getFooter(context));
        }
        out.close();
    }
    ...
    public void processExcel(File dataFile, HttpServletResponse response, boolean committing,
        Hashtable<String, MediaAsset> assetIds, Shepherd myShepherd, PrintWriter out)
    throws IOException {
                ...
                if (committing && isValid == true) {
                    ...
                    if (i % printPeriod == 0) {
                        out.println("Parsed row (" + i + "), containing Enc " +
                            enc.getEncounterNumber() + " with Latitude " +
                            enc.getDecimalLatitude() + " and Longitude " +
                            enc.getDecimalLongitude() + ", dateInMillis " +
                            enc.getDateInMilliseconds() + ", individualID " +
                            enc.getIndividualID() + ", sex " + enc.getSex() + ", living status " +
                            enc.getLivingStatus() + ", identification notes " +
