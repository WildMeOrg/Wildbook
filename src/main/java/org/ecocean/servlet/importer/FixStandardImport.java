...
public class
FixStandardImport extends HttpServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException,  IOException {
    ...
    for (int i=1+offset; i<rows&&i<(maxRows+offset); i++) {
      ...
      try {
        ...
        if (verbose) {
          out.println("<li>Fixed row ("+i+") with Enc "+getEncounterDisplayString(enc)+"<ul>"
          +"<li> year "+enc.getYear()+"</li>"
  ...
  public Encounter fixEncounter(Row row) {
    ...
    if (patterningCode==null) patterningCode = getIntegerFromString(row, "Encounter.patterningCode");
...
//  	localPath = localPath.replace(" ","\\ ");
  ...
  public MarkedIndividual loadIndividual(Row row, Encounter enc) {
    ...
    if (mark==null) return mark;
  	...
  	if (Util.shouldReplace(species, mark.getSpecificEpithet())) mark.setSpecificEpithet(species);
