...
public class EncounterSetLifeStage extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if ((myShepherd.isEncounter(encNum)) && (request.getParameter("lifeStage") != null)) {
            ...
            try {
                if (lifeStage.equals("")) { enc.setLifeStage(null); } else {
                    enc.setLifeStage(lifeStage);
                }
                enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                    (new java.util.Date()).toString() + "</em><br>Changed life stage to " +
                    request.getParameter("lifeStage") + ".</p>");
            } catch (Exception le) {
                locked = true;
            ...
            if (!locked) {
                ...
                out.println(
                    "<strong>Success!</strong> I have successfully changed the lifeStage for encounter "
                    + encNum + " to " + lifeStage + ".</p>");
                ...
                String message = "The lifeStage for encounter " + encNum + " was set to " +
                    lifeStage + ".";
            } else {
                // out.println(ServletUtilities.getHeader(request));
                out.println(
            ...
            // out.println(ServletUtilities.getHeader(request));
            out.println(
                "<strong>Error:</strong> I was unable to set the lifeStage. I cannot find the encounter that you intended in the database.");
            // out.println(ServletUtilities.getFooter(context));
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
        }
        out.close();
