...
public class EncounterRemoveSpots extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (request.getParameter("number") != null) {
            ...
            try {
                ...
                if (despotMe.getIndividualID() == null) {
                    if ((request.getParameter("rightSide") != null) &&
                        ...
                        side = "right";
                        despotMe.addComments("<p><em>" + request.getRemoteUser() + " on " +
                            (new java.util.Date()).toString() + "</em><br>Removed " + side +
                            "-side spot data.</p>");
                        // despotMe.setNumRightSpots(0);
                    } else if ((request.getParameter("rightSide") != null) &&
                        (request.getParameter("rightSide").equals("false"))) {
            ...
            if (!locked) {
                out.println("<strong>Success:</strong> I have removed spot data for encounter " +
                ...
                String message = "The spot-matching data for encounter " +
                ...
                if (assigned) {
                    out.println(
                        "<strong>Failure:</strong> I was NOT able to remove the spot data because the encounter has been assigned to a marked individual. Please try to remove the spot data again after removing the encounter from the individual if appropriate.");
                    ...
                    out.println(
                        "<strong>Failure:</strong> I was NOT able to remove the spot data because another user is currently modifying this encounter, or you did not specify a side to remove spot data from. Please try to remove the spot data again in a few seconds.");
                    out.println("<p><a href=\"http://" +
                        CommonConfiguration.getURLLocation(request) +
                        "/encounters/encounter.jsp?number=" + request.getParameter("number") +
                        "\">Return to encounter #" + request.getParameter("number") + "</a></p>\n");
