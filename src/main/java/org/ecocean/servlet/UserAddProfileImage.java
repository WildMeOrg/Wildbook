        ...
        try {
            ...
            while ((part = mp.readNextPart()) != null) {
                ...
                if (part.isFile()) {
                    ...
                    if (fileName != null) {
                        File thisSharkDir = new File(encountersDir.getAbsolutePath() + "/" +
                            username);
            ...
            if (myShepherd.getUser(username) != null) {
                ...
                try {
                    SinglePhotoVideo spv = new SinglePhotoVideo(username,
                    ...
                    // enc.addComments("<p><em>" + request.getRemoteUser() + " on " + (new java.util.Date()).toString() + "</em><br>" + "Submitted new
                    // encounter image graphic: " + fileName + ".</p>");
                ...
                if (!locked) {
                    ...
                    out.println(
                        "<strong>Success!</strong> I have successfully uploaded the user profile image file.");
                    ...
                    if (request.getRequestURL().indexOf("MyAccount") != -1) {
                        ...
                        out.println("<p><a href=\"" + request.getScheme() + "://" +
                            CommonConfiguration.getURLLocation(request) +
                            "/appadmin/users.jsp?context=context0&isEdit=true&username=" +
                            username + "#editUser\">Return to User Management.</a></p>\n");
                    }
                    ...
                    out.println(ServletUtilities.getHeader(request));
                    out.println(
                        "<strong>Failure!</strong> This User account is currently being modified by another user. Please wait a few seconds before trying to add this image again.");
                    out.println("<p><a href=\"" + request.getScheme() + "://" +
                        CommonConfiguration.getURLLocation(request) +
                        "/appadmin/users.jsp?context=context0\">Return to User Management</a></p>\n");
                    out.println(ServletUtilities.getFooter(context));
                }
            } else {
                myShepherd.rollbackDBTransaction();
                ...
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                out.println(
                    "<strong>Error:</strong> I was unable to upload your image file. I cannot find the username that you intended it for in the database.");
                out.println(ServletUtilities.getFooter(context));
            }
            ...
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            out.println(
                "<strong>Error:</strong> I was unable to upload your image file. Please contact the webmaster about this message.");
            out.println(ServletUtilities.getFooter(context));
