...
public class MergeIndividual extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            oldName1 = mark1.getDisplayName(request,
                myShepherd) + "(" + Util.prettyUUID(mark1.getIndividualID()) + ")";
            oldName2 = mark2.getDisplayName(request,
                myShepherd) + "(" + Util.prettyUUID(mark2.getIndividualID()) + ")";
            ...
            boolean throwaway = Util.stringExists(throwawayStr) &&
                !throwawayStr.toLowerCase().equals("false");
            ...
            if (currentUsername != null) {
                ...
                if (request.isUserInRole("admin") ||
                    (Collaboration.canUserFullyEditMarkedIndividual(mark1,
            ...
            if (canMergeAutomatically) {
                ...
                boolean incrementalIdAndProjectNameListsAreTheSameSize =
                    deprecatedIncrementIdsArr.size() == desiredIncrementalIdArr.size() &&
                    deprecatedIncrementIdsArr.size() == projNamesArr.size() &&
                    deprecatedIncrementIdsArr.size() == projIdsArr.size() &&
                    desiredIncrementalIdArr.size() == projNamesArr.size() &&
                    projNamesArr.size() == projIdsArr.size();
                if (desiredIncrementalIdArr.size() > 0 &&
                    incrementalIdAndProjectNameListsAreTheSameSize) {
                    for (int i = 0; i < desiredIncrementalIdArr.size(); i++) {
                        if (!deprecatedIncrementIdsArr.get(i).equals("_")) {
                            ...
                            mark1.addName("Merged " + projNamesArr.get(i),
                                deprecatedIncrementIdsArr.get(i));
                    ...
                    System.out.println(
                        "Error! incrementalIdAndProjectNameListsAreTheSameSize was false! ProjectID-related names were not successfully added to marked individual: "
                        + mark1.getIndividualID());
                }
        ...
        if (!locked && canMergeAutomatically) {
            out.println("<strong>Success!</strong> I have successfully merged individuals " + id1 +
                " and " + id2 + ".</p>");
            out.close();
            ...
            out.println(
                "<strong>Pending:</strong> Participating user have been notified of your request to merge individuals "
                + id1 + " and " + id2 + ".</p>");
            out.close();
            response.setStatus(HttpServletResponse.SC_OK);
        } else {
