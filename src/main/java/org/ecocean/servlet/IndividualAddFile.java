        ...
        try {
            ...
            while ((part = mp.readNextPart()) != null) {
                ...
                if (part.isFile()) {
                    ...
                    if (fileName != null) {
                        // File individualsDir = new File(getServletContext().getRealPath(("/" +
                        // CommonConfiguration.getMarkedIndividualDirectory())));
                        /*if (!individualsDir.exists()) {
                           individualsDir.mkdirs();
                           }*/

                        File thisSharkDir = new File(individualsDir.getAbsolutePath() + "/" +
                            individualName);
                        if (!(thisSharkDir.exists())) {
                            thisSharkDir.mkdirs();
                        }
                        long file_size = filePart.writeTo(new File(thisSharkDir, fileName)
                            );
                    }
                }
            }
            ...
            if (myShepherd.isMarkedIndividual(individualName)) {
                ...
                try {
                    ...
                    add2shark.addComments("<p><em>" + request.getRemoteUser() + " on " +
                        (new java.util.Date()).toString() + "</em><br>" + "Submitted new file: " +
                        fileName + ".</p>");
                } catch (Exception le) {
                    locked = true;
                ...
                if (!locked) {
                    ...
                    out.println(
                        "<strong>Success:</strong> I have successfully uploaded your data file.");
                    // out.println("<p><a href=\""+request.getScheme()+"://" + CommonConfiguration.getURLLocation(request) +
                    // "/individuals.jsp?number=" + individualName + "\">Return to " + individualName + "</a></p>\n");
                    // out.println(ServletUtilities.getFooter(context));
                    // String message="A new data file named "+fileName+" has been added to "+request.getParameter("individual")+".";
                } else {
                    // out.println(ServletUtilities.getHeader(request));
                    out.println(
                        "<strong>Failure:</strong> I failed to add your file. This record is currently being modified by another user. Please try to add the file again in a few seconds.");
                    // out.println("<p><a href=\""+request.getScheme()+"://" + CommonConfiguration.getURLLocation(request) +
                    // "/individuals.jsp?number=" + individualName + "\">Return to " + individualName + "</a></p>\n");
                    // out.println(ServletUtilities.getFooter(context));
                }
                ...
                // out.println(ServletUtilities.getHeader(request));
                out.println(
                    "<strong>Error:</strong> I was unable to upload your file. I cannot find the record that you intended it for in the database.");
                // out.println(ServletUtilities.getFooter(context));
