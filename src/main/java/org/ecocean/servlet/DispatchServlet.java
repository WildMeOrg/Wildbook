...
/**
 * Servlet base class which dispatches calls to delegate methods based on the value of the extra path info in the servlet URL. For example, if the
 * servlet is called with:
 *     {@code &lt;servletPath&gt;/foo}
 * then the method
 *     {@code foo(HttpServletRequest, HttpServletResponse)}
 * will be called within the servlet. Within the context of this class, the method {@code foo} is considered the delegate method.
 * <p>
 * Sub-classes should register which methods they use with the various
 * {@code registerXXX()} methods, otherwise they will not be accessible. This is best done in the {@code init()} method.
 *
 * @author Giles Winstanley
 */
abstract class DispatchServlet extends HttpServlet {
    /** Class references for reflection. */
    protected static final Class[] SERVLET_ARGS = {
     ...
     */
    protected File getDataDir(String context)
     ...
     * @return The data directory used for web application storage.
     */
    protected File getUsersDataDir(String context)
    throws FileNotFoundException {
     ...
     */
    protected final void registerMethodGET(String... methodName)
     ...
     * @throws DelegateNotFoundException if the named method is not found
     */
    protected final void unregisterMethodGET(String... methodName)
    throws DelegateNotFoundException {
        for (String s : methodName)
            methodsGET.remove(s);
    }
     ...
     * @throws DelegateNotFoundException if the named method is not found
     */
    protected final void registerMethodPOST(String... methodName)
    throws DelegateNotFoundException {
        for (String s : methodName) {
            if (getDelegateMethod(s) != null)
                methodsPOST.add(s);
     ...
     * @throws DelegateNotFoundException if the named method is not found
     */
    protected final void unregisterMethodPOST(String... methodName)
    throws DelegateNotFoundException {
        for (String s : methodName)
     ...
     */
    protected void dispatchToDelegate(HttpServlet target, Method method, HttpServletRequest req,
    ...
    /**
     * Returns the delegate {@code Method} of the class instance with the specified method name. The delegate method must have the standard
     * request/response parameter arguments.
     *
     * @param methodName name of delegate method
     */
    protected Method getDelegateMethod(String methodName)
    throws DelegateNotFoundException {
        try {
     ...
     * @throws IOException
     */
    abstract protected void handleDelegateNotFound(HttpServletRequest req, HttpServletResponse res)
    throws ServletException, IOException;
