...
/**
 * Populates a patterning passport for a specific image/mediafile encounterId mediaId
 *
 * @author estastny
 */
public class EncounterSetPatterningPassport extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        MultipartRequest multipartRequest = new MultipartRequest(request, fileUploadLocation);
...
// String originalFilename = multipartRequest.getOriginalFileName(name);
        ...
        if (mediaId != null && encounterId != null && xmlFile != null) {
            responseMsg += this.setPassportForMediaObject(mediaId, encounterId, xmlFile, context);
        } else {
            responseMsg += "<br/>Not enough data to setPassportForMediaObject. <ul><li>" + mediaId +
                "<li>" + encounterId + "</ul> ";
        }
        // response
