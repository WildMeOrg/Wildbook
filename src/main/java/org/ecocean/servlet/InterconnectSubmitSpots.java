...
public class InterconnectSubmitSpots extends HttpServlet {
    private void deleteOldScans(String side, String num) {
        try {
            ...
            file = new File(getServletContext().getRealPath(("/encounters/" + num + "/lastBoost" +
                side + "Scan.xml")));
            if (file.exists()) {
                file.delete();
            }
        ...
        if (myShepherd.isEncounter(num)) {
            ...
            try {
                if (enc.getIndividualID() == null) {
                    ok2add = true;
                    for (int i = 0; i < 200; i++) {
                        if ((request.getParameter("spotx" + (new Integer(i)).toString()) != null) &&
                            (request.getParameter("spoty" + (new Integer(i)).toString()) != null)) {
 ...
 */
                        ...
                        enc.addComments("<p><em>" + user + " on " +
                            (new java.util.Date()).toString() + "</em><br>" + "Added " + side +
                            "-side spot data." + "</p>");
                    }
                    ...
                    out.println(
                        "<p>You are not allowed to modify spot data for an encounter that belongs to a shark. Please remove the encounter from the shark before attempting to modify its spot data.</p>");
                ...
                out.println(ServletUtilities.getHeader(request));
                out.println(
                    "<p>The spot pattern was only partially transmitted, resulting in a NullPointerException.</p>");
                out.println(ServletUtilities.getFooter(context));
                npe.printStackTrace();
                ok2add = false;
                ...
                out.println(ServletUtilities.getHeader(request));
                out.println(
                    "<p>This encounter object is in a locked state and may be in use by another user or may be locked in error.</p>");
                out.println(ServletUtilities.getFooter(context));
            ...
            if (!locked && ok2add) {
                ...
                try {
                    ...
                    if (locked) {
                        ...
                        out.println(
                            "<p>This object is currently in use by another user. Please wait a few seconds and then attempt to add spot data again.</p>");
                    } else if ((enc.getSpots() != null) && haveData && (side.equals("left"))) {
                        out.print("<strong>Step 1 Confirmed:</strong> Thank you for submitting " +
                            side + "-side spot data for encounter number " + num + "!</p>\n");
                        if (side.equals("right")) {
                            out.println("<p>" + enc.getNumRightSpots() +
                                " right-side spots were successfully added.</p>");
                        } else {
                            out.println("<p>" + enc.getNumSpots() +
                                " left-side spots were successfully added.</p>");
                        }
                        response.setStatus(HttpServletResponse.SC_OK);
                        out.println("<p><strong>Step 2: Upload the spot " + side +
                            "-side image that you extracted the spot data from.</strong>");
                        out.println(
                            "<form action=\"EncounterAddSpotFile\" method=\"post\" enctype=\"multipart/form-data\" name=\"addSpotsFile\">");
                        out.println(
                            "<input name=\"action\" type=\"hidden\" value=\"fileadder\" id=\"action\">");
                        out.println("<input name=\"number\" type=\"hidden\" value=\"" + num +
                            "\" id=\"number\">");
                        if (side.equals("right")) {
                            out.println(
                                "<input name=\"rightSide\" type=\"hidden\" value=\"true\" id=\"rightSide\">");
                        }
                        out.println("<p>Add spot data image file:</p>");
                        out.println("<p><input name=\"file2add\" type=\"file\" size=\"20\"></p>");
                        out.println(
                            "<p><input name=\"addtlFile\" type=\"submit\" id=\"addtlFile\" value=\"Upload spot image file\"></p>");
                    ...
                    // check for right-side spot submissions
                        ...
                        out.println(
                            "<p>Unfortunately, no spots were added to this shark. Check to see if you correctly added any data in the previous form.</p>");
                        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    }
                    out.println("<p><a href=\"http://" +
                        CommonConfiguration.getURLLocation(request) +
                        "/encounters/encounter.jsp?number=" + num + "\">Return to encounter #" +
                        num + "</a></p>\n");
                    out.println(ServletUtilities.getFooter(context));
            ...
            try {
                out.println(ServletUtilities.getHeader(request));
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                out.println("<p>You did not specify a valid number for this encounter: " + num +
                    "</p>");
                out.println(ServletUtilities.getFooter(context));
            } catch (Exception e) {
                out.println(
                    "I couldn't find the template file to write to, but the spots were added successfully.");
                e.printStackTrace();
            }
        }
        out.close();
