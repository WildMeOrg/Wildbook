...
public final class MantaMatcher extends DispatchServlet {
    ...
    @Override protected void handleDelegateNotFound(HttpServletRequest req, HttpServletResponse res)
    ...
    public void displayResults(HttpServletRequest req, HttpServletResponse res)
    throws ServletException, IOException {
    ...
    public void displayResultsRegional(HttpServletRequest req, HttpServletResponse res)
    throws ServletException, IOException {
        String context = "context0";
        ...
        try {
            ...
            Map<String, File> mmMap = MantaMatcherUtilities.getMatcherFilesMap(spv);
            MMAResultsProcessor.MMAResult mmaResults = parseResults(req, mmMap.get("TXT-REGIONAL"),
                spv);
            req.setAttribute(REQUEST_KEY_RESULTS, mmaResults);
            getServletContext().getRequestDispatcher(JSP_MMA_RESULTS).forward(req, res);
        } catch (Exception ex) {
    ...
    private MMAResultsProcessor.MMAResult parseResults(HttpServletRequest req, File mmaResults,
    ...
    public void resetMmaCompatible(HttpServletRequest req, HttpServletResponse res)
    throws ServletException, IOException {
        String context = "context0";
        ...
        shepherd.beginDBTransaction();
        File dataDir = new File(ServletUtilities.dataDir(context,
        ...
        try {
            ...
            for (Iterator<Encounter> iter = shepherd.getAllEncounters(); iter.hasNext();) {
                ...
                if ((hasCR && encCR) || (!hasCR && !encCR)) {
                    ...
                    try {
                        ...
                        failed++;
                        log.warn("Failed to set MMA-compatible flag for encounter: " +
                            enc.getCatalogNumber(), ex);
                    }
                }
            }
            shepherd.commitDBTransaction();
            ...
            if (failed > 0) {
                out.println(String.format(
                    "<strong>Failure!</strong> I failed to reset all the MMA-compatibility flags; %d failed to update, %d were updated successfully, and %d were already correct.",
                    failed, changed, ok));
            } else {
                out.println(String.format(
                    "<strong>Success!</strong> I have successfully reset the MMA-compatibility flag for %d encounters (%d were already correct).",
                    changed, ok));
            }
            out.println("<p><a href=\"http://" + CommonConfiguration.getURLLocation(req) +
    ...
    public void deleteAllOrphanMatcherFiles(HttpServletRequest req, HttpServletResponse res)
    throws ServletException, IOException {
        String context = "context0";
        ...
        String pageUrlFormatEnc = "//" + CommonConfiguration.getURLLocation(req) +
            "/encounters/encounter.jsp?number=%s";
        ...
        try {
            ...
            for (Iterator<Encounter> iter = shepherd.getAllEncounters(); iter.hasNext();) {
                ...
                RegexFilenameFilter ff1 = new RegexFilenameFilter("^.+_(CR|EH|FT)\\." +
                    MediaUtilities.REGEX_SUFFIX_FOR_WEB_IMAGES);
                RegexFilenameFilter ff2 = new RegexFilenameFilter("^.+\\.FEAT$");
                RegexFilenameFilter ff3 = new RegexFilenameFilter(
                    "^.+_mma(In|Out)put(Regional)?\\.(txt|csv)$");
            ...
            try (PrintWriter out = res.getWriter()) {
                ...
                if (!failed.isEmpty()) {
                    out.println(String.format(
                        "<strong>Failure!</strong> I failed to delete all orphan MantaMatcher algorithm files (%2$d of %1$d couldn't be deleted).",
                        files.size(), failed.size()));
                } else {
                    out.println(String.format(
                        "<strong>Success!</strong> I have successfully deleted all orphan MantaMatcher algorithm files (%d were found).",
                        files.size()));
                }
                ...
                for (Map.Entry<File, String> me : files.entrySet()) {
                    ...
                    if (failed.containsKey(me.getKey())) {
                        out.println(String.format(
                            "<li>Failed to delete file: %s (<a href=\"%s\">%s</a>)</li>",
                            f.getName(), url, encNum));
                    } else {
                        out.println(String.format(
                            "<li>Successfully deleted file: %s (<a href=\"%s\">%s</a>)</li>",
                            f.getName(), url, encNum));
                    }
                }
                out.println("</ul>");
