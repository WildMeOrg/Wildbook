...
public class SurveyCreate extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (date != null) {
            try {
                ...
                message +=
                    "<p><strong>Error: </strong> This survey did not recieve a valid date.</p>";
                e.printStackTrace();
            }
        } else {
            message +=
                "<p style=\"color:red;\"><strong>Error: </strong>Survey must be submitted with a date.</p>";
        }
        try {
            if (date != null) {
                String project = null;
                ...
                message += "<p><strong>Success: </strong> A new survey on " + date +
                    " was created.</p>";
                message +=
                    "<p><strong>View Survey Page: </strong><a href=\"/surveys/survey.jsp?surveyID="
                    + sv.getID() + "\">Survey ID: " + sv.getID() + "</a></p>";
                message +=
                    "<p>From the Survey home page you can add occurrences and other info.</p>";
            }
        } catch (Exception e) {
            message +=
                "<p><strong>Error: </strong>There was data included in the submission that the server could not process.</p>";
            myShepherd.rollbackDBTransaction();
            e.printStackTrace();
        }
    ...
    private void printResultMessage(HttpServletRequest request, String context) {
        ...
        message +=
            "<p><strong>Back: </strong><a href=\"/surveys/createSurvey.jsp\">Return to Survey Creation Page</a></p>";
        out.println(message);
