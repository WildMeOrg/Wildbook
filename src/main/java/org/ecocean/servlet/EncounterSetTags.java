    ...
    throws ServletException, IOException {
        ...
        if (myShepherd.isEncounter(encNum)) {
            ...
            try {
                ...
                if ("metalTags".equals(tagType)) {
                    ...
                    for (String metalTagParamName : metalTagParamNames) {
                        ...
                        String location = metalTagParamName.substring(
                            METAL_TAG_PARAM_START.length(), metalTagParamName.length() - 1);
                        ...
                        sb.append(MessageFormat.format("<br/>Metal Tag {0} set to {1}", location,
                            getParam(request, metalTagParamName)));
                    }
                    ...
                    sb.append(MessageFormat.format("<br/>{0} set to {1}", ACOUSTIC_TAG_ID,
                        getParam(request, ACOUSTIC_TAG_ID)));
                    sb.append(MessageFormat.format("<br/>{0} set to {1}", ACOUSTIC_TAG_SERIAL,
                        getParam(request, ACOUSTIC_TAG_SERIAL)));
                    ...
                    satelliteTag.setArgosPttNumber(getParam(request,
                        SATELLITE_TAG_ARGOS_PTT_NUMBER));
                    ...
                    sb.append(MessageFormat.format("<br/>{0} set to {1}",
                        SATELLITE_TAG_ARGOS_PTT_NUMBER,
                        getParam(request, SATELLITE_TAG_ARGOS_PTT_NUMBER)));
                    sb.append(MessageFormat.format("<br/>{0} set to {1}", SATELLITE_TAG_SERIAL,
                        getParam(request, SATELLITE_TAG_SERIAL)));
                    sb.append(MessageFormat.format("<br/>{0} set to {1}", SATELLITE_TAG_NAME,
                        getParam(request, SATELLITE_TAG_NAME)));
                } else {}
            } catch (Exception ex) {
                ex.printStackTrace();
                locked = true;
            ...
            if (!locked) {
                ...
                out.println(
                    "<p><strong>Success!</strong> I have successfully set the following tag values:");
            ...
            out.println(
                "<strong>Error:</strong> I was unable to set the tag. I cannot find the encounter that you intended in the database.");
            out.println(ServletUtilities.getFooter(context));
        }
        out.close();
        myShepherd.closeDBTransaction();
