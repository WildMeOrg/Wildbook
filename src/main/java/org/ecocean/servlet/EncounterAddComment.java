...
public class EncounterAddComment extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if ((request.getParameter("number") != null) && (request.getParameter("user") != null) &&
            (request.getParameter("autocomments") != null) &&
            ...
            try {
                commentMe.addComments("<p><em>" + request.getParameter("user") + " on " +
                    ...
                    request.getParameter("autocomments") + "</p>");
            } catch (Exception le) {
                locked = true;
                le.printStackTrace();
                myShepherd.rollbackDBTransaction();
            ...
            if (!locked) {
                ...
                String message = "A new comment has been added to encounter #" +
                ...
                out.println(
                    "<strong>Failure:</strong> I did NOT add your comments. Another user is currently modifying the entry for this encounter. Please try to add your comments again in a few seconds.");
                // out.println("<p><a href=\"http://" + CommonConfiguration.getURLLocation(request) + "/encounters/encounter.jsp?number=" +
                // request.getParameter("number") + "\">Return to encounter #" + request.getParameter("number") + "</a></p>\n");
            }
