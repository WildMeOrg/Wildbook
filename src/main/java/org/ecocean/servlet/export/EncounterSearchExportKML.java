            ...
            for (int i = 0; i < numMatchingEncounters; i++) {
                ...
                if ((enc.getDecimalLongitude() != null) && (enc.getDecimalLatitude() != null)) {
                    ...
                    if (enc.getIndividualID() == null) {
                        ...
                        nameText = enc.getIndividualID() + ": Encounter " +
                    ...
                    if (!bareBonesPlacemarks) {
                        ...
                        String descHTML = "<p><a href=\"https://" +
                            CommonConfiguration.getURLLocation(request) +
                            "/encounters/encounter.jsp?noscript=true&number=" +
                            enc.getEncounterNumber() + "\">Direct Link</a></p>";
                        descHTML += "<p> <strong>Date:</strong> " + enc.getDate() + "</p>";
                        if (enc.getLocation() != null) {
                            descHTML += "<p> <strong>Location:</strong><br>" + enc.getLocation() +
                                "</p>";
                        }
                        // trying to find problematic sizes...
                        try {
                            if (enc.getSizeAsDouble() != null) {
                                descHTML += "<p> <strong>Size:</strong> " + enc.getSize() +
                                    " meters</p>";
                            }
                        } catch (Exception npe) {
                            npe.printStackTrace();
                            System.out.println("NPE on size for encounter: " +
                        ...
                        if ((enc.getComments() != null) && (!enc.getComments().equals(""))) {
                            descHTML += "<p> <strong>Comments:</strong> " + enc.getComments() +
                                "</p>";
                        }
                        ...
                        for (int imgNum = 0; imgNum < imgsNum; imgNum++) {
                            descHTML += ("<br />" + "<a href=\"https://" +
                                CommonConfiguration.getURLLocation(request) +
                                "/encounters/encounter.jsp?noscript=true&number=" +
                                enc.getEncounterNumber() + "\"><img src=\"http://" +
                                request.getServerName() + "/" +
                                CommonConfiguration.getDataDirectoryName(context) + "/encounters/" +
                                enc.getEncounterNumber() + "/" +
                                imgs.get(imgNum).getDataCollectionEventID() + ".jpg\" /></a>");
                        }
                    ...
                    String coordsString = enc.getDWCDecimalLongitude() + "," +
                        enc.getDWCDecimalLatitude();
