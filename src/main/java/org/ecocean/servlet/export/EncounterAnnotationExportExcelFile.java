...
public class EncounterAnnotationExportExcelFile extends HttpServlet {
  ...
  private void setMediaAssetCounts(Vector rEncounters, Shepherd myShepherd) {
    System.out.println("EncounterAnnotationExportExcelFile: setting environment vars for "+Math.max(rEncounters.size(), rowLimit)+" encs.");
    int maxNumMedia = 0;
    ...
    for (int i=0;i<rEncounters.size() && i< rowLimit;i++) {
      ...
      if(enc.getMeasurements().size() > 0){
        for(MeasurementEvent currentMeasurement: enc.getMeasurements()){ // populate a list of measurementColName with measurements in current encounter set only
    ...
    System.out.println("EncounterAnnotationExportExcelFile: environment vars numMediaAssetCols = "+numMediaAssetCols+"; maxNumKeywords = "+maxNumKeywords+" and maxNumNames = "+numNameCols);
  }
    ...
    try {
      ...
      for (int i=0;i<numMatchingEncounters && i<rowLimit;i++)
      {
        ...
        for (ExportColumn exportCol: columns) {
          ...
          else System.out.println("EncounterAnnotationExportExcelFile: no object found for class "+exportCol.getDeclaringClass());
        }
