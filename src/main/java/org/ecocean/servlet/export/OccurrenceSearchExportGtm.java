...
public class OccurrenceSearchExportGtm extends HttpServlet {
    ...
    private static Map<Long, List<String> > getMillisToOccIdMap(Vector initialOccs) {
        ...
        for (int i = 0; i < initialOccs.size(); i++) {
            ...
            if (millis != null) {
                if (!millisToOccIdMap.containsKey(millis))
                    millisToOccIdMap.put(millis, new ArrayList<String>());
    ...
    private Set<Object> getDatelessOccurrences(Vector initialOccs) {
        ...
        System.out.println("getDatelessOccurrences found " + badOccs.size() +
            " dateless occurrences out of " + initialOccs.size());
        return badOccs;
    }
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            try {
                ...
                boolean removedDateless = rOccurrences.removeAll(getDatelessOccurrences(
                    rOccurrences));
                int numMatchingOccurrences = rOccurrences.size();
                System.out.println("Returning " + numMatchingOccurrences +
                    " occurrences. removedDateless = " + removedDateless);
                ...
                WritableSheet individualsSheet = workbookOBIS.createSheet(
                    "Individual Number Reference", 0);
                ...
                for (int i = 0; i < sortedMillis.length; i++) {
                    ...
                    for (String occId : millisToOccIdMap.get(millis)) {
                        ...
                        // members
                        List<String> members = new ArrayList<String>(occurrenceMap.get(
                            occId).getIndividualIDs());
                        for (int j = 0; j < members.size(); j++) {
                            Integer newID = indivIDMap.get(members.get(j));
                            gtmSheet.addCell(new Label(2 + j, i + 1, newID.toString()));
                        }
                        if ((i % 50) == 0)
                            System.out.println("printing occurrence " + i + " = " + occId);
                    }
                    count++;
                    numResults++;
                } // end for loop iterating encounters
