...
public class MarkRecaptureEncounters extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            EncounterQueryResult result = EncounterQueryProcessor.processQuery(myShepherd, request,
                order);
            ...
            if (request.getParameter("includeQueryComments") != null) {
                ...
                footer = "\r\n\r\n/*\r\nSourceURL:\r\n" + "http://" +
                    CommonConfiguration.getURLLocation(request) + "/MarkRecaptureEncounters?" +
    ...
    public static String addHistories(Vector<Encounter> rEncounters, int numSessions,
        HttpServletRequest request, DateTime[] start, DateTime[] end) {
        ...
        Map<String, String> indIDToDisplayName = (includeIndID) ? new HashMap<String,
            String>() : null;
        ...
        for (Encounter enc : rEncounters) {
            ...
            for (int session = 0; session < numSessions; session++) {
                if (enc.wasInPeriod(start[session], end[session]))
                    markRecapture[rowNum][session] = true;
            }
