...
public class SimpleCMROutput extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            if (request.getParameter("includeQueryComments") != null) {
                ...
                footer = "/*\r\nSourceURL:\r\n" + "http://" +
                    CommonConfiguration.getURLLocation(request) + "/SimpleCMROutput?" +
    ...
    private String addHistories(Vector<MarkedIndividual> rIndividuals, int numSessions,
        ...
        for (int i = 0; i < numIndividuals; i++) {
            ...
            for (int j = 0; j < numSessions; j++) {
                ...
                if (request.getParameter("locationCodeField") != null) {
                    if (indie.wasSightedInPeriod(start[j].getYear(),
                        (start[j].getMonthOfYear() - 1), start[j].getDayOfMonth(), end[j].getYear(),
                        (end[j].getMonthOfYear() - 1), end[j].getDayOfMonth(),
                        request.getParameter("locationCodeField").trim())) {
                        wasSighted = true;
                    }
                } else {
                    if (indie.wasSightedInPeriod(start[j].getYear(),
