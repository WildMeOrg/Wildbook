    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            try {
                ...
                outp.write(
                    "Sample_ID,Individual_ID,Latitude,Longitude,Date_Time,Region,Sex,Haplotype" +
                    locusString.toString() + ",Occurrence_ID\n");
                for (int i = 0; i < numMatchingEncounters; i++) {
                    ...
                    if (enc.getLocationID() != null) {
                        locationID = enc.getLocationID().replaceAll(",", "-");
                    }
                        ...
                        if (!foundMsMarkers) {
                            ...
                            for (int l = 0; l < aSize; l++) {
                                ...
                                if (ga.getAnalysisType().equals("MicrosatelliteMarkers")) {
                                    foundMsMarkers = true;
                                    MicrosatelliteMarkersAnalysis ga2 =
                                        (MicrosatelliteMarkersAnalysis)ga;
                                    for (int m = 0; m < numLoci; m++) {
                                        String locus = allLoci.get(m);
                                        if (ga2.hasLocus(locus)) {
                                            Locus loc = ga2.getLocus(locus);
                                            if (loc.getAllele0() != null) {
                                                msMarkerString += "," + loc.getAllele0();
                                            } else { msMarkerString += ","; }
                                            if (loc.getAllele1() != null) {
                                                msMarkerString += "," + loc.getAllele1();
                                            } else { msMarkerString += ","; }
                                        } else { msMarkerString += ",,"; }
                                    }
                    ...
                    if (myShepherd.getOccurrenceForEncounter(enc.getCatalogNumber()) != null) {
                        Occurrence occur = myShepherd.getOccurrenceForEncounter(
