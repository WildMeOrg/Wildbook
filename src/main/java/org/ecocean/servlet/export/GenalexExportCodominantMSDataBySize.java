...
public class GenalexExportCodominantMSDataBySize extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            String order = "";
            HttpServletRequest request1 = (HttpServletRequest)request.getSession().getAttribute(
                "locationSearch1");
            if ((request != null) && (request1 != null)) {
                MarkedIndividualQueryResult queryResult1 = IndividualQueryProcessor.processQuery(
                    myShepherd, request1, order);
                ...
                try {
                    ...
                    WritableSheet sheet = workbookOBIS.createSheet(
                        "Shepherd Project GenAlEx Export Microsatellite Data", 0);
                    if (request.getParameter("exportHaplos") != null) {
                        Label label0 = new Label(0, 0, "1");
                        sheet.addCell(label0);
                    } else {
                        Label label0 = new Label(0, 0,
                            (new Integer(myShepherd.getAllLoci().size())).toString());
                        sheet.addCell(label0);
                    }
                    Label label1 = new Label(1, 0,
                        (new Integer((numSearch1Individuals + numSearch2Individuals))).toString());
                    sheet.addCell(label1);
                    Label label2 = new Label(2, 0, (new Integer(numPopulations)).toString());
                    sheet.addCell(label2);
                    Label label2a = new Label(3, 0,
                        (new Integer(numSearch1Individuals)).toString());
                    ...
                    Label label5 = new Label(3, 1,
                        "Search 1:" + queryResult1.getQueryPrettyPrint());
                    sheet.addCell(label5);
                    Label label6 = new Label(4, 1,
                        "Search 2:" + queryResult2.getQueryPrettyPrint());
                    ...
                    if (request.getParameter("exportHaplos") != null) {
                        ...
                        for (int r = 0; r < numLoci; r++) {
                            ...
                            if ((request.getParameter("hasMSMarkers") != null) ||
                                ((request.getParameter(locus) != null) &&
                                (!request.getParameter(locus).equals("")) &&
                                (request1.getParameter(locus) != null) &&
                                (!request1.getParameter(locus).equals("")))) {
                    ...
                    for (int i = 0; i < numPopulations; i++) {
                        ...
                        for (int k = 0; k < iterateMe.size(); k++) {
                            ...
                            Label lNumber = new Label(0, count,
                                indy.getIndividualID() + "_" + (i + 1));
                            ...
                            if (request.getParameter("exportHaplos") != null) {
                                if (numHaplos > 0) {
                                    ...
                                    if (indy.getHaplotype() != null) {
                                        ...
                                        Label lociLabel = new Label(locusColumn, count,
                                            haploNum.toString());
                                        sheet.addCell(lociLabel);
                                    } else {
                                ...
                                for (int r = 0; r < numLoci; r++) {
                                    ...
                                    if ((request.getParameter("hasMSMarkers") != null) ||
                                        ...
                                        if (indy.hasLocus(locus)) {
                                            ArrayList<Integer> vals = indy.getAlleleValuesForLocus(
                                                locus);
                                            if (vals.size() == 1) {
                                                Label lociLabel = new Label(locusColumn, count,
                                                    vals.get(0).toString());
                                                sheet.addCell(lociLabel);
                                                locusColumn++;
                                                Label lociLabel2 = new Label(locusColumn, count,
                                                    vals.get(0).toString());
                                                sheet.addCell(lociLabel2);
                                                locusColumn++;
                                            } else if (vals.size() == 2) {
