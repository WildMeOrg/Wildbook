...
public class EncounterSearchExportShapefile extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            for (int i = 0; i < numMatchingEncounters; i++) {
                ...
                if ((enc.getDecimalLongitudeAsDouble() != null) &&
                    (enc.getDecimalLatitudeAsDouble() != null)) {
                    /* Longitude (= x coord) first ! */
                    org.locationtech.jts.geom.Point point = geometryFactory.createPoint(
                        new Coordinate(enc.getDecimalLongitudeAsDouble().doubleValue(),
                        enc.getDecimalLatitudeAsDouble().doubleValue()));
                            ...
                            if (thisSample.getNumAnalyses() > 0) {
                                ...
                                for (int analy = 0; analy < numAnalyses; analy++) {
                                    ...
                                    if (thisAnalysis.getAnalysisType().equals("MitochondrialDNA")) {
                                        MitochondrialDNAAnalysis thisDNA =
                                            (MitochondrialDNAAnalysis)thisAnalysis;
                    ...
                    featureBuilder.set("URL",
                        ("https://" + CommonConfiguration.getURLLocation(request) +
                        "/encounters/encounter.jsp?number=" + enc.getCatalogNumber()));
            ...
            if (featureSource instanceof SimpleFeatureStore) {
                ...
                /*
                 * SimpleFeatureStore has a method to add features from a SimpleFeatureCollection object, so we use the ListFeatureCollection class to
                ...
                try {
                    // Create the ZIP file
                    String outFilename = shapeFile.getParentFile().getAbsolutePath() +
                        File.separator + gisZipFilename;
                    ...
                    response.setHeader("Content-Disposition",
                        "attachment;filename=" + gisZipFilename);
            ...
            out.println("<p>Please let the webmaster know you encountered an error at: " +
                this.getServletName() + " servlet</p></body></html>");
            out.println(ServletUtilities.getFooter(context));
        }
        myShepherd.rollbackDBTransaction();
        myShepherd.closeDBTransaction();
     ...
     * Coordinate Reference System for the FeatureType and a a maximum field length for the 'name' field dddd
     */
     ...
     * private static SimpleFeatureType createFeatureType(String context) {
        ...
        // add attributes in order builder.add("Location", org.locationtech.jts.geom.Point.class);
