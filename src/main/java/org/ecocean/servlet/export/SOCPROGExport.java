        ...
        try {
            ...
            if (request != null) {
                ...
                try {
                    ...
                    WritableSheet sheet = workbookOBIS.createSheet("Wildbook SOCPROG Data Export",
                        0);
                    ...
                    List<MeasurementDesc> bioMeasurementTypes = Util.findBiologicalMeasurementDescs(
                        "en", context);
                    ...
                    WritableCellFormat numbersFormat = new WritableCellFormat(
                    ...
                    HiddenIndividualReporter hiddenData = new HiddenIndividualReporter(iterateMe,
                        request, myShepherd);
                    ...
                    for (int k = 0; k < iterateMe.size(); k++) {
                        ...
                        for (int j = 0; j < numEncs; j++) {
                            ...
                            if ((enc.getLocationID() != null) ||
                                ((enc.getLongitudeAsDouble() != null) &&
                                (enc.getLatitudeAsDouble() != null))) {
                                if ((enc.getDateInMilliseconds() != null) &&
                                    (enc.getDateInMilliseconds() > 0)) {
                                    count++;
                                    ...
                                    jxl.write.DateTime encLabel = new jxl.write.DateTime(0, count,
                                        localCalendar.getTime(), dateFormat);
                                    sheet.addCell(encLabel);
                                    if ((enc.getLongitudeAsDouble() != null) &&
                                        (enc.getLatitudeAsDouble() != null)) {
                                        jxl.write.Number popLabel1a = new jxl.write.Number(1, count,
                                            enc.getLatitudeAsDouble(), numbersFormat);
                                        sheet.addCell(popLabel1a);

                                        jxl.write.Number popLabel2a = new jxl.write.Number(2, count,
                                            enc.getLongitudeAsDouble(), numbersFormat);
                                        sheet.addCell(popLabel2a);
                                    } else {
                                        jxl.write.Label popLabel1a = new jxl.write.Label(1, count,
                                            "NaN");
                                        sheet.addCell(popLabel1a);

                                        jxl.write.Label popLabel2a = new jxl.write.Label(2, count,
                                            "NaN");
                                        sheet.addCell(popLabel2a);
                                    }
                                    if ((enc.getMaximumDepthInMeters() != null) ||
                                        (enc.getMaximumElevationInMeters() != null)) {
                                        if (enc.getMaximumDepthInMeters() != null) {
                                            jxl.write.Number popLabel3c = new jxl.write.Number(3,
                                                count, enc.getMaximumDepthInMeters(),
                                                numbersFormat);
                                            sheet.addCell(popLabel3c);
                                        } else {
                                            jxl.write.Number popLabel3c = new jxl.write.Number(3,
                                                count, enc.getMaximumElevationInMeters(),
                                                numbersFormat);
                                            sheet.addCell(popLabel3c);
                                        }
                                    } else {
                                        jxl.write.Label popLabel3c = new jxl.write.Label(3, count,
                                            "NaN");
                                        sheet.addCell(popLabel3c);
                                    }
                                    if (enc.getLocationID() != null) {
                                    ...
                                    if (enc.getIndividual() != null) {
                                        ...
                                        if (enc.getIndividual().getDefaultName() != null)
                                            defaultName = enc.getIndividual().getDefaultName();
                                        Label popLabel4a1 = new Label(5, count,
                                            defaultName.replaceAll("[^a-zA-Z0-9]", ""));
                                        sheet.addCell(popLabel4a1);

                                        Label popLabel4a2 = new Label(0, count, defaultName);
                                        sheet2.addCell(popLabel4a2);
                                    }
                                    if (myShepherd.getOccurrenceForEncounter(
                                        enc.getCatalogNumber()) != null) {
                                        Occurrence oc = myShepherd.getOccurrenceForEncounter(
                                            enc.getCatalogNumber());
                                        Label popLabel7b = new Label(1, count,
                                            oc.getOccurrenceID());
                                        sheet2.addCell(popLabel7b);
                                    }
                                    List<String> mySocialUnits =
                                        myShepherd.getAllSocialUnitsForMarkedIndividual(
                                        indy.getIndividualID());
                                    if (mySocialUnits.size() > 0) {
                                        Label popLabel7c = new Label(2, count,
                                            mySocialUnits.get(0));
                                    ...
                                    if (indy.getDateSortedEncounters(
                                        true)[0].getCatalogNumber().equals(
                                        enc.getCatalogNumber())) {
                                        cmrStatus = "New";
                                    }
                                    ...
                                    for (int m = 0; m < numBioMeasurementTypes; m++) {
                                        ...
                                        if (enc.hasBiologicalMeasurement(measureName)) {
                                            BiologicalMeasurement bm = enc.getBiologicalMeasurement(
                                                measureName);
                                            if ((bm != null) && (bm.getValue() != null)) {
                                                jxl.write.Number popLabelX = new jxl.write.Number(
                                                    (m + 7 + numMeasurementTypes), count,
                                                    bm.getValue(), numbersFormat);
                                                sheet2.addCell(popLabelX);
                                            }
                                        }
                                    }
                    ...
                    out.println(
                        "<p>Please let the webmaster know you encountered an error at: SOCPROGExport servlet</p></body></html>");
