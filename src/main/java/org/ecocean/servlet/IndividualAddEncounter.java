...
public class IndividualAddEncounter extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        StringBuilder failureMessage = new StringBuilder("<strong>Failure:</strong> Encounter " +
            request.getParameter("number") + " was NOT added to " +
            request.getParameter("individual") + ". Error unknown.");
        ...
        if ((request.getParameter("number") != null) && (indivID != null) &&
            (request.getParameter("matchType") != null)) {
            myShepherd.beginDBTransaction();
            Encounter enc2add = myShepherd.getEncounter(request.getParameter("number"));
            try {
                if (enc2add == null) {
                    failureMessage = new StringBuilder("<p>Invalid encounter id=" +
                        request.getParameter("number") + "</p>");
                    throw new RuntimeException("invalid encounter id=" +
                ...
                if (enc2add.getIndividual() == null) {
                    ...
                    if (forceNew) {
                        ...
                        try {
                            ...
                            checkForDuplicateAnnotations(enc2add, failureMessage, addToMe,
                                myShepherd);
                        ...
                        System.out.println(
                            "IndividualAddEncounter: Retrieving an existing individual=" + indivID);
                        addToMe = myShepherd.getMarkedIndividual(indivID);
                        if (addToMe == null) {
                            failureMessage = new StringBuilder("<p>Invalid individual id=" +
                                indivID + "</p>");
                    ...
                    try {
                        ...
                        enc2add.addComments("<p><em>" + request.getRemoteUser() + " on " +
                            ...
                            request.getParameter("individual") + ".</p>");
                        addToMe.addComments("<p><em>" + request.getRemoteUser() + " on " +
                            (new java.util.Date()).toString() + "</em><br>" + "Added encounter " +
                            request.getParameter("number") + ".</p>");
                        if ((addToMe.getSex() != null) && (enc2add.getSex() != null) &&
                            (!addToMe.getSex().equals(enc2add.getSex()))) {
                            sexMismatch = true;
                        } else if (((addToMe.getSex() == null) ||
                            (addToMe.getSex().equals("unknown"))) && (enc2add.getSex() != null)) {
                    ...
                    if ("false".equals(request.getParameter("noemail"))) {
                        try {
                            System.out.println(
                                "About to send emails to interested users in IndividualAddEncounter.java");
                            executeEmails(myShepherd, request, addToMe, newIndy, enc2add, context,
                                langCode);
                    ...
                    out.println(
                        "<strong>Error:</strong> You can't add this encounter to a marked individual when it is already assigned to another one.");
            ...
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            out.println(
                "<strong>Error:</strong> I didn't receive enough data to add this encounter to a marked individual.");
        }
        out.close();
        myShepherd.closeDBTransaction();
    }

    private void checkForDuplicateAnnotations(Encounter enc2add, StringBuilder failureMessage,
        MarkedIndividual addToMe, Shepherd myShepherd) {
        // check for duplicate individual IDs represented by another annotation with the same acmId
        ArrayList<Encounter> conflictingEncs = new ArrayList<Encounter>();

        for (Annotation annot : enc2add.getAnnotations()) {
            conflictingEncs.addAll(Annotation.checkForConflictingIDsforAnnotation(annot,
                addToMe.getIndividualID(), myShepherd));
        }
        conflictingEncs.remove(enc2add);
        if (conflictingEncs.size() > 0) {
            failureMessage = new StringBuilder("Failure: ");
            failureMessage.append(
                "<p>The following Encounters contain the same annotation but have a different individual ID. An annotation can only have one ID inherited from its Encounters. <ul>");
            for (Encounter enc : conflictingEncs) {
                // failureMessage.append("<li>"+enc.getEncounterNumber()+" ("+enc.getIndividual().getIndividualID()+")</li>");
                failureMessage.append("<li><a target=\"_blank\" href=\"encounter.jsp?number=" +
                    enc.getEncounterNumber() + "\">" + enc.getEncounterNumber() + "</a></li>");
            }
    ...
    public static void executeEmails(Shepherd myShepherd, HttpServletRequest request,
        MarkedIndividual addToMe, boolean newIndy, Encounter enc2add, String context,
        String langCode) {
        ...
        Map<String, String> tagMap = NotificationMailer.createBasicTagMap(request, addToMe,
            enc2add);
        ...
        NotificationMailer mailer = new NotificationMailer(context, langCode, mailTo, emailTemplate,
            tagMap);
        ...
        if (enc2add.getPhotographerEmails() != null)
            cSubmitters.addAll(enc2add.getPhotographerEmails());
        if (enc2add.getInformOthersEmails() != null)
            cSubmitters.addAll(enc2add.getInformOthersEmails());
        ...
        for (String emailTo : cSubmitters) {
            if (!"".equals(emailTo)) {
                tagMap.put(NotificationMailer.EMAIL_NOTRACK,
                    "number=" + enc2add.getCatalogNumber());
                tagMap.put(NotificationMailer.EMAIL_HASH_TAG,
                    Encounter.getHashOfEmailString(emailTo));
                es.execute(new NotificationMailer(context, langCode, emailTo, emailTemplate,
                    tagMap));
            }
        }
        ...
        String rssDescription = request.getParameter("individual") + " was resighted on " +
            enc2add.getShortDate() + ".";
