...
public class IAGateway extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        // this seems to feed iaResults.jsp and is an unexpected dependency and extra URL call that
        // shouldn't be needed
    ...
    throws ServletException, IOException {
        ...
        String context = ServletUtilities.getContext(request); // note! this *must* be run after
                                                               // postStream stuff above
        ...
        try {
            ...
            if (j.optBoolean("enqueue", false) || j.optBoolean("v2", false)) { // short circuits and
                                                                               // just blindly
                                                                               // writes out to
                                                                               // queue and is done!
                                                                               //  magic?
                ...
                // incoming json *probably* (should have) has taskId set... but if not i guess we
                // use the one we generated???
                ...
                if (tuser == null) { // "anonymous" but we want to make sure we zero these out to
                                     // prevent them from being passed in
                ...
                if (j.optJSONArray("annotationIds") != null) {
                    // if this is just a single Encounter call, put it in the fast/detection lane to
                    // unblock small batch users
    ...
    throws ServletException, IOException {
        ...
        Task task = Task.load(taskId, myShepherd); // might be null in some cases, such as
                                                   // non-queued  ... maybe FIXME when we dump
                                                   // cruft?
         ...
         *      if ((set != null) && (set.getMediaAssets() != null) && (set.getMediaAssets().size()
         * > 0)) mas.addAll(set.getMediaAssets());
         *  }
         * }
         */
        else {
 ...
 */
        ...
        int limitTargetSize = j.optInt("limitTargetSize", -1); // really "only" for
                                                               // debugging/testing, so use if you
                                                               // know what you are doing

        // currently this implies each annotation should be sent one-at-a-time TODO later will be
        // allow clumping (to be sent as multi-annotation
        ...
        // i think that "in the future" co-occurring annotations should be sent together as one set
        // of query list; but since we dont have support for that
 ...
 * considerations?   */
        ...
        for (int i = 0; i < anns.size(); i++) {
            ...
            } catch (Exception ex) { // unsure if maybe _some_ exceptions should be treated
                                     // differently here?
 ...
 */
    ...
    // TODO clean this up!  now that this is moved here, there is probably lots of redundancy with
    // above no?
...
// System.out.println("DEBUG: IAGateway.processQueueMessage -> " + message);
        ...
        // this must have a taskId coming in, cuz otherwise how would (detached, async) caller know
        // what it is!
        // __context and __baseUrl should be set -- this is done automatically in IAGateway, but if
        // getting here by some other method, do the work!
    ...
    // weirdly (via StartupWildbook) stuff put in the queue is processed by.... the method right
    // above us!  :)  :(
    ...
    throws IOException {
        ...
        // real IA sends "jobid=jobid-xxxx" as body on POST, but this gives us a url-based
    ...
    throws ServletException, IOException {
        ...
        // System.out.println("IAGateway.handleBulkImport() preparing to parse " +
        // maMap.keySet().size() + " encounter detection jobs");
        // if (maMap == null) return res;  // "should never happen"
        /*
         *  maMap is just js_jarrs from imports.jsp, basically { encID0: [ma0, ... maN], ... encIDX:
         * [maY, .. maZ] }
        ...
        // System.out.println("[ERROR] IAGateway.handleBulkImport() maMap could not find JSONArray
        // of MediaAsset ids at encId key=" + encId);
        // continue;
        // }
        Task task = new Task();
