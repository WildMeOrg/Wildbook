...
private final String UPLOAD_DIRECTORY = "/tmp";
  ...
  private List<Measurement> getMeasurements(HashMap formValues, String encID, String context) {
...
//System.out.println("about to do int stuff");
            ...
            if((getVal(formValues, "datepicker")!=null)&&(!getVal(formValues, "datepicker").trim().equals(""))){
              ...
              try{
                ...
                String datepickerVal = getVal(formValues, "datepicker");
...
System.out.println("depth --> " + formValues.get("depth").toString());
     ...
     // enc.setPhotographerEmail(getVal(formValues, "photographerEmail"));
      ...
      if (!getVal(formValues, "population").equals("")) {
        enc.setPopulation (getVal(formValues, "population"));
      }
      if (!getVal(formValues, "governmentArea").equals("")) {
        enc.setGovernmentArea(getVal(formValues, "governmentArea"));
      }
      if (!getVal(formValues, "huntingState").equals("")) {
        enc.setHuntingState(getVal(formValues, "huntingState"));
      }
      ...
      if ((formValues.get("easting") != null) && (formValues.get("northing") != null) && !formValues.get("easting").toString().equals("") && !formValues.get("northing").toString().equals("")) {
      //enc.setGPSLatitude(lat + "&deg; " + gpsLatitudeMinutes + "\' " + gpsLatitudeSeconds + "\" " + latDirection);


      try {
        int easting = (new Integer(formValues.get("easting").toString())).intValue();
        int northing = (new Integer(formValues.get("northing").toString())).intValue();

        String epsgProjCode = formValues.get("epsgProjCode").toString();
