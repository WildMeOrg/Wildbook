        ...
        if ((myShepherd.isEncounter(sharky)) && (request.getParameter("sampleID") != null) &&
            ...
            try {
                ...
                if (myShepherd.isTissueSample(request.getParameter("sampleID"), sharky)) {
                    genSample = myShepherd.getTissueSample(request.getParameter("sampleID"),
                        sharky);
                    enc.removeTissueSample(genSample);
                    enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                        (new java.util.Date()).toString() + "</em><br />" +
                        "Removed tissue sample ID " + request.getParameter("sampleID") + ".<br />");
            ...
            if (!locked) {
                ...
                out.println("<strong>Success!</strong> I have successfully removed tissue sample " +
                    request.getParameter("sampleID") + " for encounter " + sharky + ".</p>");
            ...
            out.println(ServletUtilities.getHeader(request));
            out.println(
                "<strong>Error:</strong> I was unable to remove the tissue sample. I cannot find the encounter that you intended it for in the database.");
            out.println(ServletUtilities.getFooter(context));
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
        }
        out.close();
