...
public class MediaAssetAttach extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        /*
           ok, leaving this for prosperity. this FAILS on the .getString() for reasons i cant explain when passed an int (non-string) however, the
              conditional still evaluates as TRUE !!!!   good luck on this one.
            see also:   https://github.com/stleary/JSON-java/issues/472 if (args.optString("MediaAssetID", null) != null)
               maIds.add(args.getString("MediaAssetID"));
         */
        if (args.optString("MediaAssetID", null) != null)
            maIds.add(args.optString("MediaAssetID", null));
        ...
        if (encID == null || maIds.size() < 1) {
            throw new IOException(
                      "MediaAssetAttach servlet requires both a \"MediaAssetID\" and an \"EncounterID\" argument. Servlet received maIds="
                      + maIds + " and encID=" + encID);
        }
        ...
        try {
            ...
            if (enc == null)
                throw new ServletException("No Encounter with id " + encID + " found in database.");
            ...
            if (args.optString("attach") != null && args.optString("attach").equals("true")) {
                for (MediaAsset ma : mas) {
                    ...
                    enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                        (new java.util.Date()).toString() + "</em><br>" + comments + " </p>");
                }
            ...
            // DETACH MEDIAASSET FROM ENCOUNTER
                ...
                for (MediaAsset ma : alreadyAttached) {
                    ...
                    String undoLink = request.getScheme() + "://" +
                        CommonConfiguration.getURLLocation(request) +
                        "/MediaAssetAttach?attach=true&EncounterID=" + encID + "&MediaAssetID=" +
                        ma.getId();
                    String comments = "Detached MediaAsset " + ma.getId() +
                        ". To undo this action, visit " + undoLink;
                    enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                        (new java.util.Date()).toString() + "</em><br>" + comments + " </p>");
                    success = true;
                }
