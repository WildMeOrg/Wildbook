...
public class EncounterSetCountry extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if ((myShepherd.isEncounter(encNum)) && (request.getParameter("country") != null)) {
            ...
            try {
                if (country.equals("")) { enc.setCountry(null); } else {
                    enc.setCountry(country);
                    enc.addComments("<p><em>" + request.getRemoteUser() + " on " +
                        (new java.util.Date()).toString() + "</em><br />Changed country to " +
            ...
            if (!locked) {
                ...
                out.println(
                    "<strong>Success!</strong> I have successfully changed the reported country for encounter "
                    + encNum + " to " + country + ".</p>");
            ...
            // out.println(ServletUtilities.getHeader(request));
            out.println(
                "<strong>Error:</strong> I was unable to set the country. I cannot find the encounter that you intended in the database.");
            // out.println(ServletUtilities.getFooter(context));
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
        }
        out.close();
