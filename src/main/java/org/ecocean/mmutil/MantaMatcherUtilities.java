...
public final class MantaMatcherUtilities {
    ...
    /**
     * Converts the specified file to a map of files, keyed by string for easy referencing,
     ...
     * <li>File representing MantaMatcher algorithm regional input file (key:
     * MMA-INPUT-REGIONAL).</li>
     ...
     * </ul>
     * All files are assumed to be in the same folder, and no checking is performed to see if they
     * exist.
    ...
    public static Map<String, File> getMatcherFilesMap(SinglePhotoVideo spv) {
        ...
        // MMA input files.
        map.put("MMA-INPUT",
            new File(spv.getFile().getParentFile(),
            spv.getDataCollectionEventID() + "_mmaInput.txt"));
        map.put("MMA-INPUT-REGIONAL",
            new File(spv.getFile().getParentFile(),
            spv.getDataCollectionEventID() + "_mmaInputRegional.txt"));
        // MMA results files: global.
        map.put("TXT",
            new File(spv.getFile().getParentFile(),
            spv.getDataCollectionEventID() + "_mmaOutput.txt"));
        map.put("CSV",
            new File(spv.getFile().getParentFile(),
            spv.getDataCollectionEventID() + "_mmaOutput.csv"));
        // MMA results files: regional.
        map.put("TXT-REGIONAL",
            new File(spv.getFile().getParentFile(),
            spv.getDataCollectionEventID() + "_mmaOutputRegional.txt"));
        map.put("CSV-REGIONAL",
            new File(spv.getFile().getParentFile(),
            spv.getDataCollectionEventID() + "_mmaOutputRegional.csv"));
        return map;
    }
     ...
     * <li>File representing MantaMatcher feature file (key: FEAT).</li>
     * </ul>
     * All files are assumed to be in the same folder, and if existing web-compatible image files
     * exist (based on file extension) they will be referenced, otherwise a generic name is used
     * (with the same extension as the original file).
     * <p>
     * This allows simple determination of, for example, existence of a CR file for a specified
     * original:
     * <pre>
    ...
    public static Map<String, File> getMatcherFilesMap(File f) {
        ...
        // Use existing image files if possible, otherwise use generic name with same file
        // extension.
        File cr = crOpts.isEmpty() ? new File(pf,
            name.replaceFirst(regex, "_CR.$1")) : crOpts.get(0);
        String crExt = cr.getName().substring(cr.getName().lastIndexOf(".") + 1);
        File eh = ehOpts.isEmpty() ? new File(pf,
            name.replaceFirst(regex, "_EH." + crExt)) : ehOpts.get(0);
        File ft = ftOpts.isEmpty() ? new File(pf,
            name.replaceFirst(regex, "_FT." + crExt)) : ftOpts.get(0);
    ...
    /**
     * Checks whether the pre-processed MantaMatcher algorithm files exist for the specified base
     * image file (only checks the files required for running
     * {@code mmatch}).
     * @param f base image file from which to reference other algorithm files
    ...
    public static boolean checkMatcherFilesExist(File f) {
        ...
        return mmFiles.get("O").exists() && mmFiles.get("CR").exists() &&
                   mmFiles.get("EH").exists() && mmFiles.get("FT").exists() &&
                   mmFiles.get("FEAT").exists();
    }

    /**
     * Checks whether the pre-processed MantaMatcher algorithm files exist for an encounter (which
     * may be used to determine MMA-compatibility status).
    ...
    public static boolean checkEncounterHasMatcherFiles(Encounter enc, File dataDir) {
        for (SinglePhotoVideo spv : enc.getSinglePhotoVideo()) {
            if (MediaUtilities.isAcceptableImageFile(spv.getFile()) &&
                checkMatcherFilesExist(spv.getFile())) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Removes all MantaMatcher algorithm files relating to the specified base image file.
     * @param spv {@code SinglePhotoVideo} instance denoting base reference image
     */
    ...
    /**
     * Collates text input for the MantaMatcher algorithm. The output of this method is suitable for
     * placing in a temporary file to be access by the {@code mmatch} process.
     ...
     */
    @SuppressWarnings("unchecked") public static String collateAlgorithmInput(Shepherd shep,
        File encDir, Encounter enc, SinglePhotoVideo spv) {
        // Validate input.
        ...
        if (enc.getPatterningCode() != null) {
            ...
            if (enc.getPatterningCode().matches("^(normal|white).*$"))
                sbf.append(
                    " || this.patterningCode.startsWith('normal') || this.patterningCode.startsWith('white')");
            else
...
// sb.append(spv.getFile().getParent()).append("\n\n");
        ...
        for (Encounter x : list) {
            if (!enc.getEncounterNumber().equals(x.getEncounterNumber()))
                sb.append(encDir.getAbsolutePath()).append(File.separatorChar).append(
                    x.subdir()).append("\n");
        }
        // Clean resources.
        query.closeAll();
     ...
     * @return text suitable for MantaMatcher algorithm input file
     */
    @SuppressWarnings("unchecked") public static String collateAlgorithmInputRegional(Shepherd shep,
        File encDir, Encounter enc, SinglePhotoVideo spv) {
        // Validate input.
        if (enc.getLocationID() == null)
            throw new IllegalArgumentException("Invalid location ID specified");
...
// sb.append(spv.getFile().getParent()).append("\n\n");
    ...
    /**
     * Removes any previously generated algorithm match results for the specified encounter. It's
     * recommended this method be called whenever any match critical data fields are changed (e.g.
     * species/patterningCode/locationID).
