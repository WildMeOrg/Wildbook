...
public final class MantaMatcherUtilities {
    ...
    /**
     * Converts the specified file to a map of files, keyed by string for easy referencing, comprising the following elements:
     * <ul>
     ...
     * </ul>
     * All files are assumed to be in the same folder, and no checking is performed to see if they exist.
     ...
     * <li>File representing MantaMatcher feature file (key: FEAT).</li>
     * </ul>
     * All files are assumed to be in the same folder, and if existing web-compatible image files exist (based on file extension) they will be
     * referenced, otherwise a generic name is used (with the same extension as the original file).
     * <p>
     * This allows simple determination of, for example, existence of a CR file for a specified original:
     * <pre>
    ...
    /**
     * Checks whether the pre-processed MantaMatcher algorithm files exist for the specified base image file (only checks the files required for
     * running
     * {@code mmatch}).
     * @param f base image file from which to reference other algorithm files
    ...
    /**
     * Checks whether the pre-processed MantaMatcher algorithm files exist for an encounter (which may be used to determine MMA-compatibility status).
    ...
    /**
     * Collates text input for the MantaMatcher algorithm. The output of this method is suitable for placing in a temporary file to be access by the
     * {@code mmatch} process.
...
// sb.append(spv.getFile().getParent()).append("\n\n");
    ...
    /**
     * Removes any previously generated algorithm match results for the specified encounter. It's recommended this method be called whenever any match
     * critical data fields are changed (e.g. species/patterningCode/locationID).
