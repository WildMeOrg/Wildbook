...
/**
 * Class providing centralized image-related services, such as image rescaling, JPEG saving,
...
public final class MediaUtilities {
    ...
    private static final ICC_ColorSpace CS_sRGB = new ICC_ColorSpace(ICC_Profile.getInstance(
        ColorSpace.CS_sRGB));
    /** Regex pattern string suffix for matching image filenames (case-insensitive, capturing
       group). */
    ...
    /** Regex pattern string suffix for matching video filenames (case-insensitive, capturing
       group). */
     ...
     */
    private static ImageWriter getImageWriter_JPEG()
    throws IOException {
        // Obtain ImageWriter instance for JPEG images.
        for (Iterator<ImageWriter> iter = ImageIO.getImageWritersByMIMEType("image/jpeg");
            iter.hasNext();) {
    ...
    /**
     * Checks filename extension for supported image type. This method had been recreated here to
     * allow static and centralized access;
    ...
    public static boolean isAcceptableImageFile(String filename) {
        return (filename ==
                   null) ? false : filename.matches("^.+\\." + REGEX_SUFFIX_FOR_WEB_IMAGES);
    }

    /**
     * Checks filename extension for supported video type. This method had been recreated here to
     * allow static and centralized access;
     * original version requires a {@code Shepherd} instance.
     * @param filename filename of file to check
     * @return true if filename is supported, false otherwise
    ...
    throws IOException {
        if (f == null)
            ...
            throw new FileNotFoundException(String.format("File %s doesn't exist",
                    f.getAbsolutePath()));
    ...
    /**
     * Loads the specified image from the specified file, converting it to the sRGB color profile if
     * necessary.
     * @param f file to which to save image
     * @return {@code BufferedImage} instance (converted to sRGB color profile)
     * @throws IOException if an I/O problem occurs
     */
    public static BufferedImage loadImageAsSRGB(File f)
    throws IOException {
    ...
    /**
     * Creates an appropriate affine transform to correct the EXIF orientation specified. The
     * orientation number should be obtained from the EXIF IDFD0 Orientation metadata field.
     ...
     */
    public static void saveImageJPEG(BufferedImage img, File f, boolean overwrite, float quality,
        boolean progressive)
    throws IOException {
        if (img == null)
        ...
        if (f == null)
            ...
            throw new IllegalArgumentException(String.format("File %s already exists",
                    f.getAbsolutePath()));
        if (quality < 0f || quality > 1f) {
            throw new IllegalArgumentException(
                      "Invalid JPEG quality specified (0 <= quality <= 1)");
        }
        if (f.exists()) {
            if (overwrite) {
                if (!f.delete()) {
    ...
    /**
     * Convenience method to convert a {@code BufferedImage} to the sRGB color space if not already
     * compatible.
    ...
    /**
     * Convenience method that returns a scaled instance of the provided {@code BufferedImage}. Any
     * transparency/alpha of the original image is not preserved.
     *
     ...
     */
    public static BufferedImage rescaleImage(BufferedImage img, int targetWidth, int targetHeight,
        Object hint) {
        if (img == null)
            throw new NullPointerException("Invalid (null) image specified");
    ...
    /**
     * Convenience method that returns a scaled instance of the provided {@code BufferedImage} with
     * a text overlay. Any transparency/alpha of the original image is not preserved. This is a
     * simple implementation which was created solely for the purpose of reproducing thumbnail
     * images with a copyright message overlay, as done by the deprecated Sunwest Technologies
     * Dynamic Images library.
     */
    public static BufferedImage rescaleImageWithTextOverlay(BufferedImage img, int tw, int th,
        String text) {
        if (img == null)
            throw new NullPointerException("Invalid (null) image specified");
    ...
    /**
     * Extracts all the EXIF tags from the specified Metadata instance. This method ignores all
     * GPS-related tags for data security reasons (identifiers starting with &quot;GPS&quot;).
