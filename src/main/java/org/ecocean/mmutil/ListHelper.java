...
/**
 * Utility class to allow convenient building of lists by method chaining. This class is just a wrapper around a {@code List} implementation which
 * provides convenience methods to quickly create/edit lists. Lists can also be easily created using the standard method
 * {@link java.util.Arrays#asList(java.lang.Object...)}, so this technique is just a added option.
 *
...
public final class ListHelper<E> {
     ...
     */
    @SuppressWarnings("unchecked") private <T extends List> ListHelper(Class<T> c) {
        try {
            if (List.class.isAssignableFrom(c))
                this.list = (List<E>)c.newInstance();
            else
                throw new IllegalArgumentException(
                          "Invalid class type specified; not a List implementation");
        } catch (Exception ex) {
    ...
    /**
     * Creates a new instance using the specified list implementation, and with the specified initial list item.
    ...
    /**
     * Creates a new instance (using an {@code ArrayList} implementation), and with the specified initial list item.
     * @param <E> list item type
     * @param o initial item in list
     * @return {@code ListHelper} instance ready for further use.
     */
    ...
    /**
     * Returns a single delimited string comprising the string representations of the constituent items.
     * @param <E> list element type
    ...
    /**
     * Converts a list of items to a single delimited string comprising the string representations of the constituent items.
     * @param <E> list element type
    ...
    /**
     * Converts a list of items to a single delimited string comprising the string representations of the constituent items, and also quotes items
    ...
    private static String quoteString(String s) {
        assert s != null;
        if (s.contains(" "))
            return new StringBuilder().append("\"").append(s.replace("\"",
                    "\\\"")).append("\"").toString();
        return s.replace("\"", "\\\"");
    }
}
