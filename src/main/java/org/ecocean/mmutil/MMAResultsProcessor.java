...
/**
 * Utility to convert text file results from the MantaMatcher algorithm plain text output format to
 * HTML format for display.
 * <p>Compared to the algorithm functionality, it is restricted in only allowing a single
 * <em>test</em> and single <em>reference</em> folder each to be specified.
 * <p>For a specified {@code SinglePhotoVideo} instance, its folder it scanned for an associated MMA
 * output file (with <em>.txt</em> extension). This is parsed for results, and a FreeMarker template
 * is used to generate and XHTML output file for display.
 *
...
public final class MMAResultsProcessor {
    ...
    private static boolean checkResultsFormat(String text) {
        ...
        final String[] CHECKS = {
            "Manta Matcher version " + MMA_VER, "Reference Set: ", "SIFT files found",
                "Test Image: ", "### Ranking List ###", "Confidence: "
        };
     ...
     */
    public static MMAResult parseMatchResults(Shepherd shepherd, String text, SinglePhotoVideo spv,
        File dataDir)
    throws IOException, ParseException {
        ...
        for (int i = 0; i < sections.length; i++) {
            if (i == 0)
                ...
                parseMatchResultsCore(shepherd, result, sections[i].trim(), dataDir,
                    result.dirTest);
        }
        return result;
    }
     ...
     */
    private static void parseMatchResultsCore(Shepherd shepherd, MMAResult result, String text,
        File dataDir, File dirTest)
...
// log.trace("Confidence: {}", result.confidence);
                    ...
                    if (mr.group(4) != null && mr.group(5) != null) {
                        ...
                        try {
                            ...
                            if (enc != null) {
                                ...
                                // Only add items with a non-zero score (only lines with 'best
                                // match' anyway).
                                matches.add(res);
                            }
                        } catch (Exception ex) {
                            // Ignore; just means previously matched encounter can't now be found.
                            log.trace("Failed to find encounter for match: " +
                                res.matchEncounterNumber);
                        }
                    }
                    try {
                        sc.nextLine();
     ...
     */
    private static void parseMatchResultsHeader(Shepherd shepherd, MMAResult result, String text)
    throws ParseException {
        try (Scanner sc = new Scanner(text).useDelimiter("[\\r\\n]+")) {
            ...
            // Parse reference set.
            Pattern p = Pattern.compile("^(?:Reference Set: )?(.+)(?<!SIFT files found)$",
                Pattern.MULTILINE);
            while ((s = sc.findInLine(p)) != null) {
...
// log.trace("Score combination: {}", result.scoreCombination);
    ...
    /**
     * Utility method to locate an original reference image file using a path reference to a related
     * CR image file. This needs to be derived by examining the file-system.
     ...
     */
    private static File findReferenceFile(File dir, String root, String crPath)
    throws IOException {
        ...
        if (files.isEmpty())
            return null;
        if (files.size() > 1)
            log.warn(String.format("Found %d matching image files in folder: %s", files.size(),
                dir.getAbsolutePath()));
