...
/**
 * Utility to convert text file results from the MantaMatcher algorithm plain text output format to HTML format for display.
 * <p>Compared to the algorithm functionality, it is restricted in only allowing a single <em>test</em> and single <em>reference</em> folder each to
 * be specified.
 * <p>For a specified {@code SinglePhotoVideo} instance, its folder it scanned for an associated MMA output file (with <em>.txt</em> extension). This
 * is parsed for results, and a FreeMarker template is used to generate and XHTML output file for display.
 *
...
// log.trace("Score combination: {}", result.scoreCombination);
    ...
    /**
     * Utility method to locate an original reference image file using a path reference to a related CR image file. This needs to be derived by
