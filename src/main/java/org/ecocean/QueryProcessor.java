...
public abstract class QueryProcessor {
    ...
    protected static String stringFieldExactSubFilter(HttpServletRequest request,
        String fieldName) {
        String val = request.getParameter(fieldName);
    ...
    protected static String filterWithBasicStringField(String filter, String fieldName,
        HttpServletRequest request) {
        String subFilter = stringFieldSubFilter(request, fieldName);

        System.out.println("filterWithBasicStringField: field " + fieldName + " made subFilter " +
            subFilter);
        return filterWithSubfilter(filter, subFilter);
    }

    protected static String filterWithExactStringField(String filter, String fieldName,
        HttpServletRequest request) {
        String subFilter = stringFieldExactSubFilter(request, fieldName);

        System.out.println("filterWithExactStringField: field " + fieldName + " made subFilter " +
            subFilter);
        if (!Util.stringExists(subFilter)) return filter;
        filter = prepForCondition(filter);
        filter += subFilter;
    ...
    protected static String filterWithExactStringField(String filter, String fieldName,
        HttpServletRequest request, StringBuffer prettyPrint) {
    ...
    protected static String filterObservations(String filter, HttpServletRequest request,
        StringBuffer prettyPrint, String objectType) {
        filter = prepForCondition(filter);
        ...
        if (allParams != null) {
            ...
            if (obQuery.length() > 0) {
                if (!filter.equals("SELECT FROM org.ecocean." + objectType +
                    " WHERE 'ID' != null &&")) {
    ...
    protected static String prettyPrintUpdateForBasicString(String fieldName,
        HttpServletRequest request) {
        if (!Util.stringExists(request.getParameter(fieldName))) return "";
        return (fieldName + " contains \"" + request.getParameter(fieldName) + "\".<br />");
    }

    protected static String prettyPrintUpdateForExactString(String fieldName,
        HttpServletRequest request) {
        if (!Util.stringExists(request.getParameter(fieldName))) return "";
    ...
    protected static String gpsBoxSubFilter(String latQueryStr, String lonQueryStr,
        HttpServletRequest request) {
        ...
        if (!(Util.stringExists(ne_latStr) && Util.stringExists(ne_longStr) &&
            Util.stringExists(sw_latStr) && Util.stringExists(sw_longStr))) return "";
        try {
            ...
            if ((sw_long > 0) && (ne_long < 0)) {
                subFilter += "(" + latQueryStr + " <= " + ne_latStr + ") && (" + latQueryStr +
                    " >= " + sw_latStr + ")";
                subFilter += " && ((" + lonQueryStr + " <= " + ne_longStr + ") || (" + lonQueryStr +
                    " >= " + sw_longStr + "))";
            } else {
                subFilter += "(" + latQueryStr + " <= " + ne_latStr + ") && (" + latQueryStr +
                    " >= " + sw_latStr + ")";
            ...
            System.out.println(
                "Exception when trying to process lat and long data in DataSheetQueryProcessor!");
            ee.printStackTrace();
        }
        return "";
    }
    ...
    protected static String filterWithGpsBox(String latQueryStr, String lonQueryStr, String filter,
        HttpServletRequest request) {
    ...
    protected static String updateJdoqlVariableDeclaration(String jdoqlVariableDeclaration,
    ...
    protected static String updateParametersDeclaration(String parameterDeclaration,
        String typeAndVariable) {
    ...
    public static String filterDateRanges(HttpServletRequest request, String filter,
        ...
        try {
            ...
            if (request.getParameter("startTimeFrom") != null &&
                request.getParameter("startTimeFrom").length() > 8) {
        ...
        try {
            ...
            if (request.getParameter("startTimeTo") != null &&
                request.getParameter("startTimeTo").length() > 8) {
        ...
        try {
            ...
            if (request.getParameter("endTimeFrom") != null &&
                request.getParameter("endTimeFrom").length() > 8) {
