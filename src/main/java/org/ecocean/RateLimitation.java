/*
    RateLimitation is meant to track a resource which can only be used a certain number of iterations per unit time. Usage would be like: (will not do
       more than 10 actions in a 2 hour period)

        private static RateLimitation rl = new RateLimitation();  //best to be static on a class for example while (rl.numSinceHoursAgo(2) > 10) {
            //wait or do something else or give up etc
        }
        //code HERE that does limited action rl.addEvent();  //if you want to be paranoid/conservative, can move *before* event action
