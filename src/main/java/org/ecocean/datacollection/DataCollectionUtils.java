...
public class DataCollectionUtils {
    public static String[] getDataCollectionEventsForClass(String className, String context) {
        List<String> measurements = CommonConfiguration.getIndexedPropertyValues("measurement",
            context);
        List<String> units = CommonConfiguration.getIndexedPropertyValues("measurementUnits",
            context);
        List<String> classes = CommonConfiguration.getIndexedPropertyValues("measurementClasses",
            context);
        ...
        for (int i = 0; i < measurements.size(); i++) {
            if (classes.get(i) != null &&
                classes.get(i).toLowerCase().contains(className.toLowerCase())) {
