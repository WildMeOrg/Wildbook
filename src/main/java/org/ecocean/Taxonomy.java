...
public class Taxonomy implements java.io.Serializable {
    ...
    // There should be only one record per scientificName value in the Taxonomy table, though they sometimes change e.g. when giraffes were
    // reclassified.
    ...
    // while convenience string-only methods will use noun "Species" (and might require Shepherds to see which Taxonomy objects exist in the DB, for
    // e.g. putting a species string on an Encounter)
    ...
    public static List<Taxonomy> findMatch(Shepherd myShepherd, String regex) {
        ...
        while (it.hasNext()) {
            Taxonomy tx = it.next();
            if (!tx.getScientificName().matches(regex) && (tx.filterCommonNames(regex).size() < 1))
                continue;
    ...
    public static List<Occurrence> getOccurrences(Shepherd myShepherd, Taxonomy tax) {
        ...
        String jdoql =
            "SELECT FROM org.ecocean.Occurrence WHERE taxonomies.contains(tx) && tx.id == '" +
            tax.getId() + "' VARIABLES org.ecocean.Taxonomy tx";
