...
public class EncounterQueryProcessor extends QueryProcessor {
  ...
  public static String queryStringBuilder(HttpServletRequest request, StringBuffer prettyPrint, Map<String, Object> paramMap){
    ...
    if((request.getParameter("organizationId")!=null) && (!request.getParameter("organizationId").equals("")) && Util.isUUID(request.getParameter("organizationId"))){
      ...
      filter = "SELECT FROM org.ecocean.Encounter WHERE user.username == this.submitterID && org.members.contains(user) && org.id == '" + orgId + "'";
      String variables_statement = " VARIABLES org.ecocean.User user; org.ecocean.Organization org";
...
*/
  ...
  //------------------------------------------------------------------
    ...
    //study site name filters-------------------------------------------------
    String[] studySiteNames=request.getParameterValues("studySiteName");
    if((studySiteNames!=null)&&(!studySiteNames[0].equals("None"))&&(!studySiteNames[0].equals(""))){
      System.out.println("Study site name query!");
      prettyPrint.append("studySiteName is one of the following: ");
      int kwLength=studySiteNames.length;
        String stageFilter="(";
        for(int kwIter=0;kwIter<kwLength;kwIter++) {
          String kwParam=studySiteNames[kwIter].replaceAll("%20", " ").trim();
          if(!kwParam.equals("")){
            if(stageFilter.equals("(")){
              stageFilter+="(studySiteID == stu.id && stu.name == \""+kwParam+"\")";
            }
            else{
              stageFilter+=" || (studySiteID == stu.id && stu.name == \""+kwParam+"\")";
            }
        ...
        if(jdoqlVariableDeclaration.equals("")) {
          jdoqlVariableDeclaration=" VARIABLES org.ecocean.StudySite stu";
        } else if(!jdoqlVariableDeclaration.contains("org.ecocean.StudySite stu")) {
          jdoqlVariableDeclaration+=";org.ecocean.StudySite stu";
        }

    }
    //end study site name filters
