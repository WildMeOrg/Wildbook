...
public class SUTime {
    ...
    private static Properties getTimeAnnotatorProperties(String rootDir) {
        ...
        ruleFile = getRuleFilepaths(rootDir, "defs.sutime.txt", "english.sutime.txt",
        ...
        if (ruleFile != null) {
            ...
            try {
                props.setProperty("sutime.binder.1.xml",
                    (rootDir + "/WEB-INF/data/holidays/Holidays_sutime.xml"));
            } catch (Exception npe) {
                props.setProperty("sutime.binder.1.xml",
                    "/data/wildbook_data_dir/WEB-INF/data/holidays/Holidays_sutime.xml");
                npe.printStackTrace();
            }
...
// props.setProperty("gutime.path", getServletContext().getRealPath("/packages/gutime"));
    ...
    private static List<CoreMap> getTimeAnnotations(String query, Annotation anno,
    ...
    private static List<CoreMap> getDates(String query, String rootDir, SUTimePipeline pipeline)
    throws IOException {
    ...
    throws IOException {
        ...
        if (!StringUtils.isNullOrEmpty(query)) {
            ...
            if (timeAnnotator != null) {
                ...
                System.out.println("<br><br>Error creating annotator for " +
                    StringEscapeUtils.escapeHtml4(annotatorType));
            }
        }
        return null;
    }
    ...
    public static SUTimePipeline createPipeline(String rootDir) {
        ...
        try {
            ...
            System.setProperty("de.jollyday.config",
                (rootDir + "/WEB-INF/classes/holidays/jollyday.properties"));
        } catch (Exception npe) {
            // OK, we couldn't find a servlet context, so let's try to get the files from a hardcoded override directory
            dataDir = "/data/wildbook_data_dir/WEB-INF/data";
            System.setProperty("de.jollyday.config",
                ("/data/wildbook_data_dir/WEB-INF/classes/holidays/jollyday.properties"));
            npe.printStackTrace();
        }
    ...
    public static ArrayList<String> parseStringForDates(String rootDir, String text,
        ...
        // String[] text1 = text.replaceAll("[^A-Za-z0-9 ]", " ").toLowerCase().split("\\s+"); //TODO I think this does a better version of what the
        // above (text = text.replaceAll("[,.!?;:]", "$0 ");) does?? -Mark Fisher
    ...
    public static String selectBestDateFromCandidates(ArrayList<String> candidates)
    throws Exception {
        String selectedDate = "";
        ...
        if (candidates.size() < 1) {
            ...
            try {
                validDatesWithExcessiveDatesRemoved = removeExcessivePastDates(
                    validDatesWithFutureDatesRemoved);
            ...
            try {
                validDatesFilteredByYesterday = removeYesterdayDatesIfTheyAreNotTheOnlyDates(
                    validDatesWithExcessiveDatesRemoved);
            ...
            if (validDatesFilteredByYesterday == null || validDatesFilteredByYesterday.size() < 1) {
                throw new Exception(
                          "validDatesFilteredByYesterday is null or empty before selecting the longest string");
            }
            if (validDatesFilteredByYesterday.size() > 1) {
                for (int j = 0; j < validDatesFilteredByYesterday.size(); j++) {
                    for (int k = j + 1; k < validDatesFilteredByYesterday.size(); k++) {
                        if (validDatesFilteredByYesterday.get(j).length() >
                            validDatesFilteredByYesterday.get(k).length()) {
                            selectedDate = validDatesFilteredByYesterday.get(j);
                        } else if (validDatesFilteredByYesterday.get(j).length() <
                            validDatesFilteredByYesterday.get(k).length()) {
        ...
        if (selectedDate == null || selectedDate.equals("")) {
            throw new Exception(
                      "selectedDate either null or empty after selecting for longest one");
        } else {
            return selectedDate;
        }
    }

    public static ArrayList<String> removeInvalidDates(ArrayList<String> candidates)
    throws Exception {
        ArrayList<String> validDates = new ArrayList<String>();
        ...
        if (validDates == null | validDates.size() < 1) {
            throw new Exception(
                      "validDates arrayList is empty or null in removeInvalidDates method");
        } else {
            return validDates;
        }
    }

    public static ArrayList<String> removeFutureDates(ArrayList<String> candidates)
    throws Exception {
    ...
    public static ArrayList<String> removeYesterdayDatesIfTheyAreNotTheOnlyDates(
    ...
    throws Exception {
        ...
        if (returnCandidates == null | returnCandidates.size() < 1) {
            throw new Exception(
                      "removeYesterdayDatesIfTheyAreNotTheOnlyDates method returned null or empty arrayList");
        } else {
            return returnCandidates;
        }
    }
...
// use this function to remove false positive from NLP that are clearly before the project started, such as "1492"
    public static ArrayList<String> removeExcessivePastDates(ArrayList<String> candidates)
    throws Exception {
// TODO add handling for tweets coming from future in datelines
        ArrayList<String> returnCandidates = new ArrayList<String>();
...
// overloaded version to deal with tweets
    public static String parseDateStringForBestDate(String rootDir, Status tweet)
    throws Exception {
