...
public class GoogleOcr {
    public static List<byte[]> makeBytesFrames(ArrayList<MediaAsset> frames) {
        ...
        try {
            ...
            System.out.println(
                "Yikes, exception while trying to convert frames into jpg bytes array. For more info check 'providin image' in here:https://cloud.google.com/vision/docs/request#providing_the_image");
        }
        return null;
    }
    ...
    /*
       ...
       // Instantiates Stella's credentials Properties googleProps = ShepherdProperties.getProperties("googleKeys.properties","");
       ...
       if (vision !=null) {
        try {
          ...
          for (byte[] byteFrame : bytesFrames) {
           // Builds the image annotation request ImmutableList.Builder<AnnotateImageRequest> requests = ImmutableList.builder();
            requests.add(
                ...
                if((batchResponse==null)||(batchResponse.getResponses()==null)||(batchResponse.getResponses().get(0).getTextAnnotations()==null)||(batchResponse.getResponses().get(0).getTextAnnotations().get(0).getDescription()
                   == null)){
    ...
    public static String detectText(List<byte[]> byteFrames)
    throws Exception, IOException {
        ...
        for (byte[] byteFrame : byteFrames) {
            ...
            AnnotateImageRequest request = AnnotateImageRequest.newBuilder().addFeatures(
