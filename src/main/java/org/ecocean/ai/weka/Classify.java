...
public class Classify {
    ...
    /*
     * Assumes you're using a Weka FilteredClassifier that will do any necessary filter to the Instance (e.g., StringToWord) as a part of the
     * classification.
     *
     */
    public static Double classifyWithFilteredClassifier(weka.core.Instance instance,
        String fullPathToClassifierFile) {
        Double result = -1.0;

        try {
            // load classifier from file
            Classifier cls_co = (Classifier)weka.core.SerializationHelper.read(
                fullPathToClassifierFile);
    ...
    /*
       //partially complete public static Double createNewModel(String classifyMe, String classifierName, String classifierOps, String filterName,
          String filterOps) {
       ...
       try {
        ...
        //set classifier m_Classifier = AbstractClassifier.forName(classifierName, (String[])classifierOptions.toArray());

        //set filter m_Filter = (Filter) Class.forName(filterName).newInstance();
     ...
     */
    public static Instances makeInstance(List<String> attributesToClassifyNames,
        List<String> valuesToClassify, List<String> classValues) {
