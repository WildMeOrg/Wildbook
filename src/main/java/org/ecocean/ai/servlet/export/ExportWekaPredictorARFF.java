...
public class ExportWekaPredictorARFF extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        File dataDir = CommonConfiguration.getDataDirectory(
            request.getSession().getServletContext(), context);
        ...
        out.println(
            "<p><strong>Export underway. It's a slow process. When it is done, you will find it at: "
            + outputFile.getAbsolutePath() + "</strong></p>");
        ...
        try {
            String filter =
                "SELECT FROM org.ecocean.media.MediaAsset WHERE store instanceof org.ecocean.media.YouTubeAssetStore";
            ...
            for (int i = 0; i < numResults; i++) {
                ...
                if ((ma.getMetadata() != null)) {
                    ...
                    if (md.getData() != null) {
                        String videoID = ma.getMetadata().getData().getJSONObject(
                            "detailed").optString("id");
                        String videoTitle = "[unknown]";
                        String videoTitleShort = videoTitle;
                        if (videoTitle.length() > 1000) {
                            videoTitleShort = videoTitle.substring(0, 1000);
                        }
                        ...
                        if (videoTags.length() > 1000) {
                            videoTagsShort = videoTags.substring(0, 1000);
                        }
                        ...
                        String qFilter =
                            ...
                            videoID +
                            "') != -1) && ( state=='approved' || state=='unidentifiable')";
                        ...
                        if (numEncs > 0) {
                            ...
                            videoDescription = DetectTranslate.translateIfNotEnglish(
                                videoDescription);

                            sb.append("'" +
                                AIUtilities.youtubePredictorPrepareString(videoTitle + " " +
                                videoDescription + " " + videoTags) + "',good \n");
                        } else {
                            String pFilter =
                                "SELECT FROM org.ecocean.Encounter WHERE (occurrenceRemarks.indexOf('"
                                + videoID + "') != -1) && ( state=='auto_sourced')";
                            ...
                            if (encresults2.size() == 0) {
                                ...
                                sb.append("'" +
                                    StringEscapeUtils.escapeEcmaScript(videoTitle + " " +
                                    videoDescription + " " + videoTags) + "',poor\n");
                            }
                        }
                    }
                }
            }
            sb.append("%Num discard assets (negative training data): " + poorDataVideos.size() +
