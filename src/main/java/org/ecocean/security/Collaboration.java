...
/**
 * a Collaboration is a defined, two-way relationship between two Users. It can exist in various
 * states, but once fully approved, generally will allow the referenced two users to have more
...
// NOTE the first user, by convention, is the initiator
    ...
    // copied with Shepherd instead of context in hopes this fixes the issue where we couldn't save
    // an updated collab with another shepherd
...
// System.out.println("qry -> " + queryString);
    ...
    public static Collaboration collaborationBetweenUsers(String username1, String username2,
        ...
        try {
            ...
            query.closeAll();
            // System.out.println("collaborationBetweenUsers(String username1, String username2,
            // String context)");
    ...
    public static boolean canCollaborate(String context, String u1, String u2) {
        if (User.isUsernameAnonymous(u1) || User.isUsernameAnonymous(u2)) return true;          // TODO
                                                                                                // not
                                                                                                // sure???
        if (u1.equals(u2)) return true;
        Collaboration c = collaborationBetweenUsers(u1, u2, context);
    ...
    public static boolean canUserViewOwnedObject(String ownerName, HttpServletRequest request,
        Shepherd myShepherd) {
        if (request.isUserInRole("admin")) return true;          // TODO generalize and/or allow
                                                                 // other roles all-access
    ...
    public static boolean canUserViewOwnedObject(User viewer, User owner,
        ...
        if (viewer != null && owner != null && viewer.getUUID() != null &&
            viewer.getUUID().equals(owner.getUUID())) return true;                                                                  //
                                                                                                                                    // should
                                                                                                                                    // really
                                                                                                                                    // be
                                                                                                                                    // user
    ...
    public static boolean canUserAccessOwnedObject(String ownerName, HttpServletRequest request) {
        ...
        if (request.isUserInRole("admin")) return true;          // TODO generalize and/or allow
                                                                 // other roles all-access
        if (User.isUsernameAnonymous(ownerName)) return true;          // anon-owned is "fair game"
                                                                       // to anyone
        if (request.getUserPrincipal() == null) {
            return canCollaborate(context, ownerName, "public");
        }
        String username = request.getUserPrincipal().getName();
        // System.out.println("canUserAccessOwnedObject(String ownerName, HttpServletRequest
        // request)");
    ...
    public static boolean canUserAccessEncounter(Encounter enc, String context, String username) {
        ...
        if (User.isUsernameAnonymous(owner)) return true;          // anon-owned is "fair game" to
                                                                   // anyone
        // System.out.println("canUserAccessEncounter(Encounter enc, String context, String
        // username)");
    ...
    public static boolean canUserAccessOccurrence(Occurrence occ, HttpServletRequest request) {
        ...
        for (Encounter enc : all) {
            if (canUserAccessEncounter(enc, request)) return true; // one is good enough (either
                                                                   // owner or in collab or no
                                                                   // security etc)
        }
        return false;
    }
    ...
    public static boolean canUserAccessMarkedIndividual(MarkedIndividual mi,
        ...
        if ((all == null) || (all.size() < 1)) return true;
        for (Encounter enc : all) {
            if (canUserAccessEncounter(enc, request)) return true;              // one is good
                                                                                // enough (either
                                                                                // owner or in
                                                                                // collab or no
                                                                                // security etc)
    ...
    public static boolean canUserFullyEditMarkedIndividual(MarkedIndividual mi,
        HttpServletRequest request) {
        if (request.isUserInRole("admin")) return true;     // TODO generalize and/or allow other
                                                            // roles all-access
        Vector<Encounter> all = mi.getEncounters();
        if ((all == null) || (all.size() < 1)) return false;
        for (Encounter enc : all) {
            if (!canEditEncounter(enc, request)) return false;    // one is good enough (either
                                                                  // owner or in collab or no
                                                                  // security etc)
        }
        return true;
    ...
    public static boolean canUserAccessSocialUnit(SocialUnit su, HttpServletRequest request) {
        ...
        for (MarkedIndividual indy : all) {
            if (canUserAccessMarkedIndividual(indy, request)) return true;    // one is good enough
                                                                              // (either owner or in
                                                                              // collab or no
                                                                              // security etc)
        }
    ...
    public String getEditInitiator() {
        if (editInitiator != null && !"".equals(editInitiator)) {
            ...
            this.editInitiator = this.username1;             // probably old collaboration request
                                                             // where position 1 is always initiator
            return editInitiator;
        }
    }
...
/*   CURRENTLY NOT USED public static boolean doesQueryExcludeUser(Query query, HttpServletRequest
   request) {
                ...
                for (Organization org : orgs) {
                    ...
                    for (User user : users) {
                        if (user.getUsername() != null && !user.getUsername().equals(username)) {
                            // System.out.println("dding collab for:
                            // "+username+":"+user.getUsername());

                            // so this is someone else than the orgAdmin and therefore someone we
