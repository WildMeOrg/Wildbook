...
// java sucks for making us add three import lines just to use a multimap (without calling a single
// method on it!). INELEGANT. NEXT!
...
/**
 * a HiddenDataReporter is a simple, non-persistent class. Its intended use is mainly in exporters
 * or search results. It keeps track of data that was hidden from a particular user so that that
 * user can request collaborations with the data owners. Inheriting classes are defined by their
 * choice of the data type <T>
...
abstract class HiddenDataReporter<T> {
    ...
    protected final HttpServletRequest request;     // the whole thing only makes sense in the
                                                    // context of a request (which has attached user
                                                    // object)
    public final String className;     // frustrating that I can't figure out how to get this
                                       // generically from <T>
    ...
    // this is useful to provide a link to the hidden data, which prompts the user to start a
    // collaboration
    ...
    // makes an excel report about this hidden data, exposing only the usernames, number encounters,
    // and a prompt for collaboration
