...
/*
    making a decision here to set *both* scistarter and ppsr values when possible for example, there is "name" (scistarter) and "ProjectName" (ppsr)
       -- these both will be set no matter what.  they will be identical *unless* the property files specifically set them differently!

    note: (intentionally) exculdes api key.... add that after when needed
 */
    public static JSONObject generateJSONObject(String context)
    throws IOException {
        ...
        req = _setJSON(context, jobj, "description", "ProjectDescription",
            CommonConfiguration.getHTMLDescription(context));
        if (!req)
            throw new IOException(
                      "one of properties description or ProjectDescription is required");
        req = _setJSON(context, jobj, "origin", "ProjectDataProvider", "PPSR:wildbook.org:" + guid);
        if (!req)
            throw new IOException("one of properties origin or ProjectDataProvider is required");
        req = _setJSON(context, jobj, "contact_name", "ProjectContactName", wbname + " Admin");
        if (!req)
            throw new IOException(
                      "one of properties contact_name or ProjectContactName is required");
        req = _setJSON(context, jobj, "contact_email", "ProjectContactEmail",
            CommonConfiguration.getAutoEmailAddress(context));
        if (!req)
            throw new IOException(
                      "one of properties contact_email or ProjectContactEmail is required");
        ...
        String keywords = getProperty(context, "ppsr.ProjectKeywords",
            CommonConfiguration.getHTMLKeywords(context));
    ...
    static boolean _setJSON(String context, JSONObject jobj, String ssProp, String ppsrProp,
        String def) {
    ...
    public static JSONObject register(String context)
    throws IOException {
        ...
        if (apiKey == null)
            throw new IOException("you must set sciStarterApiKey in ppsr.properties");
    ...
    public static String apiUrl(String context) {
        return getProperty(context, "sciStarterApiEndpoint",
                "https://scistarter.com/api/project/add/");
    }
    ...
    public static String getProperty(String context, String label, String def) {
        ...
        if (p == null) {
            System.out.println("PPSR.getProperty(" + label +
                ") has no properties; ppsr.properties unavailable?");
            return null;
        }
