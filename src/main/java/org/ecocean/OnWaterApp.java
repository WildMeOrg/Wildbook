...
public class OnWaterApp {
    ...
    public static void init(String context) {
        if (props == null)
            props = ShepherdProperties.getProperties("on-water-app.properties", "", context);
    ...
    throws IOException {
        if (!hasBeenInitialized())
            throw new IOException("Looks like OnWaterApp.init(context) has not been called yet.");
    }
 ...
 */
    ...
    public static Encounter toEncounter(JSONObject jin, Shepherd myShepherd)
    throws IOException {
    ...
    throws IOException {
        ...
        if (id == null) {
            ...
            if (myShepherd.isEncounter(id)) {
                if (createDuplicate) {
                    System.out.println(
                        "OnWaterApp.toEncounter(): attempt to create duplicate Encounter ID " + id +
                        "; generating random");
                    id = Util.generateUUID();
                } else {
                    System.out.println(
                        "OnWaterApp.toEncounter(): will not create duplicate Encounter ID " + id);
                    throw new IOException("Encounter with id=" + id +
                            " already exists; will not created duplicate");
                }
            }
        }
        ...
        if (ma != null) {
            ...
            System.out.println("OnWaterApp is calling IA.intakeMediaAssetsOneSpecies on " + tax +
                ", " + ma);
            // compare this to prev. logic in detectionQueueJob method below
            List<MediaAsset> mas = new ArrayList<MediaAsset>();
            mas.add(ma);
            IA.intakeMediaAssetsOneSpecies(myShepherd, mas, tax, task);
 ...
 */
    ...
    public static Taxonomy findTaxonomy(Shepherd myShepherd, String tstring) {
        ...
        List<String> configuredCommonNames = CommonConfiguration.getIndexedPropertyValues(
            "commonName", myShepherd.getContext());
        ...
        for (int i = 0; i < configuredCommonNames.size(); i++) {
            ...
            if (cn.toLowerCase().equals(tstring.toLowerCase()) && (i < configuredSpecies.size()))
                return new Taxonomy(configuredSpecies.get(i));
        }
        return null;
    }
...
//// API-related calls (both flavors)
    ...
    public static JSONArray getList(String context)
    ...
    public static JSONArray getListSince(int since)
    throws RuntimeException, MalformedURLException, IOException, NoSuchAlgorithmException,
        InvalidKeyException {
        System.out.println(">>> getListSince grabbing since " +
 ...
 */
    ...
    public static JSONArray apiGetArray(String apiUrl)
    throws RuntimeException, MalformedURLException, IOException, NoSuchAlgorithmException,
        InvalidKeyException {
        checkInit();
        URL getUrl = new URL(apiUrlPrefix + apiUrl);
