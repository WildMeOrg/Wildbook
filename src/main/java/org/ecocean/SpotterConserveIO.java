...
public class SpotterConserveIO {
    ...
    public static void init(String context) {
        if (props == null)
            props = ShepherdProperties.getProperties("spotter-conserve-io.properties", "", context);
    ...
    throws IOException {
        if (!hasBeenInitialized())
            throw new IOException(
                      "Looks like SpotterConserveIO.init(context) has not been called yet.");
    }
 ...
 ******************************************/
    ...
    public static Survey ciToSurvey(JSONObject jin, Shepherd myShepherd) {
        ...
        survey.addComments("<p>Observer Names: <b>" + jin.optString("Observer Names",
            "<i>none provided</i>") + "</b>, Channel Island trip ID: <b>" + jin.optInt("_tripId",
            0) + "</b></p>");
...
///TODO do we .setEffort based on survey track lengths or what???
    ...
    public static SurveyTrack ciToSurveyTrack(JSONObject jin, Shepherd myShepherd) {
        ...
        if (jin.optJSONArray("sightings") != null) {
            ...
            if (jocc.length() % 2 == 1)
                throw new RuntimeException("sightings JSONArray is odd length=" + jocc.length());
            int halfSize = (int)jocc.length() / 2;
            for (int i = 0; i < halfSize; i++) {
                Occurrence occ = ciToOccurrence(jocc.optJSONObject(i),
                    jocc.optJSONObject(i + halfSize), jin, myShepherd);
...
/* also notable?
   Other Vessels On Scene: 0, Other Species: "", Certainty: "Certain", Distance Category: "B"
 */

        // it actually appears the jin2 array contains WhaleAlert type sightings data, fwiw; but we only care about these 2:
        occ.addComments("<p class=\"import-source\">conserve.io source: <a href=\"" +
            jin2.optString("url") + "\"><b>" + jin2.optString("id") + "</b></a></p>");
 ...
 */
    public static Encounter ciToEncounter(JSONObject jin, JSONObject occJson, String occId,
        ...
        if ((imageStart < 0) || (imageEnd < 0) || (imageEnd < imageStart)) {
            enc.addComments(
                "<p class=\"error\"><b>NOTE:</b> invalid range for image start/end; ignored</p><p class=\"json\">"
                + jin.toString(4) + "</p>");
            System.out.println("WARNING: " + enc + " had no valid image range [" + imageStart +
                " - " + imageEnd + "]");
        } else if ((imageEnd - imageStart) > sanityMaxNumberImages) {
            enc.addComments("<p class=\"error\"><b>NOTE:</b> too many images detected (" +
                (imageEnd - imageStart) + " > " + sanityMaxNumberImages +
                "); ignored</p><p class=\"json\">" + jin.toString(4) + "</p>");
            System.out.println("WARNING: " + enc + " number images > sanity check (" +
                sanityMaxNumberImages + ") [" + imageEnd + " - " + imageStart + "]");
        } else {
            ...
            for (int i = imageStart; i <= imageEnd; i++) {
                ...
                params.put("description",
                    "Image number " + i + " (in " + imageStart + "-" + imageEnd +
                    "); Card Number " + jin.optString("Card Number",
                    "Unknown") + ", PID Code " + jin.optString("PID Code", "Unknown"));
                Feature ft = new Feature("org.ecocean.MediaAssetPlaceholder", params);
                Annotation ann = new Annotation(ciSpecies(occJson.optString("CINMS Species", null)),
                    ft);
 ...
 ******************************************/
    ...
    public static Survey cwToSurvey(JSONObject jin, Shepherd myShepherd) {
        ...
        String comments = "<p>Data Collector: <b>" + jin.optString("Data Collector",
            "<i>none provided</i>") + "</b>; ";
        comments += "Assistants: <b>" + jin.optString("Assistants",
            "<i>none provided</i>") + "</b>; ";
        comments += "Operator: <b>" + jin.optString("Operator", "<i>none provided</i>") + "</b>; ";
        comments += "Departure port: <b>" + jin.optString("Departure Port",
            "<i>none provided</i>") + "</b>; ";
 ...
 */
    public static SurveyTrack cwToSurveyTrack(JSONObject jin, Shepherd myShepherd) {
        ...
        if (jin.optJSONArray("sightings") != null) {
            ...
            int halfSize = (int)jocc.length() / 2;
            for (int i = 0; i < halfSize; i++) {
                Occurrence occ = cwToOccurrence(jocc.optJSONObject(i),
                    jocc.optJSONObject(i + halfSize), jin, myShepherd);
                if (occ != null) occs.add(occ);
            }
            st.setOccurrences(occs);
    ...
    public static Occurrence cwToOccurrence(JSONObject jin, JSONObject jin2, JSONObject allJson,
        Shepherd myShepherd) {
        int tripId = allJson.optInt("_tripId", 0);
        Occurrence occ = new Occurrence();
        ...
        String bool = "<p>Photos taken? <b>" + (jin.optBoolean("Photos Taken?",
            false) ? "yes" : "no") + "</b>; ";
        bool += "Calf present? <b>" + (jin.optBoolean("Calf Present?",
            false) ? "yes" : "no") + "</b>; ";
        bool += "Birds present? <b>" + (jin.optBoolean("Birds Present?",
            false) ? "yes" : "no") + "</b>; ";
        bool += "</p>";
 ...
 */
    ...
    public static User cwToUser(JSONObject jin, Shepherd myShepherd) {
        ...
        if (jin.optString("creator").equals("ShaneGero"))
            return myShepherd.getUserByUUID("6811f404-aaa5-4b13-9d2c-111e3738955b");
        // return ciGetSubmitterUser(myShepherd);
        return null;
    }
 ...
 ******************************************/
    ...
    public static List<Occurrence> waToOccurrences(JSONObject jin, Shepherd myShepherd) {
        ...
        int halfSize = (int)jocc.length() / 2;
        for (int i = 0; i < halfSize; i++) {
            Occurrence occ = waToOccurrence(jocc.optJSONObject(i), jocc.optJSONObject(i + halfSize),
                jin.optInt("_tripId", 0), myShepherd);
            if (occ != null) occs.add(occ);
        }
    ...
    public static Occurrence waToOccurrence(JSONObject jin, JSONObject jin2, int tripId,
        Shepherd myShepherd) {
        Occurrence occ = new Occurrence();
    ...
    public static Encounter waToEncounter(String photoUrl, JSONObject occJson, Occurrence occ,
        Shepherd myShepherd) {
        URLAssetStore urlStore = URLAssetStore.find(myShepherd);

        if (urlStore == null)
            throw new RuntimeException("Could not find a URLAssetStore to store images");
        Encounter enc = new Encounter();
 ...
 */
        ...
        if (photoUrl != null) {
            ...
            try {
                ...
                System.out.println(
                    "WARNING: SpotterConserveIO.waToEncounter() on updateMetadata() of " +
                    photoUrl + " threw " + iox.toString());
            }
    ...
    public static User waToUser(JSONObject jin, Shepherd myShepherd) {
        ...
        return findOrMakeUser(jin.optString("Whale Alert Submitter Email", null),
 ...
 ******************************************/
    ...
    public static List<Occurrence> oaToOccurrences(JSONObject jin, Shepherd myShepherd) {
        ...
        int halfSize = (int)jocc.length() / 2;
        for (int i = 0; i < halfSize; i++) {
            Occurrence occ = oaToOccurrence(jocc.optJSONObject(i), jocc.optJSONObject(i + halfSize),
                jin.optInt("_tripId", 0), myShepherd);
            if (occ != null) occs.add(occ);
        }
        checkIntegrity(occs, false, myShepherd);
    ...
    public static Occurrence oaToOccurrence(JSONObject jin, JSONObject jin2, int tripId,
        Shepherd myShepherd) {
        Occurrence occ = new Occurrence();
    ...
    public static Encounter oaToEncounter(String photoUrl, JSONObject occJson, Occurrence occ,
        Shepherd myShepherd) {
        URLAssetStore urlStore = URLAssetStore.find(myShepherd);
        ...
        if (photoUrl != null) {
            ...
            try {
                ...
                System.out.println(
                    "WARNING: SpotterConserveIO.oaToEncounter() on updateMetadata() of " +
                    photoUrl + " threw " + iox.toString());
            }
            MediaAssetFactory.save(ma, myShepherd);
            ma.updateStandardChildren(myShepherd);
...
/*  this should be redundant now tax = myShepherd.getTaxonomy(sciName);
        if (tax != null) return tax;
 */
...
// System.out.println("SEGPTS " + segPts);
    ...
    public static Path trackToPath(JSONObject track) { // for now (see comments above) this is just convenience method
        if ((track == null) || (track.optJSONObject("gpx") == null) ||
            (track.getJSONObject("gpx").optJSONObject("trk") == null)) return null;
    ...
    public static PointLocation trkptToPointLocation(JSONObject trkpt) {
        ...
        try {
            ...
            System.out.println("WARNING: trkptToPointLocation() could not convert '" + tt +
                "' to DateTime");
        }
...
//// API-related calls (both flavors)
    ...
    public static JSONObject ciGetTripList(String context)
    ...
    public static JSONObject waGetTripList(String context)
    throws RuntimeException, MalformedURLException, IOException, NoSuchAlgorithmException,
        InvalidKeyException {
    ...
    public static JSONObject ciGetTripListSince(int since)
    throws RuntimeException, MalformedURLException, IOException, NoSuchAlgorithmException,
        InvalidKeyException {
        System.out.println(">>> ciGetTripListSince grabbing since " +
            new DateTime(new Long(since) * 1000));
        return apiGet("/project/" + PROJECT_ID_CI + "/trip_data/" + since + "/0");
    }

    public static JSONObject waGetTripListSince(int since)
    throws RuntimeException, MalformedURLException, IOException, NoSuchAlgorithmException,
        InvalidKeyException {
        System.out.println(">>> waGetTripListSince grabbing since " +
            new DateTime(new Long(since) * 1000));
    ...
    public static String tripFlavor(JSONObject tripData) {
        ...
        if ((tripData.optString("Ship Name", null) != null) || (tripData.optString("Data Collector",
            null) != null)) return "cw";
        if ((tripData.optJSONObject("track") != null) ||
            (tripData.optJSONArray("CINMS Weather") != null) || (tripData.optString("CINMS Vessel",
                null) != null)) return "ci";
        JSONArray sarr = tripData.optJSONArray("sightings");
        // ITIS Species TSN seems to only exist in Ocean Alert, not Whale Alert
        if ((sarr != null) && (sarr.length() > 0) && (sarr.optJSONObject(0) != null) &&
            (sarr.getJSONObject(0).optString("ITIS Species TSN", null) != null)) return "oa";
        return "wa";
    }

    // CI & WA trips use same call
    public static JSONObject getTrip(int tripId)
    throws RuntimeException, MalformedURLException, IOException, NoSuchAlgorithmException,
        InvalidKeyException {
    ...
    public static List<User> ciGetVolunteerUsers(Shepherd myShepherd, List<String> fullNames) {
        ...
        if ((org == null) || (vorg == null))
            throw new RuntimeException("You must have valid " + PROP_ORGID_CI + " and " +
                    PROP_ORGID_CI_VOLUNTEER + " set in SpotterConserverIO properties file");
    ...
    public static String checkIntegrity(Path path, Shepherd myShepherd) {
        ...
        if (Util.collectionIsEmptyOrNull(path.getPointLocations()))
            return "empty .path on SurveyTrack; ";
        String reason = "";
        ...
        if (invalidPts > 5)
            reason += "found " + invalidPts + " invalid lat/lon points in SurveyTrack path; ";
        if (reason.equals("")) return null;
        return reason;
    }
    ...
    public static String checkIntegrity(List<Occurrence> occs, boolean override,
        ...
        for (Occurrence occ : occs) {
            ...
            if (Util.stringExists(c))
                reason += "occ " + occ.getOccurrenceID() + " failed integrity: [" + c + "]";
        }
    ...
    public static String checkIntegrity(Occurrence occ, boolean override, Shepherd myShepherd) {
        ...
        if (!Util.collectionIsEmptyOrNull(occ.getTaxonomies()))
            for (Taxonomy tx : new HashSet<Taxonomy>(occ.getTaxonomies())) {
        ...
        if (!override && reason.equals("")) return null;
        occ.addComments("<p class=\"spotter-review\">pending approval: <i>" + reason +
            (override ?
            "overridden (likely bad integrity in sibling Occurrences or SurveyTrack); " : "") +
            "</i></p>");
        System.out.println("WARNING: checkIntegrity() on " + occ + " failed due to: " + reason +
            " override=" + override);
        return reason;
    }
    ...
    public static String checkIntegrity(Encounter enc, boolean override, Shepherd myShepherd) {
        ...
        enc.addComments("<p class=\"spotter-review\">pending approval: <i>" + reason +
            (override ? "overridden (likely bad integrity in Occurrence); " : "") + "</i></p>");
        enc.setState(ENCOUNTER_STATE_REVIEW);
        System.out.println("WARNING: checkIntegrity() on " + enc + " failed due to: " + reason +
            " override=" + override);
        return reason;
    }
    ...
    public static String checkIntegrity(Double lat, Double lon, Shepherd myShepherd) {
        ...
        if (Util.isValidDecimalLatitude(lat) && Util.isValidDecimalLongitude(lon) &&
            !Util.nearWater(myShepherd, lat, lon, WATER_DISTANCE_RADIUS))
            reason += "lat/lon is not near water; ";
        if (reason.equals("")) return null;
        return reason;
    }
