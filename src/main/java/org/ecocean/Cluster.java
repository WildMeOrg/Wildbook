 ...
 *
 *  This class could basically be a set of static utility functions off of Occurrence Keeping separate for cleanliness sake right now.
 *
 **/

public class Cluster {
    public static List<Occurrence> makeNOccurrences(int n, List<MediaAsset> assets,
        Shepherd myShepherd) {
    ...
    public static List<Occurrence> fromSmartXml(File smartXmlFile, List<MediaAsset> assets)
    throws IOException {
        Document xdoc = null;
...
///////TODO verify that this produces correct output
                ...
                NodeList olist = wel.getElementsByTagName("observations"); // these will map to separate Occurrences, so kind of "inherit" the above
                                                                           // from waypoint. :/
                ...
                for (int o = 0; o < olist.getLength(); o++) {
                    ...
                    if (!"animals.liveanimals.".equals(oel.getAttribute("categoryKey"))) {
                        System.out.println(" - skipping observation with non-animal categoryKey=" +
                            oel.getAttribute("categoryKey") + " in waypoint id=" +
                            wel.getAttribute("id"));
                        continue;
                    }
        ...
        for (int i = 0; i < occs.size(); i++) {
            if (photoOffsets.get(i) == null) {
                System.out.println(" # Occurrence[" + i +
                    "] did not have a photoOffset -- not assigning MediaAssets");
            } else {
...
// System.out.println(" - Occurrence[" + i + "] assets -> [" + photoOffsets.get(i) + "," + toIndex + "] " + occs.get(i).getAssets());
                System.out.println(" - Occurrence[" + i + "] assets -> [" + photoOffsets.get(i) +
                    "," + toIndex + "]");
            }
            // persist the occ!
        }
 ...
 */
    ...
    public static String buildConsoleCommand(List<TimePlace> data) {
        ...
        return ("python3 /var/lib/tomcat7/webapps/wildbook/config/occurrence_blackbox.py " +
                   lats.toString() + lons.toString() + secs.toString());
    }

    public static List<List<MediaAsset> > groupAssetsByJonsOutput(List<MediaAsset> assets,
        int[] occIndices)
    throws IOException {
    ...
    public static List<Occurrence> putAssetsInOccs(List<List<MediaAsset> > assetGroups,
        Shepherd myShepherd) {
        List<Occurrence> occs = new ArrayList<Occurrence>();
    ...
    public static String runJonsScript(List<MediaAsset> assets, Shepherd myShepherd)
    throws IOException {
