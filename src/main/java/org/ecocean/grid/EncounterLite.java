...
public class EncounterLite implements java.io.Serializable {
    ...
    public MatchObject getPointsForBestMatch(SuperSpot[] newspotsTemp, double epsilon, double R,
        ...
        try {
            ...
            SuperSpot[] newspots;
            // set up variables needed to normalize spots and make sure that list A is always the
            // smallest of the two lists
            ...
            // System.out.println("     I expect "+(numSpots*(numSpots-1)*(numSpots-2)/6)+"
            // triangles.");
            ...
            for (int i = 0; i < newTriangles.size(); i++) {
                ...
                if (tempTriangle.D13 / newSpan >= Sizelim) {
                    // System.out.println("Removing large triangle: "+tempTriangle.D13+" "+newSpan+"
                    // "+tempTriangle.D13/newSpan);
            ...
            for (int i = 0; i < spotArrayL; i++) {
                for (int j = i + 1; j < (baseSpots.length - 1); j++) {
                    for (int k = j + 1; k < baseSpots.length; k++) {
                        ...
                        orient = 0;
                        if (tempTriangle.clockwise) orient = 1;
                        // System.out.println("New "+i+" "+j+" "+k+" "+tempTriangle.C+"
                        // "+tempTriangle.tC2+" "+tempTriangle.R+" "+tempTriangle.tR2+"
                        // "+tempTriangle.D13+" "+orient);
                        // System.out.println(i+" "+j+" "+k+" "+tempTriangle.Dxs+"
                        // "+tempTriangle.Dys+" "+tempTriangle.Dxl+" "+tempTriangle.Dyl+" "+orient);
            ...
            for (int i = 0; i < baseTriangles.size(); i++) {
                ...
                if (tempTriangle.D13 / baseSpan >= Sizelim) {
                    // new way
                    // if (tempTriangle.D13/baseSpan < Sizelim) {
                    // System.out.println("Removing large triangle: "+tempTriangle.D13+"
                    // "+baseSpan+" "+tempTriangle.D13/baseSpan);
                    baseTriangles.remove(i);
                    i--;
                } else {
                    // System.out.println("Keeping: "+i+" "+tempTriangle.D13+" "+baseSpan+"
                    // "+tempTriangle.D13/baseSpan);
                }
            }
            // System.out.println("     Now using "+baseTriangles.size()+" base encounter
            // triangles.");
            ...
            // VmatchesA are the matched triangles of the new encounter whose spots were passed into
            // this method
            ...
            // below, 'A' refers to tArray which is the array of the new encounter triangles, 'B' to
            // baseArray which is the array of this database encounter's triangles
            double RA, RB, CA, CB;
            ...
            for (int iter1 = 0; iter1 < arrayL; iter1++) {
                ...
                for (int iter2 = 0; iter2 < baseArrayL; iter2++) {
                    ...
                    // System.out.println(iter1+" "+iter2+" RB "+RB+" RA-sqrttR2sum
                    // "+(RA-sqrttR2sum)+" RA+sqrttR2sum "+(RA+sqrttR2sum));
                    if ((RB > (RA - sqrttR2sum)) && (RB < (RA + sqrttR2sum))) {
                        ...
                        if ((Rdiff2 < 1.0) && (Cdiff2 < 1.0) && (rotdiff < 1.0)) {
                            ...
                            // System.out.println("Match: "+iter1+" "+iter2+" RA "+RA+" RB "+RB+" CA
                            // "+CA+" CB "+CB+" CWA "+tArray[iter1].clockwise+" CWB
                            // "+baseArray[iter2].clockwise+" "+sumdiffs);
                            // System.out.println("PerA "+tArray[iter1].logPerimeter+" PerB
                            // "+baseArray[iter2].logPerimeter);
                            // added the requirement here that matched trianlges be of the same
                            // orientation - jah 1/19/04
                            if (sumdiffs < bestsum) {
                                // check to make sure that the triangles are not extreme rotations
                                // of each other
                ...
                if (matched) {
                    ...
                    // System.out.println("Match: "+bestsum+" "+tArray[iter1].R+"
                    // "+baseArray[bestiter2].R+" "+tArray[iter1].C+" "+baseArray[bestiter2].C+"
                    // "+tArray[iter1].D13/newSpan+" "+baseArray[bestiter2].D13/baseSpan+"
                    // "+tArray[iter1].clockwise+" "+baseArray[bestiter2].clockwise+" "+iter1+"
                    // "+bestiter2);
                    VmatchesA.add(tArray[iter1]);
            ...
            // System.out.println("I am now about to start filtering with "+VmatchesA.size()+"
            // triangles!");
            // now begin filtering
            ...
            int numIterations = 0;
            // System.out.println("   Going into the logM filter with "+VmatchesA.size()+" matching
            // triangles. Before filtering, N+="+nPLUS+" N-="+nMINUS);
            double oldStdDeviationLogM = 10000;
            while (stillIterate && (numIterations < 20) && (VmatchesA.size() > 0)) {
                // System.out.println("          iterating filter with "+VmatchesA.size()+"
                // triangles!");
                numIterations++;
                ...
                // System.out.println("The real std dev. should be:
                // "+Math.pow((stdDeviationLogM/(logM.size()-1)), 0.5));
                if (nPLUS > 1) {
                ...
                // System.out.println("     Removed "+logMremovals+" triangles on logM filter pass
                // "+numIterations+" with a filter/multiple value of "+multiple);
                // System.out.println("          leftsideRemovals="+leftsideRemovals+"
                //     rightsideRemovals="+rightsideRemovals);
                ...
                // System.out.println("          Going into the next round with mT, mF:
                // "+mT+","+mF);
            }
            ...
            // System.out.println("Going into scoring with "+VmatchesA.size()+" matching
            // triangles.");
            if (VmatchesA.size() == 0) {
            ...
            // now run Groth's algorithm again if there are enough spots. if not, exit as this is
            // not a match.
            ...
            if ((secondNewSpots.length > 3) && (secondBaseSpots.length > 3)) {
                ...
                if (secondPassSpots.length < 3) {
                    // System.out.println("Exiting after the second pass because the returned number
                    // of spots was less than three.
                    // "+scoredSpots.length+"-"+(scoredSpots.length-secondPassSpots.length)+"="+secondPassSpots.length);
                    return (new MatchObject(belongsToMarkedIndividual, 0, 0, encounterNumber));
                }
                // System.out.println("     The second pass cut out
                // "+(scoredSpots.length-secondPassSpots.length)+" spots.");
            } else {
                // System.out.println("Exiting processing because there were less than three spots
                // going into the second filter pass. This is not a match.");
                return (new MatchObject(belongsToMarkedIndividual, 0, 0, encounterNumber));
            }
            // end second run

            // let's create and pass along an array of the logM values of the matched and scored
            // triangles.
            int logMSize = logM.size();
            ...
            while ((iter14 < scoresLength) && ok2iterate) {
                ...
                // modification...remove the drop in half limit that Groth recommends in his paper
                // b/c point scores are so low.
            ...
            // if newspots and spots were swapped at the beginning of this method to decrease
            // processing time, we need to correct this for eventual spot mapping
            if (swappedSpots) {
    ...
    private VertexPointMatch[] secondGrothPass(SuperSpot[] secondNewSpots,
        ...
        try {
            ...
            // System.out.println("     Now using "+newTriangles.size()+" new encounter
            // triangles.");
            ...
            for (int i = 0; i < baseTriangles.size(); i++) {
                ...
                /*if (tempTriangle.D13/baseSpan >= Sizelim) {
                   ...
                   //if (tempTriangle.D13/baseSpan < Sizelim) {
                    ...
                    //System.out.println("Removing large triangle: "+tempTriangle.D13+" "+baseSpan+"
                       "+tempTriangle.D13/baseSpan);
                    baseTriangles.remove(i);
                    i--;
                   } else {
                ...
                for (int iter2 = 0; iter2 < baseArrayL; iter2++) {
                    ...
                    if ((RB > (RA - sqrttR2sum)) && (RB < (RA + sqrttR2sum))) {
                        ...
                        if ((Rdiff2 < 1.0) && (Cdiff2 < 1.0) && (rotdiff < 1.0)) {
                            ...
                            if (sumdiffs < bestsum) {
                                // if
                                // ((sumdiffs<bestsum)&&(tArray[iter1].clockwise==baseArray[iter2].clockwise))
                                // {
    ...
    public com.reijns.I3S.Point2D[] getThreeLeftFiducialPoints() {
        ...
        if ((getLeftReferenceSpots() != null) && (getLeftReferenceSpots().length == 3)) {
            ...
            // is:"+refsLeft[0].getTheSpot().getCentroidX()+","+refsLeft[0].getTheSpot().getCentroidY()+"
            // and
            // "+refsLeft[1].getTheSpot().getCentroidX()+","+refsLeft[1].getTheSpot().getCentroidY()+"
            // and
