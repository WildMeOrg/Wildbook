...
public class EC2RequestThread implements Runnable, ISharkGridThread {
    ...
    private void getEC2Instances() {
        try {
            ...
            AWSCredentials credentials = new PropertiesCredentials(new File(props.getProperty(
                "credentialsFileFullPath")));
            AmazonEC2Client amazonEC2Client = new AmazonEC2Client(credentials);
            amazonEC2Client.setEndpoint(props.getProperty("endpoint"));

            /*
               //Step 2: create a security group CreateSecurityGroupRequest csgr = new
                  CreateSecurityGroupRequest();
               csgr.withGroupName("WhaleSharkJavaSecurityGroup").withDescription("whaleshark.org
                  security group");
               CreateSecurityGroupResult createSecurityGroupResult =
                   amazonEC2Client.createSecurityGroup(csgr);

               //Step 3. Create a key pair CreateKeyPairRequest createKeyPairRequest = new
            ...
            com.amazonaws.services.ec2.model.Filter runningInstancesFilter =
                new com.amazonaws.services.ec2.model.Filter("instance-state-name",
                allowedInstanceValues);

            int maxInstances = (new Integer(props.getProperty("maxInstances"))).intValue();
            List<Reservation> reservList = amazonEC2Client.describeInstances(
                (new DescribeInstancesRequest()).withFilters(
                runningInstancesFilter)).getReservations();
            int numReservations = reservList.size();
            int numInstances = 0;
            for (int i = 0; i < numReservations; i++) {
                if (reservList.get(i).getInstances().size() > 0)
                    numInstances += reservList.get(i).getInstances().size();
            }
            int numInstancesToLaunch = maxInstances - numInstances;
            System.out.println(".....There are already " + reservList.size() +
                " instances in the pending or running states. So I will launch: " +
            ...
            /*
               ...
               // Initializes a Spot Instance Request RequestSpotInstancesRequest requestRequest =
               ...
               // Amazon Linux AMI id or another of your choosing. LaunchSpecification
               ...
               // Call the RequestSpotInstance API. RequestSpotInstancesResult requestResult =
                  amazonEC2Client.requestSpotInstances(requestRequest);
            ...
            /*
             * This basic example does not handle stderr, which is sometimes dangerous (please read
             * the FAQ).
             */
