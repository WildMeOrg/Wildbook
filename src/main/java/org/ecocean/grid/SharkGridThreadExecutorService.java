    ...
    public synchronized static ThreadPoolExecutor getExecutorService() {
        try {
            if (threadPool == null) {
                ...
                // threadPool=new
                // ThreadPoolExecutor(gm.getCreationDeletionThreadQueueSize(),gm.getCreationDeletionThreadQueueSize(),0,TimeUnit.SECONDS, (new
                // ArrayBlockingQueue(100)));
                if (!gm.getCreationThread()) {
                    threadPool = new SharkGridThreadPoolExecutor(
                        gm.getCreationDeletionThreadQueueSize(),
                        gm.getCreationDeletionThreadQueueSize());
                }
