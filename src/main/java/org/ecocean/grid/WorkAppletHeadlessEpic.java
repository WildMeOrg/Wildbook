...
public class WorkAppletHeadlessEpic {
     ...
     * Obtain a connection to the server to send/receive serialized content
       ...
       private URLConnection getConnection(String action, String newEncounterNumber, int groupSize,
        * String nodeID, int numProcessors) throws IOException {
       String encNumParam = "";
       if (!newEncounterNumber.equals("")) {
        encNumParam = "&newEncounterNumber=" + newEncounterNumber;
       }
       URL u = new URL("http://" + thisURLRoot + "/scanAppletSupport?version=" + version +
        * "&nodeIdentifier=" + nodeID + "&action=" + action + encNumParam + "&groupSize=" +
    ...
    public void getGoing(int numProcessors) {
        ...
        try {
            ...
            // URL recoverURL = new URL("http://" + thisURLRoot +
            // "/encounters/sharkGrid.jsp?groupSize=1&autorestart=true" + targeted +
            ...
            // set the start groupSize used-i.e. number of scans to tackle in the first returned
            ...
            while (repeat) {
                ...
                try {
                    ...
                    // allow 55 minutes
                    // update 2017-11-08 AWS now allows per-second billing, so limit this way down
                    // to maximum 180 seconds (two polling intervals)
                    ...
                    try {
                        ...
                        // con = getConnection("getWorkItemGroup", holdEncNumber, groupSize, nodeID,
                        // numProcessors);
                        ...
                        try {
                            ...
                            if ((workItems != null) && (workItems.size() > 0)) {
                                ...
                                /*
                                   ...
                                   System.out.println("\n\nI hit the timeout and am shutting down
                                      after "+(timeDiff/1000/60)+" minutes.");
                    ...
                    if (successfulConnect) {
                        ...
                        if (swi.getTotalWorkItemsInTask() <= 0) {
                            try {
                                // waiting for last workItem to finish elsewhere, wait quietly and
                                // check later
                                // long currentTime=(new GregorianCalendar()).getTimeInMillis();
                                // long timeDiff=currentTime-startTime;
                                // if ((swi.getTotalWorkItemsInTask() ==
                                // -1)&&(timeDiff<allowedDiff)) {
                        ...
                        // kick it like Poison!
                            ...
                            for (int q = 0; q < vectorSize; q++) {
                                ...
                                // we also pass in workItemResults, which is a threadsafe vector of
                                // the results returned from each thread
                                try {
                            ...
                            /**
                               ...
                               ScanWorkItemResult swir = (ScanWorkItemResult)
                                  workItemResults.get(d);
                               MatchObject thisResult = swir.getResult();
                               if ((thisResult.getMatchValue() * thisResult.getAdjustedMatchValue())
                                  >= 115) {
                                numMatches++;
                               }
                               }
                            ...
                            if (resultsSize > 0) {
                                ...
                                try {
                                    ...
                                    // outputToFinalServlet = new
                                    // ObjectOutputStream(finishConnection.getOutputStream());
                                ...
                                if (line.equals("success")) {
                                    ...
                                    // recoverURL = new URL("http://" + thisURLRoot +
                                    // "/encounters/sharkGrid.jsp?groupSize=1&autorestart=true" +
