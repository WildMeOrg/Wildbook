...
public final class SpamChecker {
    ...
    /**
     * Loads words-matching patterns from a text file resource. The specified resource should contain each entry on a separate line, with
     * case-insensitive substring matches entered as plain text, and regular-expression matches (for entire text) delimited by forward slashes (e.g.
     * &quot;/.&#42;([Ff]oo|[Bb]ar)&#42;/&quot;).
     * @param rName resource name to be found on classpath
     * @return list of {@code Pattern} instances for word matching
     * @throws IOException
     */
    private static List<Pattern> loadPatterns(String rName)
    throws IOException {
    ...
    /**
     * Checks if the specified {@code Encounter} is a spam submission, based on the text content submitted. Note: the specified encounter will
     * generally not have any stored media files when passed to this method, so spam determination should only be via text fields.
    ...
    public Result isSpam(Encounter enc) {
        ...
        if (containsDefiniteSpam(enc.getSubmitterName()) ||
            containsDefiniteSpam(enc.getSubmitterPhone()))
            return Result.SPAM;
        if (containsDefiniteSpam(enc.getPhotographerName()) ||
            containsDefiniteSpam(enc.getPhotographerPhone()))
            return Result.SPAM;
        if (containsDefiniteSpam(enc.getLocation()) || containsDefiniteSpam(enc.getComments()) ||
            containsDefiniteSpam(enc.getBehavior()))
            return Result.SPAM;
        // Checks for possible spam.
        if (containsPossibleSpam(enc.getSubmitterName()) ||
            containsPossibleSpam(enc.getSubmitterPhone()))
            return Result.POSSIBLE_SPAM;
        if (containsPossibleSpam(enc.getPhotographerName()) ||
            containsPossibleSpam(enc.getPhotographerPhone()))
            return Result.POSSIBLE_SPAM;
        if (containsPossibleSpam(enc.getLocation()) || containsPossibleSpam(enc.getComments()) ||
            containsPossibleSpam(enc.getBehavior()))
            return Result.POSSIBLE_SPAM;
        return Result.NOT_SPAM;
    }

    /**
     * Checks if the specified text contains anything considered spam. These detections are absolute, and should only be specified for definite spam
     * text.
