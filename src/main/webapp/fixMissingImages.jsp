...
static File getEncDataFolder2(File imgDir, Encounter enc) {
  ...
  System.out.println(enc.getCatalogNumber()+": begin getEncDataFolder2");
...
while (allEncs.hasNext()) {
  ...
  %></br>
  <%=enc.getCatalogNumber()%> DP: <%=enc.getDynamicProperties()%>
  <%
}
allEncs=myShepherd.getAllEncounters(encQuery);
*/
...
while(allEncs.hasNext()){
  ...
  if(images.size()>0) {
    SinglePhotoVideo spv = images.get(0);
    String tempfname = spv.getFullFileSystemPath();
    File img = spv.getFile();
    if (img==null){
      %></br>IMAGE NULL! <%=encID%>spv fpath = <%=tempfname%></br><%
    }
  }
  //System.out.println("\t before if(images.size()==0):"+enc.getDynamicProperties());
  if(images.size()==0) {
    badEncs.add(encID);
    System.out.println("\n"+encID+" before calling gEDF:"+enc.getDynamicProperties());
    File encDataFolder = getEncDataFolder2(sharkImgs,enc);
    System.out.println("\t after calling gEDF:"+enc.getDynamicProperties());

/*
    while (allEncs.hasNext()) {
      Encounter testEnc=allEncs.next();
      %></br></br>AFTER images.size():</br>
      <%=testEnc.getCatalogNumber()%> DP: <%=testEnc.getDynamicProperties()%>
      <%
    }
*/

    String fname = enc.getCatalogNumber();
    File picture = new File(encDataFolder, fname+".jpg");
    File fgp = new File(encDataFolder, fname+".fgp");
    String flank = enc.getDynamicPropertyValue("flank");


    numMissing++;
    %></br></br>
    <%=encID%> has zero images, but we can find them in <%=encDataFolder.getName()%></br>
    picture.exists() = <%=picture.exists()%> and fgp.exists() = <%=fgp.exists()%> and flank = <%=flank%></br>
    dynamic properties = <%=enc.getDynamicProperties()%>
    <%
    ...
    if (flank==null) {
    ...
    else if (picture.exists() && fgp.exists()) {
      %></br>
      <%=encID%> has picture and FGP.</br>
      <%

      numWithFiles++;

      FileInputStream fStream = new FileInputStream(fgp);
      ...
      if(printEncs){%></br>Processing image <%=encID%><%}
      ...
      try {
        ...
        if ((flank!=null)&&(flank.equals("R"))){
          ...
          if(printEncs){%>
            <br/>n R-spots: <%=nPoints%>
            <br/>R-reference spots: <%=printSpotList(reference_spots)%>
            <br/>R-spots: <%=printSpotList(spots)%><%
          }
        }
        else {
          if (uploading) {
          ...
          if (printEncs) {%>
            <br/>n spots: <%=nPoints%>
            <br/>reference spots: <%=printSpotList(reference_spots)%>
            <%=printSpotList(spots)%><%
          }
        }
        if (printEncs) {
          %></br>FGP file parsed!</br><%
        }
      } catch (Exception e) {
      ...
      if(uploading){enc.addComments("<p><em>" + request.getRemoteUser() + " on " + (new java.util.Date()).toString() + "</em><br>" + "batchUpload process added spots from " + fname+".fgp"+".</p>");}

      // need to copy image
      try {
        File oldPicture = picture;
        if (printEncs){%><br/>oldPicture is in <%=oldPicture.getCanonicalPath()%><%}
        String extractName;
        if ((flank!=null)&&(flank.equals("R"))){
        ...
        double bytes = newPicture.length();
        File extractFile = new File(encounterFolder, extractName);
        if (!newPicture.exists() || bytes==0.0) {
          if(uploading){FileUtils.copyFile(oldPicture, newPicture);}
          if (printEncs){%><br/>image copied to <%=newPicture.getCanonicalPath()%><%}
        } else if (printEncs){
          %><br/>image copy already found in <%=newPicture.getCanonicalPath()%>
        </br>And it has <%=bytes%> bytes.<%
        }

        if (uploading && !extractFile.exists()) {
          FileUtils.copyFile(oldPicture, extractFile);
          if (printEncs){%><br/>Copying extractFile to <%=extractFile.getCanonicalPath()%><%}
        }

        if (uploading && (flank!=null)&&(flank.equals("R"))){
          enc.setRightSpotImageFileName(extractFile.getName());
          %>Set spot image file: <%=extractFile.getAbsolutePath()%><%
        ...
        loadPicture = true;
        if (printEncs){%><br/>PictureSPV filename: <%=pictureSPV.getFilename()%><%}
        if (printEncs){%><br/>PictureSPV filename: <%=pictureSPV.getFilename()%><%}
      } catch (Exception e) {
        ...
        String emsg=sw.toString(); // stack trace as a string
        %><br/>EXCEPTION on image copy for encounter  <%=encID%>:<br/><%=emsg%><%
      }
      ...
      try {
        ...
        if (loadPicture) {
          if (printEncs){
            %><br/>PictureSPV is null: <%=(pictureSPV==null) %>
            <br/>PictureSPV filename= <%=pictureSPV.getFilename()%>
            <br/>About to refresh asset formats with baseDir = <%=baseDir%><%
          }
          if (uploading) {
        ...
        String exceptionAsString = sw.toString().replace("at","\n at");
    ...
    } // <- ends the "if we have picture and fgp"
  }
...
%>
</br></br><%=numMissing%> missing images total
</br><%=numWithFiles%> have their files though </br>
</br><%=nAdded%> encounters had images added and refreshAssetFormats
</br><%=nullFlanks%> null flanks </br>
...
<%
  ...
  %></br>An error occurred on page fixSomeFields.jsp<%
