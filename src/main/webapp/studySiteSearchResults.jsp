<%@ page contentType="text/html; charset=utf-8" language="java"
  ...
  context=ServletUtilities.getContext(request);
    ...
    stuProps = ShepherdProperties.getProperties("studySite.properties");
    ...
    myShepherd.setAction("studySiteSearchResults.jsp");
    ...
    Vector<StudySite> rIndividuals = new Vector<StudySite>();
    myShepherd.beginDBTransaction();
    String order ="";

    StudySiteQueryResult result = StudySiteQueryProcessor.processQuery(myShepherd, request, order);
...
<div class="container maincontent">
      ...
      <h1 class="intro">
        <%=stuProps.getProperty("StudySiteSearchResults")%>
      </h1>
...
<table width="810" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>
      ...
      <p><%=stuProps.getProperty("searchResultsInstructions")%>
      </p>
    </td>
  </tr>
</table>
...
*/
  {
    key: 'name',
    label: '<%=stuProps.getProperty("name")%>',
  ...
  {
    key: 'typeOfSite',
    label: '<%=stuProps.getProperty("typeOfSite")%>',
    value: _notUndefined('typeOfSite'),
  },
  {
    key: 'governmentArea',
    label: '<%=stuProps.getProperty("governmentArea")%>',
    value: _notUndefined('governmentArea'),
  },
  {
    key: 'population',
    label: '<%=stuProps.getProperty("population")%>',
    value: _notUndefined('population'),
  },
  {
		key: 'latitude',
		label: 'latitude',
...
/*
	for (var i = 0 ; i < searchResults.length ; i++) {
		searchResultsObjects[i] = new wildbook.Model.StudySite(searchResults[i]);
	}
*/
...
function rowClick(el) {
	console.log(el);
	var w = window.open('studySite.jsp?number=' + el.getAttribute('data-id'), '_blank');
	w.focus();
	return false;
}
...
function doTable() {
	...
	inds = new wildbook.Collection.StudySites();
	var addedCount = 0;
...
if (percentage % 3 == 0) console.log(percentage);
	...
	_.each(searchResults, function(o) {
		inds.add(new wildbook.Model.StudySite(o));
	});
	$('#progress').remove();
...
function _colID(o) {
  if (o.ID == undefined) {
    if (o.DataCollectionEventID == undefined) return '';
...
<table width="810" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="left">
      <p><strong><%=stuProps.getProperty("matchingStudySites")%>
      </strong>: <span id="count-total"></span>
      </p>
      <%myShepherd.beginDBTransaction();%>
      <p><strong><%=stuProps.getProperty("totalStudySites")%>
    </strong>: <%=(myShepherd.getNumStudySites())%>
      </p>
    </td>
    <%
      myShepherd.rollbackDBTransaction();
