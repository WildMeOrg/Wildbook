...
<p>Observer: <b><%=occ.getObserver()%></b></p>
<p>Distance: <b><%=occ.getDistance()%> m</b></p>
<% if (occ.getDecimalLatitude() != null) { %>
<p>Lat/Lon: <b><%=occ.getDecimalLatitude()%> / <%=occ.getDecimalLongitude()%></b></p>
<% } %>
<p>Bearing: <b><%=occ.getBearing()%></b></p>
<p>Date: <b><%=(occ.getDateTime() == null) ? "Unknown" : occ.getDateTime().toString().substring(0,10)%></b></p>
<p>Vegetation/Habitat: <b><%=occ.getVegetation()%></b></p>
<p>Terrain: <b><%=occ.getTerrain()%></b></p>
<p>Monitoring zone: <b><%=(occ.getMonitoringZone() == null) ? "" : occ.getMonitoringZone()%></b></p>
<div class="indiv-counts">
    <p class="group-size"><span>Group size:</span> <b><%=occ.getGroupSize()%></b></p>
    <p><span>Adult males:</span> <b><%=occ.getNumAdultMales()%></b></p>
    <p><span>Adult females:</span> <b><%=occ.getNumAdultFemales()%></b></p>
    <p><span>Sub-males:</span> <b><%=occ.getNumSubMales()%></b></p>
    <p><span>Sub-females:</span> <b><%=occ.getNumSubFemales()%></b></p>
    <p><span>Juveniles:</span> <b><%=occ.getNumJuveniles()%></b></p>
</div>
	
<p>WP: <b><%=(occ.getWp() == null) ? "N/A" : occ.getWp()%></b></p>

<p><%=props.getProperty("numMarkedIndividuals") %>: <b><%=occ.getMarkedIndividualNamesForThisOccurrence().size() %></b></p>
