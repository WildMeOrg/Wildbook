...
<%!//shepherd must have an open trasnaction when passed in
	...
	public boolean checkAccessKey(HttpServletRequest request, Encounter enc) {
		...
		for (MediaAsset ma : enc.getMedia()) {
			...
			if (p == null)
				return false;
			if (!accessKey.equals(p.optString("accessKey", null)))
				return false;
		}
		return true;
	}%>
<link type='text/css' rel='stylesheet'
	href='../javascript/timepicker/jquery-ui-timepicker-addon.css' />
...
<style type="text/css">
#spot-image-wrapper-left, #spot-image-wrapper-right {
	position: relative;
	height: 510px;
}

#spot-image-left, #spot-image-canvas-left, #spot-image-right,
	#spot-image-canvas-right {
	position: absolute;
	left: 0;
	top: 0;
	max-width: 600px;
					...
					try {
						...
						if (!visible) {
							...
							if (!User.isUsernameAnonymous(uid) && (request.getUserPrincipal() != null)) {
								blocker = "<script>$(document).ready(function() { $.blockUI({ message: '" + cmsg
										+ "' + _collaborateHtml('" + uid + "', '" + name.replace("'", "\\'")
										+ "') }) });</script>";
							} else {
								blocker = "<script>$(document).ready(function() { $.blockUI({ message: '<p>" + cmsg
										+ "' + collabBackOrCloseButton() + '</p>' }) });</script>";
							}
							out.println(blocker);
						}
...
$(function() {
  ...
  $(function() {
    ...
    $( "#releasedatepicker" ).datepicker( $.datepicker.regional[ "<%=langCode%>" ] );
			...
			<%
				//int stateInt=-1;
						...
						while (moreStates) {
							...
							if (CommonConfiguration.getProperty(currentLifeState, context) != null) {
								...
								if (CommonConfiguration.getProperty(currentLifeState, context).equals(enc.getState())) {
									//stateInt=taxNum;
									moreStates = false;
									if (CommonConfiguration.getProperty(("encounterStateCSSClass" + cNum),
											context) != null) {
						...
						if (enc.getIndividualID() != null) {
							individuo = encprops.getProperty("of") + "&nbsp;<a href=\"../individuals.jsp?number="
									+ enc.getIndividualID() + "\">" + enc.getIndividualID() + "</a>";
						}
			%>
			...
			<p class="caption">
				<em><%=encprops.getProperty("description")%></em>
			</p>
			<table
				style="border-spacing: 10px; margin-left: -10px; border-collapse: inherit;">
				<tr valign="middle">
					<td>
						<!-- Google PLUS-ONE button --> <g:plusone size="medium"
							annotation="none"></g:plusone>
					</td>
					<td>
						<!--  Twitter TWEET THIS button --> <a
						href="https://twitter.com/share" class="twitter-share-button"
						data-count="none">Tweet</a> <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>
					</td>
					<td>
  ...
  $("#closeEditLocation").click(function() {
    $(".editFormLocation, .editTextLocation, .resultMessageDiv").hide();
				...
				<br> <a
					...
					alt="Help" border="0" align="absmiddle"></a> <em><%=encprops.getProperty("locationID")%></em><span>:
					<span id="displayLocationID"><%=enc.getLocationCode()%></span>
				</span> <br> <a
					href="<%=CommonConfiguration.getWikiLocation(context)%>country"
					target="_blank"><img src="../images/information_icon_svg.gif"
					alt="Help" border="0" align="absmiddle"></a> <em><%=encprops.getProperty("country")%></em>
				<%
				...
				%>
				<br /> <em><%=encprops.getProperty("depth")%> <%
    ...
    $("#locationInput").click(function() {
      ...
      $("#addLocation").show();
				...
				<div>
					...
					<p class="editTextLocation">
						<strong><%=encprops.getProperty("setLocation")%></strong>
					</p>
					<form name="setLocation" class="editFormLocation">
						<input name="number" type="hidden" value="<%=num%>"
							id="setLocationNumber" /> <input name="action" type="hidden"
							value="setLocation" /> <input name="encounter" type="hidden"
							value="<%=num%>" id="setLocationEncounter">
						...
						<div class="form-group row">
							...
							<div class="col-sm-3">
								<input name="Add" type="submit" id="addLocation"
									value="<%=encprops.getProperty("setLocation")%>"
									class="btn btn-sm" /> <span class="form-control-feedback"
									id="setLocationCheck">&check;</span> <span
									class="form-control-feedback" id="setLocationError">X</span>
							</div>
						</div>
					</form>
				</div>
    ...
    $("#selectCountry").click(function() {
      ...
      $("#countryFormBtn").show();
				...
				<div>
					...
					<p class="editTextLocation">
						<strong><%=encprops.getProperty("resetCountry")%></strong>
					</p>
					...
					<form name="countryForm" class="editFormLocation">
						...
						<div class="form-group row">
							<div class="col-sm-5">
								<select name="country" id="selectCountry" size="1"
									class="form-control">
									<option value=""></option>
							...
							<div class="col-sm-3">
								<input name="<%=encprops.getProperty("set")%>" type="submit"
									id="countryFormBtn" value="<%=encprops.getProperty("set")%>"
									class="btn btn-sm editFormBtn" /> <span
									class="form-control-feedback" id="countryCheck">&check;</span>
    ...
    $("#selectCode").click(function() {
      ...
      $("#setLocationBtn").show();
				...
				<div>
					...
					<p class="editTextLocation">
						<strong><%=encprops.getProperty("setLocationID")%></strong>
					</p>
					<form name="addLocCode" class="editFormLocation">
						<input name="number" type="hidden" value="<%=num%>"
							id="locationIDnumber" /> <input name="action" type="hidden"
							value="addLocCode" />
						...
						<div class="form-group row">
							...
							<div class="col-sm-3">
								<input name="Set Location ID" type="submit" id="setLocationBtn"
									value="<%=encprops.getProperty("setLocationID")%>"
									class="btn btn-sm" />
							</div>
						</div>
						<%
						...
						<div class="form-group row">
							<div class="col-sm-5">
								<select name="code" id="selectCode" class="form-control" size=="1">
									...
									<option
										value="<%=CommonConfiguration.getProperty(currentLoc, context)%>"><%=CommonConfiguration.getProperty(currentLoc, context)%></option>
									<%
										codeTaxNum++;
														} else {
															hasMoreLocs = false;
							...
							<div class="col-sm-3">
								<input name="Set Location ID" type="submit" id="setLocationBtn"
									value="<%=encprops.getProperty("setLocationID")%>"
									class="btn btn-sm" /> <span class="form-control-feedback"
									id="locationIDcheck">&check;</span> <span
									class="form-control-feedback" id="locationIDerror">X</span>
							</div>
						</div>
						<%
							}
    ...
    $("#depthInput").click(function() {
      ...
      $("#AddDepth").show();
				...
				<div>
					...
					<p class="editTextLocation">
						<strong><%=encprops.getProperty("setDepth")%></strong>
					</p>
					<form name="setencdepth" class="editFormLocation">
						<input name="lengthUnits" type="hidden" id="lengthUnits"
							value="Meters" /> <input name="number" type="hidden"
							value="<%=num%>" id="depthNumber" /> <input name="action"
							type="hidden" value="setEncounterDepth" />
						<div class="form-group row">
							...
							<div class="col-sm-3">
								<input name="AddDepth" type="submit" id="AddDepth"
									value="<%=encprops.getProperty("setDepth")%>"
				...
				%>
				<br /> <em><%=encprops.getProperty("elevation")%></em> &nbsp;
				<%
    ...
    $("#elevationInput").click(function() {
      ...
      $("#AddElev").show();
				...
				<div>
					...
					<p class="editTextLocation">
						<strong><%=encprops.getProperty("setElevation")%></strong>
					</p>
					<form name="setencelev" class="editFormLocation">
						<input name="number" type="hidden" value="<%=num%>"
							id="elevationNumber" /> <input name="action" type="hidden"
							value="setEncounterElevation" /> <input name="lengthUnits"
							type="hidden" id="lengthUnits" value="Meters" />
						<div class="form-group row">
        ...
        //bounds.extend(latLng);
         	<%//currently unused programatically
					...
					if ((encprops.getProperty("defaultMarkerColor") != null)
							&& (!encprops.getProperty("defaultMarkerColor").trim().equals(""))) {
      ...
      google.maps.event.addDomListener(window, 'load', initialize);
				...
				<div id="map_canvas"
					style="width: 510px; height: 350px; overflow: hidden;"></div>
				<%
					} else {
				%>
          ...
          $("#lat, #longitude, #map_canvas").click(function() {
            ...
            $("#latCheck, #longCheck").hide();
				...
				<div>
						...
						<div class="form-group row">
							...
							<div class="col-sm-3">
								<input name="lat" type="text" id="lat" class="form-control"
									value="<%=laty%>" /> <span class="form-control-feedback"
									id="latCheck">&check;</span>
							</div>
							<div class="col-sm-2">
							...
							<div class="col-sm-3">
								<input name="longitude" type="text" id="longitude"
									class="form-control" value="<%=longy%>" /> <span
									class="form-control-feedback" id="longCheck">&check;</span>
							</div>
						</div>
						<div class="form-group row">
							<div class="col-sm-3">
					...
					<br /> <span class="editTextLocation"><%=encprops.getProperty("gpsConverter")%></span><a
						class="editTextLocation"
				...
				<div
					style="background-color: #E8E8E8; padding-left: 10px; padding-right: 10px; padding-top: 10px; padding-bottom: 10px;">
					...
					<h2>
						<img align="absmiddle"
							src="../images/wild-me-logo-only-100-100.png" width="40px"
							height="40px" />
            ...
            $(".editForm, .editText, .resultMessageDiv").hide();
					...
					<h2>
						<img align="absmiddle"
							src="../images/wild-me-logo-only-100-100.png" width="40px"
							height="40px" />
						<%=encprops.getProperty("identity")%></h2>
					<%
						}
					%>
					...
					<p class="para">
						...
						<a
							href="../individuals.jsp?langCode=<%=langCode%>&number=<%=enc.getIndividualID()%><%if (request.getParameter("noscript") != null) {%>&noscript=true<%}%>">
					...
					<p>
						<img align="absmiddle"
							src="../images/Crystal_Clear_app_matchedBy.gif"> <span><%=encprops.getProperty("matched_by")%>:<span
					...
					%>
					<div>
						<p><%=encprops.getProperty("identified_as")%>
							<a
								href="../individuals.jsp?langCode=<%=langCode%>&number=<%=enc.getIndividualID()%><%if (request.getParameter("noscript") != null) {%>&noscript=true<%}%>"><span
								id="displayIndividualID"><%=enc.getIndividualID()%></span></a>
						</p>
						...
						<form name="setMBT" class="editForm">
							...
							<div class="form-group row" id="selectMatcher">
								...
								<div class="col-sm-5 col-xs-10">
									<select name="matchedBy" id="matchedBy" size="1"
										class="form-control">
								...
								<div class="col-sm-3">
									<input name="setMB" type="submit" id="setMB"
										...
										class="btn btn-sm editFormBtn" /> <span id="matchCheck">&check;</span>
                    ...
                    $(document).ready(function() {
                      ...
                      $("#individualAddEncounterInput, #matchType").click(function() {
						...
						<div class="editText">
							...
							<p>
								<em><small><%=encprops.getProperty("identityMessage")%></em></small>
							</p>
						</div>
						...
						<form name="add2shark" class="editForm">
							<input name="number" type="hidden" value="<%=num%>"
								id="individualAddEncounterNumber" /> <input name="action"
								type="hidden" value="add" id="individualAddEncounterAction" />
							<div class="form-group row" id="individualDiv">
								...
								<div class="col-sm-5 col-xs-10">
									<input name="individual" type="text" class="form-control"
										id="individualAddEncounterInput" /> <span
										class="form-control-feedback" id="individualCheck">&check;</span>
							...
							<div class="form-group row" id="matchedByDiv">
								...
								<div class="col-sm-5 col-xs-10">
									<select name="matchType" id="matchType" class="form-control"
										...
										<option value="Visual inspection"><%=encprops.getProperty("visualInspection")%></option>
										<option value="Pattern match" selected><%=encprops.getProperty("patternMatch")%></option>
									</select> <span class="form-control-feedback" id="matchedByCheck">&check;</span>
							...
							<div class="form-group row">
								<div class="col-sm-5 col-xs-10">
									<label><input name="noemail" type="checkbox"
										value="noemail" /> <%=encprops.getProperty("suppressEmail")%></label>
								</div>
							</div>
                  ...
                  $(document).ready(function() {
                    ...
                    $("#createSharkIndividual").click(function() {
                      ...
                      $("#createSharkBtn").show();
						...
						<div class="highlight resultMessageDiv"
							id="individualCreateErrorDiv"></div>

						<form name="createShark" class="editForm">
							<input name="number" type="hidden" value="<%=num%>"
								id="individualCreateNumber" /> <input name="action"
								type="hidden" value="create" id="individualCreateAction" />
							<div class="form-group row">
								<div class="col-sm-4">
									<p>
								...
								<div class="col-sm-5 col-xs-10" id="createSharkDiv">
									<input name="individual" type="text" id="createSharkIndividual"
										class="form-control"
										value="<%=getNextIndividualNumber(enc, myShepherd, context)%>" />
							...
							<input name="Create" type="submit" id="createSharkBtn"
						...
						<div id="setRemoveResultDiv" class="resultMessageDiv">
							<span class="highlight" id="removeErrorDiv"></span> <span
								class="successHighlight" id="removeSuccessDiv"></span>
						</div>
						<div class="editText" id="manageIdentityMessage">
							<p>
								<strong><%=encprops.getProperty("manageIdentity")%></strong>
							</p>
							<p>
								<em><small><%=encprops.getProperty("identityMessage")%></small></em>
							</p>
						</div>
						<form class="editForm" id="removeShark" name="removeShark">
							<div class="form-group row">
								<div class="col-sm-12 col-xs-10">
									...
									<input name="number" type="hidden" value="<%=num%>"
										id="individualRemoveEncounterNumber" /> <input name="action"
										type="hidden" value="remove" /> <input type="submit"
										name="Submit" value="<%=encprops.getProperty("remove")%>"
										id="individualRemoveEncounterBtn"
										class="btn btn-sm editFormBtn" />
								</div>
							</div>
						</form>
					...
					<form name="setAltID" class="editForm">
						...
						<div class="form-group row">
							...
							<div class="col-sm-5 col-xs-10" id="altIdDiv">
								<input name="alternateid" id="alternateid" type="text"
									class="form-control"
									placeholder="<%=encprops.getProperty("alternate_id")%>" /> <span
									class="form-control-feedback" id="altIdCheck">&check;</span> <span
									class="form-control-feedback" id="altIdError">X</span>
							</div>
							<div class="col-sm-4">
                ...
                $(document).ready(function() {
                  ...
                  $("#removeOccurrenceBtn").click(function(event) {
					...
					<div class="editText" id="occurrenceEditMessage">
						...
						<p class="editText">
							<em><small><%=encprops.getProperty("occurrenceMessage")%></small></em>
						</p>
					</div>
					<div id="occurrenceRemoveResultDiv" class="resultMessageDiv">
						<span class="highlight" id="occurRemoveErrorDiv"></span> <span
							class="successHighlight" id="occurRemoveSuccessDiv"></span>
					</div>
					<form class="editForm" name="removeOccurrence">
						<input name="number" type="hidden" value="<%=num%>"
							id="occurrenceRemoveEncounterNumber" /> <input name="action"
							type="hidden" value="remove" id="occurrenceRemoveEncounterAction" />
						<div class="form-group row">
							<div class="col-sm-12">
				...
				<table>
					<tr>
						<td width="560px"
							...
							%>
							<h2>
								<img align="absmiddle" width="40px" height="40px"
...
$(document).ready(function() {
 ...
 %>
							<h2>
								<img align="absmiddle" width="40px" height="40px"
									style="border-style: none;" src="../images/workflow_icon.gif" />
								<%=encprops.getProperty("metadata")%></h2> <%
 	}
 %>
 ...
 %>
							<p class="para">
								<%=encprops.getProperty("workflowState")%>
								<span id="displayWork"><%=state%></span>
 ...
 %> <script type="text/javascript">
                      ...
                      $("#selectState").click(function() {
                        ...
                        $("#editWork").show();
							...
							<div>
								...
								<p class="editTextMeta">
									<strong><%=encprops.getProperty("setWorkflowState")%></strong>
								</p>
									...
									<div class="form-group row">
										<div class="col-sm-5">
											<select name="state" id="selectState" class="form-control"
												...
												%>
												<option
													value="<%=CommonConfiguration.getProperty(currentLifeState, context)%>"><%=CommonConfiguration.getProperty(currentLifeState, context)%></option>
												<%
													stateTaxNum++;
																} else {
																	hasMoreStates = false;
										...
										<div class="col-sm-3">
											<input name="<%=encprops.getProperty("set")%>" type="submit"
												id="editWork" value="<%=encprops.getProperty("set")%>"
												class="btn btn-sm editFormBtn" /> <span
												class="form-control-feedback" id="workCheck">&check;</span>
 	...
 	// }
 %> <!-- END WORKFLOW ATTRIBUTE --> <!-- START USER ATTRIBUTE -->
							<%
 	...
 	User thisUser = aUserShepherd.getUser(username);
 						...
 						if (thisUser.getUserImage() != null) {
 							profilePhotoURL = "/" + CommonConfiguration.getDataDirectoryName("context0")
 									+ "/users/" + thisUser.getUsername() + "/"
										...
										<div>
											<div class="row">
												<div class="col-sm-6"
													...
													<img
														src="../cust/mantamatcher/img/individual_placeholder_image.jpg"
														class="lazyload" align="top"
														data-src="<%=profilePhotoURL%>"
												...
												<div class="col-sm-6"
													...
													<p>
														<strong><%=encprops.getProperty("affiliation")%></strong>
														<%=thisUser.getAffiliation()%></p>
													<%
														}
													...
													%>
													<p>
														<strong><%=encprops.getProperty("researchProject")%></strong>
														<%=thisUser.getUserProject()%></p>
													<%
														}
													...
													%>
													<p>
														<strong><%=encprops.getProperty("webSite")%></strong> <a
															style="font-weight: normal; color: blue" class="ecocean"
															href="<%=thisUser.getUserURL()%>"><%=thisUser.getUserURL()%></a>
													</p>
													<%
														}
 ...
 %> <!-- start set username -->
										<div>
											...
											<p class="editTextMeta">
												<strong><%=encprops.getProperty("assignUser")%></strong>
											</p>

											<form name="asetSubmID" action="../EncounterSetSubmitterID"
												method="post" class="editFormMeta">
												<input name="number" type="hidden" value="<%=num%>"
													id="assignNumber" />
												<div class="form-group row">
													<div class="col-sm-5">
        ...
        $("#tapirError").html(response.responseText);
							...
							<div>
								<div id="tapirResultDiv" class="resultMessageDiv">
									<span id="tapirSuccess" class="successHighlight"></span> <span
										id="tapirError" class="highlight"></span>
								</div>
								<form name="setTapirLink" class="editFormMeta">
									<input name="action" type="hidden" id="tapirAction"
										value="tapirLinkExpose" /> <input name="number" type="hidden"
										value="<%=num%>" id="tapirNumber" />
									<%
									...
									%>
									<label>TapirLink:</label>&nbsp; <input
										style="width: 40px; height: 40px;" align="absmiddle"
										name="approve" type="image"
										src="../images/<%=tapirCheckIcon%>" id="tapirApprove"
										value="<%=encprops.getProperty("change")%>" /> &nbsp; <a
										href="<%=CommonConfiguration.getWikiLocation(context)%>tapirlink"
										target="_blank"><img
										src="../images/information_icon_svg.gif" alt="Help" border="0"
										align="absmiddle" /></a>
								</form>
							</div> <!-- END TAPIRLINK DISPLAY AND SETTER --> <%
 	}
 %> <!-- START AUTOCOMMENTS --> <%
 	if (request.getUserPrincipal() != null) {
 %> <!-- start autocomments --> <script type="text/javascript">
  $(document).ready(function() {
    $("#manualAdd").click(function(event) {
      ...
      .fail(function(response) {
        ...
        $("#autoCommentErrorDiv").html(response.responseText);
							...
							<div>
								<p>
									<strong><%=encprops.getProperty("auto_comments")%></strong>
								</p>
								...
								<form name="addComments" class="editFormMeta">
									<input name="user" type="hidden"
										value="<%=request.getRemoteUser()%>" id="autoUser" /> <input
										name="number" type="hidden"
										value="<%=enc.getEncounterNumber()%>" id="autoNumber" /> <input
										name="action" type="hidden" value="enc_comments"
										id="autoAction" />

									<textarea name="autocomments" cols="50" id="autoComments"
										class="form-control"></textarea>
									<input name="Submit" type="submit"
										value="<%=encprops.getProperty("add_comment")%>"
										class="btn btn-sm" id="manualAdd" />

								</form>
							</div> <%
 	}
 %> <!-- END AUTOCOMMENTS --> <!-- START DELETE ENCOUNTER FORM --> <%
 	if (isOwner) {
 %>
							<table width="100%" border="0" cellpadding="1">
								<tr>
									<td height="30" class="para">
										<form
											onsubmit="return confirm('<%=encprops.getProperty("sureDelete")%>');"
											...
											<input align="absmiddle" name="approve" type="submit"
												class="btn btn-sm btn-block deleteEncounterBtn"
												id="deleteButton"
												value="<%=encprops.getProperty("deleteEncounter")%>" />
										</form>
									</td>
								</tr>
							</table> <!-- END DELETE ENCOUNTER FORM --> <%
 	}
 %>
...
$(document).ready(function() {
  ...
  $("#closeEditMeasure").click(function() {
    $(".editFormMeasure, .editTextMeasure, .resultMessageDiv").hide();
					...
					<table>
						<tr>
							<th class="measurement"><span id="displayMtype"><%=encprops.getProperty("type")%></span>
							</th>
							<th class="measurement"><span id="displayMsize"><%=encprops.getProperty("size")%></span>
							</th>
							<th class="measurement"><span id="displayMunits"><%=encprops.getProperty("units")%></span>
							</th>
							<c:if test="${!empty samplingProtocols}">
								<th class="measurement"><display id="displayMsample"><%=encprops.getProperty("samplingProtocol")%></span></th>
							</c:if>
						</tr>
						<c:forEach var="item" items="${measurements}">
							<%
								MeasurementDesc measurementDesc = (MeasurementDesc) pageContext.getAttribute("item");
												...
												Measurement event = myShepherd.getMeasurementOfTypeForEncounter(measurementDesc.getType(),
														num);
												if (event != null) {
													pageContext.setAttribute("measurementValue", event.getValue());
													pageContext.setAttribute("samplingProtocol", Util
															.getLocalizedSamplingProtocol(event.getSamplingProtocol(), langCode, context));
												} else {
							...
							%>
							<tr>
								<td class="measurement"><c:out value="${item.label}" /></td>
								<td class="measurement"><c:out value="${measurementValue}" />
								</td>
								<td class="measurement"><c:out value="${item.unitsLabel}" />
								</td>
								<td class="measurement"><c:out value="${samplingProtocol}" />
								</td>
							</tr>
						</c:forEach>
					</table>
					...
					<div>
						<p class="editTextMeasure">
							<strong><%=encprops.getProperty("setMeasurements")%></strong>
						</p>
						...
						<table cellpadding="1" cellspacing="0" bordercolor="#FFFFFF"
							class="editFormMeasure">
							<form name="setMeasurements" class="editFormMeasure"
								id="measurementForm" action="../EncounterSetMeasurements"
								method="post">
								...
								<c:forEach items="${items}" var="item">
									<%
										MeasurementDesc measurementDesc = (MeasurementDesc) pageContext.getAttribute("item");
														...
														if (measurement == null) {
															measurement = new Measurement(enc.getEventID(), measurementDesc.getType(), null,
																	measurementDesc.getUnits(), null);
														}
									...
									%>
									<tr>
										<td class="form_label"><c:out value="${item.label}" /> <input
											type="hidden" name="measurement${index}(id)"
											value="${measurementEvent.dataCollectionEventID}" /></td>
										<td><input name="measurement${index}(value)"
											value="${measurementEvent.value}"
											id="measurementEvent${index}" /> <input type="hidden"
											name="measurement${index}(type)" value="${item.type}" /> <input
											type="hidden" name="measurement${index}(units)"
											value="${item.unitsLabel}" /> <c:out
												value="(${item.unitsLabel})" /> <select
											name="measurement${index}(samplingProtocol)"
											id="selectMeasurement">
												<c:forEach items="${optionDescs}" var="optionDesc">
													<c:choose>
														<c:when
															test="${measurementEvent.samplingProtocol eq optionDesc.name}">
															<option value="${optionDesc.name}" selected="selected"><c:out
								...
								<tr>
									<td><input name="${set}" type="submit" value="${set}"
										class="btn btn-sm editFormBtn" id="addMeasurements" /></td>
								</tr>
						</table>
						</form>
					</div>
					...
					<tr>
						<td width="560px"
							...
							<c:if test="${showMetalTags}">
								...
								<h2>
									<img align="absmiddle"
										src="../images/Crystal_Clear_app_starthere.png" width="40px"
										height="40px" />
    ...
    $(".editFormTracking, .editTextTracking, .resultMessageDiv").hide();
								...
								<h2>
									<img align="absmiddle"
										src="../images/Crystal_Clear_app_starthere.png" width="40px"
										height="40px" />
									<%=encprops.getProperty("tracking")%></h2>
								...
								<div>
									<p class="editTextTracking">
										<strong><%=encprops.getProperty("resetMetalTags")%></strong>
									</p>
									<%
										pageContext.setAttribute("metalTagDescs", Util.findMetalTagDescs(langCode, context));
									%>

									<form name="setMetalTags" class="editFormTracking"
										id="tagsForm" action="../EncounterSetTags" method="post">
										<input type="hidden" name="tagType" value="metalTags"
											id="metalTagsType" /> <input type="hidden" name="encounter"
											value="${num}" id="metalTagsEncounter" />
								...
								%>
							</c:if> <c:if test="${showAcousticTag}">
								<%
								...
								%>
								<p class="para">
									<em><c:out value="${acousticTagTitle}"></c:out></em>
									<c:if test="${editable}">
&nbsp;
<%
...
%>
								...
								<table>
									<tr>
										<td><%=encprops.getProperty("serialNumber")%></td>
										<td><c:out
												value="${empty acousticTag ? '' : acousticTag.serialNumber}" /></td>
									</tr>
									<tr>
										<td>ID:</td>
										<td><c:out
												value="${empty acousticTag ? '' : acousticTag.idNumber}" /></td>
									</tr>
								</table>
								</p>
								...
								<div>
									<p class="editTextTracking">
										<strong><%=encprops.getProperty("resetAcousticTag")%></strong>
									</p>
										...
										<tr>
											<td>
												<form name="setAcousticTag" class="editFormTracking"
													id="acousticTagsForm" action="../EncounterSetTags"
													method="post">
													<input type="hidden" name="encounter" value="${num}"
														id="acousticTagEncounter" /> <input type="hidden"
														name="tagType" value="acousticTag" id="acousticTagType" />
													<input type="hidden" name="id" value="${acousticTag.id}" />
													<table>
														<tr>
															<td class="formLabel"><%=encprops.getProperty("serialNumber")%></td>
														</tr>
														<tr>
															<td><input name="acousticTagSerial"
																value="${acousticTag.serialNumber}" class="form-control"
																id="acousticTagInput" /></td>
														</tr>
														<tr>
														...
														<tr>
															<td><input name="acousticTagId"
																value="${acousticTag.idNumber}" id="acousticTagId"
																class="form-control" /></td>
														</tr>
														<tr>
															<td><input name="${set}" type="submit"
																value="${set}" class="btn btn-sm editFormBtn"
							...
							</c:if> <c:if test="${showSatelliteTag}">
								<%
								...
								%>
								<p class="para">
									<em><c:out value="${satelliteTagTitle}"></c:out></em>
									<%
								...
								<table>
									<tr>
										<td><%=encprops.getProperty("name")%></td>
										<td><c:out value="${satelliteTag.name}" /></td>
									</tr>
									<tr>
										<td><%=encprops.getProperty("serialNumber")%></td>
										<td><c:out
												value="${empty satelliteTag ? '' : satelliteTag.serialNumber}" /></td>
									</tr>
									<tr>
										<td>Argos PTT:</td>
										<td><c:out
								...
								<div>
									<p class="editTextTracking">
										<strong><%=encprops.getProperty("resetSatelliteTag")%></strong>
									</p>
									...
									%>
									<form name="setSatelliteTag" class="editFormTracking"
										action="../EncounterSetTags" method="post">
										<input type="hidden" name="tagType" value="satelliteTag"
											id="satelliteTagType" /> <input type="hidden"
											name="encounter" value="${num}" id="satelliteTagEncounter" />
										<input type="hidden" name="id" value="${satelliteTag.id}"
											id="satelliteTagId" />
										<table cellpadding="1" cellspacing="0">

											<tr>
												<td class="formLabel"><%=encprops.getProperty("name")%></td>
											</tr>
											<tr>
												<td><select name="satelliteTagName"
													class="form-control" size="1">
														<c:forEach items="${satelliteTagNames}"
															var="satelliteTagName">
															<c:choose>
											...
											<tr>
												<td><input name="satelliteTagSerial"
													class="form-control" value="${satelliteTag.serialNumber}"
				...
				<div id="dialogOccurrence"
					title="<%=encprops.getProperty("assignOccurrence")%>"
 ...
 %>
							...
							<p>
								...
								<br /> <em><%=encprops.getProperty("verbatimEventDate")%></em>:
								<%
 ...
 %>
								...
								<c:if test="${showReleaseDate}">
									<br />
									<em><%=encprops.getProperty("releaseDate")%></em>: <span
										id="displayReleaseDate"></span>
									<fmt:formatDate value="${enc.releaseDate}" pattern="yyyy-MM-dd" />
							</p> </c:if> <br> <!-- start date --> <script type="text/javascript">
      $(document).ready(function() {
        ...
        $("#datepickerField").click(function() {
          ...
          $("#addResetDate").show();
							...
							<div>
								...
								<p class="editTextDate">
									<strong><%=encprops.getProperty("resetEncounterDate")%></strong>
								</p>
								<form name="setencdate" class="editFormDate">
									<input name="number" type="hidden" value="<%=num%>"
										id="resetDateNumber" /> <input name="action" type="hidden"
										value="changeEncounterDate" />
									<div id="datepicker" class="editFormDate"></div>
									<div class="form-group row editFormDate">
										<div class="col-sm-5">
											<label><%=encprops.getProperty("setDate")%>
												(yyyy-MM-dd HH:mm)</label>
											<p>
												<font size="-1"><%=encprops.getProperty("leaveBlank")%></font>
											</p>
										</div>
										<div class="col-sm-5" id="resetDateDiv">
											<input type="text" id="datepickerField" name="datepicker"
												class="form-control" /> <span class="form-control-feedback"
												id="resetDateCheck">&check;</span> <span
												class="form-control-feedback" id="resetDateError">X</span>
										</div>
										<div class="col-sm-2">
							...
							</div> <!-- start releaseDate --> <script type="text/javascript">
      $(document).ready(function() {
        ...
        $("#releasedatepickerField").click(function() {
          ...
          $("#AddDate").show();
    ...
    </script> <c:if test="${showReleaseDate}">
								<div>
									<div class="highlight resultMessageDiv" id="releaseErrorDiv"></div>

									<p class="editTextDate">
										<strong><%=encprops.getProperty("setReleaseDate")%></strong>
									</p>
									<form name="setReleaseDate" class="editFormDate">
										<input type="hidden" name="encounter" value="${num}"
											id="releaseDateEncounter" />
										...
										<div class="form-group row editFormDate">
											...
											<div class="col-sm-5" id="releaseDiv">
												<input type="text" id="releasedatepickerField"
													name="releasedatepicker" class="form-control" /> <span
													class="form-control-feedback" id="releaseCheck">&check;</span>
							...
							</c:if> <!-- end releaseDate --> <br> <!-- start verbatim event date -->
							<script type="text/javascript">
      $(document).ready(function() {
        ...
        $("#verbatimEventDateInput").click(function() {
          ...
          $("#setVerbatimEventDateBtn").show();
							...
							<div>
								...
								<p class="editTextDate">
									<strong><%=encprops.getProperty("setVerbatimEventDate")%></strong>
								</p>
								<form name="setVerbatimEventDate" class="editFormDate">
									<input name="encounter" type="hidden" value="<%=num%>"
										id="verbatimDateEncounter">
									<div class="form-group row">
										<div class="col-sm-4">
										...
										<div class="col-sm-5 col-xs-10" id="verbatimDiv">
											<input name="verbatimEventDate" type="text"
												class="form-control" id="verbatimEventDateInput"> <span
												class="form-control-feedback" id="verbatimCheck">&check;</span>
				...
				%>
				<h2>
					<img align="absmiddle" src="../images/Note-Book-icon.png"
    ...
    $("#closeEditObservation").click(function() {
      $(".editFormObservation, .editTextObservation, .resultMessageDiv").hide();
				...
				<h2>
					<img align="absmiddle" src="../images/Note-Book-icon.png"
						width="40px" height="40px" />
					<%=encprops.getProperty("observationAttributes")%></h2>
				<%
					}
				%>
				...
				<p class="para">
					<img align="absmiddle" src="../images/taxontree.gif">
        ...
        $("#genusSpecies").click(function() {
          ...
          $("#taxBtn").show();
				...
				<div>
					...
					<p class="editTextObservation">
						<strong><%=encprops.getProperty("resetTaxonomy")%></strong>
					</p>
						...
						<div class="form-group row">
							<div class="col-sm-5">
								<select name="genusSpecies" id="genusSpecies"
									...
									<option
										value="<%=CommonConfiguration.getProperty(currentGenuSpecies, context)%>"><%=CommonConfiguration.getProperty(currentGenuSpecies, context).replaceAll("_",
										" ")%></option>
									<%
										genusTaxNum++;
														} else {
							...
							<div class="col-sm-3">
								<input name="<%=encprops.getProperty("set")%>" type="submit"
									id="taxBtn" value="<%=encprops.getProperty("set")%>"
									class="btn btn-sm editFormBtn" /> <span
									class="form-control-feedback" id="taxCheck">&check;</span> <span
									class="form-control-feedback" id="taxError">X</span>
							</div>
						</div>
					</form>
				</div>
      ...
      $("#genusSpecies").click(function() {
        ...
        $("#addStatus").show();
				...
				<div>
					...
					<p class="editTextObservation">
						<strong><%=encprops.getProperty("resetStatus")%></strong>
					</p>
						...
						<div class="form-group row">
							...
							<div class="col-sm-3">
								<input name="Add" type="submit" id="addStatus"
									value="<%=encprops.getProperty("resetStatus")%>"
									class="btn btn-sm" /> <span class="form-control-feedback"
									id="statusCheck">&check;</span> <span
									class="form-control-feedback" id="statusError">X</span>
							</div>
						</div>
					</form>
				</div>
				...
				%>
				<p class="para"><%=encprops.getProperty("sex")%>&nbsp;<span
						id="displaySex"><%=sex%></span>
					<%
    ...
    $("#selectSex").click(function() {
      ...
      $("#addSex").show();
				...
				<div>
					...
					<p class="editTextObservation">
						<strong><%=encprops.getProperty("resetSex")%></strong>
					</p>
						...
						<div class="form-group row">
							...
							<div class="col-sm-3">
								<input name="Add" type="submit" id="addSex"
									...
									class="btn btn-sm editFormBtn" /> <span
									class="form-control-feedback" id="sexCheck">&check;</span> <span
									class="form-control-feedback" id="sexError">X</span>
							</div>
						</div>
					</form>
				</div>
     ...
     $("#scarInput").click(function() {
       ...
       $("#addScar").show();
				...
				<div>
					...
					<p class="editTextObservation">
						<strong><%=encprops.getProperty("editScarring")%></strong>
					</p>
					<form name="setencsize" class="editFormObservation">
						<input name="number" type="hidden" value="<%=num%>"
							id="scarNumber" /> <input name="action" type="hidden"
							value="setScarring" id="scarAction" />
						...
						<div class="form-group row">
							...
							<div class="col-sm-3">
								<input name="Add" type="submit" id="addScar"
									value="<%=encprops.getProperty("resetScarring")%>"
									class="btn btn-sm" /> <span class="form-control-feedback"
									id="scarCheck">&check;</span> <span
									class="form-control-feedback" id="scarError">X</span>
							</div>
						</div>
					</form>
				</div>
        ...
        $("#behaviorInput").click(function() {
          ...
          $("#editBehavior").show();
				...
				<div>
					...
					<p class="editTextObservation">
						<strong><%=encprops.getProperty("editBehaviorComments")%></strong>
					</p>
					...
					<form name="setBehaviorComments" class="editFormObservation">
						<input name="number" type="hidden" value="<%=num%>"
							id="behaviorNumber" /> <input name="action" type="hidden"
							value="editBehavior" id="behaviorAction" />

						<div class="form-group row">
							<div class="col-sm-5">
								<textarea name="behaviorComment" class="form-control"
									id="behaviorInput">
            <%
          ...
          %>
							...
							<div class="col-sm-3">
								<input name="EditBeh" type="submit" id="editBehavior"
									value="<%=encprops.getProperty("submitEdit")%>"
									class="btn btn-sm" /> <span class="form-control-feedback"
									id="behaviorCheck">&check;</span> <span
									class="form-control-feedback" id="behaviorError">X</span>
							</div>
						</div>
					</form>
				</div>
    ...
    $("#colorCode").click(function() {
      ...
      $("#editPattern").show();
				...
				<div>
					...
					<p class="editTextObservation">
						<strong><%=encprops.getProperty("editPatterningCode")%></strong>
					</p>
					<span class="editTextObservation"><em><font size="-1"><%=encprops.getProperty("leaveBlank")%></font></em></span>
					...
					<form name="setPatterningCode" class="editFormObservation">
						...
						<div class="form-group row">
							<div class="col-sm-5">
								...
								%>
								<input name="patterningCode" type="text" class="form-control"
									id="colorCode" />
								<%
									} else {
													//iterate and find the locationID options
							...
							<div class="col-sm-3">
								<input name="EditPC" type="submit" id="editPattern"
									value="<%=encprops.getProperty("submitEdit")%>"
									class="btn btn-sm" /> <span class="form-control-feedback"
									id="patternCheck">&check;</span> <span
									class="form-control-feedback" id="patternError">X</span>
							</div>
						</div>
					</form>
				</div>
    ...
    $("#lifeStage").click(function() {
      ...
      $("#addLife").show();
				...
				<div>
					...
					<p class="editTextObservation">
						<strong><%=encprops.getProperty("resetLifeStage")%></strong>
					</p>
						...
						<div class="form-group row">
							<div class="col-sm-5">
								<select name="lifeStage" id="lifeStage" class="form-control"
									...
									<option
										value="<%=CommonConfiguration.getProperty(currentLifeStage, context)%>"><%=CommonConfiguration.getProperty(currentLifeStage, context)%></option>
									<%
										lifeTaxNum++;
														} else {
															hasMoreStages = false;
							...
							<div class="col-sm-3">
								<input name="<%=encprops.getProperty("set")%>" type="submit"
									id="addLife" value="<%=encprops.getProperty("set")%>"
									class="btn btn-sm editFormBtn" /> <span
									class="form-control-feedback" id="lifeCheck">&check;</span> <span
									class="form-control-feedback" id="lifeError">X</span>
							</div>
						</div>
					</form>
				</div>
    ...
    $("#commentInput").click(function() {
      ...
      $("#editComment").show();
				...
				<div>
					...
					<p class="editTextObservation">
						<strong><%=encprops.getProperty("editSubmittedComments")%></strong>
					</p>
					<form name="setComments" class="editFormObservation">
						<input name="number" type="hidden" value="<%=num%>"
							id="commentNumber" /> <input name="action" type="hidden"
							value="editComments" id="commentAction" />
						<div class="form-group row">
							<div class="col-sm-5">
							...
							<div class="col-sm-3">
								<input name="EditComm" type="submit" id="editComment"
									value="<%=encprops.getProperty("submitEdit")%>"
									class="btn btn-sm" /> <span class="form-control-feedback"
									id="commentCheck">&check;</span> <span
									class="form-control-feedback" id="commentError">X</span>
							</div>
						</div>
					</form>
				</div>
  ...
  $("#closeEditDynamic").click(function() {
    $(".editFormDynamic, .editTextDynamic, .resultMessageDiv").hide();
				...
				%>
							...
							<div class="currentSurvey">
								<strong><%=encprops.getProperty("surveyTrackID")%></strong>
								<strong><%=thisTrack%></strong>
							</div>					
				  <%
						} else { 
				  %>		
						...
						<form name="addSurveyToEncounter" class="editFormSurvey">
							...
							<div class="form-group row">
								...
								<div class="col-sm-8" id="addDiv">
									...
									<input name="surveyTrackID" id="surveyTrackID" type="text" class="form-control" placeholder="<%=encprops.getProperty("surveyTrackID")%>" />
								</div>
								<div class="col-sm-8">
									<input name="Add" type="submit" id="addEncounter" value="<%=encprops.getProperty("add")%>" class="btn btn-sm editSurveyFormBtn" />
...
<div class="container">
 ...
 %>
  ...
  $(document).ready(function() {
	...
	<p class="para">
		<img align="absmiddle" src="../images/microscope.gif" /> <strong><%=encprops.getProperty("tissueSamples")%></strong>
	</p>
	<p class="para">
		<a class="addBioSample toggleBtn" class="launchPopup"><img
			align="absmiddle" width="24px" style="border-style: none;"
			src="../images/Crystal_Clear_action_edit_add.png" /></a>&nbsp;<a
			class="addBioSample toggleBtn" class="launchPopup"><%=encprops.getProperty("addTissueSample")%></a>
	</p>
	...
	%>
	<div id="dialogSample"
		title="<%=encprops.getProperty("setTissueSample")%>"
		style="display: none">
		...
		<form id="setTissueSample" action="../EncounterSetTissueSample"
			...
			<table cellspacing="2" bordercolor="#FFFFFF">
				<tr>
					...
					<td><%=encprops.getProperty("sampleID")%> (<%=encprops.getProperty("required")%>)</td>
					<td>
						<%
 ...
 %>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("preservationMethod")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("storageLabID")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("samplingProtocol")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("samplingEffort")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("fieldNumber")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("fieldNotes")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("eventRemarks")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("institutionID")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("collectionCode")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("datasetID")%></td>
				...
				<tr>
					<td>
						...
						%> <%=encprops.getProperty("datasetName")%></td>
...
dlgSample.dialog("open");
	...
	<table width="100%"
		class="table table-bordered table-striped tissueSampleTable">
		<tr>
			<th><%=encprops.getProperty("sampleID")%></th>
			<th><%=encprops.getProperty("values")%></th>
			<th><%=encprops.getProperty("analyses")%></th>
			...
			<td><table>
					...
					<tr>
						<td style="border-style: none;"><strong><span
								class="caption"><%=encprops.getProperty("haplotype")%></strong></span></strong>:
							<span class="caption"><%=mito.getHaplotype()%> <%
 	if (!mito.getSuperHTMLString().equals("")) {
 %> <em> <br /><%=encprops.getProperty("analysisID")%>: <%=mito.getAnalysisID()%>
    ...
    $("#haplo<%=mito.getAnalysisID()%>").click(function() {
      $("#dialogHaplotype<%=mito.getAnalysisID()%>").toggle();
							...
							<div id="dialogHaplotype<%=mito.getAnalysisID()%>"
								title="<%=encprops.getProperty("setHaplotype")%>"
								style="display: none">
								<form id="setHaplotype<%=mito.getAnalysisID()%>"
									...
									<table cellpadding="1" cellspacing="0" bordercolor="#FFFFFF">
										...
										<tr>
											<td><%=encprops.getProperty("analysisID")%> (<%=encprops.getProperty("required")%>)</td>
											<td>
												<%
										...
										<tr>
											...
											%>
											<td><%=encprops.getProperty("haplotype")%> (<%=encprops.getProperty("required")%>)</td>
										...
										<tr>
											...
											%>
											<td><%=encprops.getProperty("processingLabTaskID")%></td>
										...
										<tr>
											<td>
												...
												%> <%=encprops.getProperty("processingLabName")%></td>
											<td><input name="processingLabName type="
												text" size="20" maxlength="100"
												value="<%=processingLabName%>" /></td>
										</tr>
										...
										<tr>
											<td>
												...
												%> <%=encprops.getProperty("processingLabContactName")%></td>
											<td><input name="processingLabContactName type="
												text" size="20" maxlength="100"
										...
										<tr>
											<td>
												...
												%> <%=encprops.getProperty("processingLabContactDetails")%></td>
											<td><input name="processingLabContactDetails type="
												text" size="20" maxlength="100"
										...
										<tr>
											<td colspan="2"><input name="sampleID" type="hidden"
												...
												name="action" type="hidden" value="setHaplotype" /> <input
												name="EditTissueSample" type="submit" id="EditTissueSample"
												value="<%=encprops.getProperty("set")%>" /></td>
										</tr>
									</table>
								</form>
...
$("a#haplo<%=mito.getAnalysisID() %>").click(function() {
 ...
 %></td>
					...
					<tr>
						<td style="border-style: none;"><strong><span
								class="caption"><%=encprops.getProperty("geneticSex")%></strong></span></strong>:
 ...
 %> </span></td>
						<td style="border-style: none;"><a
							id="setSex<%=thisSample.getSampleID()%>" class="launchPopup"><img
								width="20px" height="20px" style="border-style: none;"
								src="../images/Crystal_Clear_action_edit.png" /> </a> <%
 	if (isOwner && CommonConfiguration.isCatalogEditable(context)) {
 %> <!-- start genetic sex popup --> <script type="text/javascript">
  $("#setSex<%=thisSample.getSampleID()%>").click(function() {
    $("#dialogSexSet<%=thisSample.getSampleID().replaceAll("[-+.^:,]", "")%>").toggle();
							...
							<div
								id="dialogSexSet<%=thisSample.getSampleID().replaceAll("[-+.^:,]", "")%>"
								title="<%=encprops.getProperty("setSexAnalysis")%>"
								style="display: none">

								<form name="setSexAnalysis"
									action="../TissueSampleSetSexAnalysis" method="post">

									<table cellpadding="1" cellspacing="0" bordercolor="#FFFFFF">
										<tr>
											<td><%=encprops.getProperty("analysisID")%> (<%=encprops.getProperty("required")%>)<br />
												<%
										...
										<tr>
											...
											<td><input name="sex" type="text" size="20"
												maxlength="100" value="<%=haplotypeString%>" /></td>
										</tr>
										...
										<tr>
											<td><input name="sampleID" type="hidden"
												value="<%=thisSample.getSampleID()%>" /> <input
												name="number" type="hidden" value="<%=num%>" /> <input
												name="action" type="hidden" value="setSexAnalysis" /> <input
												name="EditTissueSampleSexAnalysis" type="submit"
 ...
 %></td>
					...
					<tr>
						<td style="border-style: none;">
							<p>
								<span class="caption"><strong><%=encprops.getProperty("msMarkers")%></strong></span>
								<%
									if ((enc.getIndividualID() != null) && (request.getUserPrincipal() != null)) {
								%>
								<a
									href="../individualSearch.jsp?individualDistanceSearch=<%=enc.getIndividualID()%>"><img
								...
								%>
							</p> <span class="caption"><%=mito.getAllelesHTMLString()%> <%
 	if (!mito.getSuperHTMLString().equals("")) {
 %> <em> <br /><%=encprops.getProperty("analysisID")%>: <%=mito.getAnalysisID()%>
									<br /><%=mito.getSuperHTMLString()%>
							</em> <%
 ...
 %> </span>
						...
						<td style="border-style: none;"><a class="launchPopup"
							id="msmarkersSet<%=thisSample.getSampleID()%>"> <img
								width="20px" height="20px" style="border-style: none;"
 	...
 	if (isOwner && CommonConfiguration.isCatalogEditable(context)) {
 %> <!-- start ms marker popup --> <script type="text/javascript">
  $(document).ready(function() {
    $("#msmarkersSet<%=thisSample.getSampleID()%>").click(function() {
      $("#dialogMSMarkersSet").toggle();
							...
							<div
								id="dialogMSMarkersSet<%=thisSample.getSampleID().replaceAll("[-+.^:,]", "")%>"
								...
								<form id="setMsMarkers"
									...
									<table cellpadding="1" cellspacing="0" bordercolor="#FFFFFF">
										<tr>
											<td align="left" valign="top"><%=encprops.getProperty("analysisID")%>
												(<%=encprops.getProperty("required")%>)</td>
											<td>
												<%
...
var dlgMSMarkersSet<%=thisSample.getSampleID().replaceAll("[-+.^:,]","")%> = $("#dialogMSMarkersSet<%=thisSample.getSampleID().replaceAll("[-+.^:,]","")%>").dialog({
 ...
 %>
										...
										<tr>
											<td><input name="sampleID" type="hidden"
												value="<%=thisSample.getSampleID()%>" /> <input
												name="encounter" type="hidden" value="<%=num%>" /> <input
				...
				<p>
					<span class="caption"> <a
						class="addHaplotype<%=thisSample.getSampleID()%> toggleBtn">
							<img align="absmiddle" width="20px" height="20px"
						...
						<table cellpadding="1" cellspacing="0">
							...
							<tr>
								<td colspan="2"><input name="sampleID" type="hidden"
									...
									type="hidden" value="<%=num%>" /> <input name="action"
				...
				<p>
					<span class="caption"> <a
						class="msmarkersAdd<%=thisSample.getSampleID()%> toggleBtn"> <img
							align="absmiddle" width="20px" height="20px"
							style="border-style: none;"
							src="../images/Crystal_Clear_action_edit_add.png" />
						...
						<table cellpadding="1" cellspacing="0" bordercolor="#FFFFFF">
							...
							<tr>
								<td><input name="sampleID" type="hidden"
									value="<%=thisSample.getSampleID()%>" /> <input name="number"
									type="hidden" value="<%=num%>" /> <input name="action"
									type="hidden" value="setSexAnalysis" /> <input
									name="EditTissueSampleSexAnalysis" type="submit"
									id="EditTissueSampleSexAnalysis"
									value="<%=encprops.getProperty("set")%>" /></td>
							</tr>
				...
				<p>
					<span class="caption"> <a
						class="toggleBtn addBioMeasure<%=thisSample.getSampleID()%>">
							...
							style="border-style: none;"
							src="../images/Crystal_Clear_action_edit_add.png" />
					</a> <a class="toggleBtn addBioMeasure<%=thisSample.getSampleID()%>"><%=encprops.getProperty("addBiologicalMeasurement")%></a>
					</span>
				</p> <%
 	if (isOwner && CommonConfiguration.isCatalogEditable(context)) {
...
<link rel='stylesheet prefetch'
	href='<%=pswipedir%>/default-skin/default-skin.css'>
