...
if (idString != null) {
    ...
    JSONObject tripData = OnWaterApp.getTrip(id);

    String flavor = tripData.optString("_tripFlavor", "__FAIL__");
    if (flavor.equals("ci")) {
        Survey surv = OnWaterApp.ciToSurvey(tripData, myShepherd);
        ...
        List<Occurrence> occs = OnWaterApp.waToOccurrences(tripData, myShepherd);
        for (Occurrence occ : occs) {
...
long sinceWA = (long)OnWaterApp.waGetLastSync(context);
long sinceCI = (long)OnWaterApp.ciGetLastSync(context);
...
try {
    waTripList = OnWaterApp.waGetTripList(context);
    ciTripList = OnWaterApp.ciGetTripList(context);
} catch (Exception ex) {
...
OnWaterApp.waSetLastSync(context, resetTime);
OnWaterApp.ciSetLastSync(context, resetTime);
...
Object foo = OnWaterApp.ciToSurveyTrack(jin);
*/
