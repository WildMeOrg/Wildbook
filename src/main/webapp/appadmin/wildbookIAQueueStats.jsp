...
try{
	...
	<ul>
		...
		//ISOLATED: the fastlane is optional, so a missing/uncreatable spool must not abort
		//the rest of this page -- that would hide the very lane depths it exists to show.
		long fastlaneQueueSize=0L;
		boolean fastlaneAvailable=true;
		try {
			org.ecocean.queue.Queue fastlaneQueue = QueueUtil.getBest(context, "iafastlane");
			if (fastlaneQueue==null) fastlaneAvailable=false;
			else fastlaneQueueSize=fastlaneQueue.getQueueSize();
		} catch (Exception fex) {
			fastlaneAvailable=false;
			System.out.println("wildbookIAQueueStats.jsp: fastlane queue unavailable: "+fex);
		}
		%>
			<ul>
				...
				<li><% if (!fastlaneAvailable) { %>Fastlane queue UNAVAILABLE - interactive matches are falling back to the detection queue.<% } else { %>There are currently <%=fastlaneQueueSize %> interactive match jobs waiting in the fastlane queue<%= org.ecocean.servlet.IAGateway.isFastlaneReady() ? "" : " (consumer NOT registered - interactive matches are falling back to the detection queue)" %>.<% } %></li>
			</ul>
		</li>
	</ul>
