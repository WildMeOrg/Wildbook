...
function processTask(task) {
    ...
    console.log('processTask is called on task '+JSON.stringify(task));
...
function showTaskResult(res, taskId) {
	...
	if (res.status && res.status._response && res.status._response.response && res.status._response.response.json_result &&
		...
		// ----- BEGIN Hotspotter IA Illustration -----
		...
		// for (var i = 0; i < max; i++) {
		// 	var database_annot_uuid = resJSON.dannot_uuid_list[i]['__UUID__'];
		...
		for (var i = 0 ; i < max ; i++) {
			...
			$('.annot-summary-'+acmId).append("<span>"+illustUrl+"</span>");

			// maybe hook here to attach hotspotter illustration?
		}
...
// TODO: modify displayAnnot to add the link to IA illustration here
...
console.info('qdata[%s] = %o', taskId, qdata);
            ...
            if (illustrationUrl) {
            	...
            	// TODO: generify
            	var iaBase = 'http://104.42.42.134:5007';
...
//.score_list vs .annot_score_list ??? TODO are these the same? seem to be same values
	...
	for (var i = 0 ; i < cm_dict.score_list.length ; i++) {
		...
		// here we'd like to add the database_annot_uuid for hotspotter illustration
