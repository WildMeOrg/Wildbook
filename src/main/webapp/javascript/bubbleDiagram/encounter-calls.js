...
var getEncounterTableData = function(occurrenceObjectArray, individualID) {
  var encounterData = [];
  var occurringWith = "";
    d3.json(wildbookGlobals.baseUrl + "/api/org.ecocean.MarkedIndividual/?individualID==%27" + individualID + "%27", function(error, json) {
      if(error) {
        console.log("error")
      }
      jsonData = json[0];
...
var getIndividualData = function(relationshipArray) {
  ...
  for(var i=0; i < relationshipArray.length; i++) {
    d3.json(wildbookGlobals.baseUrl + "/api/org.ecocean.MarkedIndividual/?individualID==%27" + relationshipArray[i].relationshipWith[0] + "%27", function(error, json) {
      if(error) {
        console.log("error")
      }
      jsonData = json[0];
