...
function uploaderInit(completionCallback, subdir, isImportExport) {
  if (useS3Direct()) {
    ...
    console.info(
      'uploader is using direct-to-s3 uploading to bucket %s',
      wildbookGlobals.uploader.s3_bucket
    )
    ...
    uploaderS3Bucket = new AWS.S3({
      params: { Bucket: wildbookGlobals.uploader.s3_bucket },
    })
    ...
    flow.on('fileProgress', function (file, chunk) {
      ...
      updateProgress(el, p, 'uploading')
      console.log(
        'progress %o %o. %o pending uploads',
        file._prevUploadedSize,
        file,
        pendingUpload
      )
    })
...
function findElement(key, size) {
  ...
  for (var i = 0; i < items.length; i++) {
    if (
      ...
      (size < 0 || size == items[i].getAttribute('data-size'))
    )
      return items[i]
  }
  return false
...
function filesChanged(f) {
  ...
  for (let i = 0; i < fileList.length; i++) {
    ...
    output += `
        <div class="file-item" id="file-item-${i}" data-i="${i}" data-name="${file.name}" data-size="${file.size}">
          <div class="file-name">${file.name}</div>
          <div class="file-size">${niceSize(file.size)}</div>
          <div class="file-status"></div>
          <div class="file-bar"></div>
        </div>`
  }

  document.getElementById('file-activity').innerHTML = output

  // Optional: Show selected folder name if webkitRelativePath is available
  if (fileList.length > 0 && fileList[0].webkitRelativePath) {
    const topFolder = fileList[0].webkitRelativePath.split('/')[0]
    const folderText = document.getElementById('folder-name')
    if (folderText) folderText.textContent = topFolder
  }
}
