wildbook.IA.plugins.push({
  ...
  getDomResult: function (task) {
    ...
    var h = ''
    h +=
      '<div class="task-content task-type-' +
      gt +
      '" id="task-' +
      task.id +
      '">'
    h +=
      '<div class="task-title accordion task-type-' +
      gt +
      '" onDblClick="$(\'#task-debug-' +
      task.id +
      '\').show();"><span class="task-title-id"><b>Task ' +
      task.id +
      '</b></span></div>'
    h +=
      '<div class="task-summary task-type-' +
      gt +
      '"><div class="summary-column col0" /><div class="summary-column col1" /><div class="summary-column col2" /></div>'
    h += '</div>'
    return h
  },
  ...
  isEnabled: function () {
    return (
      wildbookGlobals &&
      wildbookGlobals.iaStatus &&
      wildbookGlobals.iaStatus.map &&
      wildbookGlobals.iaStatus.map.iaEnabled
    )
  },
  ...
  imageMenuItems: function () {
    if (!this.isEnabled()) {
      console.info(
        '%s.imageMenuItems() claims IA functionality disabled',
        this.code
      )
      return
    }
    ...
    items.push([
      function (enh) {
        return wildbook.IA.getPluginByType('IBEIS')._iaMenuHelper(
          enh,
          'textStart'
        )
      },
    ...
    items.push([
      ...
      function (enh) {
        ...
        if (!ma.taxonomyString) {
          imageEnhancer.popup(
            'Set <b>genus</b> and <b>specific epithet</b> on this encounter before using visual matcher.'
          )
          return
        }
        //TODO how should we *really* get encounter number!
        wildbook.openInTab(
          'encounterVM.jsp?number=' +
            encounterNumberFromElement(enh.imgEl) +
            '&mediaAssetId=' +
            mid
        )
      },
    ...
    items.push([
      ...
      function (enh) {
        ...
        var ma = assetById(mid)
        wildbook.openInTab(
          'manualAnnotation.jsp?encounterId=' +
            encounterNumberFromElement(enh.imgEl) +
            '&assetId=' +
            mid
        )
  ...
  _iaMenuHelper: function (enh, mode) {
    ...
    if (!mid || !aid || !ma || typeof iaMatchFilterAnnotationIds == 'undefined')
      return false
    var iaStatus = wildbook.IA.getPluginByType('IBEIS').iaStatus(ma)
    var identActive =
      wildbook.IA.getPluginByType('IBEIS').iaStatusIdentActive(iaStatus)
    ...
    console.log(
      '_iaMenuHelper: mode=%o, mid=%o, aid=%o, ma=%o, iaStatus=%o, identActive=%o, requireSpecies=%o',
      mode,
      mid,
      aid,
      ma,
    ...
    if (
      ...
      detectionComplete &&
      ma.annotation &&
      !ma.annotation.identificationStatus
    ) {
      if (mode == 'textStart') {
        return '<span class="disabled">Cannot start match</span>'
      ...
      identActive &&
      (detectionComplete ||
        ma.annotation.identificationStatus == ('pending' || 'complete'))
    ) {
      if (mode == 'textStart') {
    ...
    if (
      ...
      !(
        iaStatus &&
        iaStatus.task &&
        iaStatus.task.parameters &&
        iaStatus.task.parameters.skipIdent
      )
    ) {
      if (mode == 'textStart') {
    ...
    jQuery.ajax({
      ...
      complete: function (xhr, textStatus) {
        if (typeof callback == 'function') {
          ...
          console.info(
            'IA.IBEIS.restCall() given no callback, status=%s, %o',
            textStatus,
            xhr
          )
        }
  ...
  matchFilter: function (aid, ma) {
    ...
    var numAlgosVisible = $(
      '.mfalgo-iaclass-' + iaClass.replaceAll('+', '-')
    ).length
...
function registerTaskId(taskId) {
  $('#activeTaskId').remove()
  $('body').append(
    '<p id="activeTaskId" style="display: none;">' + taskId + '</p>'
  )
}
