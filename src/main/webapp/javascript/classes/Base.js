wildbook.Model.BaseClass = Backbone.Model.extend({
  ...
  fields: function () {
    ...
    for (var fn in wildbookGlobals.classDefinitions[this.className()].fields) {
      ...
      var fh = {
        javaType: wildbookGlobals.classDefinitions[this.className()].fields[fn],
      }
      fh.value = this[fn]
      fh.settable = !(
        wildbookGlobals.classDefinitions[this.className()].permissions &&
        wildbookGlobals.classDefinitions[this.className()].permissions[fn] ==
          'deny'
      )
      rtn[fn] = fh
    }
...
wildbook.Collection.BaseClass = Backbone.Collection.extend({
  ...
  fetch: function (options) {
    ...
    if (options && options.jdoql) {
      ...
      if (options.jdoql.toLowerCase().indexOf('where') == 0)
        options.jdoql =
          'SELECT FROM ' +
          this.model.prototype.className() +
          ' ' +
          options.jdoql
      var noDecorate = ''
      var fetch = ''
      ...
      this._altUrl =
        ...
        fetch +
        'query=' +
        options.jdoql //note this does not need the classname like /api/org.ecocean.Foo
