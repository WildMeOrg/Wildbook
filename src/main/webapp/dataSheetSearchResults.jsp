<%@ page contentType="text/html; charset=utf-8" language="java"
  ...
  context=ServletUtilities.getContext(request);
    ...
    dtProps = ShepherdProperties.getProperties("dataSheet.properties");
    ...
    myShepherd.setAction("dataSheetSearchResults.jsp");
    ...
    Vector<DataSheet> rIndividuals = new Vector<DataSheet>();
    myShepherd.beginDBTransaction();
    String order ="";

    DataSheetQueryResult result = DataSheetQueryProcessor.processQuery(myShepherd, request, order);
...
<div class="container maincontent">
      ...
      <h1 class="intro">
        <%=dtProps.getProperty("DataSheetSearchResults")%>
      </h1>
...
*/
	...
	},
  {
    ...
    value: _notUndefined('samplingProtocol'),
	},
  {
    key: 'fieldNotes',
    label: 'field notes',
    value: _notUndefined('fieldNotes'),
  },
  /*
	{
		key: 'individual',
...
/*
	for (var i = 0 ; i < searchResults.length ; i++) {
		searchResultsObjects[i] = new wildbook.Model.DataSheet(searchResults[i]);
	}
*/
...
function rowClick(el) {
	console.log(el);
	var w = window.open('dataSheet.jsp?number=' + el.getAttribute('data-id'), '_blank');
	w.focus();
	return false;
}
...
function xxxshow() {
	...
	for (var i = 0 ; i < results.length ; i++) {
		...
		$('#results-table tbody tr')[i].setAttribute('data-id', searchResults[results[i]].DataCollectionEventID);
		for (var c = 0 ; c < colDefn.length ; c++) {
...
function doTable() {
	...
	inds = new wildbook.Collection.DataSheets();
	var addedCount = 0;
...
if (percentage % 3 == 0) console.log(percentage);
	...
	_.each(searchResults, function(o) {
		inds.add(new wildbook.Model.DataSheet(o));
	});
	$('#progress').remove();
...
<table width="810" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="left">
      <p><strong><%=dtProps.getProperty("matchingDataSheets")%>
      </strong>: <span id="count-total"></span>
      </p>
      <%myShepherd.beginDBTransaction();%>
      <p><strong><%=dtProps.getProperty("totalDataSheets")%>
    </strong>: <%=(myShepherd.getNumDataSheets())%>
      </p>
    </td>
    <%
      myShepherd.rollbackDBTransaction();
