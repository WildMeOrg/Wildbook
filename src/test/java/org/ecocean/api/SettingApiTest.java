 ...
 */
    ...
    @Test void apiPostInvalidIdGroup()
    throws ServletException, IOException {
        User user = mock(User.class);
    ...
    throws ServletException, IOException {
        ...
        try (MockedConstruction<Shepherd> mockShepherd = mockConstruction(Shepherd.class,
                (mock, context) -> {
            when(mock.getUser(any(HttpServletRequest.class))).thenReturn(user);
            doThrow(new RuntimeException("fail")).when(mock).getOrCreateSetting(any(String.class),
                any(String.class));
        })) {
            try (MockedStatic<ShepherdPMF> mockService = mockStatic(ShepherdPMF.class)) {
                mockService.when(() -> ShepherdPMF.getPMF(any(String.class))).thenReturn(mockPMF);
    ...
    throws ServletException, IOException {
        ...
        try (MockedConstruction<Shepherd> mockShepherd = mockConstruction(Shepherd.class,
            ...
            try (MockedStatic<CommonConfiguration> mockService = mockStatic(
                CommonConfiguration.class)) {
                mockService.when(() -> CommonConfiguration.getProperty(any(String.class),
                    any(String.class))).thenReturn("test-value");
                try (MockedStatic<ReCAPTCHA> mockCaptcha = mockStatic(ReCAPTCHA.class)) {
                    ...
                    assertEquals(jout.keySet().size(), 43);
                }
            }
        }
    }

    @Test void apiGetAnon()
    throws ServletException, IOException {
        try (MockedConstruction<Shepherd> mockShepherd = mockConstruction(Shepherd.class,
                (mock, context) -> {
            ...
            try (MockedStatic<CommonConfiguration> mockService = mockStatic(
                ...
                try (MockedStatic<ReCAPTCHA> mockCaptcha = mockStatic(ReCAPTCHA.class)) {
                    ...
                    assertEquals(jout.keySet().size(), 41);
                }
            }
        }
    }

    @Test void apiDelete401()
    throws ServletException, IOException {
        try (MockedConstruction<Shepherd> mockShepherd = mockConstruction(Shepherd.class,
                (mock, context) -> {
            doNothing().when(mock).beginDBTransaction();
    ...
    @Test void apiDeleteNonAdmin401()
    throws ServletException, IOException {
        User user = mock(User.class);
    ...
    @Test void apiDeleteInvalidIdGroup()
    throws ServletException, IOException {
        User user = mock(User.class);
