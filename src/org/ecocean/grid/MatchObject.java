...
public class MatchObject implements java.io.Serializable{
	...
	public String getLogMBreakdown() {
		...
		if(logMbreakdown==null) {return "&nbsp;";}
	    	...
	    	if(stdDeviationLogM>0.01) {return ((new Double(stdDeviationLogM)).toString().substring(0,7));}
	    	else {return "<0.01";}		
	...
	public double getRightmostSharkSpot() {
		...
		for (int i=0;i<scores.size();i++) {
			...
			if (((VertexPointMatch)scores.get(i)).oldX>greatest) {greatest=((VertexPointMatch)scores.get(i)).oldX;}
	...
	public double getHighestSharkSpot() {
		...
		for (int i=0;i<scores.size();i++) {
			...
			if (((VertexPointMatch)scores.get(i)).oldY>greatest) {greatest=((VertexPointMatch)scores.get(i)).oldY;}
	...
	public double getRightmostEncounterSpot() {
		...
		for (int i=0;i<scores.size();i++) {
			...
			if (((VertexPointMatch)scores.get(i)).newX>greatest) {greatest=((VertexPointMatch)scores.get(i)).newX;}
	...
	public double getHighestEncounterSpot() {
		...
		for (int i=0;i<scores.size();i++) {
			...
			if (((VertexPointMatch)scores.get(i)).newY>greatest) {greatest=((VertexPointMatch)scores.get(i)).newY;}
	...
	public Vector getMap2(){return Points;}
	public double getI3SMatchValue(){return i3sMatchValue;}
