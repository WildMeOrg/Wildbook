...
public class EncounterLite implements java.io.Serializable{
	...
	public MatchObject getPointsForBestMatch(SuperSpot[] newspotsTemp, double epsilon, double R, double Sizelim, double maxTriangleRotation, double C, boolean secondRun, boolean rightScan){
		...
		try{
			...
			else{spots=(SuperSpot[])getSpots().toArray(spots);}
			...
			for (int iterN1=0;iterN1<newspots.length;iterN1++){
				if (newspots[iterN1].getTheSpot().getCentroidX()>xMaxNew) {xMaxNew=newspots[iterN1].getTheSpot().getCentroidX();}
			}
			for (int iterN2=0;iterN2<newspots.length;iterN2++){
				if (newspots[iterN2].getTheSpot().getCentroidY()>yMaxNew) {yMaxNew=newspots[iterN2].getTheSpot().getCentroidY();}
			}
			for (int iterN3=0;iterN3<spots.length;iterN3++){
				if (spots[iterN3].getTheSpot().getCentroidX()>xMaxCatalog) {xMaxCatalog=spots[iterN3].getTheSpot().getCentroidX();}
			}
	
			//correction	
			for (int iterN4=0;iterN4<spots.length;iterN4++){
				if (spots[iterN4].getTheSpot().getCentroidY()>yMaxCatalog) {yMaxCatalog=spots[iterN4].getTheSpot().getCentroidY();}
			}
			double normFactorCatalog=0;
			if(xMaxCatalog>yMaxCatalog) {normFactorCatalog=xMaxCatalog;}
			else{normFactorCatalog=yMaxCatalog;}
			//System.out.println("normFactorCatalog is: "+normFactorCatalog);
			double normFactorNew=0;
			if(xMaxNew>yMaxNew) {normFactorNew=xMaxNew;}
			else{normFactorNew=yMaxNew;}
    ...
    for(int i=0;i<newSpotArrayL;i++) {
    	...
    	for(int j=i+1; j<(newspots.length-1);j++) {
    		...
    		for(int k=j+1; k<newArrayL;k++) {
				...
				if(tempTriangle.D12<newClosePairDist){newClosePairDist=tempTriangle.D12;}
    ...
    for(int i=0;i<spotArrayL;i++) {
    	...
    	for(int j=i+1; j<(baseSpots.length-1);j++) {
    		...
    		for(int k=j+1; k<baseSpots.length;k++) {
				...
				if(tempTriangle.D12<baseClosePairDist){baseClosePairDist=tempTriangle.D12;}
    			if ((tempTriangle.R<=R)&&(tempTriangle.C<=C)) {
    ...
    for(int iter3=0; iter3<VmatchesA.size(); iter3++) {
    	...
    	if(((SpotTriangle)VmatchesA.get(iter3)).clockwise==((SpotTriangle)VmatchesB.get(iter3)).clockwise) {nPLUS++;}
    	else {nMINUS++;}
    	}
    ...
    while(stillIterate&&(numIterations<20)&&(VmatchesA.size()>0)) {
    	...
    	for(int iterCount=0;iterCount<logM.size();iterCount++){
    		if((((Double)logM.get(iterCount)).doubleValue())>meanLogM){greaterThanMeanLogM++;}
    		else{lessThanMeanLogM++;}
    		}
    	...
    	if(lessThanMeanLogM>greaterThanMeanLogM){leftSideHeavy=true;}
    		else{rightSideHeavy=true;}
    		}
    	else{
    		balanced=true;}
    	...
    	for(int iter6=0; iter6<logM.size(); iter6++){
    		...
    		if(Math.abs(((Double)logM.get(iter6)).doubleValue()-meanLogM)>(multiple*stdDeviationLogM)) {
    			if(leftSideHeavy&&(((Double)logM.get(iter6)).doubleValue()<meanLogM)){leftsideRemovals++;}
    			else if(rightSideHeavy&&(((Double)logM.get(iter6)).doubleValue()>meanLogM)){rightsideRemovals++;}
    			else if(leftSideHeavy&&(((Double)logM.get(iter6)).doubleValue()>meanLogM)){rightsideRemovals++;}
    			else if(rightSideHeavy&&(((Double)logM.get(iter6)).doubleValue()<meanLogM)){leftsideRemovals++;}
    			logM.remove(iter6);
    ...
    //	System.out.println("     Removed "+logMremovals+" triangles on logM filter pass "+numIterations+" with a filter/multiple value of "+multiple);	
    	...
    	if(!haveMadeChange) {stillIterate=false;}
    	nPLUS=0;
    	nMINUS=0;
    	int iterLimit=VmatchesA.size();
    	for(int iter7=0; iter7<iterLimit; iter7++) {
    		if(((SpotTriangle)VmatchesA.get(iter7)).clockwise==((SpotTriangle)VmatchesB.get(iter7)).clockwise) {nPLUS++;}
    		else {nMINUS++;}
    		}
	...
	if(VmatchesA.size()==0) {return (new MatchObject(belongsToMarkedIndividual,0,0, encounterNumber));}
    ...
    for(int iter10=0; iter10<vMatchL; iter10++) {
    	for(int iter11=0; iter11<3; iter11++) {
    		...
    		else{mp.add(new VertexPointMatch(spotA, spotB, 1));}
    		//	}
    		}
    ...
    while(keepOnCounting&&(iter20<scoresSize)){
    	...
    	for(int iter30=0;iter30<countedSpotsA.size();iter30++){
    		...
    		if((scores[iter20].oldX==tempSpot.getCentroidX())&&(scores[iter20].oldY==tempSpot.getCentroidY())){hasNotBeenSeenYet=false;}
    	}
	...
	for(int iter25=0;iter25<scoredSpots.length;iter25++){
		...
		for(int iter26=0;iter26<secondRunSpotsA.size();iter26++){
			if((scoredSpots[iter25].newX==(((SuperSpot)secondRunSpotsA.get(iter26)).getTheSpot().getCentroidX()))&&(scoredSpots[iter25].newY==(((SuperSpot)secondRunSpotsA.get(iter26)).getTheSpot().getCentroidY()))){matchListA=true;}
			}
		if(!matchListA){secondRunSpotsA.add(new SuperSpot(new Spot(0, scoredSpots[iter25].newX, scoredSpots[iter25].newY)));}
		boolean matchListB=false;
		for(int iter26=0;iter26<secondRunSpotsB.size();iter26++){
			if((scoredSpots[iter25].oldX==(((SuperSpot)secondRunSpotsB.get(iter26)).getTheSpot().getCentroidX()))&&(scoredSpots[iter25].oldY==(((SuperSpot)secondRunSpotsB.get(iter26)).getTheSpot().getCentroidY()))){matchListB=true;}
			}
		if(!matchListB){secondRunSpotsB.add(new SuperSpot(new Spot(0, scoredSpots[iter25].oldX, scoredSpots[iter25].oldY)));}
	}
   ...
   // end second run
...
} catch (Exception e) {e.printStackTrace();System.out.println("0 points awarded due to exception.");return (new MatchObject(belongsToMarkedIndividual,0,0, encounterNumber));}
}  
	...
	private VertexPointMatch[] secondGrothPass(SuperSpot[] secondNewSpots, SuperSpot[] secondBaseSpots, double epsilon, double R, double Sizelim, double maxTriangleRotation, double C) {
	    ...
	    //System.out.println("newSpan "+newSpan+" baseSpan "+baseSpan);
			//System.out.println("     baseClosePairDist is "+baseClosePairDist);
	    	if(baseClosePairDist<(3*epsilon)){System.out.println("WARNING!!!! Spots in the catalog encounter are too close together to support this high of an epsilon value!!!");}
	   ...
	   //System.out.println("   Going into the logM filter with "+VmatchesA.size()+" matching triangles. Before filtering, N+="+nPLUS+" N-="+nMINUS);
	    ...
	    //System.out.println("Going into scoring with "+VmatchesA.size()+" matching triangles.");
		if(VmatchesA.size()==0) {return scores;}
	    MatchedPoints mp=new MatchedPoints();
	    int vMatchL=VmatchesA.size();
	    for(int iter10=0; iter10<vMatchL; iter10++) {
	    	for(int iter11=0; iter11<3; iter11++) {
		...
		scoredSpots=(VertexPointMatch[])(secondRunSpots.toArray(scoredSpots));
		return scoredSpots;
		} catch (Exception e) {e.printStackTrace();System.out.println("0 points awarded due to exception in secondGrothPass.");return scores;}
	}
	...
	public I3SMatchObject i3sScan(EncounterLite newEnc, boolean scanRight) {
		...
		else {System.out.println("Error in compare!");}
	...
	public double getRightmostSpot() {
		...
		for (int iter=0;iter<spots.length;iter++){
			if (spots[iter].getCentroidX()>rightest) {rightest=spots[iter].getCentroidX();}
			}
		return rightest;
	}
	...
	public double getLeftmostSpot() {
		...
		for (int iter=0;iter<spots.length;iter++){
			if (spots[iter].getCentroidX()<leftest) {leftest=spots[iter].getCentroidX();}
			}
		return leftest;
	}
	...
	public double getHighestSpot() {
		...
		for (int iter=0;iter<spots.length;iter++){
			if (spots[iter].getCentroidY()<highest) {highest=spots[iter].getCentroidY();}
			}
		return highest;
	}
	...
	public double getLowestSpot() {
		...
		double lowest=0;
		for (int iter=0;iter<spots.length;iter++){
			if (spots[iter].getCentroidY()>lowest) {lowest=spots[iter].getCentroidY();}
			}
		return lowest;
	}
