...
public class WriteOutScanTask extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		if((!request.getParameter("number").equals("TuningTask"))&&(!request.getParameter("number").equals("FalseMatchTask"))){	
    		...
    		try {
    			...
    			//let's check the checked-in value
    				...
    				if(taskID.startsWith("scanR")){righty=true;}
      ...
      es.execute(new ScanTaskCleanupThread(request.getParameter("number")));
			...
			String resultsURL=("http://"+CommonConfiguration.getURLLocation()+"/encounters/scanEndApplet.jsp?number="+request.getParameter("number").substring(5)+"&writeThis=true"+sideAddition);	
				...
				*/
				...
				for(int i=1; i<(resultSize+1);i++){
					...
					*/
					...
					else{boostString=boostString+"not";}
					...
					if(swi.rightScan){
						try{
							if(encB.rightSpotImageFileName.endsWith("-mapped.jpg")){bSpot = "true";}
						} catch(Exception f){f.printStackTrace();System.out.println("I couldn't find the rightSpotImageFileName for encounter "+encB.getEncounterNumber());}
						try{	
							if(encA.rightSpotImageFileName.endsWith("-mapped.jpg")){aSpot = "true";}
						} catch(Exception f){f.printStackTrace();System.out.println("I couldn't find the rightSpotImageFileName for encounter "+encA.getEncounterNumber());}
							
						}
					else{
						try{
							if(encB.spotImageFileName.endsWith("-mapped.jpg")){bSpot = "true";}
						} catch(Exception f){f.printStackTrace();System.out.println("I couldn't find the spotImageFileName for encounter "+encB.getEncounterNumber());}
						try{	
							if(encA.spotImageFileName.endsWith("-mapped.jpg")){aSpot = "true";}
						} catch(Exception f){f.printStackTrace();System.out.println("I couldn't find the spotImageFileName for encounter "+encA.getEncounterNumber());}
		...
		if(request.getParameter("number").equals("TuningTask")){
			String resultsURL=("http://"+CommonConfiguration.getURLLocation()+"/appadmin/endTuningTask.jsp?number=TuningTask");	
			response.sendRedirect(resultsURL);
		}
		else{
			String resultsURL=("http://"+CommonConfiguration.getURLLocation()+"/appadmin/endTuningTask.jsp?number=FalseMatchTask");	
			response.sendRedirect(resultsURL);
		}
    ...
    public ScanWorkItem getWI4MO(ScanWorkItemResult swir, ArrayList<ScanWorkItem> list){
    	...
    	for(int i=0;i<list.size();i++){
    		if(list.get(i).getUniqueNumber().equals(swir.getUniqueNumberWorkItem())){return list.get(i);}
    	}
    	return swi;
