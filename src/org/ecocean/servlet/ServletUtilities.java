...
public class ServletUtilities {
	...
	public static String getHeader() {
		try {
            	...
            	templateFile=templateFile.replaceAll("CSSURL", CommonConfiguration.getCSSURLLocation());
	...
	public static void informInterestedParties(String number, String message){
		...
		if(size>0){
			...
			String email=getText("dataUpdate.txt").replaceAll("INSERTTEXT", ("Encounter "+number+": "+message+"\n\nLink to encounter: http://"+CommonConfiguration.getURLLocation()+"/encounters/encounter.jsp?number="+number));
			email+=("\n\nWant to stop tracking this set of encounter data? Use this link.\nhttp://"+CommonConfiguration.getURLLocation()+"/dontTrack?number="+number+"&email=");
	...
	public static void informInterestedIndividualParties(String shark, String message){
		Shepherd myShepherd=new Shepherd();
		myShepherd.beginDBTransaction();
		...
		if(size>0){
			...
			Vector e_images=new Vector();
			String mailMe=interested[0];
			String email=getText("dataUpdate.txt").replaceAll("INSERTTEXT", ("Shark "+shark+": "+message+"\n\nLink to shark: http://"+CommonConfiguration.getURLLocation()+"/individuals.jsp?number="+shark));
			email+=("\n\nWant to stop tracking this set of shark data? Use this link.\n\nhttp://"+CommonConfiguration.getURLLocation()+"/dontTrack?shark="+shark+"&email=");
	...
	public static boolean isUserAuthorizedForIndividual(MarkedIndividual sharky, HttpServletRequest request){
		...
		for(int y=0;y<numEncs;y++){
			...
			if(request.isUserInRole(enc.getLocationCode())){return true;}
		}
		return false;
	}
