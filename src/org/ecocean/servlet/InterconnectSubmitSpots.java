...
public class InterconnectSubmitSpots extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		if(request.getParameter("number")!=null) num=request.getParameter("number").replaceAll("%20","").trim();
		...
		if (myShepherd.isEncounter(num)) {
			...
			try {
				...
				if (locked) {
					...
					ServletUtilities.informInterestedParties(num, message);
				...
				out.println("<p><a href=\"http://"+CommonConfiguration.getURLLocation()+"/encounters/encounter.jsp?number="+num+"\">Return to encounter #"+num+"</a></p>\n");
				out.println(ServletUtilities.getFooter());
			}
