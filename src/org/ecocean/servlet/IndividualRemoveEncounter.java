				...
				if ((request.getParameter("number")!=null)) {
					...
					if (!enc2remove.isAssignedToMarkedIndividual().equals("Unassigned")) {
						...
						if(!locked){
							...
							if(wasRemoved){out.println("Record <strong>"+name_s+"</strong> was also removed because it contained no encounters.");}
							...
							if(!wasRemoved){ServletUtilities.informInterestedIndividualParties(old_name, message);}
						}
						else {
							out.println(ServletUtilities.getHeader());
