...
public class Shepherd {
	...
	public Encounter getEncounter(String num) {
		...
		try {tempEnc=((Encounter)(pm.getObjectById(pm.newObjectIdInstance(Encounter.class, num.trim()),true )));}
		catch (Exception nsoe) {return null;}
		return tempEnc;
	}
	
	public Adoption getAdoption(String num) {
		Adoption tempEnc=null;
		try {tempEnc=((Adoption)(pm.getObjectById(pm.newObjectIdInstance(Adoption.class, num.trim()),true )));}
		catch (Exception nsoe) {return null;}
		return tempEnc;
	}
	...
	public ScanTask getScanTask(String uniqueID) {
		...
		try {tempTask=((ScanTask)(pm.getObjectById(pm.newObjectIdInstance(ScanTask.class, uniqueID.trim()),true )));}
		catch (Exception nsoe) {return null;}
		return tempTask;
	}
	...
	public Keyword getKeyword(String indexname) {
		...
		try {tempEnc=((Keyword)(pm.getObjectById(pm.newObjectIdInstance(Keyword.class, indexname.trim()),true )));}
		catch (Exception nsoe) {return null;}
		return tempEnc;
		}
	...
	*/
	public void addMarkedIndividual(MarkedIndividual newShark) {
		...
		while (allsharks.hasNext()) {
			...
			if (tempShark.getName().equals(newShark.getName())) {ok2add=false;};
			}
		if (ok2add) {
	...
	*/
	...
	public MarkedIndividual getMarkedIndividual(String name) {
		...
		try {tempShark=((org.ecocean.MarkedIndividual)(pm.getObjectById(pm.newObjectIdInstance(MarkedIndividual.class, name.trim()),true )));}
		catch (Exception nsoe) {
			nsoe.printStackTrace();
			return null;
		}
    ...
    public Vector getUnidentifiableEncountersForMarkedIndividual(String individual) {
      ...
      try{
		...
		public int getNumEncountersWithSpotData(boolean rightSide) {
			...
			Extent encClass=pm.getExtent(Encounter.class, true);
			String filter=""; 
			if(rightSide){filter="this.numSpotsRight > 0";}
			else{filter="this.numSpotsLeft > 0";}
			Query acceptedEncounters=pm.newQuery(encClass, filter);
			int num=0;
			try{
				Collection c=(Collection)(acceptedEncounters.execute());
	...
	public Vector getThumbnails(int startNum, int endNum) {
		Vector thumbs=new Vector();
		Extent encClass=getPM().getExtent(Encounter.class, true);
		Query query=getPM().newQuery(encClass);
		Iterator it=getAllEncounters(query);
		...
		while(it.hasNext()) {
			...
			if((count+enc.getAdditionalImageNames().size())>=startNum) {
				for(int i=0;i<enc.getAdditionalImageNames().size();i++) {
					...
					if((count<=endNum)&&(count>=startNum)) {
						...
						if(isAcceptableVideoFile(addTextFile)) {
			...
			else {count+=enc.getAdditionalImageNames().size();}
			} 
			}//end while
		return thumbs;
	}
   ...
   public int getNumMarkedIndividualThumbnails(Iterator<MarkedIndividual> it, String[] keywords) {
	...
	public Adoption getRandomAdoption(){
		...
		int ranNum=0;
		if(numAdoptions>1){ranNum = ran.nextInt(numAdoptions);}
		...
		while(it.hasNext()){
			...
			if(currentPosition==ranNum){return ad;}
			currentPosition++;
		}
		return null;
	}
