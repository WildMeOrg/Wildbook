...
<table cellpadding="5">
	<tr>
		...
		Visualization for Encounter Number: <a
			href="encounter.jsp?number=<%=num%>"><%=enc.getEncounterNumber()%></a>
		</strong></font></p>
...
%>
		<p><font size="-1">Belongs to: <%=enc.isAssignedToMarkedIndividual()%></font></p>
		<%
	} else {
%>
		<p><font size="-1">Belongs to: <a
			href="../individuals.jsp?number=<%=enc.isAssignedToMarkedIndividual()%>"><%=enc.isAssignedToMarkedIndividual()%></a></font></p>
		<%
	}
...
%>
		...
		URL encURL=new URL("http://"+CommonConfiguration.getURLLocation()+"/encounters/"+fileloc);
