...
<%@ page contentType="text/html; charset=iso-8859-1" language="java"
	import="org.ecocean.*, org.ecocean.grid.*,javax.jdo.*, java.lang.StringBuffer, java.lang.Integer, java.util.*, java.net.URL, java.net.URLConnection, java.io.InputStream, java.io.IOException, org.dom4j.*, org.dom4j.io.SAXReader, java.io.File"%>
<html>
<%
		...
		Arrays.sort(results, new MatchComparator());
		  ...
		  if ((results[p].matchValue!=0)||(request.getAttribute("singleComparison")!=null)) {%>
			...
			%>
				...
				<td><font size="-2"><%=results[p].getLogMStdDev()%></font></td>
				<td><font size="-2"><%=results[p].getEvaluation()%></font></td>
...
//or use XML output here	
	...
	root=doc.getRootElement();
		...
		while (matchsets.hasNext()) {
			...
			if(adjustedpoints==null) {adjustedpoints="&nbsp;";}
			if(adjustedpoints.length()>5) {
						adjustedpoints=adjustedpoints.substring(0,5);
					}


			else{adjustedpoints=adjustedpoints+"<br>";}
			%>
			 ...
			 %>
				...
				<td><font size="-2"><%=match.attributeValue("logMStdDev")%></font></td>
				<%
...
%>
				<td><font size="-2"><%=evaluation%></font></td>
				<td>
				<%
		String keywords="";
