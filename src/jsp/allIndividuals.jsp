...
<table id="results" border="0" width="810px">
	...
	<tr>
		<th class="caption" colspan="5"><%=text.replaceAll("COUNT", Integer.toString(totalCount))%></th>
	</tr>
	<tr class="paging">
		<td width="101" align="left">
		<%if (highCount<totalCount) {%> <a
			href="allIndividuals.jsp?start=<%=(lowCount+10)%>&amp;end=<%=(highCount+10)%><%=currentSort%>"><%=next%></a>
		<%} 
		  if ((lowCount-10)>=1) {%> | <a
			href="allIndividuals.jsp?start=<%=(lowCount-10)%>&amp;end=<%=(highCount-10)%><%=currentSort%>"><%=previous%></a>
		<%}%>
		</td>
	...
	<tr class="lineitem">
		...
		<td width="119" align="left" valign="top" bgcolor="#99CCFF"
			class="lineitem"><strong><%=sex%></strong><br />
		<%if(request.getRemoteUser()!=null){%><a
	...
	<%		
			...
			if (request.getParameter("sort")!=null) {
				...
				else if(request.getParameter("sort").equals("encountersup")) {allSharks=myShepherd.getAllMarkedIndividuals(query, "numberEncounters ascending");}
				else if(request.getParameter("sort").equals("encountersdown")) {allSharks=myShepherd.getAllMarkedIndividuals(query, "numberEncounters descending");}
				else if(request.getParameter("sort").equals("nameup")) {allSharks=myShepherd.getAllMarkedIndividuals(query, "name ascending");}
				else if(request.getParameter("sort").equals("namedown")) {allSharks=myShepherd.getAllMarkedIndividuals(query, "name descending");}
				else if(request.getParameter("sort").equals("seriesup")) {allSharks=myShepherd.getAllMarkedIndividuals(query, "seriesCode ascending");}
				else if(request.getParameter("sort").equals("seriesdown")) {allSharks=myShepherd.getAllMarkedIndividuals(query, "seriesCode descending");}
...
if (highCount<totalCount) {%> <a
  ...
  if ((lowCount-10)>=1) {%> | <a
		...
		<td align="right" colspan="4"><%=records%>: <%=lowCount%> - <%=highCount%><%=displaySort%>
		</td>
	</tr>
</table>
</div>
