...
<%@ page contentType="text/html; charset=utf-8" language="java"
	import="java.util.ArrayList,java.util.concurrent.ArrayBlockingQueue,java.util.concurrent.BlockingQueue, java.util.concurrent.ThreadPoolExecutor,java.util.Iterator, java.util.Properties, java.io.FileInputStream, java.io.File, java.io.FileNotFoundException, org.ecocean.*, org.ecocean.grid.*"%>
<%
...
<table border="1" cellpadding="2">
	...
	<%
			...
			%>
	<tr>
		<td><%=scanNum%>. <%=st.getUniqueNumber()%></td>
		<td><%=st.getSubmitter()%></td>
		<td><%=numComplete%>/<%=numTaskTot%></td>
		<td>
...
<table border="1" cellpadding="2">
	...
	<%
			...
			while(it2.hasNext()) {
				...
				if(st.getUniqueNumber().indexOf("scanR")!=-1){sideAddition="true";}
				
				scanNum++;
				%>
	<tr>

		<td><%=st.getUniqueNumber()%></td>
		<td><%=st.getSubmitter()%></td>
		<%
							String gotoURL = "http://"+CommonConfiguration.getURLLocation()+"/encounters/scanEndApplet.jsp";
							if(st.getUniqueNumber().equals("TuningTask")){gotoURL = "endTuningTask.jsp";}
						%>
...
%>
	<tr>
		...
		<td><span class="style2"><a
			href="scanTaskAdmin.jsp?zombie=<%=nd.getNodeIdentifier()%>"><%=nd.isZombie%></a></span></td>
...
startup: <%=gm.getCollisionRatePercentage()%></p>
