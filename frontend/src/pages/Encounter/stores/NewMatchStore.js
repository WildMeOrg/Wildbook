...
import { findNodeByValue, getAllDescendantValues, expandIds } from "../../../utils/treeSelectionFunction";
...
class NewMatchStore {
    ...
    get annotationIds() {
        ...
        return annotations.filter(d => d.encounterId === encounterId).map(d => d.id);
    }

    get algorithmOptions() {
        return this.iaConfigBasedOnTaxonomy?.map(d => ({
            label: d.description,
            value: d.description,
        })) || [];
    }
    ...
    get matchingAlgorithms() {
        ...
        return this.iaConfigBasedOnTaxonomy?.filter(cfg => selected.has(cfg.description)) || [];
    }
    ...
    handleStrictChange(checkedValues, _labels, extra) {
        ...
        if (!extra || !("triggerValue" in extra)) {
            this.setLocationID(expandIds(this.encounterStore.locationIdOptions, valueStrings || []));
            return;
        }
        const triggerId = typeof extra.triggerValue === 'object'
            ? extra.triggerValue.value
            : extra.triggerValue;
        const checked = !!extra.checked;
        const set = new Set(valueStrings);  
        const node = findNodeByValue(this.encounterStore.locationIdOptions, triggerId);
    ...
    async buildNewMatchPayload() {
        const locVals = (this._locationId || []).map(v => (typeof v === "object" ? v.value : v)).filter(Boolean);
