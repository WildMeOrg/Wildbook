    ...
    const filterTree = (nodes) => {
      return nodes
        .map((node) => {
          ...
          const nodeInBounds = isWithinBounds(node.geospatialInfo) || filteredChildren.length > 0;
          if (nodeInBounds) {
            return {
              ...node,
...
export const LocationFilterByMap = observer(({
  ...
  useEffect(() => {
    if (isMouseUp && bounds) {
      setShowFilterByMap(filterLocationsInBounds(treeData, bounds).length > 0);
      ...
      >
        {
          <Map
            bounds={bounds}
            setBounds={setBounds}
            center={{ lat: mapCenterLat || -1.286389, lng: mapCenterLon || 36.817223 }}
            zoom={mapZoom || 4}
          />
        }
      </div>
