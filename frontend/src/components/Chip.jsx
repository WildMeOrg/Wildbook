...
function Chip({ children }) {
  ...
  function renderFilter(filter) {
    ...
    if (Array.isArray(query)) {
      entries.push(
        `Dynamic filter: ${filterKey || filterId} is set : ${query?.map((q) => Object.keys(Object.values(q)[0])[0])[0].split(".")[1]}`,
      );
    }
    ...
    if (query?.geo_bounding_box) {
      ...
      entries.push(
        `Location within bounding box: Northeast latitude: ${top_left.lat}, longitude: ${top_left.lon}, southwest latitude: ${bottom_right.lat}, longitude: ${bottom_right.lon}`,
      );
    }
