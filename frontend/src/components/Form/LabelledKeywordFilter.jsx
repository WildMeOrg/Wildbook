...
export default function LabelledKeywordFilter({
    data,
    onChange,
}) {
    ...
    const addLabelledKeywordPair = () => {
        setLabelledKeywordPairs([...labelledKeywordPairs, { labelledKeyword: null, labelledKeywordValues: [] }]);
    };
    ...
    const updateLabelledKeywordValues = (index, selectedOptions) => {
        ...
        if (selectedOptions.length === 0) {
            onChange(null, `mediaAssetLabeledKeywords.${newPairs[index].labelledKeyword}`);
            return;
        }
        ...
        if (isChecked_keyword) {    
            ...
            const query = selectedOptions.map((option) => {
                return {
                    "term": {
                        [`mediaAssetLabeledKeywords.${newPairs[index].labelledKeyword}`]: option.value
                    }
                }
            })
            ...
            const query = {
                "terms": {
                    [`mediaAssetLabeledKeywords.${newPairs[index].labelledKeyword}`]: selectedValues
                }
            }
                ...
                <div key={index} className="d-flex flex-row gap-3 mb-3">
                    <div className="w-50">
                        ...
                        <Select
                            ...
                            value={labelledKeywordsOptions.find(option => option.value === pair.labelledKeyword)}
                        />
                    </div>
                    <div className="w-50">
                        ...
                        <Select
                            ...
                            menuPlacement="auto"
                            menuPortalTarget={document.body}
                            onChange={(selectedOptions) => updateLabelledKeywordValues(index, selectedOptions)}
